...
// Scroller version... Just for Fun - Not Profit... used for a LAN @ .DK ;-)
...
#define _(WWWW) {uv.x -= 1.1;d = WWWW(uv,d);}
...
vec3  _glob(vec2 uv)
{
	...
	c+=snow(uv,5.);
	c*=0.2/dd;
	vec3 finalColor=(vec3(0.5,0.8,0.9))*c*30.0;
	return finalColor;
}
...
{	
	...
	float t = time;
	float _d = length(uv);
	vec3 c1 = hsv(vec3(mod(t*0.3+(_d*uv.y+uv.x)*0.5,0.15),1.0,1.0));
	vec3 c2 = _glob(uv);
	uv *= sin(t+uv.y+uv.x)+4.0;
	uv.y -= 2.25;
	uv.y *= 1.0+cos(uv.x*0.2+t)*0.4;
	uv.y += sin(uv.x*0.5+t*0.4)*0.8;
	uv.x += tan(t*1.3)*.5+2.6;
	/// Lets move the chart and make a scroller !
	float d = 1.;
	_(S) _(T) _(U) _(Z) _(B) _(C);
