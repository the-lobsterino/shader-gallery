  ...
  in vec3 ambLight)
{
   ...
   float specular=pow(max(dot(refl,viewv),3.),spec);
...
{ 
  ...
  float d=sin(fp.x*0.3+time*4.0)+cos(fp.z*0.3+time*7.);
  p.y=p.y+d;
  p.xz=sim2d(p.xz,2.0);
...
void main(void){
  ...
  //Camera animation
  vec3 vuv=vec3(0,1,0);
  vec3 vrp=vec3(time*4.0,0.0,5.2);
  ...
  const vec3 ambi=vec3(0.1,0.1,4.);
