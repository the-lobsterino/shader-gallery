...
float Bu(vec2 pos){
 	float a = capsuleLength(pos + vec2(2.0, -1.5), vec2(1.2, 0.0));   
 	float b = capsuleLength(pos + vec2(-0.3, 0.3), vec2(1.3, 1.0) * 2.707);  
    float c = length(pos + vec2(-.3, -.3));
    float d = length(pos + vec2(-1.8, -1.3));
    return min(min(min(a, b), c), d);
}

float Chi(vec2 pos){
 	float a = capsuleLength(pos + vec2(1.0, -0.7), vec2(1.7, 0.0));   
...
float To(vec2 pos){
 	float a = capsuleLength(pos + vec2(0.5, 0.0), vec2(2.0, 1.0));   
