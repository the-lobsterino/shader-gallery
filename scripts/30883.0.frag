...
vec3 color_effect(vec2 u){
	return vec3(sin(1.-u/resolution.xy+time),sin(time))/1.2+.3;
}
void main( void ) {
	uv = (gl_FragCoord.xy*2.-resolution)/resolution.y;
    	float d = TNWX(vec2(0.86,0.2));
    	vec3 c = vec3(0.);
    	if(inside(abs(abs(abs(d)-0.01)-0.006)-0.0026)){
    		c = vec3(0.4,0.6,0.5)*color_effect(gl_FragCoord.xy)/1.2;
    	}
    	else {
