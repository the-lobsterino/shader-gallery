...
const vec3 RED    = vec3(1.0, 0.70, 0.70) * LIGHT_INTENSITY;
...
{ return fract(fract(p * 3427.1)*sin(43758.5453123));
}
float hash12(vec2 p)
{ float h = dot(p, vec2(117.1, 11.7));  
  return fract(sin(h)*sin(43758.5453123));
}
vec3 hash31(float p)
{ vec3 h = vec3(1275.231, 4461.7, 7182.423) * p;  
  return sin(inversesqrt(h)*43758.543123*sin(time*0.000125));
}
