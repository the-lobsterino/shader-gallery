...
vec3 borderColor = vec3(32, 32, 32)/255.;
vec3 hairColor0 = vec3(151, 200, 234)/255.;
vec3 hairColor1 = vec3(77, 135, 192)/255.;
vec3 hairColor2 = vec3(58, 103, 151)/255.;
vec3 faceColor0 = vec3(247, 223, 204)/255.;
vec3 faceColor1 = vec3(209, 173, 159)/255.;
vec3 faceColor2 = vec3(183, 148, 133)/255.;
vec3 eyeColor0 = vec3(145, 164, 176)/255.;
vec3 eyeColor1 = vec3(207, 216, 218)/255.;
vec3 eyeColor2 = vec3(255, 255, 255)/255.;
vec3 eyeColor3 = vec3(0, 0, 0)/255.;
vec3 eyeColor4 = hairColor0;
vec3 eyeColor5 = hairColor2;
vec3 eyeColor6 = hairColor1;
vec3 mouthColor0 = vec3(78, 53, 40)/255.;
vec3 mouthColor1 = vec3(152, 78, 69)/255.;
vec3 mouthColor2 = vec3(170, 92, 83)/255.;
vec3 backgroundColor = vec3(51, 124, 221)/255.;
...
bool rightEye0(out vec3 color) {
  ...
  if (inside(circle(vec2(-2.654, 1.92), 2.39))
    || outside(circle(vec2(-0.685, 0.42), 0.285 * sin(time*4.0)))
      || inside(circle(vec2(-0.683, 0.432), 0.262 * sin(time*4.0)))
        || outside(circle(vec2(-0.638, 0.516), 0.3 * sin(time*4.0))))
    return false;
  color = borderColor;
  return true;
}
...
bool rightEye3(out vec3 color) {
  ...
  if (outside(ellipse(vec2(-0.584, 0.355), vec2(-0.56, 0.518), 1.24 ))
      && inside(ellipse(vec2(-0.611, 0.31), vec2(-0.574, 0.42), 1.575 * sin(time*4.0)))
        && inside(ellipse(vec2(-0.701, 0.09), vec2(-0.58, 0.38), 1.17 * sin(time*4.0)))) {
    color = eyeColor6;
    return true;
  }
...
bool rightEye4(out vec3 color) {
  if (inside(ellipse(vec2(-0.627, 0.49), vec2(-0.589, 0.583), 1.35 * sin(time*4.0)))) {
    color = eyeColor2;
    return true;
  }
  if (inside(circle(vec2(-0.576, 0.445), 0.01 * sin(time*4.0))) || inside(circle(vec2(-0.54, 0.32), 0.01 * sin(time*4.0)))) {
    color = eyeColor4;
    return true;
  }
  return false;
...
bool mouth(out vec3 color) {
  ...
  if (inside(ellipse(vec2(-1.01, -0.04), vec2(-1.095, 0.02), 1.4))
    ...
    color = inside(ellipse(vec2(-1.0, -0.09), vec2(-1.092 + sin(time*4.0), 0.002), 1.34)) ? mouthColor2 : mouthColor1;
    return true;
  }
  color = mouthColor0;
  return true;
...
void main() {
  ...
  gl_FragColor = vec4(0.5);
	gl_FragColor.gb -= abs(fwidth(length(color)));
}
