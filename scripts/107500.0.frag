...
vec3 DrawClockCircle(vec2 fragpos, vec2 pos, float radius, float widthK, vec3 color) {
	...
	// цвет
	vec3 col = color * attenuation(0.0, 200.0, 1000.0, l) * widthK;
	// возврат
	return col;
}
...
void main() {
	...
	// рисование часовой стрелки
	outColor += DrawClockLine(p, vec2(0.0), ClockRadius, s, 1.9, clockLineHour2Col);
