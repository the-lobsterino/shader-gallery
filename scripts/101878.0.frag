...
vec3 rdr(vec2 uv)
{
    ...
    float globalShape = 199999999999999999999.;
    const float cnt = 32.;
    for (float i = 1.; i < cnt; ++i)
    {
        uv *= r2d((i/cnt)*.5);
        float th = 0.0001;
        ...
        col += pow(1.-sat(shape*5.),4.)*vec3(0.506,0.7199990,0.93993333333333333333333333333333333333333333333333333333333333333333333333386)*.1;
    }
