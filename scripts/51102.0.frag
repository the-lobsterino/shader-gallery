...
vec3 objNormal(vec3 p){
  ...
  float v1=inObj(vec3(2.*p.x+n_er,p.y-n_er,p.z-n_er)).x;
  float v2=inObj(vec3(2.*p.x-n_er,p.y-n_er,p.z+n_er)).x;
  float v3=inObj(vec3(2.*p.x-n_er,p.y+n_er,p.z-n_er)).x;
  float v4=inObj(vec3(2.*p.x+n_er,p.y+n_er,p.z+n_er)).x;
  ...
  if (f<maxd){
    ...
    if (s.y==2.0){
      ...
      if (f<maxd){
        ...
        gl_FragColor=vec4((objColor*0.8+objColor*0.692)*(1.0-f0*.03),1.0);
      } else gl_FragColor=vec4(objColor*0.08*(1.0-f0*.03),1.0);

    } else gl_FragColor=vec4(objColor*(1.0-f0*.03),1.0);
