...
void main( void ) {
	...
	for(int i = 1; i <= 48; i++){
		float a = float(i)*0.1;
		vec2 v = vec2(-1., -.5);
		v.x += 0.3*(1./sin(a*time+10./tan(v.y+v.x)) + cos(a*time*1.2));
		v.y += 0.6*(tan(a*time*1.02) + sin(1./a*time*1.2));
		v.y = .1/v.x*v.y;
		float size = 0.2;
		float b = 1.52 * (.5 - clamp(mouse.y,0.2,.8));
		float d = distance(uv, v)/size;
		float f = 1.0 - clamp(d-b*1.3, b, 1.);
		f = pow(f, 6.0);
		vec3 clr = (128.*clamp( mouse.x, 0.01, 2. ))*f*1.2 * f * vec3(0.6, 0.6, 4.0);
		gl_FragColor.rgb += clr;
		#define c gl_FragColor.rgb
		//c = .30*c + .6073*texture2D(bb, p0).xyz;
		//c = 1. - .6/c;
