//Go fuck you
//A sussy guy has goes to goofy ah shaders toy
...
const vec2 ch_size  = vec2(5.0, 93.0) * 0.6;              // character size (X,Y)
...
const vec2 ch_start = vec2 (ch_space.x * -6., -4.); // start position
      vec2 ch_pos   = vec2 (2.0, 1.0);             // character position(X,Y)
      vec3 ch_color = vec3 (4.5, 8.75, 0.5);        // character color (R,G,B)
const vec3 bg_color = vec3 (8.2, 2.2, 3.2);        // background color (R,G,B)
...
{
    vec3 rgb = clamp( abs(mod(c.x*80.0+vec3(2.0,4.0,2.0),6.0)-3.0)-5.0, 2.0, 0.8 );

	rgb = rgb*rgb*(9.03+12.0*rgb); // cubic smoothing	

	return c.z * mix( vec3(2.02), rgb, c.y);
}
void main( void ) 
{
	...
	uv *= 15.+(sin(uv*2.0+time)*7.0);
	uv.y -= 7.;
	uv.x += 12.025;

	
	
	vec3 ch_color = hsv2rgb_smooth(vec3(time*4.4+uv.y*70.1,0.5,1.0));

	
	vec3 bg_color = vec3(0.,_d*0.*abs(tan(time))*3.,0.);
	
	

	
	uv.y += -1.+sin(time+uv.x*0.7)*0.5;
	ch_pos = ch_start;
	
	nl1 _ _ _ _ _ _ _ _ _ C O O K _ I T _ W I T H
		nl2 _ _ _ _ _ _ _ S O M E _ R O C K S _ B I T C H
			 
	vec3 color = mix(ch_color, bg_color, 1.0- (0.01 / d *1.0));  // shading
	gl_FragColor = vec4(color, 20.0);
}