...
float cylinderY(vec3 p, float r, float h)
{
     ...
     d = difference(d, plane(p, vec3(0.0, -1.5, 0.0), vec3(0.0, h, 0.0)));
     d = difference(d, plane(p, vec3(0.0, 1.0, 5.0), vec3(0.5)));
     return d;
}
...
float scene(vec3 p)
{
    ...
    // try to guess
    d = _union(d, sphere(p - vec3(-0.2, -1.2, 0.9), 0.5));
    d = _union(d, sphere(p - vec3(0.2, -1.2, 0.9), 0.5));
    d = _union(d, capsuleY(rotateX(p - vec3(-0.0, -1.0, 0.9), sin(time*20.) - 3.14 / 2.0), 0.25, 1.9));
	// floor
    p += vec3(0.0, 1.0, 0.0);
...
void main(void)
{
    ...
    if(hit)
    {
        ...
        const vec3 color = vec3(1.0, 0.45, 0.22);
