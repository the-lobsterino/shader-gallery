// 300620N - mixed with http://glslsandbox.com/e#65779.0
// 030820N (created by me - necips@live.de
...
vec2 toPolar(vec2 p) {
  return vec2(length(p)*sin(time*.1), atan(p.y, p.x))*sin(time*.1);
}
...
vec4 renderText( void ) {
	...
	uv = vec2(.1*sin(time)*uv.x*uv.y + uv.x,  .1*cos(time)*uv.x*uv.y + uv.y);
	...
	color += 1. * dblend(textc+vec3(1.7, .0, .0) , d, vec2(1e88, .1));
...
void main( void ) {
	...
	float time = 0.001*(time/4.-length(position-.5));
	
	vec2 uv = position;
	float c = 0.;
	    c+=snow(uv,30.*sin(time));
	    c+=snow(uv,15.*cos(time));
	    c+=snow(uv,10.*sin(time));
	    c+=snow(uv,8.*cos(time));
	    c+=snow(uv,6.*sin(time));
	    c+=snow(uv,5.*cos(time)); 

	vec4 back=texture2D( backbuffer, fract(vec2(1,-1)/resolution+position+0.1*(5.-fract(time/256.+uv.x/2.))*snow(uv+snow(uv+snow(uv,5.),8.),30.)) );

	vec4 front=back*.98 - vec4(1.-float(fract(time)<0.5),1.+float(fract(time)<0.5),1.+float(fract(time)<0.5)*10.,1)/256. +  vec4(c);
	...
	position *= 4.;		
	position += vec2(-2.5, -2.);
	float mb = mandelbrot(position);
	// gl_FragColor = vec4(vec3(mb), 1.0);
	
	
	gl_FragColor = mb + colors * front + renderText();
}