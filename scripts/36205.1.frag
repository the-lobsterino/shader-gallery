...
// j avascript:window.open(get_img(5120, 2160));
...
vec2 snail( vec3 p, out vec4 matInfo )
{
    ...
    float dx = 0.9 + 0.5*(1.0-smoothstep(0.5,1.0,b1.y));
    d1 -= 0.005*dis*dx*0.5;
...
float mapLeafWaterDrops( in vec3 p )
{
    ...
    float d4 = sdEllipsoid( q, vec3(-0.9,0.09,-0.2), vec3(0.04,0.03,0.04) );
...
float calcSSS( in vec3 pos, in vec3 nor )
{
    ...
    for( int i=0; i<8; i++ )
    {
        ...
        vec3 dir = normalize( sin( float(i)*13.0 + vec3(90.0,2.1,4.2) ) );
...
vec3 shade( in vec3 ro, in vec3 rd, in float t, in float m, in vec4 matInfo )
{
	...
	else if( m<2.5 ) // shell
    {
        ...
        mateD *= vec3(0.06,0.03,0.0)+vec3(0.984,0.97,1.0)*bar;
