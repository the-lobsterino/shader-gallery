...
void main(){
  ...
  //unit
  col = mix(vec3(0.04,0.07,0.05),col,smoothstep(0.0,1.5/resolution.y,digit(size*uv-vec2(0.7,0),8,1)));
  col = mix(vec3(0.10,1.0,0.500),col,smoothstep(0.0,3.0/resolution.y,digit(size*uv-vec2(0.7,0),nUni,0)));
  //ten
  col = mix(vec3(0.04,0.07,0.05),col,smoothstep(0.0,1.5/resolution.y,digit(size*uv-vec2(0.0,0),8,1)));
  col = mix(vec3(0.10,1.0,0.500),col,smoothstep(0.0,3.0/resolution.y,digit(size*uv-vec2(0.0,0),nTen,0)));
  //hendred
  col = mix(vec3(0.04,0.07,0.05),col,smoothstep(0.0,1.5/resolution.y,digit(size*uv-vec2(-0.7,0),8,1)));
  col = mix(vec3(0.10,1.0,0.500),col,smoothstep(0.0,3.0/resolution.y,digit(size*uv-vec2(-0.7,0),nHun,0)));
