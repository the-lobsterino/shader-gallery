...

{
	...
	for (int i=0; i<RAY_STEPS; i++) {
		if (d.x>det && totdist<3.0) {
			p=from+totdist*dir;
			d=de(p);
			det=detail*(1.+totdist*20.)*(1.+ref*5.);
			totdist+=d.x; 
			energy=ENERGY_COLOR*(30.5+sin(time*20.+p.z*10.))*.5;
			if(d.x<0.0005)glow+=max(0.,.01-d.x)*exp(-totdist);
			if (d.y<.5 && d.x<0.03){//ONLY DOING THE GLOW WHEN IT IS CLOSE ENOUGH
				...
				(pow(max(0.,.05-glw)/.05,7.)
				+pow(max(0.,.15-abs(13.35-p.y))/1.15,8.))*1.5;
			}
		}
	}
