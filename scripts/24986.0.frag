...
// Added some siruptions! (still kloumpt)
...
vec2 next(vec2 p){
	vec2 offset = getOffset(p*.5*detailsScale)+getOffset(p*detailsScale)*.5+getOffset(p*2.*detailsScale)*.25+getOffset(p*4.*detailsScale)*.125+getOffset(p*8.*detailsScale)*.625;
	
	return mod(p+offset, vec2(1.));
}
...
void main( void ) {
	...
	vec4 density = vec4(texture2D(bb,next(p)));
	
	#ifdef GAMMA
	density = pow(clamp(density, vec4(0.), vec4(1.)), vec4(.5 / 1.0));
	#endif

	gl_FragColor = mix(vec4(cos(p.x*5.+2.*cos(time*2.*2.*PI))*.25+.75, cos(p.y*5.-2.*sin(time*1.*2.*PI))*.25+.75, cos(p.x*5.+p.y*5.)*.25+.75, 1.)*(.5+.75*random(p+vec2(random(vec2(time))))), density, .9);
	
	#ifdef MOUSE_TORCH
	float inTorch = smoothstep(0., 0.5, length((p-mouse)*resolution.xx/resolution.yx)-0.75);
