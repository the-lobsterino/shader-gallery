...
const float speed	= .0007;
const float density	= .0125;
const float shape	= .5;
...
float Cell(vec2 coord) {
    ...
    return (1. - length(cell * 2. - 1.)) * step(random(floor(coord)), density) * 5.;
}
