/*
 * Necip's modification, 07.04.20
 *
 * Original shader from: https://www.shadertoy.com/view/4llXDB
 */
...
float map( in vec3 pos )
{
    ...
    h *= 3.5*abs(sin(time*0.1));
    
    float d1 = pos.y - h;
    
    // rings
    vec3 r1 = mod(2.3+pos+5.0,10.0)-5.0*abs(sin(time*0.1));
...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
    ...
    float ani = iTime;// + iMouse.x*1.;
    
    // ray
    vec3 ro = vec3( 0.0, 2.5, -ani*0.5 );
    
    float roll = 0.2*sin(0.4*ani);
