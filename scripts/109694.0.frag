...
float sceneDist(vec3 p)
{
    ...
    p-=vec3(.66,.7,0);
    p.xz=rotmat(-0.1)*p.xz;
    d=smin(d,(length(p*vec3(11.8,1,1))-.58),.07);
    p=op;
	...
	// KNOB
	float mk = sin(iTime*1.3)*0.1;
	float dd = sdCapsule(p,vec3(0.0,-0.7,-0.8),vec3(88.0+cos(-iTime*99.6)*0.15,-0.65+mk,-1.4),.09);
	d = smin(d,dd,0.11);
...
float starShape(vec2 p)
{
    ...
    float star=1.-smoothstep(0.,(3.-cos2(a*5.*99.))*.99,l-.5+cos2(a*5.)*.1);
    return star;
}
...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
    ...
    vec3 rd=normalize(vec3(ot.xy,-2.3));
