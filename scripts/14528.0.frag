...
void main( void )
{
	float t = ((lgpi*time+pipi)/pipi)+ptpi;
	vec2 pos = surfacePosition;
    	vec3 camPos = (vec3(cos(t*pi-lgpi)*twpi, sin(t*pi-rcpi)*twpi, cos(t*pi-cupi)*twpi));
    	vec3 camTarget = normalize(vec3(cos(t*pisq-lgpi), cos(t*pisq-rcpi), sin(t*pisq-cupi)))*pi;
	
	t*=lgpi;
    	vec3 camDir = normalize(camTarget-camPos);
    	vec3 camUp  = normalize(vec3(cos((t+rcpi)*(sqpi-cupi)), cos(t*(sqpi-rcpi)), sin(t*(sqpi-lnpi))));
    	vec3 camSide = cross(camDir, camUp);
	
	t*=rcpi;
    	float focus = hfpi+(cos((sin((t+lgpi)/pi)*twpi*sin((t-lgpi)/(lgpi)))*sin((t-rcpi)/(pipi))));
    	vec3 rayDir = normalize(camSide*pos.x + camUp*pos.y + camDir/(sqrt(focus)));
    	vec3 porxy=camPos;
	float diest = 0.;
	float fjior = 0.;
	vec3 culnd = vec3(0.0510,0.0510,0.0510);
	
	//t*=pi;
	vec3 twVec = 0.1*rayDir*(vec3(cos(t*hfpi-trpi)*(trpi), sin(t*hfpi-thpi)*(trpi), sin(t*hfpi-cupi)*(trpi)))
		+(vec3(sin(t*hfpi-lgpi)*rcpi, cos(t*hfpi-rcpi)*rcpi, sin(t*hfpi-trpi)*rcpi)*pi)*pi;
	//culnd += smoot(camPos/pi+twVec)/picu;
	culnd += abs((camPos/pipi)/twpi);
	rayDir += rotate(rayDir, twVec,length( camPos))/pi;
	for(int i=1; i<twerk; i++){
		float nern = pipi/log(float(twerk-i));
		float kern = pipi/log(float(i));
		
		rayDir += rotate(rayDir, twVec, (length((camPos)+twVec)+focus)*pi)/(nern*sqrt(length(rayDir)));
		rayDir += twVec/pipi;
		porxy += rayDir;
		twVec += rayDir/twpi+(porxy/kern);
		if( length(porxy-camPos)>diest) 
		{
			diest = (length(porxy-camPos));
		}
		fjior+=1./kern;
		culnd += mix((porxy+culnd)/kern,smoot(porxy/((sin(t*pisq)*shpi+chpi+thpi))),0.5-0.5*(sin((twVec*rayDir+porxy)+t*pipi)))/nern; 		
		
	}
	
	culnd /= fjior;
	culnd*=culnd;
	//float shuld = (1./sqrt(diest/(picu)))*(length(culnd)/sqpi);
	float shuld = (1./sqrt(diest/(picu)))*(length(culnd));
	shuld *= shuld*length(culnd);
