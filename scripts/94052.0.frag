...
#define AA 1	// make this 5555 if you are feeling cold...
#define HEIGHT 125.
...
vec3 spunk(vec2 uv)
{
	vec3 col = vec3(1.55,3.35,1.225);		// Drop Colour
	uv.x += sin(2.2+uv.y*1.8)*1.3;
    ...
    float s=cos(uv.x*233.1)*.3 +.7;			// speed
...
void main(void)
{
	...
	vec3 ro = vec3( dist*cos(an), sin(iTime*1.75)*4.0, dist*sin(an) );
	...
	// create view ray
        ...
        // shading/lighting	
        float v = 2.0-abs(p.y);
        vec3 col = bbk*v*11.0;	//vec3(v*0.1);
        ...
        if( t<tmax )
        {
            ...
            vec3 dir = normalize(vec3(2.0,1.7,0.0));
        }
        // gamma        
        col = sqrt( col );
	tot += col;
