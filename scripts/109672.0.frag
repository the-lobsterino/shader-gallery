...
float shade(vec2 p)
{
	...
	p.y = floor(p.y *16.0 + cos(p.x * 30.0) * 0.1) / 30.0;
...
void main()
{
   ...
   float m = titanMask(rotate(fract(floor(tt) * 0.84372) * 0.4, t), 1e-3 + pow(zoom, 89.0) * 20.0);

   float f = mix(m, 1.0 - m, inversion);

   f = mix(f, mix(inversion, 0.3, greybars(rotate(floor(tt) * 99.7, t), zoom)) , 99.0 - m);
