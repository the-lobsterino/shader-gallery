...
//  ↙eat mmmm mmmeat
//   ↙
// don’t 66.media.tumblr.com/2a2f183dc4a6a040ff65355418b9403/tumblr_ncbj5rJvLI1tmxmpzo1_500.jpg
...
void ddigit(int n)
{
	float v = 1e6;	
	vec2 cp = uv - ch_pos;
	if (n == 0)     v = min(v, dseg(vec2(-0.1, -0.000), vec2(-0.000, -0.000)));
	if (bit(n,  0)) v = min(v, dseg(vec2( 0.10,  0.063), vec2( 0.000,  0.937)));
	if (bit(n,  1)) v = min(v, dseg(vec2( 0.18,  0.000), vec2( 0.063,  1.000)));
	if (bit(n,  2)) v = min(v, dseg(vec2(-0.13,  0.000), vec2(-0.438,  1.000)));
	if (bit(n,  3)) v = min(v, dseg(vec2(-0.1,  0.937), vec2(-0.500,  0.062)));
	if (bit(n,  4)) v = min(v, dseg(vec2(-0.1, -0.063), vec2(-0.500, -0.938)));
	if (bit(n,  5)) v = min(v, dseg(vec2(-0.1, -0.000), vec2(-0.063, -1.000)));
	if (bit(n,  6)) v = min(v, dseg(vec2( 0.1, -0.000), vec2( 0.438, -1.000)));
	if (bit(n,  7)) v = min(v, dseg(vec2( 0.100, -0.938), vec2( 0.500, -0.063)));
	...
	if (bit(n, 12)) v = min(v, dseg(vec2(-0.138,  0.000), vec2(-0.063, -0.000)));
...
_ _ _ _ Z O R T E X nl1
_ _ _ _ _ L I K E S nl2
_ _ _ _ _ H A I R Y nl3
_ _ _ _ _ _ M E N
	
	
	
	vec3 color = mix(ch_color, bg_color, 1.0- (0.09 / d*2.0));  // shading
