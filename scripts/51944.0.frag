...
#endif
/// Need help with a bird or a spaceship... ;-)    - Harley
uniform float time;
uniform vec2 mouse;
...
void main(void)
{
	...
	vec2 auv = (-1.0 + 2.0 * xy) * vec2(resolution.x/resolution.y,1.0);
	vec3 asd;
	bool istrue=true;
	if (istrue) {
	...
	for(int i = 0; i < iterations; i++){
		if((x*x+y*y) > 4.0){
			color = float(i)/float(iterations) * 2.0;
		break;	
		}

	auv.x += sin(x) - cos(y) - cos(r);
	auv.y -= sin(x) + cos(y) - sin(r);	
		r = x*x - y*y + uv.x;
		y = 2.0*x*y + uv.y;
		x=r;
...
asd = vec3(color);
	...
	float gTime = (time*2.0+m+2352.0)*.006;
//    vec2 xy = gl_FragCoord.xy / resolution.xy;
	vec2 uv = auv;
	        uv.x *= sin(asd.x) - cos(asd.y) - cos(asd.z);
        uv.y *= sin(asd.x) + cos(asd.y) - sin(asd.z);
