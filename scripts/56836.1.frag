...
vec3 GetSkyGradient( const in vec3 vDir )
{
	...
	return mix(vec3(0.0, 0.0, 0.0), vec3(0.4, 0.7, 1.0), fBlend);
}
vec3 GetLightPos()
{
	vec3 vLightPos = vec3(sin(time*3.)*20., 3.0, 3.0 + sin(time)*21.);
	#ifdef ENABLE_MONTE_CARLO        
...
vec3 GetLightCol()
{
	return vec3(24.0 + sin(time*1.1)*30., 14.0, 10.0) * 10.0 * abs(sin(time));
}
...
void ApplyAtmosphere(inout vec3 col, const in C_Ray ray, const in C_HitInfo intersection, const in vec3 vPos, const in vec3 vLightDir, const in float fLightDistance)
{
	#ifdef ENABLE_FOG
	// fog
	float fFogAmount = exp(intersection.fDistance * -kFogDensity * 1.0);
	vec3 cFog = GetSkyGradient(ray.vDir);
	float visibility = GetShadow(vPos, vLightDir, fLightDistance);
	col = mix(cFog, col, fFogAmount * visibility);
	#endif
...
{           
	...
	float fSpecPower = exp2(4.0 + 6.0 * mat.fSmoothness * 0.5);
	float fSpecIntensity = (fSpecPower + 2.0) * 0.525;
...
vec3 GetSceneColour( const in C_Ray ray, const in vec3 vPos, const in vec3 vLightDir, const in float fLightDistance )
{                                                           
	C_HitInfo intersection;
	...
	ApplyAtmosphere(cScene, ray, intersection, vPos, vLightDir, fLightDistance);
...
void main( void )
{
	...
	float fZoom = surfaceSize.y * 2.5 + 0.4;
	...
	vCameraPos += vec3(0.0, -fCameraHeight, uv.x*5.*sin(time*5.));
	#ifdef ENABLE_MONTE_CARLO             
	...
	vec3 vLightPos = GetLightPos();
	vec3 vToLight = vLightPos - vCameraPos;
	vec3 vLightDir = normalize(vToLight);
	float fLightDistance = length(vToLight);
       
	GetCameraRayLookat( vCameraPos, vec3(0.0, sin(time*3.+uv.x*7.), sin(uv.y+time)*9.), ray);
	
       
	vec3 cScene = GetSceneColour( ray,  vCameraPos, vLightDir, fLightDistance);   
