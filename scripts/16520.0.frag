...
float frequencyR = 700.0;
float frequencyG = 1000.0;
float frequencyB = 1.20;

void main()
{
	float x = gl_FragCoord.x;
	float y = gl_FragCoord.y;
	frequencyChange = frequencyChangeChange * (27000.0 * (y + time/100.0)) + frequencyChangeChange * (x / resolution.x);
