...
float sdf(in vec3 p) { //signed distance function - vorinoi inside a sphere
	...
	vec3 ppos = p;
	ppos.yx = smoothRot(ppos.yx,8.0,0.0,0.0,1.0);
	
	float xx = 0.5+sin(time*2.0)*0.5;
	xx*=0.05;
	float dd1 = sdBox(ppos,vec3(2.0,0.2+xx,0.2+xx));
...
vec3 getColor(in vec3 ro, in vec3 rd) {
    ...
    d0 = abs(1.0-d0);
    return vec3(1.2-d0,0.8-d0*0.5,0.6-d0*0.3);
}
