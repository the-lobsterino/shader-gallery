...
vec4 live = vec4(0.5,9.99,0.17,1.);
...
void main( void ) {
	...
	if (length(position - vec2(0.5 ,0.5)) < 0.1 + (0.05*sin(time * 15.))) {
		float rnd1 = mod(fract(sin(dot(position * sin(time * 1000.) * 0.1, vec2(14.9898,78.233))) * 43758.5453), 1.0);
		if (rnd1 > 0.5) {
			gl_FragColor = live;
		} else {
			gl_FragColor = blue;
		...
		if (me.g <= 0.1) {
			...
			if ((sum >= 1.00000007) && (sum <= 3.9999998)) {
				gl_FragColor = live + abs(sin((time)));
			} else {
				gl_FragColor = blue;
			}
		}
