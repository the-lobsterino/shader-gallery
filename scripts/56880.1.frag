...
// better bit function by xavierseb
...
const vec2 ch_start = vec2 (ch_space.x * -002., 2.); // start position
      vec2 ch_pos   = vec2 (0.0, 0.0);             // character position(X,Y)
      vec3 ch_color = vec3 (69.0, 69.5, 69.5);        // character color (R,G,B)
const vec3 bg_color = vec3 (.6, .0, .0);        // background color (R,G,B)

#define REPEAT_SIGN true // True/False; True=Multiple, False=Single
...
void ddigit(int n)
{
	float v = 1e6;	
	vec2 cp = uv - ch_pos;
	if (n == 0)         v = min(v, dseg(vec2(-0.705, -1.000), vec2(-1.00, -1.000)));
	...
	if (bit(n,  2048))  v = min(v, dseg(vec2( 0.000,  0.063), vec2( 0.000,  0.937)));
...
void main( void ) 
{
	...
	uv *=12.0 + sin(time);     //  set zoom size
	uv += vec2(1,-4);

nl1
         s_greater s_less _ s_greater s_less
nl2
	 C R I N G E
nl3
         _ s_greater s_less _ s_greater s_less
		
	vec3 color = mix(ch_color, bg_color, 2.0- (.29 / d));  // shading
