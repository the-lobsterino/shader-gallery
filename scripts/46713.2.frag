...
void main( void ) {
	...
	float m = mod(time, 5.0);
	vec3 c = vec3( 0, 0, 0 );
	// Decrease the 0.04 step size to make the curve look more filled.
	for( float xp = -1.0; xp < 1.0; xp += STEP_SIZE )
	{
		float t = xp / 1.0;
		float x = pulse( pos.x, xp, xp + STEP_SIZE/1.00 ); 
		float y = pulse( pos.y, -1.0, -1.0 + sin(fract( m + xp*t * 0.3)) * 2.00  );		
		c += vec3( 0, pos.y + y, pos.x + x ) * x * y;
		
		y = pulse( pos.y, 1.0, 1.0 - sin( m - xp*t ) * 2.00  );
