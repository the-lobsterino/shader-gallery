...
vec3 getNormal(vec3 p)
{
    ...
    return normalize(vec3(
        sdf(p + vec3(diff, 0, 0)) - sdf(p + vec3(-diff, 0, 0)),
...
float getLightness(vec3 v, vec3 n, vec3 l)
{
    ...
    // Phonnシェーディング
    float lightness = a + d * clamp(dot(l, n), 0.0, 1.0) + s * pow(clamp(dot(v, r),0.0,1.0), gloss);
...
void renderOne(vec3 cPos, vec3 rd, out vec3 boxColor, out float isHit)
{
    ...
    vec3 b = vec3(step(fract(rp.x), 0.5), step(fract(rp.y), 0.5), step(fract(rp.z), 0.5));
