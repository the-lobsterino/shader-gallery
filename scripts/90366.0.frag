...
const vec2 ch_size  = vec2(1.3, 2.5);              // character size (X,Y)
const vec2 ch_space = ch_size + vec2(1.0, 2.0);    // character distance Vector(X,Y)
const vec2 ch_start = vec2 (ch_space.x * -6., 2.); // start position
      vec2 ch_pos   = vec2 (0.0, 0.0);             // character position(X,Y)
      vec3 ch_color = vec3 (1.2, 0.4, 1.5);        // character color (R,G,B)
const vec3 bg_color = vec3 (0.2, 0.2, 2.6);        // background color (R,G,B)
...
void main( void ) 
{
	...
	ch_pos = ch_start + vec2(sin(time),0.0);  // set start position
         ...
         W W W _ T I K T O K _ C O M  nl1
	 _ _ M A X I M C L I E N T nl1
		
	vec3 color = mix(ch_color, bg_color, 0.7- (0.03 / d));  // shading
