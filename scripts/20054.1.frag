...
ray         emit(ray r, float t);
ray         view(in vec2 uv);
hit         map(in vec3 position, float t);
vec3        derive(in vec3 p, float t);

material     assign_material(in float material_index);
surface      shade(in ray r, in surface s,  in material m, in light l, float t);
...
float        ambient_occlusion(vec3 p, vec3 n, float t);
float        ashadow(vec3 p, vec3 d, float t);
...
void main( void ) 
{
    ...
    for(int i = 0; i < nSamples; i++) {
        float t = (time + (float(i) + rand(gl_FragCoord.xy))*blur)*speed;
        r = emit(r, t);
    
    
    
    if(r.material_range.object_id != 0. && r.material_range.distance < EPSILON)
    {        
        surface s = surface(vec4(0.), vec3(0.), 0.);
        s.color = result;
        s.range = distance(r.position, r.origin);
        s.normal = derive(r.position, t);

        material m = assign_material(r.material_range.object_id);
...
hit map(in vec3 position, float t)
{
    vec3 rotation = vec3(.09, .21, .14) * t;
    float k = kaleidoscopic_ifs(position, rotation);
    
    hit material_range = hit(0.,0.);
    
    material_range.object_id = 3.;
    material_range.distance = k;
...
hit map(in vec3 position, float t)
{
    ...
    material_range.object_id= s < c ? 1. : 2.;
    material_range.distance = min(c, s);
...
//// RENDERING
// emit rays to map the scene, stepping along the direction of the ray by the 
// of the nearest object until it hits or goes too far
ray emit(ray r, float t)
{
    float range = 0.;
    ...
    {
        r.material_range = map(r.position, t);
        
        if(r.material_range.distance < threshold || range > FARPLANE)
        {
            break;    
        }
            
        range        += r.material_range.distance * .95;
...
vec3 derive(in vec3 p, float t)
{
    ...
    vec3 normal = vec3(0.);
    normal.x = map(p+offset.yxx,t).distance-map(p-offset.yxx,t).distance;
    normal.y = map(p+offset.xyx,t).distance-map(p-offset.xyx,t).distance;
    normal.z = map(p+offset.xxy,t).distance-map(p-offset.xxy,t).distance;
...
surface shade(in ray r, in surface s,  in material m, in light l, float t)
{
    ...
    float occlusion = ambient_occlusion(r.position, s.normal,t);
        float shadow = ashadow(r.position, l.direction,t);
...
// we transform the position in the reverse order,
// with inverse operations as the individual cubes
float kaleidoscopic_ifs(vec3 position, vec3 rotation)
{
    float translation = 1.85;
    float dialation = 1.;
