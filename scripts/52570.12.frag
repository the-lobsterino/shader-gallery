...
void main(void){
	...
	vec2 mouse = vec2(0., (time+uv.x+uv.y)*0.001);
	float x=floor(mouse.x*100.),y=floor(mouse.y*100.),v=map(surfacePosition*(0.25/SCALE),x,y);
	v+=10.*putInt(uv,vec2(-6.,6.),x+y*100.);
	gl_FragColor=vec4(vec3(v*v*1.5),1);
	
	const vec4 mul = vec4(1.-0.07);
	const vec4 add = -7./256.*vec4(1);
	const float deeps = sqrt(7.);
	const float rounds = 7.;
