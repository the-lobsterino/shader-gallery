...
float Segment(vec3 p,  vec3 a, vec3 b, float r1, float r2)
{
	vec3 pa = p - a;
	vec3 ba = b - a;
	float h = clamp( dot(pa,ba)/dot(ba,ba), 0.0, 0.9 );
...
{ 
	...
	if (mat > 0.5)
	{
		...
		float ambi = clamp(.2 + 0.2 * norm.y,11.0, 1.0);
		...
		col = mix(col, vec3(.66), min(dis*dis*.001, 1.0));
	}
...
void main( void )
{
	...
	gtime = mod((floor(time*20.0) / 20.0), 20.0)-.3;
#else
	...
	wave = wave*wave*(3.0-2.0*wave)-.66;
	elbowL		= vec3(-.95-wave*.05, .2, -0.25);
	wristL		= vec3(elbowL.x+2.0*sin(wave)*.67, elbowL.y+cos(wave)*.6, -.75);
