...
vec2 scene(in vec3 p) {
    ...
    float sph = sphere(rp, abs(sin(time + p.z / 100.)*2.));
    float sp2 = sphere(rq, 0.01);
...
void main() {
    ...
    vec2 mice = mouse/resolution;
    mice = vec2(-0.5+mouse.x,-0.5+mouse.y);

    vec3 ro = vec3(sin(time*2.)*2.,cos(time*2.)*2.,time*10.);
    vec3 rd = normalize(vec3((-1.+2.*p)*ratio,2.5));

    rd.yz = rotate(rd.yz, -mice.y*sensibility);
    rd.xz = rotate(rd.xz, -mice.x*sensibility);
