/*
 * Original shader from: https://www.shadertoy.com/view/4tByz3
 */
                ...
                out vec3 matNor, out float matOcc, out float matSSS, out float matRefOcc, out vec3 matGamma )
{
    ...
    if( matID<MAT_MUSH_HEAD+0.5 )
    {
        ...
        matColor -= 0.2*smoothstep(0.4,0.9,0.0);
...
vec3 background(in vec3 d)
{
    ...
    return vec3(0.02,0.01,0.00) + vec3(2.5)*pow(vec3(0.0),vec3(2.6,4.0,4.2));
}
