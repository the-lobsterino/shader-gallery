...
const vec2 ch_start = vec2 (ch_space.x * -6., 6.); // start position
      vec2 ch_pos   = vec2 (0.0, 0.0);             // character position(X,Y)
      vec3 ch_color = vec3 (0.8, 0.5, 2.0);        // character color (R,G,B)
...
void main( void ) 
{
	...
	float _d = length(uv)*0.045;
	_d = _d*_d;
	vec3 bg = hsv2rgb_smooth(vec3(time*0.4+(uv.y+uv.x)*((_d+0.5)*0.05),1.2-_d,0.9-_d));

	
	ch_pos = ch_start + vec2(sin(time),0.0);  // set start position

       	_ K E S _ I S _ D E _ B E S T
		
		
	vec3 color = mix(ch_color, vec3(0.0) , 1.0- (0.08 / d));  // shading
