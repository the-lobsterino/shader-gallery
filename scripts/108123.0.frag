...
vec2 obj0(in vec3 p){
  ...
  float c1=length(max(abs(p)-vec3(1.2,110.2,0.2),0.0));
...
vec2 obj1(vec3 p){
  ...
  p.z=p.z-4.0;
  p.xy=rotsim(p.xy,2.0);
  float c1=length(max(abs(p)-vec3(1.2,0.2,0.2),0.0));
...
void main(void){
  ...
  else gl_FragColor=vec4(1,0,0,1); //background color
}