...
{
	return rand01(time)*smoothstep(0.0,1.0,1.0-length(touch-fc)/s);
}
...
bool cMax(vec3 cref,vec3 ccomp)
{
	...
	return dot(vec3(1.0,1.0,1.0),cross(cref,ccomp))>0.0;
}

void main(void) {
	vec4 fc = vec4(gl_FragCoord.xy,gl_FragCoord.xy/resolution.xy);
	
	vec2 pq = q(fc.xy) + vec2(0.5,0.5);
     vec3 c[5];
	c[0] =texture2D(backbuffer,pq/resolution.xy).xyz ;
	c[1] =texture2D(backbuffer,(pq+vec2(GS,0.0))/resolution.xy).xyz ;//right
	c[3] =texture2D(backbuffer,(pq+vec2(-GS,0.0))/resolution.xy).xyz ;//left
	c[2] =texture2D(backbuffer,(pq+vec2(0.0,GS))/resolution.xy).xyz ;//up
	c[4] =texture2D(backbuffer,(pq+vec2(0.0,-GS))/resolution.xy).xyz ;//down
