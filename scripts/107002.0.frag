...
void main( void ) {
	...
	vec3 color = mix(vec3(1.0, .7, 1.6), vec3(0.1, 0.1, 0.2), pow(a, 0.2)) * 0.75;
	color += mix(vec3(.8, 1.4, 1.3), vec3(0.1, 0.1, 0.2),  pow(b, 0.1)) * 0.75;
	color += mix(vec3(.3, 1.5, 1.3), vec3(0.1, 0.2, 0.2),  pow(c, 0.1)) * 0.75;
