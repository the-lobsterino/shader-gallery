...
{
    //pos.zx = smoothRot(pos.xz,32.0,0.2,1.0,7.5);
    vec3 a = vec3(-6.5,0.0,0.0);
    vec3 b = vec3(0.0,sin(iTime*1.1)*6.0,(0.5+sin(iTime*2.8)*0.5)*5.0);
    vec3 c = vec3(6.5,0.0,0.0);
    vec4 bz = sdBezierExtrude(pos,a,b,c);
    float tw = (0.5+sin(iTime*1.55)*0.5)*4.0;
    bz.xy *= rot( (3.14*tw) * bz.w);
    float d =  sdBox(bz.xyz, vec3(1.0+(bz.w*0.5),0.01+(bz.w*0.25),0.01))-0.15;
    return  min(length(bz.xyz)-(0.3+(bz.w*0.5)),d)*0.5;
}
...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
     // cam
    float an = 0.5*iTime;
    vec3 ro = vec3( 18.*cos(an), sin(iTime*1.5)*15.0, 18.0*sin(an) );
    ...
    for( int i=0; i<512; i++ )
    {
     ...
     if( t<tmax )
     {
         ...
         float sha = map(pos+rf) + .65;
         float factor = sha*length(sin(rf*3.)*0.5+0.5)/sqrt(2.);
         col = mix(vec3(0.15,0.05,0.26), vec3(0.28,0.6,0.2), factor) + pow(factor*0.8, 6.);
     }
