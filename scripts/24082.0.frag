...
vec3 Lighting( vec3 camPos, vec3 pos, vec3 normal, vec3 diffuseColor )
{
	...
	return lightAmbientColor + (lightDiffuseIntensity * diffuseColor) + (lightSpecularIntensity * lightSpecularColor * lightDiffuseIntensity);
...
vec3 RenderScene( vec2 uv )
{
	...
	//vec3 camPos = vec3( sin(time) * 3.0, 0.0, cos(time) * 3.0);
