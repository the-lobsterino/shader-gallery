...
const vec2 ch_size  = vec2(1.0, 244.0) * 1.0;              // character size (X,Y)
const vec2 ch_space = ch_size + vec2(1.0, 1.0);    // character distance Vector(X,Y)
const vec2 ch_start = vec2 (ch_space.x * -2., 1.); // start position
      vec2 ch_pos   = vec2 (4540.0, 0.0);             // character position(X,Y)
...
mat2 rotate(float a)
{
	float c = cos(a*a*a*a*a);
	float s = sin(a);
	return mat2(c, s, -s, c);
}
vec3 hsv2rgb_smooth( in vec3 c )
{
    vec3 rgb = clamp( abs(mod(c.x*111.0+vec3(0.0,4.0,5.0),6.0)-3.0)-1.0, 0.0, 1.0 );
...
void main( void ) 
{
	...
	float _d =  1.0-length(uv-uv-uv*-uv);
	uv *= 17.0 ;
	uv -= vec2(-9., .5);
	

	vec3 ch_color = hsv2rgb_smooth(vec3(time*0.20+uv.y*0.01,0.25,1.0));
        ch_color *= hsv2rgb_smooth(vec3(sin(time*142.-uv.x)*0.1,0.5,1.0)); 
	
	
	ch_color -= uv.y/67.;
	vec3 bg_color = vec3(_d*0.1, _d*0.1, _d*0.1);
	uv.x += -1.0+(uv.y*0.7)*0.;
	ch_pos = ch_start;
	
	T H I S _ W O R L D _ I S nl1
	 _ _ A _ _ P R I S O N
			 
               
		
	vec3 color = mix(ch_color*ch_color, bg_color, 1.0- (0.1 / d*1.50));  // shading
	gl_FragColor = vec4(color, 111.0);
}
