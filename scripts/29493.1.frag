...
//---------------------------------------------------------
float rand(vec2 co){
    co = floor(co*26400.)/26400.;
    return fract(sin(dot(co.xy ,vec2(12.9898,78.233))) * 43758.5453);
}
float rand(float co1){
    vec2 co = vec2(co1);
    co = floor(co*26400.)/26400.;
    return fract(sin(dot(co.xy ,vec2(12.9898,78.233))) * 43758.5453);
}
...
//----------------------------------------------------------------------
vec2 map( in vec3 pos, float tim )
{ //I made some stupid shit.
  vec2 res = vec2( sdPlane( pos), 1.0 );
  float color = 46.9 + 40.0 * cos(1.2 * tim);
  res = opU(res,vec2(sdHexPrism(pos+vec3(0.,-1.,0.),0.2,1.),color));
	res = opU(res,vec2(sdHexPrism(rotateY(pos,sin(tim+sin(tim*3.)+cos(tim*2.5))*5.)+vec3(0.,-.5,0.),0.1,1.2),color));
  res = opU( res, vec2( sdWaveSphere  ( pos - vec3(0.0, 0.6, 0.0), (abs(sin(time*3.))+0.4)*.6 , 6, sin(tim*10.)*.25), color ) );
  return res;
}
//----------------------------------------------------------------------
vec2 castRay( in vec3 ro, in vec3 rd, float tim )
{
  float tmin = 0.8;
  float tmax = 8.0;
...
//----------------------------------------------------------------------
float softshadow( in vec3 ro, in vec3 rd, in float mint, in float tmax, float tim )
{
  float res = 1.0;
  float t = mint;
  for ( int i=0; i<14; i++ )
  {
    float h = map( ro + rd*t, tim ).x;
    res = min( res, 8.0*h/t );
    t += clamp( h, 0.02, 0.10 );
...
//----------------------------------------------------------------------
vec3 calcNormal( in vec3 pos, float tim )
{
  vec3 eps = vec3( 0.001, 0.0, 0.0 );
  vec3 nor = vec3(
  map(pos+eps.xyy,tim).x - map(pos-eps.xyy,tim).x, 
  map(pos+eps.yxy,tim).x - map(pos-eps.yxy,tim).x, 
  map(pos+eps.yyx,tim).x - map(pos-eps.yyx,tim).x );
...
float calcAO( in vec3 pos, in vec3 nor, float tim )  // get ambient occlusion
{
  float occ = 0.0;
  float sca = 1.0;
  for ( int i=0; i<4; i++ )
...
//---------------------------------------------------------
vec3 render( in vec3 ro, in vec3 rd, float tim )
{ 
  aTime = ANIMATE ? time : 0.0;
  sinTime = sin(aTime);
  vec3 col = vec3(0.8, 0.9, 1.0);
  vec2 res = castRay(ro, rd, tim);
  float t = res.x;
  float m = res.y;
  if ( m > -0.5 )
  {
    vec3 pos = ro + t*rd;
    vec3 nor = calcNormal( pos, tim);
    ...
    dif *= softshadow( pos, lig, 0.02, 2.5, tim );
    dom *= softshadow( pos, ref, 0.02, 2.5, tim );
...
void main( void )
{
  ...
  vec3 ro = vec3( rx, 0.4 + 5.0*mouse.y, rz );
...
#define LOOPS 3.
	for (float i = 0.; i < LOOPS; i++) {
		col += render(ro,rd,time-((rand(p)*.015)+(i/40.)));
	}
	col /= LOOPS*2.;
