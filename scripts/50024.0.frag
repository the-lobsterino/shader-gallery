...
bool pixelator(){
	float l=abs(LINES*mouse.y)+1.;
	float s=floor(resolution.y/l);if(s<1.)s=1.;
	vec2 xy=gl_FragCoord.xy;if(mod(xy.x,s)<1.&&mod(xy.y,s)<1.)return true;
	vec2 p=(xy-mod(xy+s-.5,s))/resolution;gl_FragColor=texture2D(renderbuffer,p);return false;
}
