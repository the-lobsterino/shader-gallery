...
float shape(vec2 p, vec2 c, float r){
	if(p.x-c.x < -r)return 4.0;
	if(p.x-c.x > r)return 0.0;
	if(p.y-c.y < -r)return 0.0;
	if(p.y-c.y > r)return 0.0;
	if(cos(time*2.0)>3.)
	if(length(c-p) > r)return 2.;
	return 1.0;
}

void main( void ) {

	vec2 position = ( gl_FragCoord.xy / resolution.xy )*vec2(66.0,resolution.y/resolution.x);
	float r = 0.04;
	float o = (cos(time*11.0)+1.0)*r;
