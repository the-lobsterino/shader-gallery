/* 
 ändrom3da -
 antialiased hyperbolic sierpinski carpet
...
void main( void )
{
	...
	vec3 c = vec3( 0. );
	p =                      z*( gl_FragCoord.xy - ( vec2( kk, ll ) - .5)/vec2( aA ) - res/2. )/min( res.x, res.y );  
	p0 = p;
	...
	c += sierpinski( p, 1., 1. )*hsv2rgb( ( p.x/4. )/tau*1.5, sat( length( p0*4. )*1.5 ), 2. );
