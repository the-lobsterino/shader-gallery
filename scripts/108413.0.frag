...
{
	float h = clamp( dot(p,a)/dot(a,a), 0.0, 5.0 );
...
{	// Floor...
	...
	res.x = sMin(res.x, d);
	d = RoundBox(p-vec3(0.0, .25, 0.0), vec3(.67, .09, -.05), .09);
	res.x = sMin(res.x, d);
...
void main( void )
{
	...
	hello =  (1.0+sin(m*100.0)) * .02 * (smoothstep(8.0, .01, m) - smoothstep(0.05, .1, m));
	...
	vec3 target = mix(vec3(0.0, 1.0, 4.0),  vec3( 8.0, -.4, 0.0), t);
