// author --> cskonopka
// title ---> "blueeye4PanelJunction"
// format --> GLSL Sandbox
// link ----> http://glslsandbox.com/e#67914.1
...
void main() {
    ...
    float pct3 = 0.0;
    pct3 = distance(st*-3.2,vec2(-0.15));

    float y3 = (sin((-st.y)*(.20)))-sin(-pct2*(3.12+3.12));
    float y = smoothstep(0.32,2.5,st.x) - smoothstep(1.5,0.191,(st.x*-(cos(-.12))));
    float y2 = smoothstep(2.2,1.5,st.x) - smoothstep(-0.21,0.18,-2.92*(-st.y*1.0));
    vec3 colorA = vec3(y*(y2*9.2))+(y2*(0.013));
    colorA = (1.0)*colorA*vec3(0.130,0.19,(0.0+(sin(y3+(-10.2)))));
    ...
    colorB = (1.0)*colorB*vec3(0.2*-PI,(sin(1.0*-3.2)),(-0.20+y6)-(cos(pct3-(time*1.13))));
