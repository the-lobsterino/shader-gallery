...
void main( void )
{
	...
	// Wave...
	wave = sin(time*15.0-.8)*.5+.66;
	wave = wave*wave*(3.0-2.0*wave)-.99;
	elbowL		= vec3(-.95-wave*.05, .2, -0.25);
	wristL		= vec3(elbowL.x+1.0*sin(wave)*.6, elbowL.y+cos(wave)*.1, -.75);

    vec3 col = Render(origin, ray);
	
	col = sqrt(col);
	col *= 0.5 + 0.5*pow( 16.0*q.x*q.y*(1.0-q.x)*(1.0-q.y), 9.25 );
