...
vec3 getColors( vec2 position ) {
	vec3 res = vec3(cos(position.x), sin(position.y), 1. - 0.5 * cos(time));
	return res;
}
...
float getCheckerboardColor( vec2 position ) {
	...
	if( mod( ypos, 2.0 ) > 0.0 ) {
		col = cos(xpos*ypos + time * 5.);
	} else {
		col = sin(xpos*ypos + time * 2.);
...
vec3 shader(vec2 pos) {
...
#ifdef WOBBLE
	// Wobble effect
	position.y += 0.2 * cos(position.x /2. + time * 0.37);
#endif
	...
	v = factor*(1./abs(position.y)) + (1.-factor)*(2./r);
...
#endif	
#ifdef PATCH_MOTIF
	color *= cos( p.x * cos( time / 8.0 ) * 5. ) + cos( p.y * cos( time / 85.0 ) * 3.);
	color *= cos( p.x * cos( time / 15.0 ) * 7. ) - cos( p.y * cos( time / 125.0 ) * 4.);
#endif
#ifdef BLOB_MOTIF
	float col = 0.0;
	for(float i = 0.0; i < 5.; i++) 
	{
		...
		col += cos(TAU*(p.y * cos(a) + p.x * sin(a) /*+ mouse.y +i*mouse.x*/ + sin(time*0.004)*100.0 ));
	}
	col /= 3.0;
	color *= vec3(col);
#endif	
#ifdef STRIPE_MOTIF
	color *= float(mod(p.x+p.y + time, 2.0) > 1.);
#endif
...
void main( void ) {
	...
	vec3 color;
#ifdef RGB_DISTORSION
	color = vec3(shader(gl_FragCoord.xy + vec2(RGB_D_VALUE, 1.)).r, shader(gl_FragCoord.xy + vec2(0.,-RGB_D_VALUE)).g, shader(gl_FragCoord.xy + vec2(0.,RGB_D_VALUE)).b);
#elif defined THREE_D
