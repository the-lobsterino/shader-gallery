...
float displacement(vec3 p){p=floor(p);return hash(p.x*59.+p.y*61.+p.z*67.);}
...
float map(vec3 p){p=rotX(p,.8);p=rotY(p,-time*.4);float h=hash(floor(p.x)+floor(p.z)*133.3),o=0.;
	vec3 q=fract(p)-.5;q.y=p.y;q=rotX(q,time*(h+1.));q=rotY(q,time+h*3.);o=h<.3?sdTorus(q):sdRoundBox(q);
	return min(sdGround(p)+displacement(p*150.)*.015,o+displacement(q*150.)*.015);}

float trace(vec3 ro,vec3 rd,out float c,out vec3 n){float t=NEAR,d;vec3 p;vec2 e=vec2(.6,-.6);
...
void main(void){
	...
	float c=0.,v=trace(vec3(sin(time)*.003,sin(time*.8)*.01,sin(time*.3)*.5-1.),vec3(uv,.6),c,n)*.3;c/=float(ITER);
	float s=pow(clamp(dot(n,vec3(0.,0.,-1.)),0.,1.),200.);
	gl_FragColor=vec4(mix(vec3(1),hsv(v*.2+time*.05,.1,c*v),c)+s,1);
}