...
void main( void ) {
    ...
    vec3 sized=vec3(10);
    vec3 foreground=vec3(14);
    vec3 fracted_normalized_dot=vec3(8);
    vec3 camera=vec3(0); 
    vec3 background=normalized_dot;
    vec3 light=vec3(1.0,2.5,0.0);
    camera.y = 1.3*cos((camera.x=0.99)*(camera.z=time * 9.10));
    ...
    sized = floor( foreground )*.4;
        if( cos(sized.z) + sin(sized.x) > ++sized.y ) {
            background = (fracted_normalized_dot.y-.091*cos((foreground.x+foreground.z)*40.)>.5?light:fracted_normalized_dot.x*light.yxz) / depth;
            break;
        }
    }
