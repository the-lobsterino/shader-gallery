...
#define OVERSAMPLE 0
#define MIDDLE 0
#define ANIMATE_MIDDLE 2
#define DISTORTION 1
#define SQUISH_COORDINATES 0
#define SQUISH_COLOR 1
...
#define time 1.0
#elif SLOW
#define time (mod((time * (SLOW_FACTOR)),TAU))
#endif
...
#elif 1
	return vec3( dot(p3,p3) );
#elif 0
	vec2 a = uv/surfaceSize;
	return vec3( mod(q,a.x), q/a.x, q ) / (a.x*a.y);
#elif 0
	return vec3(uv,fn((uv)));
#elif 0
	return vec3( rotate(uv,q2*TAU), q2 );
#else
	return vec3(uv,q2);
#endif
...
//	colorC = fract(colorC);
//	colorC = fwidth(colorC);
	
#if 0
   vec4 o;
   FabriceNeyret2_mainImage( o, colorC.xy*nfc );
