...
float scene_2D( vec2 p )
{
	...
	float zoom = 1.0; zoom = pow(2.*( ( 1.0 - (m.x) ) ), 16.0);
    p *= zoom;
    ...
    {
        float im = ( ( ( 1.0-m.x ) )*1.);
	float iter = float(i) + im;
	float color = cos( float( iter )*pi );  // is flipping between -1. and +1.
	size = vec2( 1.0, 1.0 )*pow( 2.0, float( -iter ) );
