...
vec3 obj_floor_c(in vec3 p){
 return mix(vec3(0, 1, 0), vec3(0.5, 0.25, 0.1), fbm(p.xz * 0.2)) * 0.6 + fbm(p.xz * 8.0 + fbm(p.zx * 2.0)) * 0.4;
}
...
void main(void){
  ...
  const float MAX_DEPTH=200.0; //Max depth
  ...
  if (f<MAX_DEPTH){
    ...
    gl_FragColor=vec4(b*c*(1.0-f*.005),1.0);//simple phong LightPosition=CameraPosition
  }
