...
float r(in float z, in float x) {
    ...
    return /* body */.5 * (1.0 - .3 * ex(z, .8, .15) +
		s * s + .5 * ex(z, .8, .25) * c * c + .3 * c) *
		0.5 * (1. + t4z) + /* legs */ (1. - .3 * ex(z, -1.0, .8)) *
		0.5 * (1. - t4z) * (.5 * (1. + s * s + .3 * c) *
		(pow(abs(s), 8.0) + .08 * (1. + t4z) ) ) +
		/* improve butt */ 2.1 * box(c2, -.45, .45, 5., 5.) *
...
{
	/* shift butt belly */
	float dx = .1 * exp(-pow((p.z-.8),2.)/.5) - .15 * exp(-pow((p.z -.1),2.)/.5);
	...
	dx *= 3.+(sin(jiggle+iTime*9.50)*jsize);
...
//	float d1 = (.5 + .5 * smoothstep(.4,1.,p.z)) * (length(vec2(p.y, p.x - dx)) - r_expected);
	...
	float d4 = sdSphere(p+vec3(-0.30,0.0,-0.8),0.60);
	d1 = smin(d1,d4,0.5);
	...
	p.x -= dx;
	p.y = abs(p.y);
	float d2 = sdSphere(p+vec3(-0.400,-0.5,-1.8),0.5);
	float d3 = sdSphere(p+vec3(-0.85,-0.4,-1.875),0.08);
	
    d2 = smin(d2,d3,0.0300);
...
{
     // camera movement	
	float an = sin(iTime+2.00)*0.8;	//0.5*iTime - 0.8;
    // if (iMouse.z>0.5)
    ...
    vec2 p1 = (-iResolution.xy + 1.0*fragCoord)/iResolution.y;
    float val = sin(iTime+p1.x+p1.y*20.0)*(0.8+sin(p1.y*p1.x*2.5+iTime*3.0)*0.3);
    val = clamp(val+0.250,0.0,3.0);
    ...
    for( int n=0; n<AA; n++ )
    {
        ...
        if (abs(p.x)<0.8)
        {
            ...
            if( t<tmax )
            {
                ...
                float dif = clamp( dot(nor,vec3(0.8,0.5,0.5)), 0.0, 1.0 );
                float amb = 0.5 + 0.5*dot(nor,vec3(0.0,0.8,0.5));
                col = vec3(0.3,0.15,0.1)*amb + vec3(0.8,0.5,0.3)*dif;
		    col += pow(dif,13.0);
            }
        }
