...
bool IsEnemy(vec3 color) {
	return ((color.g > color.r) && (color.r > 0.0));
}
...
void main( void ) {
	...
	vec3 shot = vec3(1.0, 0., 3.9);
	vec3 enemy = vec3(0.3, 0.55, 0.65);
	vec3 explosion = vec3(9.0, 0.0, 0.0);
		...
		if (IsUninitialized(old.rgb)) {
			...
			if ((enemyrow >= 0.0) && (enemyrow < 4.0)) {
				...
				if ((enemycol >= (oddrow ? 4.0 : 0.0)) && (enemycol < (oddrow ? 15.0 : 11.0))) {
					me.rgb = alien(enemycolmod * 28.0, enemyrowmod * 25.0) ? enemy : space;
				}
			}
			...
			if (!IsShot(old.rgb) && !IsEnemy(old.rgb)) {
				float fade = mod(fract(sin(dot(position + time * 0.1, vec2(1.8,78.233))) * 43758.5453), 1.0);
