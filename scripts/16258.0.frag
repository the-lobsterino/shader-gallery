...
void main()
{
	...
	float coord = floor(p.x)*127.0+floor(p.z);
	float rval = cos(coord);
	float red = fract(17.0*rval);
	float green = fract(23.0*rval);
	float blue = fract(31.0*rval);
    gl_FragColor = color * vec4(red, green, blue, 1.0);
}