...
vec3 transform(in vec3 p) {
    ...
    float t = (fn(p.xy) * dot(p,p) * dot(surfacePosition,surfacePosition)) / a;
...
void main(void)
{
  ...
  torus_r = resolution.x/resolution.y;//M/N;
  ...
  #define AA 2
       ...
       vec2 uv = surfacePosition.xy*((2.0*(gl_FragCoord.xy) - resolution.xy)/resolution.y);
  ...
  for (int i = -AA; i < AA; i++) {
     for (int j = -AA; j < AA; j++) {
       vec2 uv = surfacePosition.xy*((2.0*(gl_FragCoord.xy+vec2(i,j)/float(AA*AA)) - resolution.xy)/resolution.y);
       vec3 r = normalize(vec3(uv, 2.0));
       r = transform(r);
       r = normalize(r);
       color += scene(p,r);
