...
void main( void ) {
	...
	vec3 normal = normalize(vec3(tan(position.x * PI), tan(position.y * PI), CORNER)); //ADD N0RMAL U FUKIN FAGIT
	float bright = dot(normal, normalize(light)); //SHIT DOESN'T KNOW NORMAL IF IT'S GONE
	bright = pow(bright, 1.);
