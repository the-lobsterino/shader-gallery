...
vec3 s(vec3 o,vec3 d){
	...
	for(int i=0;i<2550;i++){
		if(f(o+d*t)<0.005){
			...
			vec3 e=vec3(.2,.2,.2),p=o+d*t,n=-normalize(vec3(f(p+e),f(p-e.yxy),f(p-e.yyx))+vec3((sin(p*3.14)))*10.0);
			return vec3(mix( ((max(+dot(n,vec3(1.0)),-1.) + 0.125*max(-dot(n,vec3(.3,3.3,0)),1.)))*(mod(length(p.xy)*.2,1.)<1.0?vec3(.1,.3,.5):vec3(.2,.8,.1)),vec3(1.,.1,.1),vec3(pow(t/55.,1.))));
		}
		t+=.23
	;
	}
...
void main(){
	...
	vec4 color=vec4(s(vec3(sin(u*.1)*.1,sin(time)*.2,time), normalize(vec3((gl_FragCoord.xy-vec2(resolution.xy/2.))/vec2(resolution.x),.1))),1);
gl_FragColor=color;
