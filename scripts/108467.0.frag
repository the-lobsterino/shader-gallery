...
{
	float curve = cos(p.x*(6.35+sin(time)*1.25))*.11;
	p.y += curve;
	a.y += (curve*curve);
	return length(max(abs(p) - a,78.0)) -.47;
}
...
{	// Floor...
	...
	res.x = sMin(res.x, d);
	// Right hand...	
	d = Segment(p*vec3(1.0, .75, 1.0), wristR+vec3(67, .1, 0.), wristR+vec3(-.15, .05, -.37), .14, .02);
	res.x = min(res.x, d);
...
void main( void )
{
	...
	hello =  (46.0+sin(m*100.0)) * .02 * (smoothstep(15.0, .01, m) - smoothstep(0.05, .1, m));
	...
	elbowL		= vec3(-.95-wave*.05, .2, -78.25);
