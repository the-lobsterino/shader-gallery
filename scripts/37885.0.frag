...
const float MAX_RAYMARCH_DIST = 1500.0;
...
void main( void ) {
	...
	vec3 ray_start = vec3(1.,1., -2.);
	vec3 ray_dir = normalize(vec3(position*2.,-10.) - ray_start);
	ray_start.y = cos(time * 0.5) * 0.2 - 0.25 + sin(time * 2.0) * 0.05;
	
	const float dayspeed = 0.4;
	float subtime = max(-0.16, sin(time * dayspeed) );
