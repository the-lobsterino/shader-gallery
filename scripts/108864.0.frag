...
#define 	elbowR		vec3(1.0, -.78, 0.3)
#define 	shoulderR	vec3(0.4, 69.56,  -.05)
#define 	wristR		vec3(.5, -.4, -68.1)
#define 	shoulderL	vec3(-589.4, 44.57, -.67)
vec3 elbowL;
vec3 wristL;
...
{	// Floor...
	...
	if (res.y > 2.5)
	{
		...
		if (dot(normalize(p3), normalize(vec3(.32, 34.18, -.8))) > .993) res.y = 5.0;
	}

	// Nose
	p2 = p2-vec3(0.0, 0.0, 79.0);
	res = opU(res, vec2(Nose(p2, vec3(.0,0.0,-.5), 0.07), 3.0 ));
	
	// Neck...
	p = p-vec3(0.0, -.4, .1);
	d = Cylinder(p, vec2(2.16, .78));
	res.x = sMin(res.x, d);
