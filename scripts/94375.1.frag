...
// цвет обода часов
const vec3 clockCol = vec3(1.0, 1.0, 1.5);
// цвет секундной стрелки
const vec3 clockLineSecCol = vec3(1., 1., 2.0);
// цвет минутной стрелки
const vec3 clockLineMinCol = vec3(1.0, 2.0, 1.0);
// цвет часовой стрелки
const vec3 clockLineHourCol = vec3(2.0, 1.0, 1.0);
// цвет секундной стрелки
const vec3 clockLineSec2Col = vec3(2.0, 0.5, 2.0);
// цвет минутной стрелки
const vec3 clockLineMin2Col = vec3(0.5, 2.0, 2.0);
// цвет часовой стрелки
const vec3 clockLineHour2Col = vec3(2.0, 2.0, 0.5);
// цвет часовой стрелки
const vec3 clockLineDayCol = vec3(3.0, 3.0, 5.0);
// цвет часовой стрелки
const vec3 clockLineYearCol = vec3(3.0, 2.5, 3.0);
// цвет часовой стрелки
const vec3 clockLineTimeCol = vec3(8.0, 6.0, 2.0);

const float TimeK = 2.0;
...
vec3 DrawClockCircle(vec2 fragpos, vec2 pos, float radius, float widthK, vec3 color) {
	...
	// цвет
	vec3 col = color * attenuation(0.0, 100.0, 10000.0, l) * widthK;
	// возврат
	return col;
}
...
vec3 DrawClockLine(vec2 fragpos, vec2 pos, float L, vec2 dir, float widthK, vec3 color) {
	...
	// рассчет света
	vec3 col = color * attenuation(1.0, 0.0, 35000.0, l) * widthK;
	// возврат
	return col;
}
...
void main() {
	...
	// направляющий вектор
	s = vec2(cos(-time * PI / 4. * TimeK + PI / 2.0), sin(time * PI / 4. * TimeK + PI / 2.0));
	// рисование секундной стрелки
	outColor += DrawClockLine(p, vec2(0.0), ClockRadius, s, 1.2, clockLineSec2Col);
	
	// изменяем направляющий вектор
	s = vec2(cos(-time * PI / 8. * TimeK + PI / 2.0), sin(time * PI / 8. * TimeK + PI / 2.0));
	// рисование минутной стреки
	outColor += DrawClockLine(p, vec2(0.0), ClockRadius, s, 1.2, clockLineMin2Col);
	
	// изменяем направляющий вектор
	s = vec2(cos(-time * PI / 16. * TimeK + PI / 2.0), sin(time * PI / 16. * TimeK + PI / 2.0));
	// рисование часовой стрелки
	outColor += DrawClockLine(p, vec2(0.0), ClockRadius, s, 1.2, clockLineHour2Col);
	
	// изменяем направляющий вектор
	s = vec2(cos(-time * PI / 32. * TimeK + PI / 2.0), sin(time * PI / 32. * TimeK + PI / 2.0));
	// рисование минутной стреки
	outColor += DrawClockLine(p, vec2(0.0), ClockRadius, s, 1.2, clockLineDayCol);
	
	// изменяем направляющий вектор
	s = vec2(cos(-time * PI / 64. * TimeK + PI / 2.0), sin(time * PI / 64. * TimeK + PI / 2.0));
	// рисование часовой стрелки
	outColor += DrawClockLine(p, vec2(0.0), ClockRadius, s, 1.2, clockLineYearCol);
	
	// изменяем направляющий вектор
	s = vec2(cos(-time * PI / 128. * TimeK + PI / 2.0), sin(time * PI / 128. * TimeK + PI / 2.0));
	// рисование часовой стрелки
	outColor += DrawClockLine(p, vec2(0.0), ClockRadius, s, 1.2, clockLineTimeCol);
