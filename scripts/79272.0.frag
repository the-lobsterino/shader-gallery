...
void main( void ) 
{
	...
	uv = floor(128.*( gl_FragCoord.xy / resolution.y ) - aspect / 2.0)/128.0;
	float _d =  1.0-length(uv);
	uv *= 8.0 ;
	uv -= vec2(1., 5.);
	 

	vec3 ch_color = hsv2rgb_smooth(vec3(time*0.2+uv.y*0.1,0.9,1.0));

	vec3 bg_color = vec3(_d*0.4, _d*0.2, _d*0.1);
	uv.x += 0.5+sin(time+uv.y*0.9)*0.5;
	uv.x+=-9.;
	ch_pos = ch_start;
	       ...
	       _ B E N I S
		       ...
		       uv += vec2(11., 3.);;uv.x += 0.5+sin(time-uv.y*.4)*1.5;_ _ n2 n0 n2 n2
