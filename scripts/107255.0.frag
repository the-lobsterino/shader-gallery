...
mat4 rotationMatrix(vec3 axis, float angle) {
    ...
    return mat4(oc * axis.x * axis.x + c,           oc * axis.y * axis.z - axis.z * s,  oc * axis.z * axis.x + axis.y * s,  0.0,
                oc * axis.x * axis.y + axis.z * s,  oc * axis.y * axis.z + c,           oc * axis.y * axis.z - axis.x * s,  0.0,
...
void main( void ) {
	...
	if(hit){
		...
		float diff = clamp(1.3,7.8,dot(light,n));
		color = vec3(diff);
	}
	
	vec3 backgoundColor = vec3(0.9, 0.0, 0.9);
	
	
	color = mix(backgoundColor, color, exp(-0.0*rayLen));
