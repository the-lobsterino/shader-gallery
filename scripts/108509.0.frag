...
float Mouth( vec3 p, vec3 a)
{
	...
	p.y += curve;
	a.y += (curve*curve);
	return length(max(abs(p) - a,-2.0)) -.02;
}
...
{ 
	...
	if (mat > 0.5)
	{
		...
		float ambi = clamp(9.0 + 0.2 * norm.y,0.0, 1.0);
...
void main( void )
{
	...
	t = t*t*(3.0-2.0*t);
	vec3 origin = mix(vec3(0.0, 1.0, -1.2), vec3(+0.0, 1.0, -5.0), t);
	vec3 target = mix(vec3(0.0, 1.0, 4.0),  vec3( 0.0, -0.4, 0.0), t);
