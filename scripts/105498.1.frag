...
void main( void ) {
	...
	vec2 p2 = (vec2(sin(time+3.142), cos(time+8.23))*orbitDistance)+0.5;
	...
	// "bumpmapping" @Flexi23 - Improved bump mapping @dennishjorth
	vec2 d = 4./resolution;
	...
	gl_FragColor.z = pow(clamp(1.-5.5*length(uv  - mouse + d),0.,1.),12.0)+pow(clamp(1.-2.5*length(uv  - mouse + d),0.,1.),20.0)+pow(clamp(1.-1.5*length(uv  - mouse + d),0.,1.),4.0);
