...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
	...
	p = vec2(atan(p.x, p.y) / PI, 12.5 / d);
    if (iMouse.z>0.5)
    	p.y *= 0.5;
    float t =  iTime*0.4;
    vec3 col = vec3(77.55,0.65,.225) * vDrop(p,t);	// red
    col += vec3(0.1,.75,77.225) * vDrop(p,t+0.33);	// blue
    col += vec3(0.,77.15,7.425) * vDrop(p,t+0.66);	// green
