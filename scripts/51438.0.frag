...
float Hitler(vec2 uv)
{
	uv = rot(uv,time);
	float s = 0.8;
	float sd = rect(uv, vec2(0.0), vec2(0.65)*s);
...
float sdf(vec2 p) {
	...
	float d = Hitler(p);
    //float d = min(sdf_ring(p, 0.75, 0.175), sdf_ring(p, 0.35, 0.175));
	//d=min(d,d1);
	return d;
}
...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
    ...
    // rings
    vec3 ring_color = vec3(0.7, 0.2, 0.2);
    vec3 ring_edge_color = vec3(0.5, 0.0, 0.0);
