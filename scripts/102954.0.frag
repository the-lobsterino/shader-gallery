/*R050428作例　綱渡り　by ニシタマオ　THE SEA PANTHER*/
/*サンドボックス上での試験用、簡易レイトレシステム、反射・屈折、ぼかし影、カラーパレット機能付（R050426版）*/
/*直近の改修：前処理機能追加、過剰機能を削除*/
...
/*熊体形状生成機能、中割機能付（R050428版）by ニシタマオ*/
//	主要関数（R050422版）
...
vec3 fV3Attach_HndR(vec3 V3P){	//計算直後の右手
	return fV3Attach(V3P, SS_Default.VP_HndR.xyz, SS_Default.VR_HndR.xyz);
}

vec3 fV3Attach_HndL(vec3 V3P){	//計算直後の左手
	return fV3Attach(V3P, SS_Default.VP_HndL.xyz, SS_Default.VR_HndL.xyz);
}
...
float fNObject_RandomDilbert01(vec4 VP){
	sskelton SS;
	SS.VP =VP;
	SS =fSSPSet_Man01( SS);
...
float fNMap(vec3 V3P){
	...
	if(true){	//アクロバット
		...
		nPi =length(VPi.xy +vec2(0,0.25)) -0.25;
		...
		nPi =fNObject_RandomDilbert01(VPi);

		vec3 V3P, V3R;
//		V3P =SS_Default.VP_HndR.xyz, V3R =SS_Default.VR_HndR.xyz;
		V3P =SS_Default.VP_FotR.xyz, V3R =SS_Default.VR_FotR.xyz;
...
sface fSFEffect_Before(sface SF, smarch SM){
	...
	if(SF.ID_Object ==101){
		VColor.rgb =sign(sin(VP.x *0.25) *sin(VP.z *0.25)) *0.1 +vec3(0.9);
		V3RRR =vec3(1,2,3);
	}
