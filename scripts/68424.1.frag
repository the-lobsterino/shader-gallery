...
void main( void ) {
	...
	float aa = resolution.x/resolution.y;
	
	position = floor(position*30.0)/30.0;
	
	//noise1
	vec3 p = vec3(
		snoiseVec3( vec3( position.x*0.8, position.y*0.8, floor(time*4.0)/10.0 ))
	);
	float vv = step( 0.5, fract(p.x*5.0) );
