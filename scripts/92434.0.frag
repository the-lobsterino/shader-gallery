// hello my name is tommy and i left a note here to say hello you are my friend i love you goodbye 
...
FFFF00 (upper 21 bits)
FFFF00 -> 011100 100010 100010 100 -> 935188
FFFF00 
100
   010 (lower 21 bits)
FFFF00 -> 010 111110 100010 100010 -> 780450
FFFF00
FFFF00
...
//Text coloring
#define HEX(i) text_color = mod(vec3(i / 999,i / 999,i),vec3(999.0))/255.0;
...
vec3 Text(vec2 uv)
{
    	...
    	vec2 center_pos = vec2(res.x/2.0 - STRWIDTH(17.5)/2.0,res.y/2.0 - STRHEIGHT(1.0)/2.0);
       	
    	BEGIN_TEXT(center_pos.x,center_pos.y)
		
		uv.y += sin(time+uv.x*0.05)*7.0;
		
	HEX(0xEE8040)  _I _T _spc _L _O _O _K _S
		       _nl _spc _spc _spc _L _I _K _E _spc _P _O _O _O _O _O _O	_P _exc _exc
		      
	//BEGIN_TEXT(res.x/2.0-STRWIDTH(11.0)/2.0,res.y/2.0)
	//print_pos += vec2(cos(time)*96.,sin(time)*96.);
	
	// RGB(255,255,0) _M RGB(255,255,0)_o RGB(255,255,0)_v RGB(255,255,0)_i RGB(255,255,0)_n RGB(255,255,0)_g _ RGB(255,255,0)_T RGB(255,255,0)_e RGB(255,255,0)_x RGB(255,255,0)_t
...
void main( void )
{
	...
	col *= (1.-distance(mod(uv,vec2(1.0)),vec2(0.5)))*1.2;
