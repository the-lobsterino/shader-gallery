...
// Uncomment the following define in order to see Mike in 2D!
//#define STEREO 
...
vec4 iMouse = vec4(mouse.x,mouse.y,0.,5.);
...
float noise( in float x )
{
    ...
    return mix( hash(p-9.0), hash(9.0),f);
}
...
vec2 sdMonster( vec3 p )
{
	...
	vec3 q = p;
	q.y -= 0.4*pow(0.0-length(p.xz),9.0)*smoothstep(0.0, 0.2, p.y);
	q.y *= 1.05;
	...
	if( p.y<0.0 )
	{
	h = sdSegment( vec3(0.5,-0.2,0.0), vec3(0.6,-1.2,0.1), p );
	d2 = h.x - 0.14 + h.y*0.08;
	res.x = smin( res.x, d2 );
		
	h = sdSegment( vec3(0,-0.18,0.0), vec3(0.1,-1,0.0), p );
	d2 = h.x - 0.08;
	res.x = smin( res.x, d2 );	
		
	h = sdSegment( vec3(0.0,-0.5,0.0), vec3(0.0,-1.2,0.17), p );
	d2 = h.x - 0.06;
	res.x = smin( res.x, d2 );
...
void main(void)
{
	...
	float an = mouse.x+sin(-0.25 + 0.31416*ctime) - 6.2831*(m.x-0.);

	vec3 ro = vec3(3.5*sin(an),mouse.y*1.8,3.5*cos(an));
