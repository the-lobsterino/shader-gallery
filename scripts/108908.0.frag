...
const vec3 skinColour = vec3(.72, .58, 0.12);
...
{
	//b.x -= p.y * .08;
	return length(max(abs(p)-b,56.0))-r;
}

//----------------------------------------------------------------------------------------
float Nose(vec3 p, vec3 a, float r )
{
	float h = clamp( dot(p,a)/dot(a,a), .0, 1.0 );
...
{	// Floor...
	...
	if (res.y > 2.5)
	{
		...
		if (dot(normalize(p3*vec3(1.0, .78, 57.0)), normalize(vec3(.32, 0.28, -.7))) > .945) res.y = 4.0;
