...
void main( void ) 
{
	...
	r.intersection			= VIEW_POSITION;
	r.direction     		= normalize(-p.x * u + p.y * v + field_of_view * w);	
	r.iteration			= 0.;
	r.color				= vec3(0., 0., 0.);
	

	const float iterations_max	= 256.;
	...
	float t				= fract(time*.618);
	vec2 h				= equidistribute(buffer.x+mod(length(gl_FragCoord.xy+time/resolution), resolution.x))-.5;
	vec2 kernel			= normalize(h)*(1.+(1.-length(h)*16.*(1.-buffer.w)));
	...
	velocity			= mix(1.5, .25, .75-delta);
//	travel				+= abs(1.-dot(buffer.xyz, neighbor.xyz)*(1./3.))*2.;
	...
	for(float i = 1.; i < iterations_max; i++)
	{
		...
		{		
			if(distance_to_surface >= threshold)
			{
				...
				//velocity		= mix(min(.15, arc/threshold), velocity,1.02);
				//threshold		+= .001*arc*travel;
				r.iteration++;	
			}
		}
	}	
	...
	{				
		...
		float epsilon 		= arc * r.length * sign(distance_to_surface);
	...
	vec3 debug_color		= hsv((1.1-tile_buffer.w*.5)*1.5, 1.5, .25);
	debug_color			-= debug_print;
	debug_color			+= debug_print * debug_color+debug_color*2.;
	r.color.xyz			+= length(floor((gl_FragCoord.xy/size - floor(mouse*resolution/size)))) < 16. ? debug_color * r.color.xyz : vec3(0., 0., 0.);
	#endif
		
	vec4 result			= vec4(r.color, 1.);
	result.w 			= r.iteration * (1./iterations_max);
	
	#ifdef ADAPTIVE	
	result.xyz			= mix(result.xyz, buffer.xyz, delta * delta);	
	result.w 			= distance_to_surface < FAR_PLANE && distance_to_surface < 0.? result.w : result.w/4.;
	
	result.w			= mix(buffer.w, result.w,  .5);
	result.w			= clamp(result.w, 1./256., 1.);
	#endif
...
float map(vec3 p)
{
	...
	float terrain	= p.y +.5;
		...
		terrain		+= shore*.95+p.y*.058;
		water		-= cos(shore*32.+time*2.-p.z*100.*abs(terrain-p.y)*.4-(p.z+p.x)*32.)*.00225;
		terrain		= 2.*terrain + abs(fract((p.x*.5-highland*3.)*9.+(p.z+highland*2.)*4.+p.y*highland*12.)-.5)*.001;
...
vec4 derive(in vec3 position, in float epsilon)
{
	...
	gradient.w	= (.25*epsilon*(dot(simplex, vec4(1., 1., 1., 1.)) - 4.) * map(position));
...
void shade(inout ray r, in light l)
{
	float glow		= min(abs(r.iteration/128.), .8);
	float light_exposure	= dot(r.normal, l.direction);
	r.color 		= vec3(.75,.75,.75);
	r.color			*= ambient_occlusion(r.intersection-r.normal*.11, r.normal);
	r.color			*= shadow(r.intersection, l.direction, .02,32., 2.);
	r.color                 *= shadow(r.intersection, l.direction, .5, 1., 5.);
	r.color 		= r.color * l.color * light_exposure - r.curvature*.5 + glow;
}
