...
#define VIEW_POSITION   VIEW_ORBIT
...
{	
	...
	float cub	= cube(position);
	float tet	= tetrahedron(position);
	float oct	= octahedron(position);
	...
	float t		= mod(GLOBAL_TIME * 1.25, 12.);
	float lerp	= fract(t);
	float p		= floor(t);
	float poly	= p == 0. ? mix(ddc, cub, lerp) : 
		   	  p == 1. ? 	            cub : 
			  p == 2. ? mix(cub, oct, lerp) : 
			  p == 3. ? 	            oct : 
			  p == 4. ? mix(oct, ico, lerp) :	
			  p == 5. ? 		    ico :	 
			  p == 6. ? mix(ico, rtc, lerp) :	
			  p == 7. ? 		    rtc :
			  p == 8. ? mix(rtc, tic, lerp) :	
			  p == 9. ? 		    tic :
			  p == 10. ? mix(tic, ddc,lerp) :		
				    		    ddc ; 
...
void main( void ) 
{
	...
	{				
		...
		#define REFLECTION
		#ifdef REFLECTION
		ray reflection 		= emit(r.position , reflect(r.direction, r.gradient.xyz),  MIN_DISTANCE * 2.);
		...
		if(r.material.z == 1.)
		{
			...
			ray refraction		= r;
			refraction.origin 	= r.position - r.gradient.xyz * r.distance * 4.;
			...
			{				
				...
				r.color += abs(depth*.01625*r.color)*.2;
				r.color *= .75;
				r.color -= r.steps*.0025;
			}			
...
float tetrahedron(vec3 p)
{
	vec2 q   = vec2(-0.57735026919, 0.57735026919);

	return max(max(max( dot(p, q.xyy), dot(p, q.yxy)), dot(p, q.yyx)), dot(p, q.xxx)) + 2.;
}


float rhombicdodecahedron(vec3 p)
{
	vec2 q 	= vec2(sqrt(3.)/2., 0.);
	p 	= abs(p);

	return max(max(dot(p, q.yxx), dot(p, q.xyx)), dot(p, q.xxy));
}
...
float icosahedron(vec3 p)
{
	...
	p 	= abs(p);
	return max(max(max(dot(p.yz, q.xz), dot(p.xyz, q.yyy)),dot(p.xz,q.zx)),dot(p.xy,q.xz));
}


float dodecahedron(vec3 p)
{
	vec2 q 	= vec2(.5,.80901699437);	
	p 	= abs(p);	
	return max(max(dot(p.xz, q.xy), dot(p.xy, q.yx)),dot(p.yz, q.yx));
}
