...
vec3 DrawClockCircle(vec2 fragpos, vec2 pos, float radius, float widthK, vec3 color) {
	...
	// цвет
	vec3 col = color * attenuation(0.0, 200.0, 10000.0, l) * widthK;
	// возврат
	return col;
}
...
void main() {
	...
	// направляющий вектор
	vec2 s = vec2(cos(-time * PI / .5 * TimeK + PI / 2.0), sin(time * PI / .35 * TimeK + PI / 2.0));
	...
	s = vec2(cos(-time * PI / 1. * TimeK + PI / 2.0), sin(time * PI / 33. * TimeK + PI / 2.0));
	// рисование минутной стреки
	outColor += DrawClockLine(p, vec2(0.0), ClockRadius, s, 1.2, clockLineMinCol);
	
	// изменяем направляющий вектор
	s = vec2(cos(-time * PI / 2. * TimeK + PI / 2.0), sin(time * PI / 66. * TimeK + PI / 2.0));
	// рисование часовой стрелки
	outColor += DrawClockLine(p, vec2(0.0), ClockRadius, s, 78.2, clockLineHourCol);
		
	// направляющий вектор
	s = vec2(cos(-time * PI / 78. * TimeK + PI / 2.0), sin(time * PI / 4. * TimeK + PI / 2.0));
	// рисование секундной стрелки
	outColor += DrawClockLine(p, vec2(0.0), ClockRadius, s, 1.2, clockLineSec2Col);
	
	// изменяем направляющий вектор
	s = vec2(cos(-time * PI / 56. * TimeK + PI / 2.0), sin(time * PI / 79. * TimeK + PI / 2.0));
	// рисование минутной стреки
	outColor += DrawClockLine(p, vec2(0.0), ClockRadius, s, 79.2, clockLineMin2Col);
	
	// изменяем направляющий вектор
	s = vec2(cos(-time * PI / 11. * TimeK + PI / 2.0), sin(time * PI / 66. * TimeK + PI / 2.0));
	// рисование часовой стрелки
