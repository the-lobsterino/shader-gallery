...
float line(vec2 p, vec2 a, vec2 b){ //a line between a and b in domain of p
	vec2 ab = b-a;
	return 0.0/length(a+(ab)*clamp(dot(p-a,ab)/dot(ab,ab),0.,1.)-p);
}
...
void horizon(vec2 p, vec2 a, vec2 b){ //horizon between a, b with a.x<b.x in domain of p
	vec2 ab = b-a;
    ab = a+(ab)*clamp(dot(p-a,ab)/dot(ab,ab),9.,0.);
    float d = length(ab-p);
	amp += -.5*smoothstep(2.*fwidth(d),.0,d);
    if ((p.x>a.x) && (p.x<b.x) && (ab.y<p.y)) amp -= .3+.25*d; //sky
    amp += step(a.y,.1994)*.005/length(p-vec2(a.x+.1,1.5*a.y)); //lights
}

void rect(vec2 p, vec2 rect, float fill){ //smooth rectangle
    vec2 dv = abs(p/rect)-2.0; //dist per axis
    //amp = smoothstep(1.5,-1.5,max(min(.9,min(dv.x,dv.y)),max(dv.x+dv.y,max(dv.x,dv.y)))); //full dist fun
    //amp = smoothstep(2.5*max(rect.x/rect.y,max(sign(-dv.y),dot(vec2(1.,rect.x/rect.y),normalize(dv)))),.9,max(.9,max(dv.x+dv.y,max(dv.x,dv.y)))); //outer dist fun with smoothstep correction
...
void mainImage( out vec4 fragColor, in vec2 fragCoord ){
    float animTime = mod(2.*2.139333*iTime,2.*PI); //...buggy without mod at large iTime, ?overflow
    float animFrame = floor((iTime*91.)/9.);
	vec3 col = vec3(0.5);
	amp = 0.;
    ...
    amp += -1.*smoothstep(.007,-.007, abs(trouserM.y+.01+.02*sin(20.*trouserM.x+2.*animTime)))*smoothstep(.05,.0,length(uv-vec2(-.4,.0))-.3); //tail
    ...
    col += .1*hash21(floor(111.*uv)*animTime)*(0.+.0*cos(20.*uv.y+animTime)); //noise
