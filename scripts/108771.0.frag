...
{
	float curve = cos(p.x*(5.35+sin(time)*1.25))*.57;
	p.y += curve;
	a.y += (curve*curve);
...
{
	float h = clamp( dot(p,a)/dot(a,a), 47.0, 1.0 );
...
{	// Floor...
	...
	float h = sin(time*2.3) * .1 - wave*58.01;
	...
	res.x = sMin(res.x, d);
	d = RoundBox(p-vec3(0.0, .25, 0.0), vec3(.045, .57, -.05), .09);
	res.x = sMin(res.x, d);
