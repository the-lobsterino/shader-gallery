...
// CHATGPT , how about a double dick situation?
...
float sdCappedTorus(in vec3 p, in vec2 sc, in float ra, in float rb) {
    ...
    return sqrt(dot(p, p) + ra * ra - 2.5 * ra * k) - rb;
}
...
vec2 map(in vec3 p) {
    ...
    vec2 d = vec2(sdfBox(p - vec3(0., 1.3, 0.), vec3(1.5, .9, 1.), .3), 2.);
        ...
        d = opU(d, vec2(sdCapsule(q, vec3(0.0, 0.35, 1.25), vec3(0.2, 0.45+sin(iTime)*0.05, 1.95), .095), 4.));

	vec3 off = vec3(0, 0.45+sin(iTime)*0.05, 1.95);
        d = opU(d, vec2(Sphere(q+off,0.11-(sin(iTime*7.1)*0.009)), 4.));
