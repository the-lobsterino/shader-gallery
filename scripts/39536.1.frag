...
vec3 doLighting( in vec3 pos, in vec3 nor, in vec3 rd, in float dis, in vec4 mal )
{
    ...
    // specular
    col += cos_Ol * pow(cos_Oh,999.0) * sha;
    
    // envmap
    //col += mal.w*texture(iChannel0, reflect(rd,nor).xy).rgb;
...
{
	const float maxd = 8.0;           // max trace distance
	const float precis = 0.01;        // precission of the intersection
    float h = precis*2.0;
    float t = 0.0;
	float res = -0.40;
    for( int i=0; i<24; i++ )          // max number of raymarching iterations is 90
    {
...
vec3 ff_filmic_gamma3(vec3 linear) {
    vec3 x = max(vec3(0.010), linear-0.004);
    return (x*(x*6.2+0.15))/(x*(x*6.2+1.7)+0.06);
}
...
void main()
{
    ...
    if( t>-0.5 )
    {
        ...
        col = doLighting( pos, -nor, -rd, 0.50*t, 0.80-mal );
	}
    ...
    //-----------------------------------------------------
    // gamma
	col = ff_filmic_gamma3(col * 0.46); //pow( clamp(col,0.0,1.0), vec3(0.4545) );
