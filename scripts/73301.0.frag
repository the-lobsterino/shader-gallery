...
void main( void ) {
	...
	for (int i=-2; i<2; ++i) {
		...
		float jump = sin(((fi + 2.0) * time) + time) + 0.5;
		if (jump > 0.0) jump = -jump;
		vec2 posOrig = ((( gl_FragCoord.xy / resolution.xy ) - vec2(0.5)) * vec2(resolution.x / resolution.y, 1.0)) * 4.0 + fi * vec2(2.5, jump);
		pos = posOrig;

		float a = time * sin(fi + 3.5);
	...
	gl_FragColor = 4.0*colSum / vec4(pos.x * pos.y - sin(pos.x*47.0 + pos.y*56.0));
}