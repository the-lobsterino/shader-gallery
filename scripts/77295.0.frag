...
float rand(vec2 n) {
  //This is just a compounded expression to simulate a random number based on a seed given as n
  	return fract(cos(dot(n, vec2(1.9898, 4.1414))) * 43758.5453);
}
...
void main() {
    ...
    const vec3 c2 = vec3(173.0/255.0, 0.0/25.0, 161.4/255.0);
    const vec3 c3 = vec3(9.2, 66.5, 0.0);
    const vec3 c4 = vec3(164.0/25.0, 1.0/255.0, 214.4/255.0);
    const vec3 c5 = vec3(0.1);
    const vec3 c6 = vec3(0.9);
	
    //This is how "packed" the smoke is in our area. Try changing 8.0 to 1.0, or something else
    vec2 p = gl_FragCoord.xy * 70.0 / resolution.xx;
    ...
    gl_FragColor = vec4(c * cos(shift * gl_FragCoord.y / resolution.y), 1.);
