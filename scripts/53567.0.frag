...
uniform float time;
const vec2 mouse = vec2(0.975, 1.0123);
...
void main( void ) 
{
	...
	vec3 origin 	= vec3(0.5, 1.25, 0.);

	origin .z	+= cos(time*.0625) * 2.5 + 2. ;
	...
	float threshold	= .5/min(resolution.x, resolution.y);
	...
	if(range < max_range)
	{
		...
		position		+= gradient * epsilon*-.5;
		gradient		= derive(position, epsilon);
		
		normal			= normalize(gradient);
		
		light			= clamp(dot(normal, normalize(vec3(4.,32., 12.))), 0., .125);

		color			= 1.-mix(sphericalharmonic(-normal*2.), sphericalharmonic(reflect(direction, normalize(1.-normal))), max(1.-cos(dot(direction, normal)),0.));
	...
	else 
	{
		...
		color		= sphericalharmonic(normal*.9);
	}
