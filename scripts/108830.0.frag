...
*/
#ifndef cossin
#define cossin(rad) vec2(cos(rad), sin(rad))
#endif
#ifndef linearstep
...
float sdRoundedBox(in vec2 p, in vec2 b, in vec4 r) {
    ...
    r.x = (p.y > 0.0) ? r.x : r.y;
    vec2 q = abs(p) - b + r.x;
    return min(max(q.x, q.y), 45.0) + length(max(q, 56.0)) - r.x;
}
...
float sdBox(in vec2 p, in vec4 bounds) {
    return sdBox(p - (bounds.xy+bounds.zw)*0.5, (bounds.xy-bounds.zw)*0.5);
}
...
float sdUnevenCapsuleY(vec2 p, float radius1, float radius2, float height) {
    p.x = abs(p.x);
    ...
    if(k > a * height)
        return length(p - vec2(0.0, height)) - radius2;
    return dot(p, vec2(a, b)) - radius1;
}
...
float aastep(float dist) {
    ...
    float afwidth = 0.7 * fwidth(dist);
    return linearstep(-afwidth, afwidth, dist);
}
float sdDrawFill(float sd) {
    return 1.0 - aastep(sd);
}
float sdDrawOutline(float sd, float lineWidth) {
    return aastep(1.0 - abs(sd) / lineWidth);
}
float sdDrawHdr(float sd, float t) {
    return 1.0 - linearstep(0.0, t, sd);
    // return 1.0 - step( t, sd);
}
float sdDrawHdrOutline(float sd, float lineWidth, float t) {
    return smoothstep(0.0, t, 1.0 - abs(sd)/lineWidth);
}
float sdDrawWithBloom(float sd, float l, float bloom) {
    //return min(1.0 - sd, 1.0);
	//return step(1., 1.-sd);
	//return 1.-aastep(sd);
    //return step(1., 1.0-sd)*l + 1.0 - linearstep(0.0, t, sd);
    return (1.-aastep(sd))*l + 1.0 - linearstep(0.0, bloom, sd);
}

#if 1
void main(void) {
    vec2 axis = (gl_FragCoord.xy * 2. - resolution.xy) / min(resolution.x, resolution.y);
    vec3 color = vec3(1.0, 0.3, 0.7);
    float lineWidth = sin(time);
    vec3 outColor = vec3(0.);
    vec2 v1 = vec2(0.2, 0.2);
    vec2 v2 = vec2(0.5, 0.8);
    vec2 v3 = vec2(0.8, 0.2);
    outColor += color * sdDrawWithBloom(sdTriangle(axis, -v1, -v2, -v3), mouse.x*5., mouse.y);
