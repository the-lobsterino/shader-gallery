// halomasta plays goldeneye:
// https://youtu.be/kRw48LZx2oA?t=1582


// the legend of the rubble-baby
// who tried to play a PC shooter, goldeneye
// but got trolled, tenderly
// and cried vulgarly in his rage
// 'fuck you, jew nigger'

// this one is for all of those
// who have encountered a foul-mouthed kid
...
{	
	...
	float x = 0.0;
	float y = 0.0;
	vec2 line = vec2(-3., 1.75);
	
	
	
	line += h;
	x = 2.;
	d = min(d,T(uv-line+ w*(x++)));
	d = min(d,H(uv-line+ w*(x++)));
	d = min(d,I(uv-line+ w*(x++)));
	d = min(d,S(uv-line+ w*(x++)));
	x++;
	d = min(d,I(uv-line+ w*(x++)));
	d = min(d,S(uv-line+ w*(x++)));
	d = min(d,N(uv-line+ w*(x++)));
	d = min(d,T(uv-line+ w*(x++)));
	d = min(d,Astroph(uv-line+ w*(x-1.) + h *.15));
	
	
	line += h;
	x = 1.75;
	x++;
	d = min(d,Y(uv-line+ w*(x++)));
	d = min(d,O(uv-line+ w*(x++)));
	d = min(d,U(uv-line+ w*(x++)));
	d = min(d,T(uv-line+ w*(x++)));
	d = min(d,U(uv-line+ w*(x++)));
	d = min(d,B(uv-line+ w*(x++)));
	d = min(d,E(uv-line+ w*(x++)));
	
	
	/*
	d = min(d,U(uv-vec2(-4.0,1.75)));
	d = min(d,C(uv-vec2(-3.5,1.75)));
	d = min(d,K(uv-vec2(-3.0,1.75)));
	d = min(d,I(uv-vec2(-2.5,1.75)));
	d = min(d,N(uv-vec2(-2.0,1.75)));
	d = min(d,G(uv-vec2(-1.5,1.75)));
	
	d = min(d,J(uv-vec2(-3.5,1.)));
	d = min(d,E(uv-vec2(-3.0,1.)));
	d = min(d,W(uv-vec2(-2.5,1.)));
	//d = min(d,G(uv-vec2(-1.0,0.5)));
	d = min(d,N(uv-vec2(-1.5,1)));
	d = min(d,I(uv-vec2(-1.0,1)));
	d = min(d,G(uv-vec2(-0.5,1)));
	d = min(d,G(uv-vec2(0.0,1)));
	d = min(d,E(uv-vec2(0.5,1)));
	d = min(d,R(uv-vec2(1.0,1)));
	d = min(d,S(uv-vec2(1.5,1)));
	
	d = min(d,L(uv-vec2(-4.5,.25)));
	d = min(d,O(uv-vec2(-4.0,.25)));
	d = min(d,V(uv-vec2(-3.5,.25)));
	d = min(d,E(uv-vec2(-3.0,.25)));
	d = min(d,H(uv-vec2(-2.0,.25)));
	d = min(d,A(uv-vec2(-1.5,.25)));
	d = min(d,L(uv-vec2(-1.0,.25)));
	d = min(d,O(uv-vec2(-0.5,.25)));
	d = min(d,M(uv-vec2(0,.25)));
	d = min(d,A(uv-vec2(.5,.25)));
	d = min(d,S(uv-vec2(1,.25)));
	d = min(d,T(uv-vec2(1.5,.25)));
	d = min(d,A(uv-vec2(2,.25)));
	//*/
	
	float wd = 0.01;
	col = mix(vec3(1,uv.y,0),col,smoothstep(lineWidth-wd,lineWidth+wd,d));
