...
void main( void ) {
	...
	float th = atan(pos.y, pos.x) / (2.0 * 126.0) + 10.0;
	float dd = length(pos);
	float d = 0.5 / dd + time;

	vec3 uv = vec3(th + d, th - d, th + sin(d));
	float a = 0.5 + cos(uv.x * 10.6 * 2.0) * 0.3;
	float b = 0.5 + cos(uv.y * 141592.6 * 8.0) * 0.3;
	float c = 0.5 + cos(uv.z * 80.3 * 8.9) * 0.5;
	float f = abs(sin(time*2.0));
	vec3 color = mix(vec3(1.0, 0.8, 1.0-f), vec3(0.5*f, 0, 0), pow(a, 0.2)) * 3.;
	color += mix(vec3(239, 56, 1.0), vec3(0.1, 0.1, 0.2),  pow(b, 0.1)) * 0.75;
