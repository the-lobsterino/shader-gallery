...
vec2 map( in vec3 pos )
{
	...
	res = opU( res, vec2( 0.5*sdSphere(    pos-vec3( 0.4+sin(time),0.25, 0.0), 0.25 ), 46.9 ));
...
{ 
    ...
    if( m > -0.5 )
    {
        ...
        // mix in fog?
    	col = mix( col, vec3(0.8,0.0,0.0), 1.0-exp( -0.0002*t ) );
    }
...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
    ...
    for( int n=0; n<AA; n++ )
    {
        ...
        vec3 ro = vec3( 2.0 , 2.0 + 2.0*mo.y*100., 0.0  );
