...
uniform float time;
#define time (4.+.1*cos(gl_FragCoord.x/30.+time/3.))
uniform vec2 mouse;
...
const vec3 boundingSize = vec3(35,12,0.8)*scale;
...
mat3 rotX(float a) {float s=sin(a); float c=cos(a); return 
mat3(1,0,0,0,c,-s,0,s,c);}
mat3 rotY(float a) {float s=sin(a); float c=cos(a); return
...
letters = min(letters,G(p.xy-vec2(-0.70-0.5,0.9)));
letters = min(letters,L(p.xy-vec2(-0.25-0.5,0.9)));
letters = min(letters,S(p.xy-vec2( 0.25-0.5,0.9)));
letters = min(letters,L(p.xy-vec2( 0.25,0.9)));

letters = min(letters,S(p.xy-vec2(-2.,0.2)));
letters = min(letters,A(p.xy-vec2(-1.5,0.2)));
letters = min(letters,N(p.xy-vec2(-1.,0.2)));
letters = min(letters,D(p.xy-vec2( -.5,0.2)));
letters = min(letters,B(p.xy-vec2( 0.,0.2)));
letters = min(letters,U(p.xy-vec2( .5,0.2)));
letters = min(letters,T(p.xy-vec2( 1.,0.2)));
letters = min(letters,T(p.xy-vec2( 1.5,0.2))); // Spelling, please.
letters = min(letters,S(p.xy-vec2(2.0,0.2)));
...
{	
	...
	gl_FragColor = vec4(length(col/2.));
}
