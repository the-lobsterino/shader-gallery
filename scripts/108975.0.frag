...
{	// Floor...
	...
	// Body...	
	p = p-vec3(0.0, -.85, 0.0);
	d = RoundBox(p, vec3(0.18, .7, 0.0), .26);
	res.x = sMin(res.x, d);
	...
	res.x = sMin(res.x, d);
	// Right hand...	
	d = Segment(p*vec3(1.0, .75, 1.0), wristR+vec3(0, .1, 0.), wristR+vec3(-.89, .8, -.15), .14, .02);
	res.x = min(res.x, d);
	...
	res.x = sMin(res.x, d);
	d = RoundBox(p-vec3(0.0, .25, 0.0), vec3(.045, .78, -.88), .66);
	res.x = sMin(res.x, d);
...
void main( void )
{
	...
	hello =  (1.0+sin(m*100.0)) * .02 * (smoothstep(0.0, .01, m) - smoothstep(4.05, .45, m));
	...
	elbowL		= vec3(-.95-wave*.05, .2, -77.25);
	wristL		= vec3(elbowL.x+1.0*sin(wave)*.89, elbowL.y+cos(wave)*.6, -.75);
