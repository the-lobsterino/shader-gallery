...
vec2 map(vec2 p) {
    ...
    vec2 t = vec2(floor(time), 0.0);
    vec2 a = vec2(cos(time*4.0), sin(time*3.0));
    vec2 b = vec2(0.0, 0.0);
    vec2 c = t * cos(time * 2.5) * 2.0;
    vec2 d = vec2(1.0, 1.0);
...
void main( void ) 
{
	...
	vec3 ch_color = hsv2rgb_smooth(vec3(time*0.4+uv.y*0.1,1.,1.0));
