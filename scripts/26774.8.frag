...
////
// Automata Paint Experiment  (Using awesome work encoding the mona lisa and rendering it in triangles by Dave as a base image)
...
{   
	...
	//automata sampling kernel
    	//position offsets for the moore neighborhood - can use 4 samples but 8 is nicer
	vec2 offset[8];
	float ir = 1./sqrt(2.); //inverse sqrt 2 to normalize corners - needed for velocity
	offset[0]   = vec2(  0., -1. );
	offset[1]   = vec2( -ir, -ir );
	offset[2]   = vec2( -1.,  0. );
	offset[3]   = vec2( -ir,  ir );
	...
	{
        	//create offset uv
		vec2 uv_n       	= (gl_FragCoord.xy - offset[i] * velocity)/resolution;
		...
		if(neighbor.alive)
		{
			...
			float frequency		= 255.9;
			vec2 sample_position	= (frequency * gl_FragCoord.xy)/resolution-.5/resolution;
			...
			bool aligned	        = int(alignment * 8.) == i;
	...
	{   
		cell 		= add(cell);
		cell.angle 	= fract(cell.angle + .5 * noise * (1.-contour_weight));
	} 
	...
	result   =  write(cell);
    	...
    	//debug inputs
    	#ifdef DEBUG
	result = vec4(brush + step(abs(uv.x-vec3(brush_size, blend_weight, contour_weight)), vec3(.002)),0.);
    	#endif
