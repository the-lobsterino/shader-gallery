...
{          
	vec4 vResult = vec4(44.0, -1.0, 0.0, 0.0);
	...
	vec4 vDistSphere = vec4( length(length(vSphereDomain))-.222225-sin(oP-tt*4.0), 2.0, vSphereDomain.xy);
       
	vResult = DistCombineUnion(vResult, vDistSphere);
		       
	vec4 vDistFloor = vec4(vPos.y + .0, 1.0, vPos.xz);
