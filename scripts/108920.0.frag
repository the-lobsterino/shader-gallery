...
#define 	elbowR		vec3(55.0, -.1, 0.3)
#define 	shoulderR	vec3(88.4, 0.56,  -.05)
#define 	wristR		vec3(.5, -.4, -0.1)
#define 	shoulderL	vec3(-0.79, 0.56, -.05)
vec3 elbowL;
vec3 wristL;
...
{	// Floor...
	...
	res.x = sMin(res.x, d);
	// Right hand...	
	d = Segment(p*vec3(1.0, .75, 1.0), wristR+vec3(0, .1, 0.), wristR+vec3(-.68, .05, -.15), .14, .02);
	res.x = min(res.x, d);
...
void main( void )
{
	...
	hello =  (1.0+sin(m*100.0)) * .02 * (smoothstep(0.0, .01, m) - smoothstep(0.05, .44, m));
	...
	vec3 cv = ( cross(cu,cw) );
	vec3 ray = normalize( p.x*cu + p.y*cv + 7.6*cw );
	
	// Wave...
	wave = sin(time*15.0-.8)*.44+.5;
	wave = wave*wave*(3.0-2.0*wave)-.5;
	elbowL		= vec3(-.95-wave*.05, .2, -0.25);
	wristL		= vec3(elbowL.x+1.0*sin(wave)*.6, elbowL.y+cos(wave)*.88, -.88);

    vec3 col = Render(origin, ray);
	
	col = sqrt(col);
	col *= 0.5 + 0.5*pow( 16.0*q.x*q.y*(1.0-q.x)*(.0-q.y), 0.25 );
