//https://www.shadertoy.com/view/4tSXWc
#ifdef GL_ES
...
float line(vec2 p, vec2 c1, vec2 c2, float width) {
	...
	float ll = length(cx);
	float lp = length(cp);
	vec2 np = min(lp,ll )*normalize(cx);
	return width - distance(np, cp);	
	
}

void main( void ) {

	vec2 p = ( gl_FragCoord.xy / resolution.x)-.5*resolution/resolution.x;	
	vec4 back = vec4(.7, .0, 0.0, 0.) * ((1. - length(p)));
	vec3 colP = vec3(0.1, 0.6 ,.4);
	vec3 colC = vec3(.2, .2, .9);
	...
	float l = line(p, vec2(.2, 0.), vec2(a+.2, 0.), .003);
