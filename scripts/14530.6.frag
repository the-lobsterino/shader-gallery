...
/*
Forked for Oculus rift.
HmdWrap function is based on Oculus rift SDK
coded by kioku
*/
...
void main(void) {
	...
	uv = HmdWarp(uv);
	seed = time + resolution.y *(uv.x+1.0) + uv.y;
	...
	vec3 camPos = vec3(50.+side*2.5, 40.8, 169.);
	vec3 cz = normalize(vec3(50., 40., 1.6) - camPos)*.65;
	vec3 cx = vec3(1., 0., 0.);
