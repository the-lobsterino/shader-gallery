...
void main() {   
     	vec2 uvs = 2. * gl_FragCoord.xy / resolution.xy - 1.;
	uvs /= dot(uvs, uvs);
	
	float time2 = time;               
        float speed = speed2;
        speed = -.04 * cos(time2*0.02 + 3.1415926/4.0)*-1.;          
	//speed = 0.0;	
	...
	for (int r=0; r<volsteps; r++) {
		...
		if( r == volsteps-1 )
			fade *= sampleShift;
		v+=vec3(pow(s1,1.),pow(s1,1.6),0.4*pow(s1,7.0/pow(float(s1),0.8)))*a*brightness*fade; // coloring based on distance
