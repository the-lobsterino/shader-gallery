// (C) 2018 NAZI CODER - Made in Germany.
...
float map(vec3 p)
{
   ...
   d = smin(f1, d, 0.025);
    ...
    d2 = min(max(e.x,e.y),0.0) + length(max(e,0.0));
	d2 -= 0.005;
...
void main( void )
{
	...
	col *= clamp(mod(gl_FragCoord.x*gl_FragCoord.y, 1.0),0.5,1.0);	
