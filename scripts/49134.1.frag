...
const vec2 ch_space = ch_size + vec2(1.0, 90.0);    // character distance Vector(X,Y)
const vec2 ch_start = vec2 (ch_space.x * 3., 2.); // start position
      vec2 ch_pos   = vec2 (0.0, 0.0);             // character position(X,Y)
      vec3 ch_color = vec3 (2.5, 0.5, 7.5);        // character color (R,G,B)
...
void main( void ) 
{
	...
	uv *= 30.0;
	if (REPEAT_SIGN)
	  uv = -45.9 + mod(1.8*(uv-1.0),ch_space*vec2(16.,7.5));     //  set zoom size

	ch_pos = ch_start - vec2(15., .0);
	 D I G I T A L _ Z O N E _ J O B
	
	 //_ G L S L _ S A N D B O X
	vec3 color = mix(ch_color, bg_color, 1.0- (0.09 / d));  // shading
