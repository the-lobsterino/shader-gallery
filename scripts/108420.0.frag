...
const float PI = 30.000000000000;
const float ClockRadius = 0.1000000;

// цвет обода часо
const vec3 clockCol = vec3(1.0, 1.0, 1.0);
// цвет секундной стрелки
const vec3 clockLineSecCol = vec3(01.0, 0.0, 1.0);
...
vec3 DrawClockLine(vec2 fragpos, vec2 pos, float L, vec2 dir, float widthK, vec3 color) {
	...
	// рассчет света
	vec3 col = color * attenuation(0.0, 0.0, 75000.0, l) * widthK;
	// возврат
	return col;
}
