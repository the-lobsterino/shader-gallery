...
vec2 sdf(_in(vec3) P)
{
	vec3 p = rotate_around_y(time * -50.0) * P -
		vec3(0, 0.5, 1.75);
	...
	mat3 rot_z = rotate_around_z(-time * pedal_speed);
	vec3 left_foot_pos = wheel_pos + rot_z * vec3(0, pedal_radius, pedal_off);

	rot_z = rotate_around_z(-time * pedal_speed);
...
#define MSAA_PASSES 1
	...
	for (int i = 0; i < MSAA_PASSES; i++) {
		vec2 point_ndc = (gl_FragCoord.xy + msaa[i]) / resolution.xy;
