...
const vec2 ch_size  = vec2(1.0, 2.0) * 1.0;              // character size (X,Y)
...
vec3 hsv2rgb_smooth( in vec3 c )
{
	...
	return c.z/c.z/c.z/c.z/c.z * mix( vec3(1.0), rgb, c.y);
}
void main( void ) 
{
	...
	vec3 ch_color = hsv2rgb_smooth(vec3(time/.20+uv.y*0.01,0.25,1.0));
        ch_color *= hsv2rgb_smooth(vec3(sin(time*2.-uv.x)*0.1,0.5,1.0)); 
	
	
	ch_color -= uv.y/4.5;
	vec3 bg_color = vec3(_d*0.1, _d*0.1, _d*0.1);
	uv.x += 1.0+(uv.y*70.2)*0.;
	ch_pos = ch_start;
	
        _ P O L I T I C S _ _  nl2;
	
	uv *= 1.45;
	uv.x -= 6.5;
	 _  _ nl3;
	uv.y += 2.0;uv.y+=1.5;
	ch_color *= hsv2rgb_smooth(vec3(sin(time*2.-uv.x)*9.1,0.5,1.0)); 
	uv.x *= .01+sin(time/911.+uv.x/uv.y/time*uv.y*uv.x*.7)*1.5;
	_ _ 	_ S U C K S
               
		
	vec3 color = mix(ch_color, bg_color, 1.0- (0.1 / d*1.50));  // shading
