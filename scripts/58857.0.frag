...
float GetDistanceToScene( vec3 p){
  ...
  float d = length(p.xz);
  float ground = p.y - clamp(min(tri(a*4.),tri(d)),0.03,1.0/16.0);
...
void main(void){
  ...
  //Raymarching
  	const float MIN_DISTANCE = 0.71;								// Distance to scene that we will count as a hit
	const float MAX_DEPTH=34.0;								// Distance from camera that we will count as a miss
	const int MAX_STEPS = 14;								// Maxmimum amount of ray marching steps before counting the ray as a miss
	
  	float distanceToScene = 1.7;								// Initial distance to scene, should be initialized as 1
  ...
  //Lighting
	...
	const vec3 e=vec3(0.1,0,0);								// Epsilon value, used to calculate the hit normal
	
  	if (distanceFromCamera<MAX_DEPTH){							// If this is true, it means we hit something
    		
		float cy = raymarchPosition.y;
		diffuseColor = vec3(4.0*cy, 0.8, 1.0);						// Set diffuse color for cube to red
