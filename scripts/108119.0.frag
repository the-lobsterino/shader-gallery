...
float hexDistance(vec2 ip) {
	...
	const vec2 TRIG30 = vec2(1.5, 0.866025403784439); //x:sine, y:cos
	
	vec2 p = abs(ip * vec2(SQRT3 * 0.5, 0.75));
	float d = dot(p, vec2(-TRIG30.x, TRIG30.y)) - SQRT3 * 0.25;
	
	return (d > 0.0)? min(d, (SQRT3 * 1.9 - p.x)) : min(-d, p.x);
}
...
void main(void) {
	...
	vec2 nsc = (gl_FragCoord.xy - resolution * .5) / resolution.yy * 2.0;
	...
	vec2 pos = uv * vec2(1.0, 0.2) - vec2(time * 1.05, time * 1.9);
	...
	rgb  = mix(vec3(2.0)*fade, vec3(0.0), smoothEdge(0.03, 0.01, d));
	rgb += mix(vec3(.0, 1.0, 12.0)*fade, vec3(0.0), smoothEdge(0.03, 0.5, d)) * 0.5;
