...
vec3 particles( vec2 pos )
{
	...
	{
		float cs = cos( HorizontalSpeed * (i/ParticleCount) + noiseFactor ) * HorizontalAmplitude;
		float ss = sin( VerticleSpeed   * (i/ParticleCount) + noiseFactor ) * VerticleAmplitude;
		vec2 origin = vec2( cs , ss );
		
		float t = sin( ParticleBreathingSpeed * i ) * 0.5 + 0.5;
...
void main( void ) {
	...
	vec2 _uv = signedUV;
	float t;
	float freq = mix( 0.4, 1.2, sin(10.) * 0.5 + 0.5 );
	float ang =  (1. / 5.0 * 2.0 - 0.1) * 3.14159265358979;
	float ang2 =  ((1. + 2.0) / 5.0 * 2.0 - 0.1) * 3.14159265358979;
