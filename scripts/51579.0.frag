...
//--messing with city streets -emackey
...
vec2 infiniteBuildings(in vec3 p){
	...
	vec3 temp = p;
	p.y = temp.y;
	p.x = sqrt(temp.x * temp.x + temp.z * temp.z);
	p.z = asin(temp.z / p.x);
	...
	//temp = q;
	//q.y = temp.y;
	//q.x = sqrt(temp.x * temp.x + temp.z * temp.z);
	//q.z = asin(temp.z / q.x) + pi;
	
	

	vec2 pos = vec2(ceil(p.x/c.x), ceil(p.z/c.z));
	
	float height = p.z;//rand(pos)*5.0;//* 10.0 - 4.0;
	float width1 = 1.0;//rand(pos + 100.0) + 0.5;
	float width2 = 1.0;//rand(pos + 1500.0) + 0.5;
