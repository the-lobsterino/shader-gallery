...
// "Shutter", or how motion blur fill percentage ( 0.5 means that only half of frame time will be blurred )
// Play with this setting and see what it does ( normal range should be between 0 and 1 )
#define MBLUR_FILL	0.4 // 0.4 with 12 FPS corresponds to shutter 1/30

// How many samples to take for motion blur.
#define SAMPLES		16
...
void mainImage( out vec4 fragColor, in vec2 fragCoord, float iTime){
    float animTime = mod(2.*2.133333*iTime,2.*PI); // when time is big enough it has accumulated enough error to make animation buggy without mod
    float animFrame = iTime * 6.0;
	vec3 col = vec3(0.5);
	amp = 0.;
    ...
    for (float n=.0; n<15.; n++){ //horizon & sky
        horizon(uv, vec2(-1.5+.2*n, .12+.03*hash21(vec2(floor(animFrame * 2.0))+n)), vec2(-1.5+.2*(n+1.), .12+.03*hash21(vec2(floor(animFrame * 2.0))+n+1.)));
    }
...
void main(void)
{
	...
	for(int i = 0; i < SAMPLES; i++) {
		...
		float iTime = mix(float(int(time * float(FPS))) / float(FPS), float(int(time * float(FPS)) + 1) / float(FPS), float(i) / float(SAMPLES) * MBLUR_FILL);
		
		mainImage(color, gl_FragCoord.xy, iTime);
		
		meanColor += color;
	}
	
	gl_FragColor = meanColor / float(SAMPLES); 
}