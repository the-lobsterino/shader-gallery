...
{uv.x+=2.5;uv/=(1.5+sin(t)); 
	float d = B(uv,1.0);uv.x -= 1.1;
		...
		d = I(uv,d);uv.x -= 1.1;
		d = T(uv,d);
	return (1.-smoothstep(0.0,0.1,(d-(.6)*CHS)*clamp(1.-t+.05, .095,.4)))*t*(1.+sin(time*30.));
}
...
vec4 Eye(vec2 uv, float side, vec2 m, float t)
{
    ...
    d = length(uv-m*0.6);
    float pupilsize = .5-mix(0.4, 0.16, t);
    ...
    col.rgb = mix(col.rgb, vec3(1.0), highlight) + GetText((uv-m*0.45)*28.,t);
...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
    ...
    float d=GetText((uv-mouse+vec2(.1,0.))*28.,t);
