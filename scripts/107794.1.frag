...
void main()
{
    vec2 uv = resolution.xy * resolution;
	vec2 duv = floor(gl_FragCoord.xy / 1.0)-vec2(30.0,-115.0);
   ...
   vec2 p=gl_FragCoord.xy/R.y,q; p.x=p.x*1.6+.5-R.x/R.y*.8; q=1.-p;
	gl_FragColor =1.-vec4(S(p.x,p.y)||S(q.x,p.y)||S(p.x,q.y)||S(q.x,q.y))*vec4(0,1.0,1.0,0)-col.z;	
