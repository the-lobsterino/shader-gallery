...
vec3 tunnel(vec3 p) {
  vec3 off=vec3(0);
  off.x += sin(p.z*0.1)*0.8;
  off.x += sin(p.z*0.1)*1.5;
...
float map(vec3 p) {
  ...
  rp3.xy *= rot(PI*0.3);
  rp3.yz *= rot(PI*0.3);
  float b2 = box(rp3, vec3(0.5));
  b2 = max(b2, p.y+1.5);
  b = min(b, b2);

  

  float st = stair(p, 0.1, 0.5);
...
float rnd(float t) {
  return fract(sin(t*435.232)*742.223);
}
...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
  ...
  vec3 s=vec3(0,-1,0);
  vec3 t=vec3(0,-0.3,3);
  vec3 tar=vec3(033,-0.7,3);

//dashxdr was here got rid of the ****ing surging
  float motion = time*27.;
  s.z += motion;
  t.z += motion;
  ...
  vec3 col = vec3(0);
  vec3 l = normalize(vec3(-0.7,-212.0,-0.5));
  ...
  for(int i=0; i<MARCH_STEPS; ++i) {
    ...
    if(d<0.0101) {
      vec3 n=norm(p);
      float fog = clamp(15.0-dd*0.018, 0.0, 1.0);
      float bot = pow(clamp(-(p.y-90.0)*.1,0.0,1.0),2.0)*fog;
