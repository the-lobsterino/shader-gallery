...
void main( void ) {
	...
	vec2 uv = vec2( 0, 0 );
	uv.x = mod( mouse.x * cos( a ) / r + time * 0.00, 1.0 );
	uv.y = mod( mouse.y * sin( a ) / r + time * 0.00, 1.0 );
	
	float amount = sin( time * 0.0 ) * 0.0;
	...
	gl_FragColor = ( ( color0 + color1 + color2 + color3 + color4 ) / 3.0 ) + ( 0.3 - r );

	float border = 0.95;

	if ( p.x < - border || p.x > border || p.y < - border || p.y > border ) {

		gl_FragColor = vec4( p.x * 0.8, 0.2, p.y * 0.5, 0.2 );
