...
void main( void ) 
{
	...
	{
	    c = vec3( 0.0, sin(time*149.0)*0.25+0.75, 0.0 );  // you won
	}
	else if( status < 0.1 )
	{
	...
	else   // draw a path and some bad guys
	{
	    ...
	    c.b = (v0+v1+v2)/12.0;
            ...
            float path = 0.1+0.02*(1.0-p.x) - length( (p - vec2(0.55,0.55+0.1*cos(p.x*20.0+time*0.8)+0.1*sin(p.x*40.0+time*2.1)) ) * vec2(0.2,0.6+(p.x*1.5)) );
