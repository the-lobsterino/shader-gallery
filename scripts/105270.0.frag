...
{
    vec3 target = vec3(4.0, 1.5, 10.0);
    vec3 p = vec3(-15.0, 2.0, 25.0) + vec3(cos(time), 0.0, sin(time)) * 1.9;
...
// I didn't have the time to finish it, so here it is... clunky, unoptimized, etc. 
...
{
    vec3 pa = p - a, ba = b - a;
    float h = clamp( dot(pa,ba)/dot(ba,ba), 0.0, 1.5 );
    return length( pa - ba*h ) - r;
}

float sdEllipsoid( in vec3 p, in vec3 r )
{
    return (length( p/r ) - 5.0) * min(min(r.x,r.y),r.z);
}
...
float Pants(vec3 point)
{
    ...
    // Mirror
    point.x = abs(point.x);
    vec3 blendOffset = vec3(100.0, 1.5, 0.0);
    ...
    base = smin(base, hand1, 1.0);	
    base = smin(base, handBase, 1.0);
...
vec3 Render(Ray ray, Intersection isect, vec2 uv)
{
    ...
    if(isect.totalDistance < MAX_DISTANCE)
    {
        ...
        vec3 lightPosition = vec3(100., 0.0, 1.2);
