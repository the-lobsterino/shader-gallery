// Life by @Kjell
// Extending it to game-of-life-style pattern generators
...
void main()
{
	...
	else
	{
		...
		if(texture2D(backbuffer, mod(coord + vec2(  u, 0.0), 1.0)).a == 1.0)neighbours++;
		if(texture2D(backbuffer, mod(coord + vec2(  u,   v), 1.0)).a == 1.0)neighbours++;
		if(texture2D(backbuffer, mod(coord + vec2(0.0,   v), 1.0)).a == 1.0)neighbours++;
		if(texture2D(backbuffer, mod(coord + vec2( -u,   v), 1.0)).a == 1.0)neighbours++;
		if(texture2D(backbuffer, mod(coord + vec2( -u, 0.0), 1.0)).a == 1.0)neighbours++;
		if(texture2D(backbuffer, mod(coord + vec2( -u,  -v), 1.0)).a == 1.0)neighbours++;
		if(texture2D(backbuffer, mod(coord + vec2(0.0,  -v), 1.0)).a == 1.0)neighbours++;
		...
		{
			if(neighbours > 1 && neighbours < 6)cell = 1.0;
		}
		else
		{
