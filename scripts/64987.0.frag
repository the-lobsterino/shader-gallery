/*
 * Original shader from: https://www.shadertoy.com/view/wldXzX
 */
...
void init_materials() {
    ...
    material[0] = Mat(0.67, 1.0, 0.6,   0.3,   0.9 );
    material[1] = Mat(0.33, 1.0, 0.5,   0.1,   1.0 );
}

Mat get_material(int i) {
  if (i == 0) return material[0];
  else return material[1];
}

bool space = false, anim = false, soft = false, occl = false, light = false, dither = false, foggy = false, rotate = false, normal = false, depthb = false;
