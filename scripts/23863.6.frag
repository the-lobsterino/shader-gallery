...
{   
	...
	float velocity_field 	= 1.;// 1.+(prior_cell.x-prior_cell.z)*2.;
	
	float mouse_range 	= length(format_to_screen(uv)-format_to_screen(mouse));
	velocity_field		+= float(length(mouse_range)<.5)*pow(4.-mouse_range*9., 2.);
	...
	for ( int i = 0; i < 8; i++ )
		...
		//apply velocity
		neighbor_offset[i] *= 1.-velocity_field;
	...
	cell = neighbor_collision != 0. && cell.z == 0. ? vec4(cell.z, neighbor_field, cell.x, reflect_angle(1.-cell.x,neighbor_collision)) : cell;
	cell.y -= .225;
