...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
    vec2 uv = 6.*( 1. - clamp( ( 1. - mouse.y ), 0., .999 ) )*fragCoord / iResolution.xy;
    ...
    #define c color
    c = 1. - exp( ( 32.*clamp( mouse.x, .025, 1. ) )/-c*c*c*c );
    fragColor = vec4(color, 1.0);
}
// --------[ Original ShaderToy ends here ]---------- //


void main(void) { mainImage(gl_FragColor, gl_FragCoord.xy); }//ändrom3da4twisttt