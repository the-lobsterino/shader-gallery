...
void main( void ) 
{
	...
	vec3 bg = hsv2rgb_smooth(vec3(time*0.4+(uv.y+uv.x*a)*((_d+0.5)*0.05),0.3-_d,0.9-_d)) * vec3(0.3,0.5,1.2);

	
	ch_pos = ch_start + vec2(sin(time+_d*5.0),0.0);  // set start position

       	_ _ M Y _ A N U S _ L O O K S nl2
	N O T H I N G _ L I K E _ T H A T
	
	vec3 color = mix(vec3(1.0, 0.6, 0.2), vec3(0.0) , 1.0- (0.08 / d));  // shading
