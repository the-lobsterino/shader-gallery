...
vec4 render(vec2 p,float ti){
	...
	di = gd(e,r);
	if(di==far){
		return vec4(sin(p.x*p.x+p.y*p.y*time),1.0,0.0,1.0);
		}
	n = gn(e+di*r);
...
void main( void ) {
	vec2 p = 2.0*gl_FragCoord.xy/ares-vec2(1.0);
	...
	color *= sin( time / 1.0 ) * 20.5;
	vec4 r = render(p,time);
	gl_FragColor =r+vec4(color*0.1,color,color,1.0);
