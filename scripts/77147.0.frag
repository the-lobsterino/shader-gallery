...
const vec2 ch_start = vec2 (ch_space.x * -6., 2.); // start position
      ...
      vec3 ch_color = vec3 (2.0, 8.0, 1.5);        // character color (R,G,B)
const vec3 bg_color = vec3 (0.0, 0.0, 0.0);        // background color (R,G,B)

#define REPEAT_SIGN false // True/False; True=Multiple, False=Single

/* 16 segment display...Akin to 
Display.
...
void ddigit(int n)
{
	...
	if (bit(n, 4)) v = min(v, dseg(vec2( 0.000, -0.938), vec2( 0.000, -0.063)));
...
void main( void ) 
{
	...
	ch_pos = ch_start + vec2(sin(time),0.0);  // set start position
        ...
        _ I K E R _ I S _ A _   nl1 
		_ G O O D _ P E R S O N  nl2
		_ _ O F _ G O O D
