...
void main()
{
    ...
    float specularFactor = shines;
    float ambientFactor = 0.1;

    if (u_isUsingDiffuseMap == false) diffMatColor = vec4(diffuseColor, 1.0);
    vec4 diffColor = diffMatColor * u_lightPower * max(0.0, dot(usingNormal, -lightVect));
    resultColor += diffColor;
    vec4 ambientColor_l = ambientFactor * diffMatColor;
    resultColor += ambientColor_l * vec4(ambientColor, 1.0);

    vec4 specularColor_l = vec4(1.0, 1.0, 1.0, 1.0) * u_lightPower * pow(max(0.0, dot(reflectLight, -eyeVect)), specularFactor);
    resultColor += specularColor_l * vec4(specularColor, 1.0);
