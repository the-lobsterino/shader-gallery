/*
 * Original shader from: https://www.shadertoy.com/view/MdS3Rw
 */
...
{
	vec3 pos = vec3((2.*gl_FragCoord.xy-resolution)/min(resolution.x,resolution.y)*2., 5.);
	// default ray dir
	vec3 dir = vec3(0.,0.,-1.);
		//ray_dir( 45.0, iResolution.xy, fragCoord.xy );
	...
	if ( depth >= clip_far ) {
		...
		vec3 n = gradient( pos );
		fragColor = vec4( shading( pos, n, eye ) * 2.0, 1.0 );
    }
}
