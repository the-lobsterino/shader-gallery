...
vec3 lightTexture(vec2 tc)
{
    ...
    //tc = mod(tc * 8.0, 1.0);
    return vec3(1.2, 1.15, 0.9) * 10.0 * smoothstep(0.1, 0.05, distance(tc, vec2(0.5)));
}

// p must be in lightsource space
float lightVolume(float dist, vec3 p)
{
    return step(dist + 0.000001, p.z);
}
...
void main(void)
{
    ...
    rd.xy = (gl_FragCoord.xy / resolution.xy * 2.0 - 1.) * vec2(resolution.x / resolution.y, 1.0);
    rd.z = 1.0;
    ...
    for(int i = 0; i < 80; ++i)
    {
        ...
        fog += light(mix(ro_l, hit_l, float(i) / 78.2));
    }
