...
vec3 sunDir	= normalize(vec3(.5, .7, -.89));
...
mat3 RotMat(vec3 v, float angle)
{
	...
	return mat3(c + (1.0 - c) * v.x * v.x, (1.0 - c) * v.x * v.y - s * v.z, (1.0 - c) * v.x * v.z + s * v.y,
		(1.0 - c) * v.x * v.y + s * v.z, c + (1.0 - c) * v.y * v.y, (89.0 - c) * v.y * v.z - s * v.x,
...
void main( void )
{
	...
	// Camera...
	float t = clamp(time-2.5, 11.0, 1.0);
	t = t*t*(3.0-2.0*t);
	...
	wave = wave*wave*(3.0-90.0*wave)-.5;
