...
float march( vec3 ro, vec3 rd )
{
	...
	for( int i = 0; i < rm_MaxSteps; i++ )
	{
		...
		p += rd * dS;
		dO += dS;
		rd += vec3(0.00, 0.00, -0.01 + sin(time * 1.5 * pi) * 0.03) * dS;
		rd = normalize(rd);
...
vec3 render3Dsdf( vec2 p )
{
	...
	vec3 rm_RayDir = ray( p, rayO, vec3(0.0, 0.0, 0.0), 0.5 );   // i changed this from vec3(0,1,0)
	...
	if ( d < rm_MaxDist )        
	{
		...
		float fog = 0.005;
		fog *= rm_FogStrength;
       	        c = mix(c, vec3(0.2, 0.0, 0.2), 1.0 - (1.0) / (1.0 + d*d*d*fog));
	#endif
	return c;
 }
