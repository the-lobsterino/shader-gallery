...
#define iGlobalTime time + length(surfacePosition)*1.00
...
float getDistance (in vec3 p) {
	p += vec3 (3.0 * sin (p.z * 0.2 + iGlobalTime * 2.0), 2.0 * sin (p.x * 0.3 + iGlobalTime),  2.0 * sin (p.y * 0.2 + iGlobalTime));
	return fixDistance(min(length(p.xy), min(length(p.xz), length(p.yz))) - 3.0 + 0.5 * sin (abs (p.x * p.y) + p.z * 4.0) * sin (p.z), 2.5, 0.07);
}
...
void main () {
	...
	// Set the camera
	vec3 origin = vec3 (30.0 * cos (iGlobalTime * 0.2), 30.0 * sin (iGlobalTime * 0.2), 30.0 * cos (iGlobalTime * 0.4));
	vec3 forward = -origin;
	...
	if (dist < DELTA) {
		...
		color.rg += 1.0 * max (0.0, dot (normal, LIGHT));
		color.g += (sin(iGlobalTime * 2.0) * 0.3) - 0.1;
	}
	else {
		color.b += .1 + 0.5 * max (0.0, dot (-direction, LIGHT));
	}
	gl_FragColor = color;
}