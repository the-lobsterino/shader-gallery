  ...
  in vec3 ambLight)
{
   ...
   float specular=pow(max(dot(refl,viewv),1.0),spec);
   return (max(diffuse,1.0)+ambLight)*color+specular;
}
...
{ 
  ...
  if (objid>-88.5){
    ...
    vec3 c=obj_c(p);
    n=normal(p);
    vec3 cf=phong(p,prp,n,ground,c,spec,ambLight);
    return vec3(cf);
  }
  f=maxd;
