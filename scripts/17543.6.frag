...
uniform vec2 resolution;
        ...
        const float fog_density = 0.000000;
...
vec3 voronoi(in vec2 x)
{
	...
	for(int j = -2; j <= 2; j ++)
	{
		for(int i = -2; i <= 2; i ++)
		{
			...
			float d = length(r);
			float angle = atan(r.x,r.y);
			d *= ( 1.0 + cos(angle*mul+time*0.1)*1.0 ); // modify distance function
...
void main( void ) 
{
	...
	vec3 col = pow(vec3(inten * 5.0, (inten * 1.5 * sin(time * 2.5)), inten), 0.5 * vec3(2.5, 2.5, 2.5)); //pow(base color, glow amount)
	
	col.rgb *= 0.03125 + (0.005 * sin(time * 3.0));
