...
float hash2(in vec2 p) {
	return fract(sin(dot(p, vec2(15.24, 36.78))) * 43178.26);
}
...
float path(in float z) {
	return sin(z/4.0)*cos(z/8.0) + sin(z*0.1) * 5.0;
}
...
float map(in vec3 p) {
	float tn = triNoise3d(p * 0.5, 0.0) * 0.1;
	float d = p.y + tn;
	...
	float cyl = sdCappedCylinder(q, vec2(0.5, 3.0 * (1.0 + 2.0 * b)) * 0.1) - triNoise3d(p * 2.0, 0.0) * 0.03;
	d = smin(d, road, 1.0);
	d = smin(d, cyl, 0.2);
	return d;
}
...
vec3 camPos(float t) {
	return vec3(0.0, (sin(t * 2.0) * 0.5 + 0.5) * 0.01, t);
}
...
void main() { 
	...
	vec3 ro = -camPos(-time) + vec3(0.0, 1.2, 0.0);
	vec3 target = -camPos(-time - 5.0) + vec3(0.0, 1.0, 0.0);
	...
	float col = p.y;
	vec3 color = vec3(1.0);
	vec3 col_r = vec3(0.75, 0.0, 0.0);
	vec3 col_y = vec3(0.9, 0.6, 0.2);
	float depth = 0.0;
	float d = 0.0;
	float spe = 0.0;
	float bak = 0.0;
	if(t < 20.0) {
		vec3 pos = ro + rd * t;
		vec3 lig_pos = vec3(target.x, 15.0, 50.0 + time);
		vec3 lig = normalize(lig_pos - pos);
		vec3 nor = calcNormal(pos);
		float dif = clamp(dot(nor, lig), 0.0, 1.0);
		bak = 1.0 + dot(rd, nor);
		spe = pow(clamp(dot(rd, reflect(lig, nor)), 0.0, 1.0), 30.0);
