...
{
    const vec3 c = vec3(8.0);
    vec3 w = mod(p, c)-0.5*c;
    vec3 hashid=hash3(floor(p/12.0));
    w = rotationMatrix(hashid, iTime*2.5)*w;
    
    p = rotationMatrix(vec3(0.70, 1.0, 0.0), 1.391*iTime) * p;
    ...
    fractal = min(fractal, length(p)-0.95);   // center sphere
    sd = min(sd, fractal);
    ...
    if (astroids < sd)
    { 
        sd = astroids-rmf(w, 32.0, 0.5)*0.0100;
        m = 4.0;
     }  
    return sd;
}
...
void mainImage( out vec4 fragColor, in vec2 fragCoord ) 
{
    ...
    if (hit == true)
    {
	    ...
	    vec3 u_diffuseColor = mix(vec3(0.25, 0.3, 0.3)*4.0, vec3(0.6, 0.6, 0.8), clamp(mat, 0.0, 1.0));
        if (mat >= 2.0)
        {
        ...
        float brdf_spec= fresnel(u_fresnel0, halfVec, lightDir)  // *vec3(1.0,0.,0.)   // vec3(  )  red adition
            * geometry(N, halfVec, -rayDir, lightDir, u_roughness) 
            * distribution(N, halfVec, u_roughness) / (4.0 * NdotL_clamped * NdotV_clamped);
        vec3 color_spec= NdotL_clamped * brdf_spec * lightColor *vec3(1.,0.,0.) ; // vec3(1,0,0 ) red ad
        vec3 color_diff= NdotL_clamped * diffuseEnergyRatio(u_fresnel0, N, lightDir) * u_diffuseColor * lightColor;
        vec3 color = max(vec3(0.0), color_spec)+color_diff+clamp(-N.z+0.25, 0.0, 1.0)*vec3(0.0, 0.98, 0.0)*u_diffuseColor; // + sh_light(rotationMatrix(vec3(0.0, 1.0, 0.0), 1.9)*N, beach)*0.15;
        fragColor = vec4(ambo*shadow*color, 1.0);                               // de vec3(0.8, 0.8,  1.0) change
