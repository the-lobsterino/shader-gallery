...
const vec3 kAmbientColor = vec3(0.0, 0.1, 0.1);
...
float X1(vec2 p,float d){d=LR(p,d);return line2(d,p,vec4(-2,-0.25,2,-0.25)*CHS);}
float X2(vec2 p,float d){d=line2(d,p,vec4(0,-3.25,0,3.25)*CHS);p.y=abs(p.y);return line2(d,p,vec4(1.5,3.25,-1.5,3.25)*CHS);}
float X3(vec2 p,float d){d=TB(p,d);d=line2(d,p,vec4(-2,3.25,-2,-0.25)*CHS);d=line2(d,p,vec4(-2,-0.25,2,-0.25)*CHS);return line2(d,p,vec4(2,-0.25,2,-3.25)*CHS);}
float X4(vec2 p,float d){d=line2(d,p,vec4(0,-3.25,0,3.25)*CHS);return line2(d,p,vec4(2,3.25,-2,3.25)*CHS);}
...
float message(vec3 p)
{
	...
	p.y -= sin(t1+p.x*0.55)*0.6;
	vec2 uv = p.xy + vec2(1.5, -1.1);
	d = X3(uv,d); uv.x -= cw;
	d = X1(uv,d); uv.x -= cw;
	d = X2(uv,d); uv.x -= cw;
	d = X4(uv,d);
...
{   
    p.y += 0.2;
    float b = message(p);
    //float c = p.y+0.3;	//sdBox(p + vec3(0.0, 0.2, 0.0), vec3(4.0, 0.2, 4.0));
	float c = sdBox(p + vec3(0.0, 5.2, 0.0), vec3(4.0, 5.2, 4.0));	
	float t1 = fract(time*0.13)*TAU;
	float t2 = fract(time*0.16)*TAU;
	c-=sin(t2+p.x*3.4)*(0.05*sin(t1));
	c+=sin(t1+p.z*1.8)*0.2;
    return opUnion(RayResult(c, kMaterialBlue), RayResult(b, kMaterialRed));
}
