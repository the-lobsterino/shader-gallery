...
float R(vec2 p,float d){p.y+=1.75*CHS;d=min(d,abs(sdBox2(p,vec2(2.0,1.5)*CHS)));p+=vec2(0.5,-3.25)*CHS;return min(d,abs(sdBox2(p,vec2(1.5,1.75)*CHS)));} float U(vec2 p,float d){d=TB(p,d);d=line2(d,p,vec4(-2,3.25,-2,-3.25)*CHS);return line2(d,p,vec4(0,-0.25,-2,-0.25)*CHS);} float I(vec2 p,float d){d=line2(d,p,vec4(0,-3.25,0,3.25)*CHS);p.y=abs(p.y);return line2(d,p,vec4(1.5,3.25,-1.5,3.25)*CHS);} float _R(vec2 p,float d){d=line2(d,p,vec4(0.5,-0.25,2,-3.25)*CHS);d=line2(d,p,vec4(-2,-3.25,-2,0.0)*CHS);p.y-=1.5*CHS;return min(d, abs(sdBox2(p,vec2(2.0,1.75)*CHS)));} float T(vec2 p,float d){d=line2(d,p,vec4(0,-3.25,0,3.25)*CHS);return line2(d,p,vec4(2,3.25,-2,3.25)*CHS);} float X(vec2 p,float d){d = line2(d,p,vec4(-2,3.25,2,-3.25)*CHS);return line2(d,p,vec4(-2,-3.25,2,3.25)*CHS);} // DOGSHIT

float GetText(vec2 uv)
{
//	CHS = 0.05;//0.04+sin(time*3.1+uv.x*2.5)*0.01;
	float g = 0.3;
	float d = R(uv,4444.0);uv.x -= g;
	d = _R(uv,d);uv.x -= g;
	d = U(uv,d);uv.x -= g;
	d = X(uv,d);uv.x -= g;
	d = I(uv,d);uv.x -= g;
...
float map( vec3 p )
{
	...
	d = smin(p.z+0.25+v2*0.5+(sin(p.y*2.0+time*2.2+p.x*.75)*0.1),d,2.0);
	float t = 1.5*iTime;
...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
	...
	float dif = clamp( dot(nor,dir), 0.15, 1.0 );
	    ...
	    float vv = fract(pos.z*0.8+pos.x*0.8+time*0.5+pos.y*0.2);
	    vv = 1.0+sin(vv*6.28)*0.25;
	col =  vec3(vv*0.2,vv*0.6,vv)*dif;//hsv2rgb_smooth(vec3(pos.y+time*0.1,0.7,0.7))*dif;
	col+=spec;
