...
material assign_material(in float material_index, in vec3 position)
{
    ...
    else if(material_index == 2.)
    {
	    vec2 p          	= (1.+position.xz)*.5-.125;
            ...
            m.color         	= max(vec3(0.0001), hsv(1.-p.x, p.y, clamp(.5+(p.x+p.y)*.5, 0.001, 1.)));
...
//  shcgray(c);
    ...
    {               
		...
		if(r.material_range.y >= .0 || fract(r.material_range.x) != 0.)
		{
	    	...
	    	sf.color	+= .05 * (stepFog*r.material_range.x*(1./m.refractive_index)*.5);
	    	sr.color	+= .025 * (stepFog*r.material_range.x*(1./m.refractive_index)*.5);
		#endif
		...
		sf      	= caustic(r, s, mf, l);
		sf.color	+= clamp(.05 * (stepFog*r.material_range.x*(stepFog/m.refractive_index)*(1./m.roughness)*.25-m.roughness*128.),0., .25);
		}
        	#endif    

	    result = (m.transparency*.5+.25+s.color) * mix(s.color + sr.color+sr.color*result, result * s.color + sf.color * (1.-m.roughness) + sr.color + result, abs(m.transparency * m.transparency)) + m.refractive_index * result;
    }


    result.xyz += pow(lf, vec3(stepFog+.5));
    result.xyz = pow(result.xyz * .5, vec3(1.2));
...
ray emit(ray r)
{
	...
	for(int i = 0; i < ITERATIONS; i++)
	{
		if(total_range < FARPLANE)
		{
			...
			threshold          *= 1.014;
			r.position         += r.direction * r.material_range.y * .75;
...
surface shade(in ray r, in surface s,  in material m, in light l)
{
	...
	// compositing
	s.color.xyz     	= light_exposure * m.color * n * l.color + brdf * l.color;
