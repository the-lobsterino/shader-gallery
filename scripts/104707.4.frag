...
const vec2 ch_size  = vec2(1.5, 2.5) * 0.6;              // character size (X,Y)
...
void main( void )
{
	...
	uv.x += 5.0+sin((time*2.0)+uv.y*0.025)*12.5;
	uv.x+=3.;
	ch_pos = ch_start;
	
	W O W s_comma _ T H E _ W A V Y _ P A R T s_dot L O L s_question
