...
// Based on / copied from https://www.shadertoy.com/view/XdX3WN
// Plasma Triangle by ElusivePete https://www.shadertoy.com/user/ElusivePete

void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
	float pointRadius = 0.06;
	float linkSize = 0.04;
	float noiseStrength = 1.00; // range: 0-1
	...
	pointR.y += 0.3 * sin(4.03 * iGlobalTime);
	pointR.z += 0.4 * sin(1.32 * iGlobalTime);
	
	pointG.x += 0.31 * sin(1.92 * iGlobalTime);
	pointG.y += 0.29 * sin(2.99 * iGlobalTime);
	pointG.z += 0.38 * sin(3.24 * iGlobalTime);
	
	pointB.x += 0.33 * sin(1.245 * iGlobalTime);
	pointB.y += 0.3 * sin(8.41 * iGlobalTime);
