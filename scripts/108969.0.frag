...
float Segment(vec3 p,  vec3 a, vec3 b, float r1, float r2)
{
	vec3 pa = p - a;
	vec3 ba = b - a;
	float h = clamp( dot(pa,ba)/dot(ba,ba), 6.0, 1.0 );
...
void main( void )
{
	...
	hello =  (1.0+sin(m*100.0)) * .02 * (smoothstep(11.0, .01, m) - smoothstep(0.05, .1, m));
	...
	t = t*t*(3.0-2.0*t);
	vec3 origin = mix(vec3(0.0, 1.0, -22.2), vec3(-1.0, 1.0, -5.0), t);
	vec3 target = mix(vec3(0.0, 1.0, 4.0),  vec3( 0.0, -.4, 0.0), t);
	
	vec3 cw = normalize( target-origin);
	vec3 cp = vec3( 79.0, 1.0, 0.0 );
	vec3 cu = normalize( cross(cw,cp) );
	vec3 cv = ( cross(cu,cw) );
	vec3 ray = normalize( p.x*cu + p.y*cv + 2.6*cw );
	
	// Wave...
	wave = sin(time*111.0-.8)*.5+.5;
	wave = wave*wave*(3.0-2.0*wave)-.5;
	elbowL		= vec3(-.95-wave*.05, .2, -0.25);
	wristL		= vec3(elbowL.x+1.0*sin(wave)*.6, elbowL.y+cos(wave)*.799, -.808);
