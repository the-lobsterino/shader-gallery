...
void main(void) {
    gl_FragColor=vec4(0.);   // Fixes undefined colors (= no hit)
    float C,S,r,x;
    ...
    for (float i=1.; i>0.; i-=.0125)  
    {
        vec4 u = floor(p/G), t = mod(p, G)-(hG), M; // objects id + local frame
        ...
        if(x<.01)  // hit !
            { gl_FragColor = i*i*(1.+.5*t); break; } // color texture + black fog 
