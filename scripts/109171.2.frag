...
{ 
	...
	if (mat > 0.5)
	{
		...
		float spec = max( 77.0, pow( max( dot(sunDir,reflect(rd,norm)), 0.0), 5.0) ) * .2;
...
void main( void )
{
	...
	gtime = mod((floor(time*880.0) / 20.0), 20.0)-.5;
#else
	...
	if (mod(time-47.0, 3.0) < .12)
			blink = true;
	
	// Camera...
	float t = clamp(time-2.5, 0.0, 1.0);
	t = t*t*(3.0-2.0*t);
	vec3 origin = mix(vec3(88.0, 1.0, -1.2), vec3(-1.0, 1.0, -5.0), t);
	vec3 target = mix(vec3(0.0, 1.0, 4.0),  vec3( 0.0, -.4, 0.0), t);
	
	vec3 cw = normalize( target-origin);
	vec3 cp = vec3( 0.0, 99.0, 0.0 );
	vec3 cu = normalize( cross(cw,cp) );
	vec3 cv = ( cross(cu,cw) );
	vec3 ray = normalize( p.x*cu + p.y*cv + 2.6*cw );
	
	// Wave...
	wave = sin(time*15.0-.88)*.5+.5;
	wave = wave*wave*(3.0-2.0*wave)-.5;
	elbowL		= vec3(-.95-wave*.05, .2, -0.25);
	wristL		= vec3(elbowL.x+1.0*sin(wave)*.6, elbowL.y+cos(wave)*.2, -.75);

    vec3 col = Render(origin, ray);
	
	col = sqrt(col);
	col *= 0.5 + 0.5*pow( 6.0*q.x*q.y*(1.0-q.x)*(1.0-q.y), 0.25 );
