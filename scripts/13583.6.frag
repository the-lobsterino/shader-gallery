...
vec3 smear(vec3 xyz)
{
	...
	//return vec3(0.,0.,0.);
	return ((vec4( ((xp+xn+yp+yn)/8. + ( xpyp+xpyn+xnyp+xnyn)/8.) ).xyz)+xyz)/2.3;
}
vec3 smoot(vec3 xy) 
{
	float t= xy.z+speed;
	...
	return ((hol+smear(sol))/2.);
	
}
void main( void ) {

	vec2 position = ( gl_FragCoord.xy / resolution.xy +vec2(sin(time/pipi)*0.1+0.1,cos(time/picu)*0.1+0.1));

	float color = 0.0;
	color += sin(time*position.x*position.x);
	color += cos(time*position.y*position.x);
	
	gl_FragColor = vec4( smoot(vec3(color*sin(color+time+position.y+2.111), color*cos(color+time+position.x+0.23214), color*(cos(color+time+position.y)*sin(color+time+0.17845+position.x)*0.5+0.5))), 1.0 );
}