...
const vec4 iMouse = vec4(195.);

// Emulate a black texture
#define texture(s, uv) vec4(0.0)


//(customizable portion start)
#define cste 0.05 
//(set   ^^ to 0.905 for extra trippy effects)
#define dist 34.5
//(set dist to however far you want the view point to be from)
...
{  
    ...
    // Eye position
    vec3 eye = vec3(dist*cos(tetha)*sin(phi),12.*cos(phi),12.*sin(tetha)*sin(phi));
