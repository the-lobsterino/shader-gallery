...
// but what the question is what happens inside a 2d slice of a 4d%2d/3d?
...
float fn(vec2 p)
{
	float c = cos(surfaceSize.x*surfaceSize.y);
	float a = (gl_FragCoord.y * resolution.x + gl_FragCoord.x) * c;
	float b = (mouse.y*resolution.x - mouse.x) * c;
	float m = (p.x*p.y);
	m = cos( ( (m) * (TAU * time) - TAU * (p.x-p.y)) + (a));
	return cos( (abs(m)) );	
}
