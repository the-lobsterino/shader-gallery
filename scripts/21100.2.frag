...
//manga face from http://glslsandbox.com/e#743.2
...
vec3 mangaface() {
  vec3 color;
  return filter(ikachan(color) ? color : backgroundColor);
}
...
void main(void)
{
	...
	if ( depth >= clip_far ) {
		...
		//pFace = surfacePosition*2.+vec2(-1.0,0.5); //test for centering around 0,0
		vec2 pside = vec2(max(pos.x,pos.y),max(pos.y,pos.z));
		pFace = pside*2.0+vec2(-1.0,0.5);
		pFace = mod(pFace,4.0)-2.5; //wrap
		//pFace /= 10.0;
		vec3 colFace = mangaface();
		gl_FragColor.xyz = colFace;
		gl_FragColor.w = 1.0;
