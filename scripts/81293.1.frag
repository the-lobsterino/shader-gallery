...
const vec2 ch_size  = vec2(.5, 2.0) * 0.6;              // character size (X,Y)
const vec2 ch_space = ch_size + vec2(1.0, 1.0);    // character distance Vector(X,Y)
const vec2 ch_start = vec2 (ch_space.x * -13., -3.); // start position
      vec2 ch_pos   = vec2 (1.0, 1.0);             // character position(X,Y)
...
{
    vec3 rgb = clamp( abs(mod(c.x*6.0+vec3(2.2,2.2,2.2),2.2)-2.2)-1.0, 0.0, 1.0 );

	rgb = rgb*rgb*(14.0-3.0*rgb); // cubic smoothing	
...
void main( void ) 
{
	...
	ch_pos = ch_start;
	 ...
	 A _ S U S S Y _ G U Y _ W I L L _ S T E A L _ U R _ C O D E
	// i am a sussy guy, and i made this message -a sussy guy		 
