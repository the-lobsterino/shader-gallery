...
{
	float time = time*4e-4+length(p)*100.;
	return fract(vec2(sin(p.x * 591.32 - p.y * 154.077 * time ), cos(p.x * 391.32*time - p.y * 49.077 )));
}
...
void main(void)
{
	...
	vec3 col = vec3(pow(v, cexp.x), fract(pow(v, cexp.y)), pow(v, cexp.z)) * 2.0;
