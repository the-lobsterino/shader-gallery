...
void main( void ) {
	...
	vec2 rnd = vec2(mod(fract(sin(dot(position + time * 0.001, vec2(14.9898,78.233))) * 43758.5467676753), 1.0),
	                mod(fract(sin(dot(position + time * 0.001, vec2(24.9898,44.233))) * 27458.5453), 1.0));
	vec2 nudge = vec2(02.0 + 10.0 * cos(time * 0.03775),
	                  02.0 + 10.0 * cos(time * 0.02246));
	vec2 rate = -0.005 + 0.02 * (0.5 + 0.5 * cos(nudge * (position.yx - 0.1) + 0.1 + time * vec2(0.1397, 1.162)));
	...
	if (length(position-mouse) < mradius) {
		...
		rate *= 6.0 * abs(vec2(1.5, 1.5) - mouse);
		rate += 1.5 * rate.yx;
		vec2 mult = 1.0 - rate;
		vec2 jitter = vec2(1.1 / resolution.x,
		                   1.1 / resolution.y);
		vec2 offset = (rate * mouse) - (jitter * 0.0);
