...
void main( void ) {
	...
	float d = 0.5 / dd + time;
   	...
   	x=x*Scale*R/R.x;
    	...
    	c2=lungth(tan(x*sqrt(vec2(3,43))),vec3(5,6,7)*Saturation * d);
	x+=tan(x.yx*sqrt(vec2(73,5)))/5.;
    	c2=2.*lungth(tan(time+x*sqrt(vec2(33.,23.))),c2/9.);
    	x+=tan(x.yx*sqrt(vec2(93,7)))/3.;
    	c2=lungth(tan(x*sqrt(vec2(3.,1.))),c2/2.0);
    	c2=.5+.5*tan(c2*8.);
	
	vec3 uv = vec3(th + d, th - d, th + sin(d) * 0.45);
	float a = 0.5 + tan(uv.x * 3.1415926 * 2.0) * 0.5;
	float b = 0.5 + tan(uv.y * 3.1415926 * 2.0) * 0.5;
	float c = 0.5 + tan(uv.z * 3.1415926 * 6.0) * 0.5;
