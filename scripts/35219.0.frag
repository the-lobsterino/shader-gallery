...
#define Color1 vec4(0.8, 0.8, 0.8 ,1.0)
#define Color2 vec4(0.2, 0.2, 0.2 ,1.0)
#define Color3 vec4(0.2, 0.3, 1.0 ,1.0)
...
void main( void )
{
	...
	radius *= pow(min(1.0, max(0.0, gl_FragCoord.x)/200.0), 0.8);
	...
	float g = max(1.0-(mod(gl_FragCoord.y+time*300.0, 1000.0)/200.0), 0.0);
