...
vec4 nyan(vec2 p)
{
	...
	vec4 color = vec4(0.);//texture2D(iChannel1,vec2(uv.x/3.0+210.0/256.0-nt+0.05,.5-uv.y-ny));
...

{
	...
	dir.y-=.02;
	float sunsize=5.-max(0.,0.-.4)*5.; // responsive sun size
	...
	vec3 backg=vec3(0.5,0.,1.)*((1.-s)*(1.-sg)*y+(1.-sb)*sg*vec3(1.,.0,0.0)*3.);
		 backg+=vec3(0.0,1.,.0)*s;
...
void main()
{
	...
	if (mouse.y<1.) mouse=vec2(0.,-0.05);
	...
	#ifdef BORDER
	color=mix(vec3(0.0),color,pow(max(0.,.95-length(oriuv*oriuv*oriuv*vec2(1.05,1.1))),.3));
	#endif
