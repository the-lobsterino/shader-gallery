...
//  ↙
//   ↙
// https://th.bing.com/th/id/R.d72808df2efa5014f1fff0e8c775f836?rik=BFv771wf3l5HUQ&pid=ImgRaw&r=0
...
const vec2 ch_size  = vec2(1, 2.0) * 0.6;              // character size (X,Y)
const vec2 ch_space = ch_size + vec2(1.0, 1.0);    // character distance Vector(X,Y)
const vec2 ch_start = vec2 (ch_space.x * -7., 3.); // start position
...
{	
	...
	uv.x += 0.5 + sin(length(uv)*1.7 + time)*0.3;
	ch_pos = ch_start;
	      _ _ I _ H A V E _ E X P L O S I V E nl1 _ _ _ _ D I A R R E A H nl3 O H _ F U C K
