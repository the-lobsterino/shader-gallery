...
//I don't know anything about complex numbers.
//But it's just numbers in 2D?
...
vec2 f(vec2 z, vec2 w) {
	const float loops = 20.;
	float speed = 2.;
	for(float i=0.;i<loops;i++){if(mod(time*speed,loops)<i){break;}z = mix(z,iter(z),min(1.,fract(time*speed)+(mod(time*speed,loops)-i)));}
	return z;
}
