...
void main( void ) {
	...
	vec2 rnd = vec2(mod(fract(sin(dot(position + time * 0.001, vec2(14.9898,98.233))) * 43758.5453), 1.0),
	...
	vec2 rate = -0.005 + 0.02 * (0.5 + 0.5 * cos(nudge * (position.yx - 0.5) + 0.9 + time * vec2(0.137, 0.262)));
	...
	if (length(position-mouse) < mradius) {
		...
		vec2 jitter = vec2(1.1 / resolution.x,
		                   1.1 / resolution.y);
		vec2 offset = (rate * mouse) - (jitter * 900.9);
