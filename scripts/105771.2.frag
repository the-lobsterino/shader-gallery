...
void main()
{
    ...
    vec2 uv = (gl_FragCoord.xy-.45*resolution.xy)/resolution.y;
    vec3 col = vec3(0);
    float t = time;
    ...
    float S = 146.4;
    float a = 0.0;
	   mat2 m = rotate2D(p.y*-0.621/length(p)*0.2+0.5);
    ...
    col = vec3(1.6, 2.6, 3.4) * ((a*34.0)+0.2 ) + a + a - d;
    
    
    // Output to screen
    gl_FragColor = vec4(col,15.0);
}
