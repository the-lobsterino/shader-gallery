...
// mod for heavy load-- underrated PSU testing
...
void main( void ) {
    ...
    float t = 2.* PI * float(i) / 20. * sin(time*0.5);
    float x = cos(t) * sin(t * mouse.x);
    float y = sin(t * mouse.y);
    vec2 o = 0.45 * vec2(x,y);
    float r = fract(t);
    float g = 1.-r;
    float b = 1.-r;
    c += 0.00025/(length(p-o))*vec3(r,g,1);
    }
