...
// Thematica le 8 juin 2013 : Interpolation
...
vec4 lamb=vec4(0.0);
vec3 col=vec3(0.,0.,0.);
vec3 axDepart=normalize(vec3(0.,1.,0.));
vec3 axArrivee=normalize(vec3(0.,0.,1.));
...
for(int i=0;i<50;i++)
{
	...
	vec3 centre=mix(vec3(-1.,0.,-1.),vec3(1.0,1.0,0.0),temp);;
	vec4 lamb0=vec4(sdCyl(rotate(posRayon-centre,axFinal,angle*temp), 1.0, 0.2),1.,1.,0.);
