...
#define iTime time
#define iMouse vec4(1.0,1.0,0.0,1.0)
...
#ifndef SHADERTOY_APP
    vec2 rg = vec2(noise(uv+ 0.5)/10.0).yx;
#else
    vec2 rg = vec2(noise(uv+ 0.5)/256.0).yx;
#endif
    return mix( rg.x*20., rg.y*10., f.z );
}
#else
...
void main()
{
	...
	gl_FragColor.xyz = mix( back.xyz, col.xyz, col.a );
