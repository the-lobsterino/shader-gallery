...
// @mod+ changed camera animation
// @mod+ mystic lightning, watch the fishes appearing!


// comment - checkout the original work by Escher. This is interesting but...
// rotwang: sorry for breaking your piece of art...
#ifdef GL_ES
...
void main(void)
{
  ...
  //  rd.z *= dd+1.000001;
   ...
   // fog
   float d = length(pos)*0.17;
   float f = exp(-d*d);

    // ambient vignetting
	vec2 pa = mix(pixel, pos.xy, pos.z);
	vec3 ambient = vec3(0.3, 0.5, 0.5) * (1.0-length(pa));

	ambient *= 0.5+0.5*smoothstep(1.0, 0.0, dot(pa, pa));
	
    rgb *= 0.5+0.5*smoothstep(2.0, 0.5, dot(pa, pos.xy));
