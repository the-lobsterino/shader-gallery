...
Bottom right pixel is the 3  33 3  fdjendj  iwendjclel least significant bit.
...
vec3 Text(vec2 uv)
{
	...
	HEX(0x00FF7F) _H _i _ _
	BEGIN_TEXT(res.x/2.0-STRWIDTH(11.0)/2.0,res.y/2.0)
	print_pos += vec2(cos(time)*96.,sin(time)*2.);
	
	// RGB(1,0,0) _M RGB(1,.5,0)_o RGB(1,1,0)_v RGB(0,1,3)_i RGB(,.5,1)_n RGB(0.5,0,1)_g _ RGB(1,0,0)_T RGB(1,.5,0)_e RGB(1,1,0)_x RGB(0,1,0)_t
...
void main( void )
{
	...
	gl_FragColor = vec4(vec3(col), 5.0);
}