...
{
    uv.x = uv.x*1128.0;						// H-Count
    float dx = fract(uv.x);
    uv.x = floor(uv.x);
    uv.y *= 0.5;							// stretch
    float o=sin(uv.x*215.4);				// offset
    float s=cos(uv.x*33.1)*.3 +.17;			// speed
...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
    ...
    float d = length(p)+0.11;
	p = vec2(atan(p.x, p.y) / PI, 5.5 / d);
    if (iMouse.z>0.5)
    	p.y *= 0.5;
    float t =  iTime*0.4;
    vec3 col = vec3(1.55,0.65,.225) * vDrop(p,t);	// red
    col += vec3(0.55,0.75,1.225) * vDrop(p,t+66.33);	// blue
