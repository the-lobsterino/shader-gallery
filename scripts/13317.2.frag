...
vec3 smear(vec3 xyz)
{
	...
	//return vec3(0.,0.,0.);
	return ((vec4( ((xp+xn+yp+yn)/8. + ( xpyp+xpyn+xnyp+xnyn)/8.) ).xyz)+xyz*3.)/4.;
}
vec3 smoot(vec3 xy) 
{
...
const float spinspeed = 0.1;
const float movespeed = 3.0;
const float spirals = 1.0;
const float twist = 50.0;
const float twistspeed = 0.2;

// This makes me want to throw up
// PS: Expand your variable spacing you shitbag
//~// darling, be nice //~//
//~// PS: go easy on the superfluous spaces plz kthxbai //~// 
...
void main(void) {
	...
	float distortion = time * movespeed + (0.3 * sin(time * twistspeed)) * twist / sqrt(l * 5.0);
	vec2 s = 2.0 * abs(fract(vec2(pos + distortion, 6.0 * pos) / pi * spirals) - vec2(0.5));
	float d = dot(s, s);
	float f = smoothstep(0.0, 0.1, 1.0 - d);
	vec3 color = vec3(f + s.x, f * s.x, f - s.x * 0.1);
