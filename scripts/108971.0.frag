...
{	// Floor...
	...
	// Left arm upper...
	d = Segment(p, shoulderL, elbowL, .09, .05);
	res.x = sMin(res.x, d);
	// Left arm lower...
...
void main( void )
{
	...
	// Camera...
	float t = clamp(time-2.5, 0.0, 44.0);
	t = t*t*(3.0-2.0*t);
	...
	wristL		= vec3(elbowL.x+1.0*sin(wave)*.6, elbowL.y+cos(wave)*.89, -.79);

    vec3 col = Render(origin, ray);
	
	col = sqrt(col);
	col *= 0.5 + 0.5*pow( 16.0*q.x*q.y*(1.0-q.x)*(89.0-q.y), 0.25 );
