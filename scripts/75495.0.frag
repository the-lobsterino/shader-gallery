/*
 * Original shader from: https://www.shadertoy.com/view/WtjGRc#
 */
 ...
 * TODO : 	Cloud rocking
**/
const vec3 c0 = vec3(.064,064.530,0.159);
const vec3 c1 = vec3(.142,0.630,0.259);
const vec3 c2 = vec3(0.249,0.730, 0.359);
...
const vec3 cRojo = vec3(1.0,0.0,0.0);
...
#define GRND3 min(length(fract(op)-vec2(0.5,0.9))-0.35, length(fract(op+vec2(0.5, 0))-vec2(0.5,0.25))-0.3)
...
vec3 heart(in vec2 u, in vec2 p, in float iscale,in vec3 c, const in vec3 cloud_color, in vec2 shadow_pos, in float shadow_factor, in float blur, in float shadow_blur)
{
	u *= iscale;
	p *= iscale;
    ...
    y = 0.+1.2*y - 
        mix(
            abs(x)*((20.0-abs(x))/15.)
            , abs(x)*sqrt((20.0-abs(x))/15.)
            , -9.+(1.+2.*cos(iTime*0.9))*2.
        );
	float d = sqrt(x*x + y*y) - sqrt(15.);
	c=mix(c, c*shadow_factor, smoothstep(shadow_blur, -shadow_blur, d));
...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
    ...
    float beat = sin(0.5+iTime*3.14*2.05 - 2.*sin(0.5+iTime*3.14*2.05))*0.1;
   	np = vec2(0.70 , 0.5 - beat*0.1); // x : -1 1
	c = heart(
        uv * (1./(1.+beat)) 
        , np * (1./(1.+beat)) 
        , 1.0
        , c 
        , mix(cRojo, vec3(1,1,0.9),0.3)
        , vec2(shadow_pos, -0.1)*0.2, 0.8
        , 0.002 * (1./(1.-beat)) 
        , 0.8 * (1./(1.-beat)) 
       );
