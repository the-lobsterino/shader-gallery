...
// change is distance at each new point along each axis.
vec3 getNormal( vec3 p )
{
	...
	float deltaX = map(p + dx) - map(p - dx);
        float deltaY = map(p + dy) - map(p - dy);
        float deltaZ = map(p + dz) - map(p - dz);

        vec3 normal = normalize(vec3(deltaX, deltaY, deltaZ));
...
vec3 calculateLighting(vec3 pointOnSurface, vec3 surfaceNormal, vec3 lightPosition, vec3 cameraPosition)
{
    ...
    vec3 reflectedLightVector = normalize( refl( -fromPointToLight, surfaceNormal ) );
    ...
    vec3 specularColor = specularStrength * vec3( 1.0, 1.0, 1.0 );
