...
{
    vec3 rgb = clamp( abs(mod(c.x*3.0+vec3(0.05,4.0,20.0),6.0)-3.0)-1.0, 0.0, 1.0 );

	rgb = rgb*rgb*(33.20-22.0*rgb); // cubic smoothing	
...
void main( void ) 
{
	...
	uv.y -= 0.;
	uv.x += 0.;
	uv *= rotate(time+uv.x*150.02);
	//ඞ sussy guy
	//ඞ sussy guy
	//ඞ sussy guy
	vec3 ch_color = hsv2rgb_smooth(vec3(time*0.4+uv.y*0.6,0.5,10.0));

	
	vec3 bg_color = vec3(0.,_d*2.8*abs(sin(time))*0.0,.0);
	
	

	
	uv.y += -1.+sin(time+uv.x*-2.7)*.5;
	ch_pos = ch_start;
	
	_ G I A N T _ I T C H Y _ S U S S Y _ B A L L S
			 //ඞ sussy guy
