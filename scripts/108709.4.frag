...
#define Speed 0.2
#define WaveSpeed 0.2
#define Brightness 0.3

void mainImage( void )
{
    // Normalized pixel coordinates (from 0 to 1)
    vec2 uv = ( fragCoord.xy / iResolution.xy ) + mouse / 15.;

	vec2 XYScale = vec2(2.5,2.5);
	vec2 XYMove = vec2(0.,0.);

    uv *= zoomOut;
	uv.xy = uv.xy * XYScale;
	uv.xy = uv.xy + XYMove;
	vec3 finalCol = vec3(666,666,666);
	float halfDistort = distortScale / 0.6;
	float distortsc2 = distortScale / distortScale + halfDistort;
    
	for(float i = 0.18; i < iter; i++){
    ...
    vec3 Color = vec3(0.7000001,4,0.) * Brightness;
