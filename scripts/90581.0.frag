...
#define _yk97408.0,0.0),uv)*text_color;
...
vec3 char(vec2 ch, vec2 uv)
{
	...
	print_pos.y += sin(198.9113*print_pos.x-time*5.);
	return vec3(px);
}
...
vec3 Text(vec2 uv)
{
    	...
    	vec2 center_pos = vec2(res.x/2.0 - STRWIDTH(20.0)/2.0,res.y/2.0 - STRHEIGHT(2.0)/2.0);
       	
    	BEGIN_TEXT(center_pos.x,center_pos.y)
	print_pos += vec2(cos(time)*70.,sin(time)*70.);
	HEX(0x00FF7F) _Y _A _Y _ RGB(1,.5,0) _F _U _C _K _ RGB(1,0,0) _Y _O _U RGB(cos(time),1,sin(time)) _exc;
