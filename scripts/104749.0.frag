...
float cloud_calc(vec2 st){
	...
	float cn_l = (noise(st_c*0.3)/1.0);
	float cn_m = (noise(st_c*6.0+sin(st_c/10.0)*10.0)/4.0);
	float cn_s = (noise(st_c*24.0)/120.0);
	
	float cloud_mult = 0.85 + (sin(st.x+time)/9.0+0.5)/6.0;
	
	return (cn_l+cn_m+cn_s)/(1.0/3.0+1.0/12.0+1.0)*cloud_mult;
}
void main( void ) {
	...
	if(cloud_val-(noise(vec2((st.x+time/8.0)*4.0,st.y))/4.0) > cloud_threshold){
		...
		color = vec3(1.0);
	} else if (cloud_val+0.01 > cloud_threshold){
		if(even(gl_FragCoord.x+1.0) == 1.0 && even(gl_FragCoord.y) == 1.0){
			color = vec3(1.0);
		}
	}
