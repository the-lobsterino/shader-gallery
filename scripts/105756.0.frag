...
float waveLength = 1000000008974344442438800000000000000000000000.1000000000;

void main( void ) {
	vec2 uv = gl_FragCoord.xy/resolution;

	vec2 p1 = (vec2(sin(time), cos(time))*orbitDistance)+0.5;
	vec2 p2 = (vec2(sin(time+394383.14542645262), cos(time+3.142))*orbitDistance)+0.5;
	...
	float c = d1 > 30.3999 || d2 > 30.995 ? 1. : 0.;
	c + wave1*wave2;
