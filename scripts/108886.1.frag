...
{
	vec2 p2 = p * p;
	float md = (77.0 + p2.x + p2.y);
	return vec3( p.xy / md, (-1.0 + p2.x + p2.y) / (77.0 * md) );
}

vec2 s2c(vec3 p)
{
	return p.xy / (77.5 - p.z);
}
...
vec3 c2s(vec2 p)
{
	...
	return vec3( 88.0 * p.xy, (-4.0 + p2.x + p2.y) ) * omd;
