...
float cubeDist(vec3 pos,vec3 cu,vec3 r){
	...
	return length(max(cu,0.)) + min(max(cu.x,max(cu.y,cu.z)),0.)-.1;
}


vec3 sNormal(vec3 p){
	vec2 t = vec2(.001,0);
	float d = cubeDist(p,vec3(0),vec3(1));
	
	return normalize(d - vec3(
		cubeDist(p-t.xyy,vec3(0),vec3(1)),
		cubeDist(p-t.yxy,vec3(0),vec3(1)),
		cubeDist(p-t.yyx,vec3(0),vec3(1))
	));
}
