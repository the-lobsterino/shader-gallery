...
void getMaterialColor( const int material, in vec2 uv, const float decorationHash, out vec3 col )
	...
	// flag
		...
		fgcol = COL(255.,255.,255.)+mod(flagcoords.x,.1);
		...
		//fgcol = mix( fgcol, COL(0.,0.,0.), onCircleLine( uv*2., vec2(32.,30.), 11. ) );
		//fgcol = mix( fgcol, COL(0.,0.,0.), onCircleLine( uv, vec2(32.,30.), 9. ) );
		vec2 uvr = vec2( (uvc.x+uvc.y)*0.7071, (-uvc.y+uvc.x)*0.7071);
		fgcol-=(1.-GetText(uvr/7.));
		//fgcol = mix( fgcol, COL(72.,72.,72.), onRect( uvr, vec2(-1.,-1.), vec2(1.,4.) ) );//
		//fgcol*= 1.-GetText(uvr/5.);
...
void main()
	...
	vec3 ro = path( time/10. );
	vec3 ta = path( time/10.+0.1 );
	rdcenter = rotate( normalize( ta - ro), 0.3*cos(time/10.*0.75) );
