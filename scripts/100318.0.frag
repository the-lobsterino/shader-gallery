...
void mainImage( out vec4 O, in vec2 I )
{
    ...
    for( int n=0; n<AA; n++)
    {
        ...
        vec2 p_pre = (2.*I+o - iResolution.xy) / iResolution.y;

	float blob = sin(cos((p_pre.x * p_pre.x + p_pre.y*p_pre.y)*0.5+time*0.4)+time*0.7);
	    
	float ct = cos(time*0.5+blob);
	float st = sin(time*0.5+blob);
	    
	vec2 p;
	p.x = p_pre.x * ct + p_pre.y * st;
	p.y = p_pre.y * ct - p_pre.x * st;
