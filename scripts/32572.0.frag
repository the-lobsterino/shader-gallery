...
#define EXPOSURE 39.
#define GAMMA 2.1
#define SOFT_SHADOWS_FACTOR 5.
#define MAX_RAYMARCH_ITER 128
#define MAX_RAYMARCH_ITER_SHADOWS 3
...
const vec3 rightWallColor = vec3(.117, .125, .4115);
const vec3 whiteWallColor = vec3(.7295, .7355, .729);
const vec3 cameraTarget = vec3(556, 548.8, 559.2) * .5;
const vec3 sph1Color = vec3(.5, .5, .9);

float sdSphere(vec3 p, vec3 b) {
	vec3 d = abs(p) - b;
	return length(p)-80.0;
}

float sdLight(vec3 p, vec3 b) {
	vec3 d = abs(p) - b;
...
vec2 mapBlocks(vec3 p, vec3 ray_dir) { //  ray_dir may be used for some optimizations
	vec2 res = vec2(OBJ_SHORTBLOCK, sdSphere(rotateY(p + vec3(-186, -82.5, -169.5), 0.29718), vec3(83.66749, 83.522452, 82.5)));
	vec2 obj1 = vec2(OBJ_TALLBLOCK, sdSphere(rotateY(p + vec3(-368.5, -82.5, -351.5), -0.30072115), vec3(87.02012, 165, 83.6675)));
...
vec2 map(vec3 p, vec3 ray_dir) { //  ray_dir may be used for some optimizations
	...
	vec2 obj5 = vec2(OBJ_LIGHT, sdLight(p + vec3(-278, -548.8, -292), vec3(65, 0.05, 65)));
...
vec3 render(vec3 ray_start, vec3 ray_dir) {
	...
	if (p.z >= 0.) {
		...
		else if (objectID == OBJ_SHORTBLOCK) color = sph1Color;
		else if (objectID == OBJ_TALLBLOCK) color = sph1Color;
		...
		if (objectID == OBJ_LIGHT) {
			...
			vec3 c3_lightColor = lightColor * leftWallColor * 0.08;
