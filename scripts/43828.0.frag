...
//channel definitions to map the data and colors to the pixel.rgba in a more legible way - 0, 1, 2  = red, green, blue 
//the channels not carrying the angle are given colors if the COLOR define is true - they are purely decorative
#define ANGLE 		0
#define COLOR_MASK	vec4(ANGLE != 0, ANGLE != 1, ANGLE != 2, ANGLE != 3)


//the buffer is set to be cleared if the mouse is on the far left of the screen
#define CLEAR_BUFFER	(mouse.x < .0125)	
