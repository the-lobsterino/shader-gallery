...
#define BORDER_COLOR vec3(0.05, 0.2, 1.00)

#define BRIGHTNESS 0.009
...
float dfBorder(vec2 uv)
{
	...
	dist = min(dist, dfLine(vec2(0.0,0.0), vec2(1.0,1.0), uv));   // TOP
	...
	dist = min(dist, dfArc(vec2(0.0,0.0),0.0, 2.0, 0.5, uv));   // TOP LEFT
...
void main( void )
{
    ...
    vec2 offs = vec2(0.0, 0.0); //vec2(10.5, 3.5) * scale;
