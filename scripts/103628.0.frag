...
vec3 shadeCube(Ray r,vec3 cuPos,vec3 cuSize){
	vec3 c = vec3(0);
	vec3 normalCube = (ceil(abs(r.pos)-cuSize))* sign(r.pos);
	
	float diffuse = max(dot(normalCube,normalize(light-cuPos)),0.);
	c += diffuse *.5;
	c += pow(dot(normalCube,normalize(normalize(light-cuPos)+normalize(-r.dir))),100.)*diffuse;
	c = max(c,0.);
	//c = normalCube;
	return c;
} 
...
vec3 march(Ray r){
	...
	for(int st = 0;st<max_step;st++){
		...
		float dist = cubeDist(r.pos,vec3(0),vec3(1));
...
void main( void ) {
	...
	light = vec3(sin(time),1.,cos(time));
