...
void main()
{
	...
	float cam_z = 2.0 + cos(time)*0.7;
	
	vec3 ro = vec3(EYE_DISTANCE,0,cam_z); 
	...
	for (int i = 0; i < 1; i++) {
		float a = float(i); 
		color_l += rm(pos,obj_id, ro, rd, time*40.0+a*0.05);
		tot_obj_id += obj_id; 
	}
	if (tot_obj_id < 0.00) { // nothing hit
		color_l = vec3(0.0,0.0,0.0); 
	}
	// right eye
	vec3 color_r = vec3(0); 
	...
	for (int i = 0; i < 1; i++) {
		float a = float(i); 
		color_r += rm(pos,obj_id, ro, rd, time*40.0+a*0.05);
		tot_obj_id += obj_id; 
	}
	if (tot_obj_id < 0.00) { // nothing hit
		color_r = vec3(0.0,0.0,0.0); 
	}
	
	
	// letterbox
	gl_FragColor = vec4(color_r.x, color_l.y, color_l.z, 1.0); 
}