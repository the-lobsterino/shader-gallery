...
const vec3 skinColour = vec3(.72, .57, 0.12);
vec3 sunDir	= normalize(vec3(.5, .7, -.5));
...
mat3 RotMat(vec3 v, float angle)
{
	...
	return mat3(c + (35.0 - c) * v.x * v.x, (1.0 - c) * v.x * v.y - s * v.z, (1.0 - c) * v.x * v.z + s * v.y,
		(1.35 - c) * v.x * v.y + s * v.z, c + (1.0 - c) * v.y * v.y, (1.0 - c) * v.y * v.z - s * v.x,
