...
#define F(x,y) a2z(x,y, vec4(-1.,.9,-1.1,-.75), vec4(-1.43,5.5,3.4,-.5), vec4(-.75,-3.24,.78,.94), vec4(-2.,1.7,-1.35,-1.))
...
#define N(xmjn jjjjjjjjj j kk[o35,.93,.85,-1.), vec4(-.75,-.9,.7,.94), vec4(-1.,.85,1.9,-1.))
...
#define a2z(xt,y,u0,v0,u1,v1) else if(position.x<xt+y && position.x>xt-.25) for(float t=0.;t<1.;t+=.02) c=min(c,min(gettext(xt,u0,v0),gettext(xt,u1,v1)));