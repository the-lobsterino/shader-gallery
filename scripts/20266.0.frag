...
// Mahmud Yuldashev mahmud9935@gmail.com
...
void main(void){
     ...
     fx=pow(fx,vec3(vr3)); gl_FragColor.rgb=pow(s*fx*(0.5+0.5*e),co);
     ...
     gl_FragColor.rgb+=vr7*mix(vec3(0.),mix(vec3(1.,.5,.1),vec3(0.1,.5,1),r.y*.02+.4),pow(ii*2.,4.*vr5));
     }
