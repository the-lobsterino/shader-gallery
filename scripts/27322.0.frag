...
float groundPosition = -0.5;
vec3 boundingSize = vec3(35,12,0.8)*scale;
...
letters = min(letters,B(p.xy-vec2(-0.70-0.5,0.9)));
letters = min(letters,I(p.xy-vec2(-0.25-0.5,0.9)));
letters = min(letters,G(p.xy-vec2( 0.25-0.5,0.9)));


letters = min(letters,S(p.xy-vec2( 0.,0.2)));
letters = min(letters,H(p.xy-vec2( .5,0.2)));
letters = min(letters,I(p.xy-vec2( 1.,0.2)));
...
void main()
{	
	eye *= rotY( 0.5 + 0.5*sin(time));
	eye *= rotX(sin(time)+ 0.41);
	light.x = sin(time)+0.41;
