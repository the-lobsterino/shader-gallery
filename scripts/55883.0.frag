...
float traceLight(vec3 from, vec3 to, float tolerance) {
	...
	const int MAX_STEPS = 10;								// Maxmimum amount of ray marching steps before counting the ray as a miss
...
void main(void){
  ...
  // General parameter setup
	...
	float t = time*0.15;									// time value, used to animate stuff
   ...
   //Camera setup
	Camera cam;										// Make a struct holding all camera parameters
  	cam.lookAt = vec3(0.4,1,0);								// The point the camera is looking at
	cam.position = vec3(mouse.x*8.1-4.1, .15, mouse.y*8.1-4.1);						// The position of the camera
  ...
  //Light source
	vec3 lightSource =  vec3(sin(t)*2.,2.9,cos(t)*2.);

  //Raymarching
  	const float MIN_DISTANCE = 1.1;								// Distance to scene that we will count as a hit
	const float MAX_DEPTH=51.0;								// Distance from camera that we will count as a miss
	const int MAX_STEPS = 150;								// Maxmimum amount of ray marching steps before counting the ray as a miss
	const int FOG_STEPS = 64;
	
  	float distanceToScene = .05;								// Initial distance to scene, should be initialized as 1
	float distanceFromCamera = 5.0;								// The ray starts at the camera, so 0
