//source: https://github.com/keijiro/ShaderSketches/blob/master/Circles4.glsl
//MG
#ifdef GL_ES
precision mediump float;
#endif
uniform float time;
uniform vec2 resolution;
#define MONOCHROME 0

vec3 circle(vec2 coord, float bias)
{
    float repeat = sin(time * 0.2) * 10.0 + 30.0;

    float interval = resolution.x / repeat * .8;
    vec2 center = resolution.xy * 0.3;
    float dist1 = distance(coord, center);
    float num = max(floor(dist1 / interval + 0.5) + bias, 1.0);
    float radius = num  * interval;

    float phase1 = time * 3.0 + radius * 0.04;
    float phase2 = phase1 * 1.3426;
    vec2 offs = vec2(sin(phase1), sin(phase2)) * interval * 0.5;
    float dist2 = distance(coord, center + offs);

    float width = interval * 0.33;
    float c = clamp(width * 0.5 - abs(radius - dist2), 0.0, 1.0);

#if MONOCHROME
    return vec3(1, 1, 1) * c;
#else
    float c_r = 0.1 + 0.2 * cos(phase1 * 0.12);
    float c_g = 0.5 + 0.2 * sin(phase1 * 0.34);
    float c_b = 0.8 + 0.2 * sin(phase1 * 0.176);
    return vec3(c_r, c_g, c_b) * c;
#endif
}

vec3 circle2(vec2 coord, float bias)
{
	float repeat = sin(time * 0.2) * 10.0 + 30.0;

    float interval = resolution.x / repeat * .8;
    vec2 center = resolution.xy * 0.8;
    float dist1 = distance(coord, center);
    float num = max(floor(dist1 / interval + 0.5) + bias, 1.0);
    float radius = num  * interval;

    float phase1 = time * 3.0 + radius * 0.04;
    float phase2 = phase1 * 1.3426;
    vec2 offs = vec2(sin(phase1), sin(phase2)) * interval * 0.5;
    float dist2 = distance(coord, center + offs);

    float width = interval * 0.33;
    float c = clamp(width * 0.5 - abs(radius - dist2), 0.0, 1.0);

#if MONOCHROME
    return vec3(1, 1, 1) * c;
#else
    float c_r = 0.1 + 0.2 * cos(phase1 * 0.12);
    float c_g = 0.5 + 0.2 * sin(phase1 * 0.34);
    float c_b = 0.8 + 0.2 * sin(phase1 * 0.176);
    return vec3(c_r, c_g, c_b) * c;
#endif
}

void main()
{
    vec2 p = 2.0 * (gl_FragCoord.xy / resolution) - 1.0 ;
    p.x *= resolution.x/resolution.y;
    p *= resolution.x;
	
    vec3 c1 = circle(p, -1.0);
    vec3 c2 = circle(p,  0.0);
    vec3 c3 = circle(p,  1.0);
    vec3 c4 = circle2(p, 1.0);
    gl_FragColor = vec4(max(max(max(c1, c2), c3),c4), 1);
}