...
void main( void )
{
	float t = (((rcpi*time+pipi))+ptpi)/pisq;
	vec2 pos = surfacePosition*pi;
    	vec3 camPos = (vec3(cos(t*(-trpi)), sin(t*(rcpi)), 1. ))*pipi;
	...
	t*=rcpi;
    	float focus =1.0+sin(time)*0.5;
    	vec3 rayDir = (normalize(camSide*pos.x + camUp*pos.y + camDir/((focus))))*pi;
	float f=0.0;
	vec2 v=vec2(0,0);
	vec3 acCol =vec3(0.0);
	float len =0.0;
	for(float i=0.2; i<pi; i+=0.45/pipi)
	{
        	vec3 tv = rotate(camPos + (rayDir*(i*i)),camDir,i*i);
		vec3 tvl =( sin(tv)*0.7)*( cos(tv*2.)*1.2);
		vec3 lat = (cos(tv*tvl));
		lat *= lat*lat*tvl;
