...
vec3 getFrameColor(vec2 uv, float gt) {
	...
	vec3 clr = vec3(0.0);
    clr.g = 0.6-0.8*smoothstep(lineWidth*.5,lineWidth*1.5, abs(nr-0.49)); // Yellow outline
    clr.b = 0.9-0.8*smoothstep(0.49,0.49+lineWidth, nr); // The numbers
    clr.r -= 0.4-0.4*smoothstep(0.45,0.52,1.0-nr); // Background with shadow
...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
    ...
    fragColor = vec4(pow(clr/150.,vec3(1.0/2.)),1.0);
