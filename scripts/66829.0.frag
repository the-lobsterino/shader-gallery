...
float scene( vec3 p, out vec4 color ) {
	...
	vec2 mousepos = vec2(0.0,0.0);	//(mouse - vec2(0.5)) * 2.0;
	...
	vec3 ppp = rotX( p+vec3(0.0,-0.03,-0.23), 66.0+sin(time*11.0)*12.0 );	// bend
	
	d0 += 1.0 / max(meltmax, cylinder( ppp - vec3(0.0, -0.08, 0.0), vec2(0.001, 0.24) ) - 0.06);
...
void main(void) {
	...
	vec2 mousepos = vec2(-1.0,1.0);	//(mouse - vec2(0.5)) * 2.0;
