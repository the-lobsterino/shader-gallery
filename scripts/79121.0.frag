...
void main(void){
	// Что-то что отвечает за цвет фона
	...
	float a3 = atan(p.y / p.x) * 14.0;
	float l3 = 0.05 / abs(length(p) - 0.4 + sin(a + time * 23.5) * (0.1 * l2));
	destColor3 *= .5 + sin(a + time * 10.0) * 0.5;
	
	gl_FragColor = vec4(l*destColor + l2*destColor2 + l3*destColor3, 2.0);
}