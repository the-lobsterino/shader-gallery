...
const vec2 ch_start = vec2 (ch_space.x * 1., 1.); // start position
...
{
    vec3 rgb = clamp( abs(mod(c.x*2.0+vec3(0.0,1.0,2.0),6.0)-3.0)-1.0, 0.0, 1.0 );
...
void main( void ) 
{
	...
	vec3 ch_color = hsv2rgb_smooth(vec3(time*1.+uv.y*0.1,0.5,1.0));

	vec3 bg_color = vec3(_d*0.4, _d*0.2, _d*0.1);
	uv.x += 0.5+sin(time+uv.x*0.3)*1.5;
	ch_pos = ch_start;
