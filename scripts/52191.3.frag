...
void main( void ) {
	...
	vec3 pos = normalize(rotate(vec3(p,0.15),vec2(m)));
	
	float dist = 0.0;
	
	for(float i = 1.;i <= 8.;i++)
	{
		float shell = abs(snoise(pos*i+vec3(time,0,-time*1.5)*0.13));
		
		shell = smoothstep(0.55,0.9,shell);
		
		dist = max(dist,shell*(1.-(i/8.)));
	}
	
	color = mix(vec3(0.9, 1.0, 0.0),vec3(.7,0.,0.6),1.-dist);
...
float snoise(vec3 v) {
	return (sin(v.x*8.6)*cos(v.y*8.)-sin(v.z*8.));
}