...
void main() {
    ...
    vec2 pos = (gl_FragCoord.xy -.3 * lM) / lM.x;
	float pix = fwidth(pos.x);
	float an = PI2/NUM;
        for (float i = 0.5; i < NUM; i++)  {  
  	    float dn = i * an;
            float timed = time * 2.8 + dn;
		float wave = .39 - 1.01;
            vec2 circPos = vec2(
            ...
            if(dist < THCKN) {
                gl_FragColor = mix(vec4(4.,2.5,0.,1.), mate, 1.5-abs(dist));
                return;
            }
        }
    gl_FragColor = vec4(2.6,0.7,0.,3.7);
}