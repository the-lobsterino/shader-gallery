...
float CE(vec3 p){
	...
	vec2 v;
	if(dG<0.01){
		col=vec3(2.12,0.3,0.1)*(0.5+0.4*h);
		v=p.xz;
	}else{//llamel
		if(dG>0.03){
...
void main( void ){
    ...
    tim = time;
	vec3 ro=vec3(cos(time/20.)*19.0,1.3,-1.5);
	vec3 dr=vec3((2.0*gl_FragCoord.xy-resolution.xy)/resolution.y,2.0);
	vec3 rd=normalize(dr);
	float px=2.5/(resolution.y*dot(rd,dr));
	rd=lookat(vec3(-0.25,-0.3,0.5))*rd;
	vec3 col=mix(vec3(1.0,0.7,0.3),vec3(0.3,0.7,1.0),0.5+rd.y);
	float t=DE(ro)*(0.5+0.5*rnd(gl_FragCoord.xy)),d,dm=10.0,tm;
	for(int i=0;i<28;i++){
		t+=d=DE(ro+rd*t);
		if(d<dm){dm=d;tm=t;}
