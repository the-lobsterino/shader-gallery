...
float Z(vec2 p){float d=1.0;
		
		d=min(d,line(p,vec2(-1.0,5),vec2(0.0,5)));
		d=min(d,line(p,vec2(1,2),vec2(2,8)));
		
		d=min(d,line(p,vec2(4.0,5),vec2(1,5)));
...
vec3 processColor(vec3 p)
{
	...
	if (ground<d) { col = vec3(diff+spec*0.3)*vec3(0.6,0.2,0.6); d = ground; }
	if (letters<d) { col = vec3(0,0.2,p.y*0.5+0.5)+diff+spec; }
		
	col *= min(1.0, 2.0/dist);
...
{	
	...
	//letters = min(letters,C(p.xy-vec2(-0.75,0.9)));
	letters = min(letters,A(p.xy-vec2(-0.75,0.9)));
	letters = min(letters,N(p.xy-vec2(-0.25,0.9)));
	letters = min(letters,A(p.xy-vec2( 0.25,0.9)));
	letters = min(letters,L(p.xy-vec2( 0.75,0.9)));
	letters = min(letters,Z(p.xy-vec2(1.25,0.95)));
	letters = min(letters,Z(p.xy-vec2(1.8,0.95)));
