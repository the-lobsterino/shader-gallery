...
const vec2 ch_start = vec2 (ch_space.x, 1.7); // start position
      vec2 ch_pos   = vec2 (0.0, 0.0);             // character position
const vec3 ch_color = vec3(1.1,  .3, 0.8);         // character color
...
#define nl ch_pos.x = ch_start.x; ch_pos.y -= ch_space.y;
...
void main( void ) 
{
	...
	vec2 uv = ( gl_FragCoord.xy / resolution.y ) - vec2(.6,  .6);
	uv.x *= aspect;
	uv *= 30.0;
	float d = 1e6;
	ch_pos = ch_start;  // set start position

	O P E N G L nl W I L L nl G E T nl C H I C K S nl W E T
	
	
 	
		
	vec3 color = mix(ch_color, bg_color,  (.1*(sin(time)+1.0) / d));  //Shading
