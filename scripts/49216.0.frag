//by me2beats 2018
//original: http://glslsandbox.com/e#49187.1
...
vec4 texture(vec2 pos)
{
    ...
    float r=atan(uv.x,uv.y) - 0.2+0.1*sin(time);// + 0.025*sin(iTime);
    float l=length(uv);
    
    uv=vec2(sin(r)+0.1*sin(2.*time),cos(r)+0.1*cos(time))*l;
    ...
    return vec4(col*mix(0.,1.,-fract(clamp(0.1*sin(time),0.,1.)*acos(-1.))/2.+.5),1.0);
}
void main(void){
