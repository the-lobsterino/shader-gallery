...
//looks sweet at .5 resolution
//mouse to the left edge to spawn new cells
//mouse in the bottom left corner clears the screen

//how far before branching, and what angle to branch at
#define BRANCHING_PATTERN vec2(.123, .025) 

#define FREQUENCY 		169.5
...
float hash(in float v);				//random hashes used for computing spawn points
float hash(in vec2 uv); 				//hash
float value_noise(in vec2 uv);			//noise smoothed over a lattice
float fbm(in vec2 uv);				//fractal brownian motion (value noise harmonic)
...
void main() 
{
	...
	vec2 field		= mod(gl_FragCoord.xy * FREQUENCY, MAX_FLOAT)/MAX_FLOAT;
	...
	for (int i = 0; i < 8; i++)
		...
		{   
			...
			sequence 		= mod(sequence, .125);
			sequence			= fract(angle + sequence);
			
			if (int(sequence * 8.) == i) 
			{
				cell = neighbor;
				cell.w = fract(cell.w+(hash(gl_FragCoord.xy)-.5)*.01);
				break;
			}
		}
        }
...
{
	float noise		= fract(hash(gl_FragCoord.xy)*1e2)+hash(gl_FragCoord.yx);
	bool add_cell		= noise <= .001 && mouse.x < .125;
	float initial_angle	= hash(gl_FragCoord.yx-noise);
    	cell 			= add_cell ? vec4(1., BRANCHING_PATTERN, initial_angle) : cell;	
	return cell;
}
...
{
	return mod(mod(v, (sqrt(5.)+1.)/2.)*v, 1.);	
}
