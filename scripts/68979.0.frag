...
float hexDist(vec2 uv) {
    uv = abs(uv);
    return max(uv.x, dot(uv, normalize(vec2(1., 1.5))));
}
    ...
    for(int x = -1 ; x <= 1 ; x++) {
        for(int y = -1 ; y <= 1 ; y++) {
            ...
            if (n > .2) {
                ...
                vec2 shift = vec2(n1 - .75, n2 - .75)*sin(iTime + n*n1 + n2);
...
void mainImage(out vec4 fragColor, in vec2 fragCoords) {
    ...
    if (dt < MIN_DISTANCE) {
        ...
        col += pow(.005/hex.y, .8) * gridColor * b;

        col += vec3(n21(hex.ba + 100.))*.6;
    } else {
//        col = getTexture(uv + vec2(iTime/(SPEED*3.), 0.));
    }
