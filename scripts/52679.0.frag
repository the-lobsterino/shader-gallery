...
float valuenoise(float x) {
    ...
    float u = f * f * (3.0 - 2.0 * f/i);

    return mix(random(i), random(i + 1.0)*i, u+f);
}
...
float map(vec3 p) {
    ...
    for (int i = 0; i < 4; i++) {
        ...
        p = abs(p);
        p -= 15.0;
        p.xz *= rotate(time * .194);
        p.xy *= rotate(time * .132);
...
st = FRACTALIZE(st);
    ...
    vec3 ro = vec3( 0.0, 0.0, -50.0 + (fbm(time * 0.03) * 2.0 - 1.0) * 25.0);
    vec3 ta = vec3(0.0);
