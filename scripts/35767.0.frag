...
float disc(vec2 p, float r) {
	return smoothstep(0.0, 0.0025, (length(p) - r)); 
}
...
vec2 solve(vec2 p1, float r1, vec2 p2, float r2) {
	...
	float f = sqrt(r1 * r1 - k * k);
	float x1 = p1.x + (p.x * k) / d + (p.y / d) * f;
	float y1 = p1.y + (p.y * k) / d - (p.x / d) * f;
	float x2 = p1.x + (p.x * k) / d - (p.y / d) * f;
	float y2 = p1.y + (p.y * k) / d + (p.x / d) * f;
