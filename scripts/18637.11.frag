...
float DE(vec3 z) {
	...
	float FRACT_SCALE = 1.8 + 0.2 * sin(time * 0.00227);
	float FRACT_OFFSET = 1.0 + 0.5 * sin(time * 0.00153);
	
	float r;
	int n1 = 0;
	for (int n = 0; n < FRACT_ITER; n++) {
		float rotate = M_PI*0.5;
		z = vRotateX(z, rotate + M_PI*0.5*cos(time*0.00498));
		z = vRotateY(z, rotate);
		z = vRotateZ(z, rotate + M_PI*0.5*cos(time*0.00581));
		z.xy = abs(z.xy);
		if (z.x+z.y<0.0) z.xy = -z.yx; // fold 1
		if (z.x+z.z<0.0) z.xz = -z.zx; // fold 2
		if (z.y+z.z<0.0) z.zy = -z.yz; // fold 3
		z = z*FRACT_SCALE - FRACT_OFFSET*(FRACT_SCALE-0.5);
	}
...
vec3 swr(vec3 p){
	if ((abs(p.x) > 3.0) || (abs(p.z) > 3.0)) return vec3(0.03,0.07,0.15);
	float ang = p.y+length(vec3(p.x, 0.0, p.z))*1.33;
	return vec3(max(0.,sin(ang)*0.6+0.25), max(0.,sin(ang+2.08)*0.6+0.25), max(0.,sin(ang-2.08))*0.6+0.25);
}
...
void main() {
	...
	float t = time - pow(length(p), 1.3*sin(time*0.082352));
	
	//vec3 origin = vec3(0.0, 0.0, 0.0) - eyeTranslate/128.0;
	vec3 origin = vec3(0.0, -t*0.1, 0.0);
	vec3 direction = normalize(vec3(p.x,p.y,1.0));

	direction = vRotateZ(direction, time*0.1 + sin(time * 0.1));
	direction = vRotateX(direction, eyeRotate.x-M_PI/2.0);

	vec3 color = drawScene(origin, direction)*swr(ptt);
