...
vec3 firePalette(float i) {   float T = 1400. + 1300. * i;   vec3 L = vec3(7.4, 5.6, 4.4);   L = pow(L, vec3(5.0)) * (exp(1.43876719683e5 / (T * L)) - 1.0);  return 1.0 - exp(-5e8 / L); }
...
void main(void) {
	...
	for (float i = 2.5; i < number + 1.0; i++)
	{
		...
		minDist = 0.5*d * 9.*(1.-pow(mouse.y,.4)) * sqrt(minDist)-0.005*minDist;
