...
void main( void ) {
	...
	float open = fract(time + 0.3) * (sin(10.0 * time) + 1.0) / 2.0;
	float happiness = fract(time) * (sin(10.0 * time) + 1.0) / 2.0;
	
	float openSize = sin(3.1415 * 0.5 * open); // how much the mouth is open 
	float cornerPull = happiness * 2.0 - 1.0; // +/- pull at the corners for smile/frown
