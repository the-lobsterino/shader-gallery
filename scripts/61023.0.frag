...
{
	// guessing this should be a real image...
	return 1.+cos(p.x*100.+p.y*333.);
    float f;
...
vec4 mainImage( out vec4 fragColor, in vec2 fragCoord )
{
	...
	vec3 camZ   = vec3( 0.0, 0.0, 1.75);*/
	
	vec3 rO = camPos;
	vec3 rD = normalize(vec3(uv2, 1.75));//uv2.x*camX + uv2.y*camY + camZ);
	vec3 finalPos = rO;
...
void main(void){
	vec4 c = vec4(0);
	gl_FragColor = mainImage(c, gl_FragCoord.xy);
	//... thumbnail shows an image, but osx does not... :/
}