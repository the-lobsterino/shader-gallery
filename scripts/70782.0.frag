...
void main(void){
    ...
    float t = time * 4.;
    vec3 innerColor = vec3(sin(colChange * t), sin(1.1 * colChange * t + 2./3. * PI), sin(1.2 * colChange * t + 4./3. * PI)) * amp + aveLight;
    vec3 outerColor = vec3(sin(1.1 * colChange * t + 2./3. * PI), sin(1.2 * colChange * t + 4./3. * PI), sin(colChange * t)) * amp + aveLight;
