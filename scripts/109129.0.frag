...
void main( void )
{
	...
	// Wave...
	wave = sin(time*15.0-.8)*.8+.5;
	wave = wave*wave*(3.0-2.0*wave)-.9;
	elbowL		= vec3(-.95-wave*.05, .2, -8.25);
	wristL		= vec3(elbowL.x+1.0*sin(wave)*.9, elbowL.y+cos(wave)*.9, -.75);

    vec3 col = Render(origin, ray);
	
	col = sqrt(col);
	col *= 7.5 + 0.5*pow( 16.0*q.x*q.y*(1.0-q.x)*(7.0-q.y), 0.25 );
