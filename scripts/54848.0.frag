...
void main()
{
  ...
  for(int i=1;i<45;i++)
  {
    ...
    newp.x+=1.2/float(i)*sin(float(i)*p.y+time/fluid_speed+0.6*float(i)) + 1.0; // + mouse.y/mouse_factor+mouse_offset;
    newp.y+=0.6/float(i)*sin(float(i)*p.x+time/fluid_speed+0.3*float(i+10)) - 0.5; // - mouse.x/mouse_factor+mouse_offset;
    p=newp;
  }
  vec3 col=vec3(color_intensity*sin(0.3*p.x)+color_intensity,color_intensity*sin(8.0*p.y)+color_intensity,color_intensity*sin(p.x+p.y)+color_intensity
          );
