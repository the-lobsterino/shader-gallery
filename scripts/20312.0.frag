...
cState loadState()
{
    ...
    cState state;
    state.validData =  texture2D(backbuffer,( vec2(0.0,0.0) / resolution.xy )).a;
    state.alive =      texture2D(backbuffer,( vec2(1.0,0.0) / resolution.xy )).a;
    state.deadTime =   texture2D(backbuffer,( vec2(2.0,0.0) / resolution.xy )).a;
    state.birdPosY =   texture2D(backbuffer,( vec2(3.0,0.0) / resolution.xy )).a * 800.0 - 250.0;
    state.birdVelY =   texture2D(backbuffer,( vec2(4.0,0.0) / resolution.xy )).a * 80.0 - 40.0;
    state.mouseRegion = texture2D(backbuffer,( vec2(5.0,0.0) / resolution.xy )).a;
    state.score =       texture2D(backbuffer,( vec2(6.0,0.0) / resolution.xy )).a * 255.0;
    state.hiscore =     texture2D(backbuffer,( vec2(7.0,0.0) / resolution.xy )).a * 255.0;
    state.xParity  =      texture2D(backbuffer,( vec2(8.0,0.0) / resolution.xy )).a;
