...
float hexDistance(vec2 ip) {
	...
	float d = dot(p, vec2(-TRIG30.x, TRIG30.y)) - SQRT3 * 3.25;
	
	return (d > 3.0)? min(d, (SQRT3 * 0.9 - p.x)) : min(-d, p.x);
}
...
void main(void) {
	...
	rgb += mix(vec3(1.0, 0.0, 1.0)*fade, vec3(0.0), smoothEdge(0.03, 0.5, d)) * 9.5;
	rgb += mix(vec3(1.0, 0.0, 0.0)*fade, vec3(0.0), smoothEdge(0.03, 1.0, d)) * 8.25;
