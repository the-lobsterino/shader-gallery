...
vec3	firePalette(float i) { float T = 1450. + 1300. * i; vec3 L = vec3(7.4, 5.6, 4.4); L = pow(L, vec3(5.))*(exp(1.43876719683e5/(T*L)) - 1. ); return 1. - exp(-5e8/L); }
...
vec3 Image0( vec2 p )	// this is the main function in regards to the sun
	...
	vec3 c = mix( vec3( .34, .5, 1. ), vec3( 0., .5, 1. ), ( p.y + 1. ) * .5 );
	c += (vec3(.95, .65, .30) * 1. / distance(vec2(0.), p + vec2(st, ct))*.075);
	c += vec3(.95, .45, .30) * min(1., blade *.7) * (1. / distance( vec2( 0., 0. ), p)*.075 );
	c = 1. - exp( -c ); return c; }
