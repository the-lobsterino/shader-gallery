...
Sphere sphere(int i) {
	...
	if (i--==0) return	Sphere(16.5, vec3(73., 16.5, 78.), 		vec3(0.), vec3(.7, 1., .9), SPEC);
...
void main(void) {
	...
	vec4 counter = texture2D(backbuffer, vec2(0.0, 0.0));
	...
	for (int i = 0; i < SAMPLES; ++i)
		color += radiance(Ray(camPos, normalize(.53135 * (resolution.x/resolution.y*(2. * (gl_FragCoord.x + rand() * 0.5) / resolution.x - 1.) * cx + (2. * (gl_FragCoord.y + rand() * 0.5) / resolution.y - 1.) * cy) + cz)));
	float weight = clamp(255. * previous.a, 0., 254.);
	float framenum = counter.y * 65535.0 + counter.x * 255.0 + 1.0;
	float gamma = 2.2;
	color = (color / (framenum+1.0)) + pow(previous.rgb, vec3(gamma)) * (framenum/(framenum+1.0));
	
	if(length(gl_FragCoord.xy) < 1.0) {
		framenum = framenum + 1.0;
		gl_FragColor.xy = vec2(mod(framenum, 255.0) , floor(framenum / 255.0))/255.0;
