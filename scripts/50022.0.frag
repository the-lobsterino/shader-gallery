...
float RhombusSd(in vec3 p, in vec2 size) 
{
    ...
    return d * sign( q.x*size.y + q.y*size.x - size.x*size.y * mouse.y );
}
...
vec4 Tile(vec2 uvi, vec2 uvf, float eps, float t)
{
    ...
    float ts = mix(t0, 1.0-t2, step(0.5,t*t1));
    ...
    // Rhombus size:
    vec2 s = vec2(1.0, mix(1.0, sqrt(3.0)/3.0, mouse.y-ts)) / sqrt(2.0);
...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
    ...
    // Tiling:
    vec2 uvf, uvi;
    uv = uv*Count - vec2(0.05);
    uvi = floor(uv);
    uvf = 1.0*fract(uv)-1.0;
    eps *= 1.0*Count;

    // Render tile:
    vec4 tile = Tile(uvi, uvf, eps, t);
    vec4 col = mix(vec4(0.2), tile, tile.a);
