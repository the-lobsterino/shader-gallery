...
float EPSILON_NRM	= 6.0;

// sea props 
const int ITER_GEOMETRY = NUM_STEPS; // ?
const int ITER_FRAGMENT = 2; // hmm
...
const vec3 SEA_WATER_COLOR = vec3(0.1,0.5,0.9);
float SEA_TIME = 0.;
...
float hash2( vec2 p){
	float s = dot(p,vec2(0,0));
	return fract(sin(s) * 3200.0 );
...
vec3 getSeaColor(vec3 p, vec3 n, vec3 l, vec3 eye, vec3 dist) {  
    ...
    vec3 refracted = spin(p*0.001) + diffuse(n,l,80.0) * SEA_WATER_COLOR * 0.12; 
