...
float line(vec2 p, vec2 a, vec2 b){ //a line between a and b in domain of p
	vec2 ab = b-a;
	return .005/length(a+(ab)*clamp(dot(p-a,ab)/dot(ab,ab),0.,9.)-p);
}
...
void horizon(vec2 p, vec2 a, vec2 b){ ////horizon between a, b with a.x<b.x in domain of p
	vec2 ab = b-a;
    ab = a+(ab)*clamp(dot(p-a,ab)/dot(ab,ab),0.,19999999999.);
    float d = length(ab-p);
...
void rect(vec2 p, vec2 rect, float fill){ //smooth rectangle
    ...
    amp = mix(amp, fill, smoothstep(.005/rect.x,.0, d));
}
...
void mainImage( out vec4 fragColor, in vec2 fragCoord ){
    float animTime = mod(2.*2.133333*iTime,9.*PI); //...buggy without mod at large iTime, ?overflow
    float animFrame = floor((iTime*60.)/9.);
	vec3 col = vec3(0.5);
	amp = 0.;
    
    // Viewport
    vec2 uv = (2.*fragCoord-iResolution.xy) / max(iResolution.x, iResolution.y); // max -3..1
    uv *=1.5;

    // Background
    for (float n=.0; n<15.; n++){ //horizon & sky
        horizon(uv, vec2(-1.5+.2*n, .02+.03*hash21(vec2(animFrame)+n)), vec2(-1.5+.2*(n+1.), .12+.03*hash21(vec2(animFrame)+n+1.)));
    }
    ...
    vec2 trouserR = trouserM-vec2(.00,-.06);
    ...
    // Draw
    col += vec3(amp);
	col = pow(col, vec3(.49945)); //gamma correction
    col += .1*hash21(floor(111.*uv)*animTime)*(0.-.2*cos(20.*uv.y+animTime)); //noise
    col *= vec3(244,239,239)/255.; //sepia tint
