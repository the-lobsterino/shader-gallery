...
void main( void ) 
{
	...
	uv -= vec2(-3., 1.);
        ...
        vec3 ch_color = hsv2rgb_smooth(vec3(uv.y*0.015,0.6,1.0));
	...
	if(mod(time,16.)<10.) {uv.y-=3.5;uv.x+=2.;
		      
	         H E _ H A V E  _ S E X _ W I T H _ A nl1
		_ _ _ _ _ _ _ _
		if(mod(time,.5)<.25) n9 nl2;
		_ Y E A R S _ O L D _ C H I L D nl4;
		
		ch_color += hsv2rgb_smooth(vec3(uv.y*.01,.8,.5));	      
	        _ _ _ C A L L E D _ A I S H A nl5
		uv *= 3.;uv.y += 22.5;
                if(mod(time,2.5)>1.25){	    
		S  H E _ W A S _ S T I L L _ P L A Y I N G _ 	
		W I T H _ D O L L S	 } }
	       
	else { uv /= 1.15;uv.x+=1.9;uv.y-=1.;
	        W H Y _ I S _ M U H A M M A D _ A nl1
		_ _ F A L S E _ P R O P H E T nl2
		_ _ _ _ _ _ if(mod(time,.5)<.25) _ _ s_question;
			}
