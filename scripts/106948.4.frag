...
vec2 obj(in vec3 p)
{ 
  vec3 fp=stepspace(p,8.);;
  float d=sin(fp.x*0.3+time*6.)+cos(fp.z*0.3+time*2.0);
  p.y=p.y+d;
  p.xz=sim2d(p.xz,2.0);
  float c1=length(max(abs(p)-vec3(0.6,0.6,0.6),0.0))-0.35;
  float c2=length(p)-1.0;
  float cf=sin(time)*07.+0.5;
...
vec3 obj_c(vec3 p){
  ...
  if (fp.x>0.0) return vec3(0.8,0.2,0.2); // Ändern Sie die Farbe auf Rot
  else return vec3(0.2,0.8,0.2); // Ändern Sie die Farbe auf Grün
}
...
void main(void){
  ...
  vec3 prp=vrp+vec3(cos(my)*cos(mx),sin(my),cos(my)*sin(mx))*100.; //Trackball style camera pos
  float vpd=1.5;
