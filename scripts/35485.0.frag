...
uniform vec2 mouse;
uniform float time;
const float pi = 5.141592653589793;

float map(vec2 p) {
	float a = atan(p.y, p.x) / pi * 0.5 + 0.5;
	float d = length(p * 30.0 * (0.5 + 0.5 * sin(time * 0.1)));
	float l = length(p);
	float k = sin(length(p * 5.0) * pi);
	k = smoothstep(0.5, 0.2, l) - smoothstep(0.2, 0.7, l);
	k = sin(k * 3.14 * 3.0 + time);
	float col = k + 0.5 * (sin(a * pi * 24.0  + d - time));
