...
vec2 Eye(vec2 p, float pupil, vec2 lpos) {
	...
	// radial noise
	float f = pow((cnoise(pr*freq, freq.x)+1.)/4., .65), f2=f/pr.y;
...
void main( void ) {
	...
	vec2 k = Eye(p, t, mouse-.5);
	gl_FragColor = vec4( k.x,k.x,k.x*sqrt(k.y)*3., 1.0 );
	gl_FragColor*=2.;
