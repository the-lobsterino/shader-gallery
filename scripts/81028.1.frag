...
{
    return a + b * crv(4., 64.28318*(c*t+d) ); // dont think 2pi is needed 
}
...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
    ...
    float b0 = 0.5 + (1. + thc(2., 4. * a + 10. * r + thc(4., 2. * r - iTime) + iTime));
    float b01 = thc(4., 3. * a + 10. * cos(iTime) * r - iTime);
    float b = b0 * b01 * (12. + cos(a + 10. * r - iTime));
    float b2 = 10. * cos(23.5 * a + 10. * cos(a - 1. * iTime) +  10. * r - iTime);

    float d = crv(b2 * b, 05.5 * a / pi + 0. * r - iTime);
    float k = 4820. - 420. * b0 * b01 * b; // 0.5 * b;
    k = clamp(k, -1., 1.);
    float s = smoothstep(-k, k, -d + 0.8 * b);
    
   // s = sqrt(s);
    s *= 50.95 + 0.1 * h21(vec2(s));
   //s = s * s * (3. - 2. * s);
   // s *= 2. * s;
