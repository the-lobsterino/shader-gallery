...
// by @paulofalcao
//
// Author: Evan Palmer added background
...
vec2 obj(vec3 p){
  ...
  float d=sin(length(fp/2.0)*1.0-mouse.x*10.*4.0)*(sin(length(fp/50.0)*4.0-mouse.y*10.0)*0.5+0.5);
...
void main(void){
  ...
  if (f<maxd){
    ...
    gl_FragColor=vec4((b*c+pow(b,8.0))*(1.0-f*.02),0.1);
  } else {
	  //gl_FragColor=vec4(0.1,0,0,1); //background color
	  	vec2 p = 16.0 * (gl_FragCoord.xy / resolution.x);

	float dist = 1.0;
	float range = 1.0 * sin(pow(clamp(dist, 0.0, 1.0), 0.1) * (PI / 2.0));
	p = p * range;

	float texture = 0.;//pow(sin(16.0 * p.x) * 0.5 + 0.5, 64.0);
	//texture += pow(sin(16.0 * p.y) * 0.5 + 0.5, 64.0);
	
	
	vec4 c=vec4(.8,.8,.8,1.);
	
	//c.x = texture * (cos(time * 0.5) + 0.1) - cos(time * 0.5) + 1.0;
	//c.y = texture * (sin(time * 0.7) + 0.1) - sin(time * 0.7) + 1.0;
	//c.z = texture * (cos(time * 0.1) + 0.1) - cos(time * 0.1) + 1.0;
	//c.w = 1.0;
	
	gl_FragColor = c * pow(dist, 0.5);
	gl_FragColor.w = 1.0;
}
}