#ifdef GL_ES
precision mediump float;
#endif

uniform float time;
uniform vec2 mouse;
uniform vec2 resolution;

struct Ray {
	vec3 pos;
	vec3 dir;
};
	
float udRoundBox( vec3 p, vec3 b, float r )
{
	return length(max(abs(p)-b,0.0))-r;
}
float distFunc(vec3 pos)
{
	pos = mod(pos, vec3(10)) - vec3(5);
	return udRoundBox(pos, vec3(2.0), 1.0);
}

vec3 getNormal(vec3 pos)
{
	const float d = 0.0001;
	return normalize(
		vec3(
			distFunc(pos + vec3(d, 0, 0)) - distFunc(pos - vec3(d, 0, 0)),
			distFunc(pos + vec3(0, d, 0)) - distFunc(pos - vec3(0, d, 0)),
			distFunc(pos + vec3(0, 0, d)) - distFunc(pos - vec3(0, 0, d))
		)
	);
}

void main( void ) {

	vec2 pos = (gl_FragCoord.xy - resolution * 0.5)  / resolution.y + mouse - 0.5;
	Ray ray;
	ray.pos = vec3(sin(time) * 20.0 + pow(time, sin(time) * 1.3 + 2.0), cos(time) * 20.0, -10.0);
	ray.dir = normalize(vec3(pos * 2.0, 1.0));
	float d;
	for(int i = 0; i < 32; ++i)
	{
		d = distFunc(ray.pos);
		ray.pos += d * ray.dir;
		if (abs(d) < 0.001) break;
	}
	if (abs(d) < 0.001)
	{
		float color = (dot(getNormal(ray.pos), vec3(0, 0, 1)) + 0.5) * 4.0;
		gl_FragColor = vec4(vec3(color), 1.0);
	}
	else
		gl_FragColor = vec4(0.3, 0.7, 1.0, 1.0);
}