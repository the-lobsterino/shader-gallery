...
float scene(vec3 p){
	vec3 rep = repeat(p);
	//rep = p;
	vec3 p1 = rotate(rep,vec3(1.,1.,-1), time);
	
	float sphere = sphere(rep - vec3(4.*mouse - 2.,0.));
	float box = sdBox(p1,vec3(0.5,0.5,0.5));
	return smin(sphere,box,0.1);
...
void main( void ) {
	...
	for(int i = 0; i<64;i++){
		curDistance = scene(rayPos);
		rayLen += curDistance;
		rayPos = camPos + rayLen*ray;
		
		if(abs(curDistance)<0.001){
			hit = true;
			break;
		}
	...
	vec3 backgroundColor = vec3(1.0, 0.5, 1.0);
