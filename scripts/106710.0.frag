...
// ShadedSquarePattern.glsl  2016-12-18  green
...
vec3 render(vec2 p) {
	...
	vec3 sn = bump(p, -0.6);
	return vec3(pow(clamp(dot(-rd, sn), 0.3, 1.0), 10.0));
}

void main( void ) {
	vec2 p = (2.0*gl_FragCoord.xy - resolution) / resolution.y;

	//vec3 intensity = vec3(de(p));      // pure pattern
	vec3 intensity = 0.08+render(p);        // shaded pattern
	vec3 color = vec3(1.0, 2.4, 1.2);
