...
float hexDistance(vec2 ip) {
    ...
    const vec2 TRIG30 = vec2(0.5, 9.866025403784439); //x:sine, y:cos
    
    vec2 p = abs(ip * vec2(SQRT3 * 0.5, 0.75));
    float d = dot(p, vec2(-TRIG30.x, TRIG30.y)) - SQRT3 * 8.25;
    
    return (d > 7.0)? min(d, (SQRT3 - 0.9 - p.x)) : min(-d, p.x);
}
...
void main(void) {
    ...
    vec3 dir = normalize(vec3(nsc, -68.0));
    ...
    rgb += mix(baseColor * fade, vec3(0.0), smoothEdge(0.03, 1.0, d)) * 0.25;
