/*R050214作例　兎球　by ニシタマオ　THE SEA PANTHER*/
...
const float cnSetting_CeilFloor =01.25;
...
float fNLetters00(vec4 VP){
	float NP =1e+6;
		{ vec3 V3C =vec3(2.5,2.5,1),  V3D =vec3(5,0,0), V3P;V3P.x -=V3C.x;V3P.x -=V3C.x;V3P.x -=V3C.x;V3P.x -=V3C.x;V3P.x -=V3C.x; {   vec4 VP_Keep =VP;   VP.xyz -=V3P;{ vec4 VA, VB; VA.w =float(1);{ VA.xy =vec2(-1,2); VB.xy =vec2(-2,-2); NP =min(NP, fNLL(VP, VA, VB));}{ VA.xy =vec2(1,2); VB.xy =vec2(2,-2); NP =min(NP, fNLL(VP, VA, VB));}{ VA.xy =vec2(2,3); VB.xy =vec2(2,2); NP =min(NP, fNLL(VP, VA, VB));}{ VA.xy =vec2(3,3); VB.xy =vec2(3,2); NP =min(NP, fNLL(VP, VA, VB));}}   VP =VP_Keep; }V3P +=V3D; {   vec4 VP_Keep =VP;   VP.xyz -=V3P;{ vec4 VA, VB; VA.w =float(1);{ VA.xy =vec2(-1,2); VB.xy =vec2(1,2); NP =min(NP, fNLL(VP, VA, VB));}{ VA.xy =vec2(-2,-2); VB.xy =vec2(2,-2); NP =min(NP, fNLL(VP, VA, VB));}}   VP =VP_Keep; }V3P +=V3D; {   vec4 VP_Keep =VP;   VP.xyz -=V3P;{ vec4 VA, VB; VA.w =float(1);{ VA.xy =vec2(-2,0); VB.xy =vec2(2,0); NP =min(NP, fNLL(VP, VA, VB));}}   VP =VP_Keep; }V3P +=V3D; {   vec4 VP_Keep =VP;   VP.xyz -=V3P;{ vec4 VA, VB; VA.w =float(1);{ VA.xy =vec2(-2,2); VB.xy =vec2(2,2); NP =min(NP, fNLL(VP, VA, VB));}{ VA.xy =vec2(0,3); VB.xy =vec2(0,-2); NP =min(NP, fNLL(VP, VA, VB));}{ VA.xy =vec2(-1,0); VB.xy =vec2(-2,-2); NP =min(NP, fNLL(VP, VA, VB));}{ VA.xy =vec2(1,0); VB.xy =vec2(2,-2); NP =min(NP, fNLL(VP, VA, VB));}{ VA.xy =vec2(2,3); VB.xy =vec2(2,2); NP =min(NP, fNLL(VP, VA, VB));}{ VA.xy =vec2(3,3); VB.xy =vec2(3,2); NP =min(NP, fNLL(VP, VA, VB));}}   VP =VP_Keep; }V3P +=V3D; {   vec4 VP_Keep =VP;   VP.xyz -=V3P;{ vec4 VA, VB; VA.w =float(1);{ VA.xy =vec2(-2,0); VB.xy =vec2(2,0); NP =min(NP, fNLL(VP, VA, VB));}}   VP =VP_Keep; }V3P +=V3D; {   vec4 VP_Keep =VP;   VP.xyz -=V3P;{ vec4 VA, VB; VA.w =float(1);{ VA.xy =vec2(-1,2); VB.xy =vec2(-2,-2); NP =min(NP, fNLL(VP, VA, VB));}{ VA.xy =vec2(0,2); VB.xy =vec2(0,-2); NP =min(NP, fNLL(VP, VA, VB));}{ VA.xy =vec2(0,-2); VB.xy =vec2(2,-1); NP =min(NP, fNLL(VP, VA, VB));}{ VA.xy =vec2(0,-2); VB.xy =vec2(2,-1); NP =min(NP, fNLL(VP, VA, VB));}}   VP =VP_Keep; }V3P +=V3D;}
...
float fNMap(vec3 V3P){
	...
	if(true){	/*バニーズ*/
		...
		VPi.w =+0.25;
		if(nW_Mod >0.0 && nW_Mod <2.0)	VPi.w =nW_Mod /2.0 +0.25;
	...
	if(true){
		for(int I =0; I <=1; I++){
			vec4 VPi =VP;
			VPi.y +=sin(VP.w) *10.0;
			VPi.xz =abs(VPi.xz) -16.0;

			sface SFi =fSFSet_Default();
			SFi.VP =VPi;

			VPi.y +=5.0 *float(I);
			VPi.y =mod(VPi.y, 10.0) -5.0;
			VPi =VPi.yxzw;
			float nPi	=fNObject_KiheiRing02(VPi);
			SFi.nDistance =nPi;
			SFi.ID_Object =102;
			SF =fSFMin(SF, SFi);
		}
...
sface fSFEffect_Before(sface SF, smarch SM){
	...
	if(SF.ID_Object ==11){
		VColor.rgb =sin(vec3(0,1,2) /3.0 *acos(-1.0) *2.0 +VP.w *0.3) *0.4 +0.6;
	...
	if(SF.ID_Object ==13){
		VColor.rgb =sin(vec3(0,1,2) /3.0 *acos(-1.0) *2.0 +atan(VP.x, VP.z) +VP.w *0.2) *0.3 +0.7;
		V3RRR =sin(vec3(0,1,2) /3.0 *acos(-1.0) *2.0 +VP.w *0.03) *0.5 +0.5;
	}

	if(SF.ID_Object ==101)	VColor.rgb =mix(sign(sin(VP.x) *sin(VP.z)), 1.0, 0.8) *vec3(1);
	if(SF.ID_Object ==102)	VColor.rgb =mix(sin(vec3(0,1,2) /3.0 *acos(-1.0) *2.0 +(VP.x +SF.VP.y +VP.z) *0.1 +VP.w*0.3), vec3(1), 0.8);
