...
vec3 hsl_rgb (float hue, float saturation, float brightness)
{
	...
	// maybe find largest valid value that intersects the line between (input value) and vec3(0., brightness, 0.)?
	
	vec3 color = vec3(0.,0.,0.);
	float cubex = saturation*cos(hue);
	float cubey = brightness*sqrt(3.);
	float cubez = saturation*sin(hue);
