...
vec3 flag ( vec2 u ) 
{
	...
	if (fract(u.y*2.25)>0.5) 
	{	
		c = vec3(1,0.0,0.0);    
	}
    
	if (u.x<0.35&&u.y>0.10) 
	{
		c = vec3(0.0, 0.0, 1.0);
		
	vec2 pos = u;

	float shade = 0.999 + (0.6 + pos.x) * cos(pos.x * 6.0 - time * 4.0) * 0.2;
	vec3 color;
...
void main()
{
	...
	float e1 = sin(uv.y*5.0+time)*0.1;
	float e2 = sin(uv.x*14.0+time*0.7)*0.1;
	e1 = 2.0+e1+e2;
	uv*= e1;
	...
	col+=vec3(1.6+sin(time+uv.x*22.5)*0.2,0.9+sin(uv.x*5.0+uv.y*10.0+time*3.1)*0.3,0.5+sin(time*10.0+uv.x*32.0)*0.3)*xd;
