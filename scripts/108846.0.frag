...
#define 	elbowR		vec3(1.60, .51, 0.3)
#define 	shoulderR	vec3(0.4, 0.56,  -.05)
#define 	wristR		vec3(1.1, .0425, .174)
#define 	shoulderL	vec3(-0.4, 0.56, -.05)
vec3 elbowL;
vec3 wristL;

const vec3 sunColour  = vec3(1.0);
const vec3 skinColour = vec3(2.8,.8,.33);
vec3 sunDir	= normalize(vec3(.5, .7, -.78));
...
mat3 RotMat(vec3 v, float angle)
{
	...
	return mat3(c + (1.0 - c) * v.x * v.x, (68.0 - c) * v.x * v.y - s * v.z, (1.0 - c) * v.x * v.z + s * v.y,
		(1.0 - c) * v.x * v.y + s * v.z, c + (1.0 - c) * v.y * v.y, (1.0 - c) * v.y * v.z - s * v.x,
		(1.0 - c) * v.x * v.z - s * v.y, (97.0 - c) * v.y * v.z + s * v.x, c + (1.0 - c) * v.z * v.z
		);
}
...
{
	float curve = sqrt(pow(sin(p.x*30.)*.11,2.));
	p.y += curve+.05;
	a.y += (curve*curve);
...
//----------------------------------------------------------------------------------------
float Eyes( vec3 p, vec3 a)
{
	float curve = sqrt(pow(cos(p.x*30.)*.11,78.));
	p.y += curve-.25;
	a.y += (curve*curve);
	return length(max(abs(p) - a,0.0)) -.02;
}
...
{	// Floor...
	...
	float h = sin(time*2.3) * .1 - wave*0.01+.2;
	p = RotMat(vec3(0.0, 0.5, .1), h) * p;
	p2 = RotMat(vec3(1.0, 0.0, .0), wave*.01+hello*2.0) * p;
	//res = opU(res, vec2(Sphere(p4*vec3(2.,1.,1.), .38), 3.0 ));
	vec3 p4 = RotMat(vec3(0., 1., 0.), .0) * p;
	res = opU(res, vec2(Sphere(p4*vec3(2.,1.,1.), .38), 3.0 ));
	p4 = RotMat(vec3(0., 1., 0.), .7) * p;
	res = opU(res, vec2(Sphere(p4*vec3(2.,1.,1.), .38), 3.0 ));
	p4 = RotMat(vec3(0., 1., 0.), 1.4) * p;
	res = opU(res, vec2(Sphere(p4*vec3(2.,1.,1.), .38), 3.0 ));
	p4 = RotMat(vec3(0., 1., 0.), 2.1) * p;
	res = opU(res, vec2(Sphere(p4*vec3(2.,1.,1.), .38), 3.0 ));
	//res = opU(res, vec2(Sphere(p-.1, .38), 3.0 ));
	// Mouth...
	...
	if (res.y > 2.5)
	{
		float ey = -Sphere(p2*vec3(1.,-1.,1.)-vec3(0.2, -.1-hello, -0.3), .08);
		if (res.x  < ey ) res = vec2(ey, 2.0);
		float ey2 = -Sphere(p2*vec3(1.,-1.,1.)-vec3(-0.2, -.1-hello, -0.3), .08);
		if (res.x  < ey2 ) res = vec2(ey2, 2.0);
	...
	// Nose
	float ns = -Sphere(p2*vec3(1.,-1.,1.)-vec3(0., .0-hello, -0.43), .08);
	if (res.x  < ns ) res = vec2(ns, 2.0);
	...
	res.x = sMin(res.x, d);
	// Right hand...	
	d = Segment(p*vec3(1.0, .75, 1.90), wristR+vec3(0, .1, 0.), wristR+vec3(-.15, .05, -.215), .14, .02);
	res.x = min(res.x, d);
	
	// Left arm upper...
	d = Segment(p, shoulderL, elbowL+vec3(.2,-.1,-.1), .19, .05);
	res.x = sMin(res.x, d);
	// Left arm lower...
	d = Segment(p, elbowL+vec3(.20,-.10,-.2), wristL+vec3(-.0,-1.8,0.4), .14, .05);
	res.x = sMin(res.x, d);
	
	// Hand...	
	p = p+vec3(.0,0.8,.2);//(p-wristL);
	p.z -= p.x*.5;
	//d = RoundBox(p-vec3(0.0, -.1, 0.0), vec3(.045, .09, -.05), .09);
	//res.x = sMin(res.x, d);
	d = Sphere(p-vec3(.04, -.35+4.52*mod(time,1.), 0.75-12.*mod(time,1.)), .03);// vec3(.045, .09, -.05), .09);
	res.x = sMin(res.x, d);
	d = Sphere(p-vec3(0.10, -.18, -0.15), .08);// vec3(.045, .09, -.05), .09);
	res.x = sMin(res.x, d);
	d = Sphere(p-vec3(-0.05, -.2, -0.05), .08);// vec3(.045, .09, -.05), .09);
	res.x = sMin(res.x, d);
	//p = RotMat(vec3(0.0, 0.60, 1.0), -wave*.5) * p*vec3(-.75,1.3,1.0);
	d = Segment(p, vec3(0), -vec3(-.1, -0.2, 0.75), .06, -.0);
	res.x = sMin(res.x, d);
	
	//p = pos;
	// Mirrored legs...
	p.x = abs(p.x);
	vec3 topLeg = vec3(0.22, -.4, 0.20);
	vec3 ankle  = vec3(0.6, -1.0, 0.0);
...
void main( void )
{
	...
	float s=(1.3-mod(time-1.57,1.))*abs(p.y-1.5);
	float c=(s+sin(p.x*4.))*(s+sin(p.y*6.+time*6.))/12.;
	float d=sin(p.y*8.+time*8.)/12.*(1.-mod(time-1.57,1.))*c*8.;
	p+=d;
	
	float m = mod(time/10.1,.19);//fract(time*.19);
	hello =  (1.0+sin(m*120.0)) * .1 * (  smoothstep(0.015, .1, m));
	owwww = (smoothstep(0.05, .1, m)-smoothstep(0.12, .15, m))*.12;
	if (mod(time-1.0, 3.0) < .12)
			blink = true;
	
	// Camera...
	float t = clamp(time, 0.0, 1.0)*clamp(sin(time/2.)+.5,1.,1.); //0.,1.
	//t = t*t*(3.0-2.0*t);
	...
	wristL		= vec3(0., elbowL.y+cos(wave)*.8, -.75);
	...
	gl_FragColor = vec4(clamp(col, 0.0, 1.0)+clamp((c-.2)*5.,0.,1.),1.);	
