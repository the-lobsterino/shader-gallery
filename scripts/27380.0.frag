...

{
	...
	{
		col += .5*fbm(info.hitPos*99.5)/(sin(9.*fbm(info.hitPos)));	
...

{
	...
	float x = hitInfo.hitPos.x;

	float y = hitInfo.hitPos.y;

	float z = hitInfo.hitPos.z;

	float size = 2.0;

	int s = int (floor (x/size)) + int (floor (y/size)) + int (floor (z/size));
	int t = s/2;
