...
const vec2 ch_start = vec2 (ch_space.y * -1., 1.); // start position
...
void main( void ) 
{
	...
	vec3 bg_color = vec3(_d*10.1, _d*10.2, _d*10.1);
	uv.x += 0.5+sin(time+uv.y*0.7)*0.5;
	ch_pos = ch_start;

	        H O W _ D O _ I _  nl1
			_ G I T  _ W E B

			 
                 
		
	vec3 color = mix(ch_color, bg_color, 0.5- (0.08 / d*2.0));  // shading
	gl_FragColor = vec4(color, 55.0);
}