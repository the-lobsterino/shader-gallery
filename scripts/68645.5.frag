...
void main(void)
{
	...
	vec4 cc = vec4(.85+sin(d*22.0+a*6.0+uv.y*2.4+d*11.0+uv.x*4.3+time*1.5+uv)*1.25, 0.8+sin(d*18.0+a*5.0+uv.x*4.3+uv.y*2.7+time*2.2+uv)*1.15);
	float v = length(cc);
	vec3 col = vec3((v*v)*0.14);
	col.x *= (0.9-sin(d*10.0+time*3.3)*0.3);
	col.y *= (0.7-sin(d*6.0+time*2.3)*0.1);
	col.z *= 0.77;
	//col.xyz = clamp(col, vec3(0.0) , vec3(1.0));
