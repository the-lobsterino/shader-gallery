...
//Mouse X = zoom
//Mouse Y = marching speed multiplier
...
void main( void ) 
{
	...
	for(int i = 0; i < 14; i++)
	{
		...
		layer_col = hsv2rgb(vec3(float(i + 1) / float(layers) + (time * 0.3), 0.9, inten));
		col += pow(layer_col, (sin(time * 0.1) + 1.0) * 10.0 * hsv2rgb(vec3(time * 0.075, 0.9, 1.0)));
	}
