...
//Edited by Robobo1221.
//http://www.robobo1221.net/
//Mie color is being changed based of the total scattering.
//Making it look and be accurate.
...
//	vec3 rayleighAtX = TotalRayleigh(primaryWavelengths) * rayleighCoefficient;
	...
	float sunAngle = max(0.0, cosSunUpAngle * 0.9 + 0.1);
	...
	vec3 mieXtoEye = mieAtX * hgPhase(cosViewSunAngle, mieDirectionalG) * Fex0;
    ...
    gl_FragColor.rgb = pow(0.02 * (sky+sun), vec3(1.0/2.4));
    gl_FragColor.a = 1.0;
}