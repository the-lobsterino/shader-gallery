...
vec4 add_new_cells(inout vec4 cell, in vec2 position);			//adds new cells everywhere randomly
...
float hash(float x)
{
	x = fract(x) * 1234.5677490234375; 						//b -—█———█——█——██—█——█—█——█———█—█—█ //f 1234.5677490234375
...
{
	vec2 n;
	n.x = (fract(cos(v.y - v.x * 1234.5677490234375) * (v.y + v.x) * 1234.5677490234375));
	n.y = (fract(sin(v.x + v.y * 1234.5677490234375) * (v.x - v.y) * 1234.5677490234375));
	return n;
}
...
void main() 
{
	...
	for (int i = 0; i < 8; i++)
		...
		flow.x			= flow.x == 0. && positive_angle > 0. ?               positive_angle : flow.x;
		flow.x			= positive_angle > 0. ? mix_angle(positive_angle, flow.x, viscosity) : flow.x;
		...
		flow.y			= flow.y == 0. && negative_angle > 0. ?               negative_angle : flow.y;
