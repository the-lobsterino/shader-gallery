...
vec3 pos, pfus, pmot, pcab, parm, pwin, ptim, ptai, tpos ,pos20,pp20, pos30, pp30 ;
vec3 ppos=vec3(0.),ppos2=vec3(0.); 
vec3 ppos20 =vec3(10.*0.,-8.*0.,-1.08*0.),ppos22=vec3(10.*0.,-8.*0.,-1.08*0.) ;
vec3 ppos30 =vec3(0.,0.,0.),ppos32=vec3(0.,0.,0.) ;
...
float pampa(vec3 p) {     // first Pampa
    p.z+=2.8*0.5*2. ;  // to back  => 0 point(genntenn)
    p.y-=.8;
    p.yz*=rotyz;
 //   p.xz*=rotxz;
    p.z-=2.8*1.5/1.5;      // to front => 0 point
    p.y+=.8;
    p.xy*=rotxy;
    ...
    if (d==tim) id=5.;
    if (d==arm) id=6.;
    float t=step(abs(pcab.z+.3*0.2-pcab.y*.5*0.),.07*1.3);
    t=max(t,step(abs(pcab.z+1.4*0.7+pcab.y*.3),.05*1.3));
    id-=step(1.,t);
    id=max(0.,id);
    pos=p;
    return d*1.2;
...
////////////////////////////// second pampa20 
float pampa20(vec3 p) {
    p.z+=2.8*3.;    // to back
    p.y-=.8;
 //   p.yz*=rotyz;
 //   p.xz*=rotxz;
    p.z-=2.8*0.;
    p.y+=-.8*3.;  p.x -=6.*1.;  // y-0.8*2=x up  // y=x-6 to right
    p.xy*=rotxy;
    float bound=box(p,vec3(5.,3.,6.),0.);
#ifdef noplane
    bound=box(p,vec3(1.,1.,2.3),0.);id=1.;return bound;
    ...
    objcol=vec3(1.);
    p.z*=-1.05;
    float cab20=cabina(p);
    float fus20=fuselaje(p,cab20);
    float mot20=motor(p);
    float win20=alas(p);
    float tai20=cola(p);
    float tim20=timon(p);
    cab20=abs(cab20)-.02;
    float arm20=armas(p);
    float d=smin(fus20,cab20,max(.001,.02*p.z));
    d=min(d,mot20);
    d=min(d,win20);
    d=min(d,tai20);
    d=min(d,tim20);
...
////////////////////////////// Pampa 30 start
  float pampa30(vec3 p) {
    p.z+=2.8*3.*1.5;    // to back
    p.y-=.8;
 //   p.yz*=rotyz;
 //   p.xz*=rotxz;
    p.z-=2.8*0.;
    p.y+=-.8*3.*1.3;  p.x -=-6.*1.;  // y-0.8*2=x up  // y=x-6 to right
    p.xy*=rotxy;
    float bound=box(p,vec3(5.,3.,6.),0.);
#ifdef noplane
    bound=box(p,vec3(1.,1.,2.3),0.);id=1.;return bound;
    ...
    objcol=vec3(1.);
    p.z*=-1.05;
    float cab30=cabina(p);
    float fus30=fuselaje(p,cab30);
    float mot30=motor(p);
    float win30=alas(p);
    float tai30=cola(p);
    float tim30=timon(p);
    cab30=abs(cab30)-.02;
    float arm30=armas(p);
    float d=smin(fus30,cab30,max(.001,.02*p.z));
    d=min(d,mot30);
    d=min(d,win30);
    d=min(d,tai30);
    d=min(d,tim30);
    ...
    id-=step(1.,t);
    id=max(0.,id);
    pos30=p;        // pos =p; first   pos20 , pos30
    return d*1.2;
}
...
vec2 contrail(vec3 p) {
    ...
    chorro=max(chorro,pp.z-.1);  // trail x axis start pos
    chorro=max(chorro,-pp.z-14.);   // trail        end pos
    return vec2(chorro,w);
}

//////////////////////////////////////////  second trail
 vec2 contrail20(vec3 p) {
    vec3 pp20=p-ppos20;
    float t2=(tpos.z/3.)*speed-2.+.04*speed;
    float chx20=(interhash(0.,t2)-.5)*4.5 +0.3*1.0 ; // 0.5*0. trail x axis
    float chy20=.8+(interhash(-10.,t2*.75)-.5)*1.5+.018-baja +0.10*1.5 ; //0.10 trail y axis
    float w20=.003+min(.05,-pp20.z*.005)*2.  ;   // 0.5*2. z axis
    w20=min(.03,w20);
    float chorro20=length(p.xy-vec2(chx20,chy20))-snoise(tpos.zz*15.)*w20*.4;
    chorro20=max(.003,abs(chorro20));
    chorro20=max(chorro20,pp20.z-(-.3*2.0 )); // z axis start pos of trail
  //  chorro20=max(chorro20,-pp20.z-14.);
    return vec2(chorro20,w20);
}

//////////////////////////////////////////  second end
////////////////////////////////////////// third   start
   vec2 contrail30(vec3 p) {
    vec3 pp30=p-ppos30;
    float t2=(tpos.z/3.)*speed-2.+.04*speed;
    float chx30=(interhash(0.,t2)-.5)*4.5 -0.3*1.0 ; // 0.5*0. trail x axis
    float chy30=.8+(interhash(-10.,t2*.75)-.5)*1.5+.018-baja +0.10*1.9 ; //0.10 trail y axis
    float w30=.003+min(.05,-pp30.z*.005)*2.  ;   // 0.5*2. z axis
    w30=min(.03,w30);
    float chorro30=length(p.xy-vec2(chx30,chy30))-snoise(tpos.zz*15.)*w30*.4;
    chorro30=max(.003,abs(chorro30));
    chorro30=max(chorro30,pp30.z-(-.3*2.0*1.3 )); // z axis start pos of trail
  //  chorro20=max(chorro20,-pp20.z-14.);
    return vec2(chorro30,w30);
}
...
float de(vec3 p) {
    ...
    float d=min(wat,ter);
       vec3 pos20=p-ppos20;
       float pam20=pampa20(pos20*20.)/20.;
    d=min(d,pam20);
    //////////////////////////// second
  //    vec3 pos20=p-ppos20;
  //  float pam20=pampa20(pos20*20.)/20.;
    ...
    //////////////////////////   third start
    vec3 pos30=p-ppos30;
       float pam30=pampa30(pos30*20.)/20.;
...
vec3 pampaColors20() {
    vec3 base=vec3(117.,170.,219.)/255.;
    vec3 sec=vec3(1.*0.1,.97,.93);
    vec3 ter=mix(sec,base,(1.+cos(pos20.z*3.))*.5);
    ter=base;
    vec3 col=mix(base,vec3(.9),smoothstep(.55,.6,max(noise(floor(pos20*2.)),noise(floor(pos20*4.)))));
    col=mix(col,base,abs(pmot.x)*.25);    
    vec3 colg=mix(vec3(0.2,0.8,0.3),vec3(.9),smoothstep(.55,.6,max(noise(floor(pos20*2.)),noise(floor(pos20*4.)))));
  
  if (id==20.) {
        col=mix(col,ter,step(abs(pfus.x+(noise(pos20*5.+4.)-.5)*.0),.2));
    }
    if (id==22.) {
        vec3 cab=vec3(.51);
        float t=step(.07,abs(pcab.z+.3-pcab.y*.5));
    ...
    if (id==21.) {
        col=mix(colg,sec,step(2.1,-pmot.z)*step(0.,dcab-.1));
        col=mix(col,sec,step(2.2,pmot.z));
    }
    if (id==23.) {
        pwin.x=abs(pwin.x);
        col=mix(colg,sec,step(4.3,abs(pwin.x)));
        col-=step(abs(pwin.z-.7),.03)*.2*step(pwin.x,4.3);
        if (pwin.z>0.7) {
            col-=step(abs(pwin.x-3.5),.03)*.2;
            col-=step(abs(pwin.x-1.5),.03)*.2;
    ...
    if (id==24.) {
        ptai.x=abs(ptai.x);
        col=mix(colg,sec,step(1.6,ptai.x));
        col-=step(abs(ptai.z-.35),.03)*.2*step(ptai.x,1.2);
        col-=step(abs(ptai.x-1.2),.03)*.2;
    }
    if (id==25.) {
        col=mix(colg,sec,step(1.3,ptim.y));
        col-=step(abs(ptim.z-.5),.03)*.2*step(ptim.y,1.3);
    }
    if (id==26.) {
        col=mix(col,ter,step(length(parm.xy),.25));
...
/////////////////////////////////////////////// Color third start
  vec3 pampaColors30() {
    vec3 base=vec3(117.*2.,170.,219.)/255.;
    vec3 sec=vec3(1.,.7*0.1,.3);
    vec3 ter=mix(sec,base,(1.+cos(pos30.z*3.))*.5);
    ter=base;
    vec3 col=mix(base,vec3(.9),smoothstep(.55,.6,max(noise(floor(pos30*2.)),noise(floor(pos30*4.)))));
    col=mix(col,base,abs(pmot.x)*.25);    
    
    if (id==30.) {
        col=mix(col,ter,step(abs(pfus.x+(noise(pos30*5.+4.)-.5)*.0),.2));
    }
    if (id==32.) {
        vec3 cab=vec3(.51);
        float t=step(.07,abs(pcab.z+.3-pcab.y*.5));
...
vec3 shade(vec3 p, vec3 dir) {
    ...
    vec3 col;
    float oc=1.;
    if (id<0.) col=terrainColors(tpos,n,y),oc=ao(p,n); 
       else col=pampaColors() ;
     if( id>=20. && id<30. )  col =pampaColors20() ;   // second color ad
     if( id >=30.  )   col =pampaColors30() ;                     //   3 color ad
...
vec3 shadeVid(vec3 p, vec3 dir) {
    vec3 cab=vec3(0.91,0.97,0.9) ; // de(0.1) (.91);
    float t=step(.07,abs(pcab.z+.3-pcab.y*.5));
    t=min(t,step(.05,abs(pcab.z+1.4+pcab.y*.3)));
    vec3 col=cab;
    vec3 n = normal3(p);
...
vec3 march(vec3 from, vec3 dir, vec2 uv) 
{
    ...
    float                           g20=0.,     chorro20=1. ;
    float                           g30=0.,     chorro30=1. ; // 3 jet ad
    ...
    for (int i=0; i<250; i++) {
        ...
        pp20 =p-ppos20;
        /////////////////////////////////////second
         if (pp20.z<-.1 &&p.z>-14.-2.5){
            vec2 ctrail20=contrail20(p /*pp20*/); //which one?
            float chorro20=ctrail20.x;
            float w20=ctrail20.y ;
            d=min(d,chorro20*(.5+w20*17.));
            g20=max(g20,max(0.,w20-chorro20)/w20*smoothstep(-14.,0.,pp20.z)*(1.-ref*.5)
                *smoothstep(-.07,-.35,pp20.z));              
        }
        ...
        pp30 =p-ppos30;
         ...
         if (pp30.z<-.1 &&p.z>-14.-2.5){
            vec2 ctrail30=contrail30(p /*pp20*/); //which one?
            float chorro30=ctrail30.x;
            float w30=ctrail30.y ;
            d=min(d,chorro30*(.5+w30*17.));
            g30=max(g30,max(0.,w30-chorro30)/w30*smoothstep(-14.,0.,pp30.z)*(1.-ref*.5)
                *smoothstep(-.07,-.35,pp30.z));              
        }
    ...
    col=mix(col,vec3(.85),g*.5);
      //////////////////////////////  second trail
     g20*=smoothstep(0.5,1.,t);
    col=mix(col,vec3(.95,0.76,0.3),g20 *.5);  // red color
     //////////////////////////////   second trail end
     
     /////////////////////////////    3      trail start
     g30*=smoothstep(0.5,1.,t);
    col=mix(col,vec3(.95,0.276,0.3),g30 *.5);  // red color
...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
    ...
    ppos.x=(interhash(0.,t1)-.5)*4.5;  // -0.5*0.
    ...
    rotyz=rot((ppos.y-ppos2.y)*.9);
    rotxy=rot((ppos.x-ppos2.x)*.8);
    rotxz=rot((ppos.x-ppos2.x)*.3);
    
    //////////////////////////////////////  second 
     ppos20.x=(interhash(0.,t1)-.5)*4.5;  // 2nd jet
    ppos22.x=(interhash(0.,t2)-.5)*4.5 +0.3;    // 2nd trail
    
    baja=smoothstep(125.,130.,time)*1.65;
    ppos20.y=.8+(interhash(-10.,t1*.75)-.5)*1.5-baja;//2nd jet
    ppos22.y=.8+(interhash(-10.,t2*.75)-.5)*1.5-baja +0.15 ; //2nd trail
    rotyz=rot((ppos20.y-ppos22.y)*.9);
    rotxy=rot((ppos20.x-ppos22.x)*.8);
    rotxz=rot((ppos20.x-ppos22.x)*.3);
    
    //////////////////////////////////////  second end
    
    //////////////////////////////////////    3    start
      ppos30.x=(interhash(0.,t1)-.5)*4.5;  // 3   jet
    ppos32.x=(interhash(0.,t2)-.5)*4.5 -0.3;    // 3ird trail
    
    baja=smoothstep(125.,130.,time)*1.65;
    ppos30.y=.8+(interhash(-10.,t1*.75)-.5)*1.5-baja;//3 jet
    ppos32.y=.8+(interhash(-10.,t2*.75)-.5)*1.5-baja+0.19  ; //3 trail
    rotyz=rot((ppos30.y-ppos32.y)*.9);
    rotxy=rot((ppos30.x-ppos32.x)*.8);
    rotxz=rot((ppos30.x-ppos32.x)*.3);
    ...
    if (mouse.x<1.) {
        ...
        from.yz*=rot(.5-(1.-m.y)*1.5*0.7);
        from.xz*=rot(-m.x*6.*0.5);
        mouseon=true;
    }
#endif
#ifdef onlymouse
