/*R040904作例　by ニシタマオ*/
/*サンドボックス上での試験用、簡易レイトレシステム、二段階反射・屈折機能、ぼかし影機能付（R040904版）*/
...
//ゲームキャラ用共通動作ここから
vec4 fVGame_FireFly(vec4 VP, int I){
	vec4 VPi =VP, VRandom =fVN(fNRandom(I));
	VPi.xyz -=sin(normalize(VRandom.xyz -0.5) *VPi.w) *vec3(6,8,4);
	return VPi;
}
//共通動作ここまで
...
vec4 fVPSet(vec3 V3P){
	...
	VP.xyz *=fM3Rotate(-VMouse.yx *0.2 +vec2(0.5,0));
...
/*ここから下に所要の図形の表面距離を、座標vec VPと、距離float NPとの関係で記述*/
	int iSQ_Twist =fISequencer(40, 4), iSQ_Type =fISequencer(40 *3, 3);

	if(true){	//メビウスの輪
		vec4 VPi =VP;
		VPi.y -=2.0;
		VPi.xyz *=fM3Rotate(VMouse.yx *8.0);

		float nTh =atan(VPi.x, VPi.y);
		nTh *=float(iSQ_Twist) /4.0;

		if(iSQ_Type ==0)	nTh *=2.0;
		nTh +=VPi.w *0.5;

		vec2 V2P =vec2(length(VPi.xy) -4.0, VPi.z);
		...
		if(iSQ_Type ==0){
			V2P.x =abs(V2P.x) -1.25;
			nPi =length(V2P) -0.75;
		}

		if(iSQ_Type ==1){
			nPi =length(max(abs(V2P) -0.75, 0.0)) -0.1;
		}
		...
		if(NP >nPi){
			SFi= SFo;
			SFi.VColor.rgb =sin((vec3(0,1,2) /3.0 +VPi.w *0.01) *acos(-1.0) *2.0) *0.2 +0.8;
		}
		NP =min(NP, nPi);
	...
	if(true){	//水面
		vec4 VPi =VP;
		VPi.xyz +=vec3(0,0,1) *VPi.w;
		float nPi =abs(VPi.y +2.0 +sin(VPi.w) *2.0 +sin(VPi.x +sin(VPi.z +VPi.w)) *sin(VPi.z +sin(VPi.x +VPi.w)) *0.05) -0.1;
	...
	if(true){	/*鬼火*/
		const int ciFire =3;
		for(int I =0; I <ciFire; I++){
			vec4 VPi = fVGame_FireFly(VP, I);
...
vec4 fVEffect(vec4 VColor, smarch SM){
	...
	if(true){	/*鬼火*/
		...
		for(int I =0; I <ciFire; I++){
			vec4 VPi = fVGame_FireFly(VP, I);
			float nL =length(VPi.xyz), nDot =dot(normalize(VPi.xyz),+V3NL), nV;
			nV +=clamp(+nDot *pow(nL, -2.0), 0.0, 1.0) *4.0;
			vec3 V3C =sin((vec3(0,1,2) /3.0 +VPi.w *0.1 +float(I) /float(ciFire)) *acos(-1.0) *2.0) *0.5 +0.5;
			VColor.rgb +=nV *V3C;
		}
	}
