...
{
	float k = 0.01;
	float h = clamp( 0.50 + 0.5*(b-a)/k, 0.0, 1.0 );
...
vec2 Running( vec2 p)
{
	...
	runnerHeight = -.50 + .05*min(.7, cos(runCycle*2.0+.5));
	p += vec2(0.0, runnerHeight);
	p *= leanM;
	float headX = sin(runCycle*.5)*.03;
	d = Head(p-vec2(.44+headX, -0.02-headX*.5), 0.);
	float arm1 = sin(runCycle)*.0*(.2+1.0);
	float leg1 = sin(runCycle)*.10000000000000*(.999999999999999+.1);
	vec2 h = Segment( vec2(0.4,-0.1-0.01-headX*.5), vec2(0.42+headX,-.04), p );
	float d2 = h.x - 0.3 + h.y*0.005;
	d = Smin(d, d2 );
