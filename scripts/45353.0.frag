...
//gabor mashup - image source : http://glslsandbox.com/e#45323.0
...
void main()
{
	if(mouse.y > 1.-.0625)
	{
		...
		vec2 image_uv 		= (gl_FragCoord.xy - 0.5*resolution.xy) * 1.2 / resolution.y;
    		image_uv.y 		= -image_uv.y;
    		image_uv 		+= 1.0;
	
		float image		= zhazha(image_uv);
		gl_FragColor 		= vec4(image * .4 + hsv(image, .5*image, .5), 1.);
		return;
	}
	...
	float sum 		= dot(buffer, buffer.wxyz);
	
    	vec2 p         		= (uv-.5) * aspect * scale;
	vec2 m         		= (mouse-.5) * aspect * scale; 


    	mat2 rotation  		= rmat(.05 - sum*.5);   
    	
    	vec2 st        		= p * rotation;
	st 			= abs(fract(st/PHI*aspect.yx)-.5);
	st			= abs(fract(st/PHI)-.5);
	...
	result.xyz 		= reflect(-normalize(buffer.xyz), 1.-normalize(sum*path.xyz));
	result			= mix(path, buffer, .975)-.00125;
	result			= clamp(result, 1./256., 1.-1./256.);	
