...
//tutorial: https://www.youtube.com/watch?v=13wiXc0377g
...
//Automatically generated from the sprite sheet here: http://f0nt.com/sov-digital/.png
...
vec3 rgb() {
	...
	color.y = sin((0.03125) + animation + 1.0);
...
vec3 Text(vec2 uv)
{
	...
	// Feel free to edit:
	_ _ _ _o _b _j _e _c _c ;
		
	
	BEGIN_TEXT(res.x/2.0-STRWIDTH(11.0)/2.0,res.y/2.0)
	print_pos += vec2(cos(time)*96.,sin(time)*96.);
	
	// RGB(1,0,0) // RGB(1,0,0) _M RGB(1,.5,0)_o RGB(1,1,0)_v RGB(0,1,0)_i RGB(0,.5,1)_n RGB(0.5,0,1)_g _ RGB(1,0,0)_T RGB(1,.5,0)_e RGB(1,1,0)_x RGB(0,1,0)_tt
...
void main( void )
{
	...
	vec3 col = pixel*1.5+0.0;
	col *= (1.-distance(mod(uv,vec2(1.0)),vec2(0.50)))*1.0;
