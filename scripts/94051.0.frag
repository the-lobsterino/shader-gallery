...
const float streetDistance = 1.6;
const vec3 streetColor = vec3(1.0, 5.0, 20.0);

const float fogDensity = .3;
const float fogDistance = 2.0;
const vec3 fogColor = vec3(1.34, 2.37, 0.4);

const float windowSize = 1.1;
const float windowDivergence = 7.2;
const vec3 windowColor = vec3(0.1, 0.2, 0.5);

const float beaconProb = 2.0003;
const float beaconFreq = 0.6;
const vec3 beaconColor = vec3(2.5, 1.2, 0.0);


const float tau = 6.283185;

float hash1(vec2 p2) {
    p2 = fract(p2 * vec2(12.3983, 5.4427));
...
vec2 hash2(vec2 p2) {
    vec3 p3 = fract(vec3(4.3983 * p2.x, 5.4427 * p2.y, 6.9371 * p2.x));
    p3 += dot(p3, p3.yzx + 29.19);
    ...
    for (int i = 0; i < 100; ++i) {
        ...
        if (h.x < prob) {
            ...
            if (t < dist) {
                ...
                t = distance(center, p);
                fog *= smoothstep(2.0, 0.5, cos(tau * (beaconFreq * iTime + h.y)));
                beacon += fog * pow(clamp(1. - 2.0 * t, .0, 1.0), 4.0);
            }
        }
