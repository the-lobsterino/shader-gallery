// just modified the shader
//
// lowed down and removed the black bars
//
//
// cheers A5 aka inc
//
...
{
	float fogAmount = clamp(dis*dis* 0.000072, 0.0, 1.0);
...
vec3 GrassBlades(in vec3 rO, in vec3 rD, in vec3 mat, in float dist)
{
	...
	float rCoC = CircleOfConfusion(dist*.1);
	float alpha = 0.0;
	
	vec4 col = vec4(mat*0.05, 0.0);
...
vec3 TerrainColour(vec3 pos, vec3 dir,  vec3 normal, float dis, float type)
{
	...
	{
		// Random colour...
		mat = mix(vec3(.0,.6,1.0), vec3(1.0,1.0,1.0), Noise(pos.xy*.065));
		// Random shadows...
...
void main(void)
{
	...
	float gTime = (time*1.5+m+2352.0)*.006;
