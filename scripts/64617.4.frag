...
vec3 getLight(in vec3 p) {
	...
	//lParallel *= (mouse.x-.5)*10.;
	lParallel *= -1.;
	
	vec3 lfire = vec3(0.);
	vec2 lfirexy = vec2(p.x, p.y);
	lfirexy *= vec2(0.25,5.);
	lfirexy += vec2(0.5,0.0);
	lfirexy *= resolution;
	//lfire += fire(vec2(max(p.x, p.z), p.y)*500., 0.3).rgb*(mouse.x-.5)*10.;
	lfire += 1.-length(p)*1.;
	lfire *= fire(lfirexy, 1.).rgb;
	lfire = max(vec3(0.), lfire);
    
    return (lPoint + lParallel + lPoint2) * _lightColor+lfire;
}
...
void main(void)
{
	...
	//gl_FragColor = fire(gl_FragCoord.xy, .3);
	//gl_FragColor += fire(gl_FragCoord.xy, -.3);
}