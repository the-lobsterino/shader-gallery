...
//Originally written by srtuss. Modified by Vlad. Modi^WSeizurfied some more by dbtx
//Mouse X = zoom
//Mouse Y = time multiplier
...
void main( void ) 
{
	...
	uv = (uv * mouse.x - mouse.x / 2.0) * 10.0;
	uv.x *= resolution.x / resolution.y;


	// ray origin
	vec3 ro = vec3(10.0, 10.0, time * 0.0);
	ro.y = 0.0;
	// camera look at
	...
	for(int i = 0; i < 12; i++) //aforementioned n layers here, failed when variable
	{
		float layer = float(i);
		float inten = 0.0;
		...
		if(its.x > 0.0)
		{
			...
			// add some special timed fx to each layer
			{
				...
				float fxi = cos(vo.x * 0.2 + time * (mouse.y / 5.0) * float(i + 1));//abs(crd - vo.x);
				fx = clamp(smoothstep(0.9, 1.0, fxi), 0.0, 0.9) * 1.0 * rand12(vo.xy);
				fx *= exp(-3.0 * vo.z) * 2.0;
			}
			inten += v * 0.1 + fx;
		}
		layer_col = hsv2rgb(vec3(float(i + 1) / float(layers), 0.9, inten));
		col += pow(layer_col, 0.1 * hsv2rgb(vec3(float(i + 13) / float(layers), 0.95, inten / 0.)));
	}
