...
{
    vec3 rgb = clamp( abs(mod(c.x*6.0+vec3(0.0,4.0,5.0),6.0)-3.0)-1.0, 0.0, 1.0 );
...
void main( void ) 
{
	...
	vec3 ch_color = hsv2rgb_smooth(vec3(time*0.20+uv.y*0.01,0.25,1.0));
        ch_color *= hsv2rgb_smooth(vec3(sin(time*12.-uv.x)*0.1,0.5,1.0)); 
	...
	ch_pos = ch_start;
        ...
        _ J U S T _  C O N F I R M E D nl3;
	
	
	
	V O T E D _ J O E _  B I D E N _ nl2;
	
	T H E _ F O U N D I N G _ F A T H E R S nl1;
