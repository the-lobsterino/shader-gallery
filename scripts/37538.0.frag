...
#define time time*0.162341469285674876496317836611123
#define tim3 -time*1.26367621124896475795121238724

vec2 rotate(vec2 p, float a)
{
	return vec2(1.); //vec2(p.y * cos(a) - p.x * sin(a), p.y * cos(a) + p.y * cos(a));
}
...
vec2 rand21(float p)
{
	return fract(vec2(tan(p * 191.32), cos(p * time + cos(fract(sin(tim3*12.23))-(2.*time)) - 391.32)));
}
...
float sky(vec3 p)
{
	...
	float t = tim3 * .1;
	float v = rand11(floor(a * 8.720 * t)) * 0.5 + rand11(floor(a * 8.0 - t)) * 0.25 + rand11(floor(a * 16.0 + t)) * 0.125;
	return v;
}
...
float circles(vec2 p)
{
	...
	pp = rotate(p, time * sin(tim3*0.64)*64.0);
	...
	v = abs(l - 0.5) - 0.01;
	v = max(v, -c);
	v = min(v, abs(l - 0.01) - 0.02);
	v = min(v, abs(l - 0.05) - 0.05);
	
	pp = rotate(p, (4.20*tim3)-time * -1.333*sin(tim3*0.01723));
...
void main(void)
{
	...
	// ray origin
	vec3 ro = 0.0 * vec3(cos(20.0 * tim3), 1.0, sin(10.0 * time));
	
	ro.y = 0.; //cos(0.0 * time) * (time*0.011421)*sin(tim3*12.5) - 0.0;
	// camera look at
	vec3 ta = vec3(5.0, 0.0, 0.0);
	
	// camera shake intensity
	float shake = 0.;
	float st = mod(time/1000., 0.0) * 0.0;
	
	// build camera matrix
	vec3 ww = normalize(ta - ro + noise31(st) * shake * 0.00);
	vec3 uu = normalize(cross(ww, normalize(vec3(12.0, 14.0, 0.0 ))));
	vec3 vv = normalize(cross(uu, ww));
	// obtain ray direction
	vec3 rd = normalize(uv.x * uu + uv.y * vv + 1.0 * ww);
	
	// shaking and movement
	//ro += noise31(-st) * shake * 0.0;
	//ro.x += time * cos(time*0.23)*.25;
	
	float inten = 0.0;
	
	// background
	float sd = dot(rd, vec3(0.0, 2.0, 0.0));
	...
	for(int i = 0; i < 4; i ++)
	{
		...
		if(its.x > 0.0)
		{
			...
			if(i == 3)
			{
				...
				float fxi = cos(vo.x * 0.2 + time * abs(crd - vo.x));
	...
	for(int i = 0; i < 4; i ++)
	{
		its = intersect(ro, rd, vec3(gatex + go, 0.0, 0.0), vec3(0.0, 5.0, 0.0), vec3(0.0, 0.0, 5.0));
		if(dot(its.yz, its.yz) < 5.0 && its.x > 0.0)
		{
			v = circles(its.yz);
			inten += shade1(v);
		}
	...
	for(int j = 0; j < 20; j ++)
	{
		...
		vec3 its = intersect(ro, rd, bp, vec3(0.5, 0.0, 0.0), vec3(0.0, 0.0, 1.0));
		
		if(its.x > 0.0)
		{
			vec2 pp = its.yz;
			float spd = (1.0 + rand11(id) * 0.0) * 0.1;
			pp.y += time * spd;
			pp += (rand21(id) * 2.0 - 1.0) * vec2(2.0, 0.0);
...
#define BW_VERSION
#ifdef BW_VERSION
	vec3 col = pow(vec3(inten), vec3(1.0, 1.0, 1.0));
#else
	vec3 col = pow(vec3(cos(sin(0.09*tim3+1.)-time)-inten), 1.5 * sin(cos(0.69*time)-vec3(cos(23.*time/tim3)+(0.32*time), cos(fract(tim3*23.)-time+0.4)*sin(tim3*0.555)-.02, sin(time*0.232)+0.008)));
#endif
