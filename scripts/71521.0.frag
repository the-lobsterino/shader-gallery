...
vec3 addVertex(vec4 vertex, vec3 color, mat4 projMat, vec2 camUV) {
	...
	return color * star(camUV-vec2(.5)-p.xy,.02,.3);
}
...
void main(void) {
	//vec2 uv = (gl_FragCoord.xy - vec2(resolution.x * 0.5, resolution.y * .5)) / -resolution.y;
	vec2 uv = (gl_FragCoord.xy - vec2(resolution.x * 0.25, resolution.y * .0)) / resolution.y;
