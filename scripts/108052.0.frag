...
vec2 rand21(float p)
{
	return fract(vec2(sin(p * 4.), cos(p * 391.32)));
}
...
void main( void ) 
{
	...
	vec3 col = pow(vec3(inten.r, (inten.g * 9.9), inten.b), 0.5 * vec3(cos(time*0.0)/6.0+0.33)); //pow(base color, glow amount)
