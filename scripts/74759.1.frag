...
#define time TAU*(dot(surfacePosition.xy,surfacePosition.yx))
...
float fn(vec2 p)
{
	...
	float m = (p.x*p.y);
	m = cos( floor( (m) * (TAU * time) - TAU * (p.x-p.y)*sin(time)) + (a));
	return fwidth(abs(m));	
}
...
bool eq(mat4 m, mat4 n) {
  return eq(m[0],n[0]) && eq(m[1],n[1]) && eq(m[2],n[2]) && eq(m[0],n[0]);
}
...
vec3 applylighting(vec3 baseColor, vec3 p, vec3 n, vec3 r) {
  ...
  c += specular*s*specularcolor*sin(time);
  return c;
}
...
void main(void)
{
  ...
  torus_r = M/N;
  torus_R = sqrt(torus_r*torus_r+.50);
  ...
  for (int i = 0; i < AA; i++) {
     for (int j = 0; j < AA; j++) {
       vec2 uv = (2.0*(gl_FragCoord.xy+vec2(i,j)/float(AA)) - resolution.xy)/resolution.y;
