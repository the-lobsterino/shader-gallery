...
const float kPipeRadius = 0.5;
const float kPipeThickness = 0.1;
 float kPipeHeight = 0.5+abs(sin(time))*4.0;
//float kPipeHeight = 2.0 + sin(iGlobalTime);

const float kWaterNoiseScale = 0.027;

 float kWaterVelocity = 0.75+abs(sin(time)) * 0.5;
const float kWaterAccel = -1.0;

const float kWaterAnimSpeed = 60.0;
const float kTrenchWaterAnimSpeed = 30.0;
...
const float kInverseTransparency = 0.27;
