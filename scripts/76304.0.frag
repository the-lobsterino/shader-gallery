...
void main() {
    ...
    color = mix(color, vec3(1.0, 0.145, 0.729), sunLight * 0.3 * (0.6 + noise(vec2(uv.x * 10. + u_time, uv.y * 60. + u_time)) * 0.4));
    ...
    ///
    float sy = mapRange(pow(1.0 - uv.y + 0.0, 7.), 9.0,  seaLimit, 0.0, 1.0) ;
    ...
    float seaGradient = mapRange(uv.y, 8.0, seaLimit, 0.0, 1.0);
