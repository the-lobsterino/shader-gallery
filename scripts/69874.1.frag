...
//#define WINDOW_MUSIC_BLINK // Disabled by default - Blinks can be bad for some people.
#define CARS
#define I_MAX 70
...
void mainImage( out vec4 fragColor, in vec2 fragCoord ) {
 ...
 //   float time = -iTime;       // t => ti (t is reused in PolyigonXY )
 //   float tt = -iTime-0.5;
    
    
    vec3 camPos = vec3(5.0+12.0*sin(time*0.05),5.0+ 7.0*cos(time*0.05), 1.9);
    
    vec3 camTarget = vec3(5.0+0.0,5.0+7.0*sin(time*0.05), 0.0);
    if (fract(time/20.0)<0.25){                  // de t/12.0
    	camPos = vec3(5.*time,3.1*time,2.1);
		camTarget = vec3(5.+5.*(time+0.),10.+3.1*(time+0.),1.7);
    }  
    if (fract(time/12.0)>0.5){                 //   de t/12.0
    	camPos = vec3(35.,3.1,1.);
		camTarget = vec3(35.+sin(time/10.0),3.1+cos(time/10.0),0.7);
    } 
