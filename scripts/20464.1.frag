...
vec3 global_color_hack = vec3(0.1);

#define ASPECT		resolution.x/resolution.y
#define EPSILON		.001
#define FOV 		1.1
#define FARPLANE	12.1
#define ITERATIONS	128

#define VIEWPOSITION	vec3(0., .0, -6.18)
#define VIEWTARGET	vec3(0.5, -.2, 4.)

#define LIGHTPOSITION	vec3(-5., 6., -10.)
#define LIGHTCOLOR	vec3(.95, 0.95,  0.96)
...
vec2 map(in vec3 position)
{
	vec3 rotation		= vec3(.09, 0.21, .24) * time;	
...
vec2 map(in vec3 position)
{
	...
	float s 		= sphere(position+vec3(-.1, -.1 + cos(time) * -.1, -0.1), 2.);
...
surface shade(in ray r, in surface s,  in material m, in light l)
{
	...
	float occlusion		= ambient_occlusion(r.position, s.normal);
    	float shadow		= ashadow(r.position, l.direction);
...
{
	vec3 d = (abs(p) - s);
  	return min(max(d.x,max(d.y,d.z)),0.0) + length(max(d,0.4));
}
...
mat3 rmat(in vec3 r)
{
	vec3 a = vec3(cos(r.x)*acos(r.y),sin(r.y),sin(r.x)*acos(r.y));
