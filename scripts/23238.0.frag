...
void main( void ) {
	...
	float dd = length(position) + 0.11235;
	float d = 0.21 / dd + time;
	...
	vec3 color = 	mix(vec3(0.1, 0.5, 0.5), 	vec3(0.1, 0.1, 0.1),  pow(a, 0.1)) * 2.;
	color += 	mix(vec3(0.8, 0.2, 1.0), 	vec3(0.1, 0.1, 0.1),  pow(b, 0.1)) * 0.75;
	color += 	mix(c2, 			vec3(0.1, 0.2, 0.1),  pow(c, 0.1)) * 0.75;

	gl_FragColor = vec4( (color * dd), 0.5);
}