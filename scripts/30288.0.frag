...
//------------------------------------------------------------------------------------
// Shader:    Mike   created by inigo quilez - iq 2013-05-03
//            https://www.shadertoy.com/view/MsXGWr 
// tags:      3d, raymarching, distancefield
...
// type "mike wazowski".
//
// Raymarching (a simplified version of) Mike's procedural distance field. 
// Modeling and shading are pretty arbitrary, and made after the first picture you get 
// in google image when you type his name. 
//------------------------------------------------------------------------------------
...
void main( void )
{
  ...
  bool gUseMin = /*mouse.z>0.0 && */ mouse.x > gl_FragCoord.x;
  ...
  //-----------------------------------------------------
	...
	//float an = sin(-0.25 + 0.31416*time) - 6.2831*(m.x-0.5);
  float an = 1.0 - mouse.x*2.0;
  ...
  if( tmat.z>-0.5 )
  {
    ...
    else if( tmat.z<1.5 )
    {
      ...
      float f = 1.0; //texturize( iChannel0, 0.15*pos, nor ).x * texturize( iChannel0, 0.2*0.25*pos, nor ).x;
      mate.xyz = mix( 0.8*mate.xyz, vec3(0.3,0.4,0.0), f );

      f = 1.0; //texturize( iChannel0, pos, nor ).x * texturize( iChannel0, 0.25*pos, nor ).x;
      f = f*f;
      mate.xyz = mix( mate.xyz, vec3(0.3,0.4,0.0), 0.6*f );
      //vec3 bnor = -1.0 + 2.0 * texturize( /*iChannel0*/ 1.0 , 4.0*pos, nor );
      vec3 bnor = nor;
...
//  if( mouse.z>0.0 )
  {
    float f = abs(mouse.x-gl_FragCoord.x);
