...
void main()
{	
	vec2 uv = (2.0 *gl_FragCoord.xy - resolution.xy) / resolution.yy+0.5;
	uv.x += 0.13;
	...
	d = min(d,H(uv-vec2(-1.0,1.5)));
	d = min(d,E(uv-vec2(-0.5,1.5)));
	d = min(d,L(uv-vec2(0.0,1.5)));
	d = min(d,L(uv-vec2(0.5,1.5)));
	d = min(d,O(uv-vec2(1.0,1.5)));

	d = min(d,G(uv-vec2(-2.5,0.5)));
	d = min(d,L(uv-vec2(-2.0,0.5)));
	d = min(d,S(uv-vec2(-1.5,0.5)));
	d = min(d,L(uv-vec2(-1.0,0.5)));
	d = min(d,S(uv-vec2(-0.5,0.5)));
	d = min(d,A(uv-vec2(0.0,0.5)));
	d = min(d,N(uv-vec2(0.5,0.5)));
	d = min(d,D(uv-vec2(1.0,0.5)));
	d = min(d,B(uv-vec2(1.5,0.5)));
	d = min(d,O(uv-vec2(2.0,0.5)));
	d = min(d,X(uv-vec2(2.5,0.5)));
	
	float w = fwidth(d);
	col = mix(vec3(1,uv.y,0),col,smoothstep(lineWidth-w,lineWidth+w,d));
	
// 256 angle steps
        float angle = atan(uv.y,uv.x)/(2.*3.14159265359);
        angle -= floor(angle);
        float rad = length(uv);
        ...
        for (int i = 0; i < 5; i++) {
            ...
            col += max(0.,.5-dist*(mouse.y*100.)/adist)*(.5-abs(angleFract-.5))*5./adist/radDist;
	...
	gl_FragColor = vec4(col*vec3((40.*mouse.x)*abs(d)),1.0);
}