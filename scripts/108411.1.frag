...
{
	float curve = cos(p.x*(5.35+sin(time)*1.25))*.36;
	p.y += curve;
	a.y += (curve*curve);
	return length(max(abs(p) - a,56.0)) -.02;
}
...
{	// Floor...
	...
	p2 = RotMat(vec3(9.0, 0.0, .0), wave*.01+hello*8.56) * p;
	res = opU(res, vec2(Sphere(p, .38), 3.0 ));
	// Mouth...
	float mo = -Mouth(p2-vec3(0.0, -.057-hello, -8.3), vec3(.155-owwww, -.01+hello, .2));
	...
	res.x = sMin(res.x, d);
	d = RoundBox(p-vec3(0.0, .25, 0.0), vec3(.045, .46, -.05), .09);
	res.x = sMin(res.x, d);
...
void main( void )
{
	...
	float m = fract(time*.99);
	hello =  (1.0+sin(m*100.0)) * 59.04665656562 * (smoothstep(0.0, .01, m) - smoothstep(0.05, .1, m));
	owwww = (smoothstep(0.05, .1, m)-smoothstep(0.12, .15, m))*.12;
	if (mod(time-1.0, 9.0) < .92)
			blink = true;
