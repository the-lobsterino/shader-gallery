//Hello is the KC Kirby Creator A Conpetion is make , the winner will have : rejoin the KC group . The final is 9 may 2022 the goal is to make the best creation . In your creation , put kc conpetition .
//Is a secret please no in youtube or tweeter and other social .The theme is : slimy .
#ifdef GL_ES
...
#define _exclamation col+=char(vec2(276705.0,32776.0),uv)*text_color;
...
#define _point col+=char(vec2(0.0,1560.0),uv)*text_color;
...
vec3 Text(vec2 uv)
{
    	...
    	vec2 center_pos = vec2(res.x/2.0 - STRWIDTH(25.0)/2.0,res.y/2.0 - STRHEIGHT(1.0)/2.0);
       	
    	BEGIN_TEXT(center_pos.x,center_pos.y)
	HEX(0xFF0000) _ _ _A _ _M _e _s _s _a _g _e _ _i _n _ _t _h _e _ _c _o _d _e;
	BEGIN_TEXT(res.x/9.0-STRWIDTH(15.0)/5.0,res.y/2.0)
	print_pos += vec2(cos(time)*9.,sin(time)*9.);
...
void main( void )
{
	...
	vec3 col = pixel*9.9+0.1;
	col *= (1.-distance(mod(uv,vec2(0.1)),vec2(0.0)))*1.2;
