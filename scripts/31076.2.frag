...
//maximum battery life reduction 
//just kinda shoved this together - mouse bottom left to reset
...
void main(void) 
{
	...
	vec4 cell		= texture2D(renderbuffer, uv);
	
	float r			= cell.w;
	for (float i = 0.; i < 8.; i++)
    	{
		vec2 neighbor_uv 	= gl_FragCoord.xy - neighbor_offset(floor(hash(fract(i*uv*17.))*8.));
		...
		float sequence		= abs(fract(r + q) - .5) < .25 ? field.x : field.y;
		sequence 		= fract(sequence) * .125;
		sequence		= fract(r + q + sequence);

		if(floor(sequence * 8.) != i)
		{
			cell = mix(cell, neighbor, cell.w);
		}
	}	
	
	//cell	 	= mouse.x + mouse.y < .1 || floor(time) < 8. ? tree_scene() * vec4(1., 1., 1., steps/768.) : cell;
	
	cell		= mix(cell, tree_scene() * vec4(1., 1., 1., steps/768.), .025); 
	
	/*
	vec4 prior	= texture2D(renderbuffer, uv);


	vec4 result 	= vec4(r.c, distance(r.o, r.p)*.005);
	result 		= cells(result);
	
	result 		= mouse.x + mouse.y < .1 ? vec4(r.c, prior.w) : result;
*/
	gl_FragColor = cell;
}//sphinx
...
vec4 tree_scene( void ) {
	...
	return vec4(r.c, distance(r.o, r.p));
}
