...
void main( void ) {
	...
	float time = time + length(uv)*2.*cos(length(uv)*pow(10.,3.*length(mouse)));
	sph1.x = 0.5*cos(time); 
	sph1.z = 0.5*sin(time);
	
	//ray and direction
	vec3 ro = vec3(0.0, 1.0, 3.0);
	vec3 rd = normalize( vec3( (-1.0+2.0*uv)*vec2(1.3,1.0), -1.0) );
	...
	if(id > 0.5 && id < 1.5) {
		...
		col = vec3(0.9, 0.8, 0.6) * dif + vec3(0.1, 0.2, 0.4)*ao;
	} else if( id > 1.5 ){
		vec3 pos = ro + t*rd;
		vec3 nor = nPlane(pos);
		float dif = clamp( dot(nor, light), 0.0, 1.0);
