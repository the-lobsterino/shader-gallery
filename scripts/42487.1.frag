...
float map(vec3 p){
	vec3 _p = p;
	p.y-=0.;p.yz*=rotate(.25);p.xz*=rotate(time*.003);
	p.x+=hash(floor(p.z*.125))*6.;
	p.xz=mod(p.xz,4.)-2.;
	p.xz=foldRotate(p.xz,11.);p.xz*=rotate(1.54);
	vec3 size=vec3(.01,.9,.01)*2.5;
	float d=min(sdBox(p,size),p.y+1.);
	for(int i=0;i<5;i++){
		vec3 q=p;q.x=abs(q.x);q.y-=size.y;q.xy*=rotate(-.4+0.1*cos(time*.3+float(i)*10.+_p.z*32.));
...
void main(void){vec2 uv=(gl_FragCoord.xy-0.5*resolution.xy)/resolution.y;gl_FragColor=vec4(.1,.2,.3,1.)*vec4(trace(vec3(0,6,-20),vec3(uv,.4))/FAR);}
