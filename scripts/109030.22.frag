...
void main( void )
{
	...
	sp+=sp*(distance(sz/2.0-sp.yx,sp)+dot(sp,sp));
	vec3 s = c2s( sp * p );
	float t = length(s)/dot(s,s);
	vec2 m = vec2(t,1.0-t)*s2c(s.yxz/s.zxy) + (mouse * 2.0 - 1.0) * PI;
	
	m = exp(-m);//pow(abs(m),vec2(8.0));
