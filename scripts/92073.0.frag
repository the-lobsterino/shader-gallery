...
const vec2 ch_size  = vec2(1.0, 2.0) * 0.13;              // character size (X,Y)
...
const vec2 ch_start = vec2 (ch_space.x * -5., 1.); // start position
      vec2 ch_pos   = vec2 (1.0, 0.0);             // character position(X,Y)
...
void main( void ) 
{
	...
	float _d =  -800000.0-length(uv);
	uv *= 18.0 ;
	uv.y -= 3.;
	uv *= 1.0+(sin(time+length(uv)*0.1)*0.7);
	//uv *= rotate(time+uv.x*0.3);
	...
	vec3 bg_color = vec3(0.1,_d*0.8*cos(sin(time))*0.7,0.1);

	

	
	uv.y += -2.4+sin(time+uv.x*0.3)*0.5;
	ch_pos = ch_start;
			         nl1  E V E R E T T _ I S _ T H E
					 nl2 B E S T _ A T _ S L O P E
