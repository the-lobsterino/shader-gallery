...
// Update 3: it would appear the flickering is now fixed. Everything is looking smoooth.
...
const vec2 ch_start = vec2 (ch_space.x * -6., 2.); // start position
      ...
      vec3 ch_color = vec3 (0.0, 5.0, 3.);        // character color (R,G,B)
...
void main( void ) 
{
	...
	uv *= 25.0 + sin(time);     //  set zoom size
	uv.x += 11.;
	if (REPEAT_SIGN)
	...
	ch_pos = ch_start + vec2(sin(time),0.0);  // set start position
        ...
        _ _ _ I _ T H I N K _ T H E nl1
	F L I C K E R I N G _ I S _ F I X E D
