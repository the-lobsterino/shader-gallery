...
void main(){	//vec2 p = -surfacePosition.yx*6.; // pan zoom around this, a little broken now, maybe later
	vec2 p=-(gl_FragCoord.yx/resolution.yx-.5)/resolution.xy*min(resolution.x,resolution.y)*11.;
	p.x+=.5;
	float t = -0.0;
	float l=0.,b=0.,m=0.,n=1.,r=0.,q=0.,qq=0.;
	q=-zin(time*3.)/50.;qq=zin(time*(coz(216.)/coz(612.)-1.)*3.)/30.;//(un)comment this line to toggle dancing
	vec4 heart=vec4(0.);
