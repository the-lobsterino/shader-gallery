...
void main( void )
{
	...
	if (iMouse.z < 0.5)
	{
		...
		mou = (mou+1.0)*iResolution.xy;
	}
	vec2 z = ((-iResolution.xy+2.0*gl_FragCoord.xy)/iResolution.y);
	vec2 p = ((-iResolution.xy+2.0+mou)/iResolution.y);
	for( int i = 0; i < 20; i++) 
	{
		float d = dot(z,z);
		z = (vec2( z.x, -z.y ) / d)+p; 
