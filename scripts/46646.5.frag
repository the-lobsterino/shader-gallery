...
const vec3 inkColor = vec3(0.11, 0.11, 0.1);
const vec3 paperColor = vec3(1.0, 0.98, 0.9);
...
void main(void /*out vec4 fragColor, in vec2 fragCoord */)
{
    ...
    vec3 p = vec3(uv, time * speed / 2.0);
    
    //Sample a noise function
    float blot = fbm(p * 7.0 + 4.0);
    float shade = fbm(p * 3.0 + 16.0);
    ...
    gl_FragColor.rgb *= 1.0 - pow(max(length(coord) - 0.3, 0.0), 5.0);
}