...
void main( void )
{
	...
	if (mod(time-1.0, 3.0) < 0.12)
			blink = true;
	
	// Camera...
	float t = clamp(time-2.5, 0.0, 2.9);
	t = t*t*(3.0-2.0*t);
	vec3 origin = mix(vec3(0.0, 1.0, -1.5), vec3(-1.0, 1.0, -5.0), t);
	vec3 target = mix(vec3(0.0, 1.0, 0.0),  vec3( 0.0, -.4, 0.0), t);
