...
vec2 obj0(in vec3 p){
  ...
  float c1=length(max(abs(p)-vec3(35.2,0.2,0.2),0.0));
...
void main(void){
  ...
  vec3 prp=vec3(cos(time*0.2),0.1,sin(time*4.2))*45.0; //Trackball style camera pos
  float vpd=1.5;  
  ...
  if (f<maxd){
    ...
    gl_FragColor=vec4((b*c+pow(b,8.0))*(45.0-f*.01),1.0);//simple phong LightPosition=CameraPosition
  }
