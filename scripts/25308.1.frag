...
//Added a 4x4 Bayer matrix so the dithering works without textures.
...
void InitPalette()
{
	...
	//4x4 Bayer matrix
	bayer4x4[0] = vec4( 1, 9, 3,11);
	bayer4x4[1] = vec4(13, 5,15, 7);
	bayer4x4[2] = vec4( 4,12, 2,10);
	bayer4x4[3] = vec4(16, 8,14, 6);
	bayer4x4 /= 17.0;
}
...
vec3 GetDitheredPalette(float x,vec2 pixel)
{
	...
	#ifdef DITHER
		float dith = GetBayerValue(mod(pixel,vec2(4.0)));
...
void main()
{
	...
	vec2 uv = (floor(gl_FragCoord.xy / DOWN_SCALE) * DOWN_SCALE) / resolution.y;
	
	vec2 mouse = mouse.xy - 0.5;
	
	vec2 camAngle = vec2(0);
	
	#ifdef AUTO_MODE
	camAngle.x = PI * (-1.0 / 8.0) * sin(time * 0.5);
		camAngle.y = -time;
	#else
	...
	vec3 outColor = Shade(scenePosition,Normal(scenePosition), rayOrigin, rayDirection, gl_FragCoord.xy / DOWN_SCALE);
	
	//Palette preview
	if(uv.x < 0.05) 
	{
		outColor = GetDitheredPalette(uv.y, gl_FragCoord.xy / DOWN_SCALE);
	}
