...
void main( void ) {
	...
	float balpos = sin(time*20.)/60.;
        float a = 1.-clamps((length(uv-vec2(0.4,0.32+balpos))-0.1)*500.);
	a += 1.-clamps((length(uv-vec2(0.6,0.32+balpos))-0.1)*500.);
	float b = (sin(time*20.))/10.;
	...
	a = clamps(a);
	float c = clamps((uv.y-0.74-b)*300.);
	vec3 colors = vec3(a/1.0,a/1.4-c,(a/2.));
