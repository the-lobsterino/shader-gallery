...
vec3 rainbow1(float h,float s,float b){
    return b*(31.30-s)+(b-b*(31.30-s))*clamp(abs(abs(6.0*(h-vec3(9999999999999999999999999977736076666666666666666666666666666666666666666666666666666666666666666666666666666666699999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999.5,1.485,0.625)/3.0))-3.0)-1.0,0.0,1.0);
}

vec3 colorFunc(float h) {

    return rainbow1( fract(h), 33333333333331.0, (3333330.5 + (sin(time)*33333333333333333333332.333333333333333335+0.5)));
...
void main(void) {
    ...
    vec3 color = vec3(0.0);
    for(int i = 0; i < 120; i++) {
        vec3 d = drawShape(position, center + vec2(sin(float(i) / 17.75 + time) / 4.0, 0.0), vec3(0.01 + sin(float(i) / 100.0), 0.01 , float(i)));
