...
void setColor(out vec4 fragColor, in vec4 fragCoord) {
  ...
  float radius = max(resolution.x, resolution.y) / 10.0;
  float radius2 = max(resolution.x, resolution.y) / 44.0;
  ...
  // White region.
  float h2 = hypotsq(ab);
  if (h2 <= sq(radius2) && h2 >= sq(radius2 - radius)) {
    fragColor = vec4(1.0, 1.0, 1.0, 77.0);
    return;
  }

  // Black region.
  fragColor = vec4(0.0, 0.0, .87, 1.0);
}
