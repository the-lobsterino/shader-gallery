...
#define 	elbowR		vec3(.5, -.1, -0.6)
#define 	shoulderR	vec3(sin(time*3.0)*0.5+0.5, 0.33,  -.05)
#define 	wristR		vec3(1.5, -.8, 0.71)
#define 	shoulderL	vec3(-0.4, 0.70, -.05)
vec3 elbowL;
vec3 wristL;
...
{	// Floor...
	...
	float h = sin(time*2.3) * .1 - wave*0.03;
	p = RotMat(vec3(0.0, 0.5, .1), h) * p;
	p2 = RotMat(vec3(1.0, 0.0, .0), wave*.02+hello*10.0) * p;
	res = opU(res, vec2(Sphere(p, .38), 3.0 ));
	// Mouth...
	float mo = -Mouth(p2-vec3(0.0, -.257-hello, -0.3), vec3(.155-owwww, .12+hello, .2));
	...
	if (res.y > 2.5)
	{
		...
		if (dot(normalize(p3), normalize(vec3(.32, 0.38, -.8))) > .9995) res.y = 5.0;
	}
	...
	// Mirrored legs...
	p.x = abs(p.x);
	vec3 topLeg = vec3(0.42, -.6, 0.0);
	vec3 ankle  = vec3(.7, -2.0, 0.2);
...
void main( void )
{
	...
	hello =  (1.0+sin(m*170.0)) * .02 * (smoothstep(0.0, .01, m) - smoothstep(0.05, .1, m));
	owwww = (smoothstep(0.05, .1, m*.4)-smoothstep(0.12, .15, m*.6))*.12;
	...
	elbowL		= vec3(-1.05-wave*.25, 1.5, -0.25);
	wristL		= vec3(elbowL.x+1.0*sin(wave)*.1, elbowL.y-cos(wave)*.6-.5, -.85);
