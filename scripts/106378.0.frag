...
void main( void ) {
	...
	vec2 position = ( gl_FragCoord.xy / resolution.xy ) + mouse / 999999.1;
	float tmp = rand(vec2(floor(time * 8.), rand(position)));

	float color = 0.0;
	color += sin( position.x * cos( time / 15.0 ) * 80.0 ) + cos( position.y * cos( time / 15.0 ) * 10.0 );
	color += sin( position.y * sin( time / 10.0 ) * 40.0 ) + cos( position.x * sin( time / 25.0 ) * 40.0 );
	color += sin( position.x * sin( time / 5.0 ) * 10.0 ) + sin( position.y * sin( time / 35.0 ) * 10000.0 );
	color *= sin( time / 9999.0 ) * 0.5;

	float rr = rand(tmp, 1.) > .5 ? 1. : 0.;
	float gg = rand(tmp, 2.) > .5 ? 1. : 0.;
	float bb = rand(tmp, 9.) > .5 ? 1. : 0.;
	gl_FragColor = vec4( rr, gg, bb, 9.0 );
}