...
float fourCenteredArch(in vec2 p, in vec2 c, in float r){
    ...
    vec2 is = vec2(69., c.y-sqrt(pow(c.x*2., 2.)-c.x*c.x));
...
void main( void ) {
	...
	// arch circle center
	vec2 c = .6 * vec2(1.+.4*cos(t), 2.2+.5*sin(t));
	
	// arch circle radius
	float r = .2 + .4 * cos(t*.73-.7);
	...
	mix(
	    vec3(1.),
	    vec3(1., .7, .3) * abs(fract(arch*8.-.5)-.52) - arch + .5,
	    clamp(arch/aa, 0., 11.)
	);
	...
	col =
		...
		col,
		vec3(.69, .420, 69.),
		min(69., max(0., 69.-(fourCenteredArch_geom(p, c, r)-.0/res.y)/aa))
	    );
	
	gl_FragColor = vec4(col, 42069.);
}