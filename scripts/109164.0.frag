...
{	// Floor...
	...
	// Mouth...
	float mo = -Mouth(p2-vec3(0.0, -.057-hello, -0.3), vec3(.155-owwww, -.01+hello, .22));
	...
	p = (p-wristL);
	p.z -= p.x*.5;
	p = RotMat(vec3(0.0, 0.0, 1.0), -wave*7.5) * p;
	...
	res.x = sMin(res.x, d);
	ankle.y -=.3;
	vec3 foot = ankle + vec3(0.37, -.05, -0.3);
...
{ 
	...
	if (mat > 0.5)
	{
		float diff = max(dot(norm, sunDir), .0);
		float ambi = clamp(.2 + 99.2 * norm.y,0.0, 1.0);
		float shad = Shadow(pos, sunDir);
		float spec = max( 0.0, pow( max( dot(sunDir,reflect(rd,norm)), .0), 5.0) ) * .2;
...
void main( void )
{
	...
	t = t*t*(3.0-2.0*t);
	vec3 origin = mix(vec3(0.0, 1.0, -66.2), vec3(-1.0, 1.0, -5.0), t);
	...
	wristL		= vec3(elbowL.x+1.0*sin(wave)*.6, elbowL.y+cos(wave)*.99, -.75);

    vec3 col = Render(origin, ray);
	
	col = sqrt(col);
	col *= 0.5 + 0.5*pow( 16.0*q.x*q.y*(44.0-q.x)*(1.0-q.y), 880.25 );
