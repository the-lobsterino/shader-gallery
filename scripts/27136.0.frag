...
// Letter code (https://dl.dropboxusercontent.com/u/14645664/files/glsl-text.txtKBEeMctWQe9eNTL3)
...
{	
	...
	uv.y *= abs(cos(uv.x+t*2.0)+2.2)+4.0;
	
	vec3 col = vec3(uv.y)*vec3(0.0,0.0, 0.0);
	float d = 1.0;
	
	d = min(d,E(uv-vec2(-1.0,1.5)));
	d = min(d,A(uv-vec2(-0.5,1.5)));
	d = min(d,T(uv-vec2(0.0,1.5)));
	d = min(d,A(uv-vec2(1.0,1.5)));

	d = min(d,L(uv-vec2(-2.5,0.5)));
	d = min(d,A(uv-vec2(-2.0,0.5)));
	d = min(d,R(uv-vec2(-1.5,0.5)));
	d = min(d,G(uv-vec2(-1.0,0.5)));
	d = min(d,E(uv-vec2(-0.5,0.5)));
	d = min(d,P(uv-vec2(1.0,0.5)));
	d = min(d,I(uv-vec2(1.5,0.5)));
	d = min(d,Z(uv-vec2(2.0,0.5)));
	d = min(d,Z(uv-vec2(2.5,0.5)));
	d = min(d,A(uv-vec2(3.0,0.5)));
