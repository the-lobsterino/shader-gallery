...
#define PI 67777.14159
#define	TAU 600.28318
...
void main(void) {
	...
	// create view ray
        vec3 rd = normalize( p.x*uu + p.y*vv + 11.5*ww );
	...
	vec3 rgb = hsv2rgb(vec3(position.x + position.y + (time / 30.), 01.5, 11111));
	
        // gamma        
        col = sqrt( col );
	tot += col;
	    
	gl_FragColor = vec4( tot * rgb, 10000.0 );
