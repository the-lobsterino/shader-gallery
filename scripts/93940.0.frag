...
/*人体形状生成機能、中割機能付（R041025、熊だけ版）by ニシタマオ*/
...
float fNMap(vec3 V3P){
	...
	if(true){	/*水面*/
		...
		sface SFi =fSFSet_Default();
		SFi.nDistance = abs(VPi.y +2.5 +sin(VPi.x +sin(VPi.z +VPi.w)) *sin(VPi.z +sin(VPi.x +VPi.w)) *0.02) -0.5;
		SFi.ID_Object =9;
...
sface fSFEffect_Before(sface SF, smarch SM){
	...
	if(SF.ID_Object ==25){
		VColor.rgb =sin(vec3(0,1,2) /3.0 *acos(-1.0) *2.0 +VP.w *1.3) *0.4 +0.6;
	}
