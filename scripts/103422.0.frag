...
void main( void ) {
	...
	if (length(position-mouse) < 0.11) {
		float rnd1 = mod(fract(sin(dot(position + time * 1., vec2(14.998,08.233))) * 43758.5453), 1.0);
		if (rnd1 > 0.5) {
			gl_FragColor = live;
		} else {
			gl_FragColor = blue;
		...
		if (me.g <= 0.1) {
			if ((sum >= 1.9) && (sum <= 3.1)) {
				...
				gl_FragColor = vec4(0., 0., max(me.b - 0.004, 0.05), 0.);
			} else {
				gl_FragColor = dead;
			}
		} else {
