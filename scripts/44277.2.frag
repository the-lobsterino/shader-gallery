...
////
// Automata Paint Experiment  (Using awesome work by Dave as a base image)

// controls :
// red slider   : brush size 
// green slider : paint blend
// blue slider  : path contouring
// right button : freeze paint outside of brush
// left button  : reset
// these effects are mixed together for fun, so dont fret about exactly what the sliders do
//
// what to do :
// add color pallet and come up with some magic to toggle paint on off?
// replace monalisa with raytracer for real normals to contour?
// extend into actual glslsandbox based paint program?
...
#define TAU 	(8.*atan(1.))   //http://tauday.com/
//#define DEBUG 			//uncomment this to see if the ui is working
...
float       mix_angle(float angle, float target, float weight);
float       angle_to_mouse(void);
vec2        uv_to_screen(vec2 uv);
float       hash(float v);
float       position_hash(vec2 uv);
float       time_position_hash(vec2 uv);
bool	    screen_border(float width);	
...
void main(void)
{
	...
	//set a base color using any function you like
	vec3 base_color		= mona_lisa();
	
	
	//paint with colors
	gl_FragColor		= cell_paint(base_color);
}


vec4 cell_paint(vec3 color) 
{   
	//input formatting and layout
	vec2 aspect	     	= vec2(resolution.x/resolution.y, 1.);
	vec2 position  	 	= uv_to_screen(uv);
	vec2 uv_index    	= floor(   uv * aspect * 24.);				 
	vec2 mouse_index 	= floor(mouse * aspect * 24.);
	
	//state 
	bool memory_area 	= uv.x < 1./resolution.x && uv.y < 1./resolution.y;
	vec4 memory      	= texture2D(renderbuffer, vec2(0., 0.));
	
	float brush_size	= memory.x;
	float blend_weight 	= memory.y;
	float contour_weight	= memory.z;
	
	bool clear       	= mouse_index.x ==  0. && mouse_index.y == 0. || time < 1.;
	bool freeze      	= mouse_index.x == 43. && mouse_index.y == 0. || memory.w == 1.;
	bool magic 		= mouse.y < .9;
	...
	if(clear)
	{
		...
		return memory_area ? vec4(brush_size,blend_weight, contour_weight, 0.) : vec4(color, dot(color,vec3(1.))/3.);
	}
	
	//buttons & sliders
	if(uv_index.y <= 1. && !clear)
	{

		bool brush_position 	= false;
		bool blend_position 	= false;
		bool contour_position	= false;
		bool slider_brush       = false;
		bool slider_blend       = false;
		bool slider_contour     = false;
		bool button_freeze  	= false;
		bool button_reset	= false;
		vec4 ui_background	= vec4(0.);
		
		//slider inputs
		if(mouse_index.y < 4.)
		{
			if(mouse_index.x > 1. && mouse_index.x < 14.)
			{
				brush_size   = mouse.x;
			}
			else if(mouse_index.x > 15. && mouse_index.x < 28.)
			{
				blend_weight = mouse.x;
			}
			else if(mouse_index.x > 29. && mouse_index.x < 42.)
			{
				contour_weight   = mouse.x * 1.01;
			}
		}
		
		button_reset		= uv_index.x == 0.;
		button_freeze  		= uv_index.x == 43.;
		
		slider_brush       	= uv_index.x > 1.  && uv_index.x < 14.;
		slider_blend       	= uv_index.x > 14. && uv_index.x < 28.;
		slider_contour     	= uv_index.x > 28. && uv_index.x < 42.;
		brush_position 		= uv_index.x == floor(brush_size*43.);
		blend_position 		= uv_index.x == floor(blend_weight*43.);
		contour_position	= uv_index.x == floor(contour_weight*43.);
		
		ui_background 		= vec4(slider_brush , slider_blend, slider_contour, 0.) * .125 + vec4(color, 0.) * .5;
		ui_background 		+= vec4(brush_position, blend_position, contour_position, gl_FragColor.w);
		ui_background.w		= 1.;
		float pulse_animation	= abs(cos(time)) * .5 + .25;
		
		if(memory_area)
		{
			return vec4(brush_size, blend_weight, contour_weight, freeze);	
		}   
		else if(button_freeze)
		{
			return freeze ? vec4(.125, .125, .125, freeze)  : vec4(0., pulse_animation, 0., freeze);
		}
		else if(button_reset)
		{
			return freeze ? vec4(0., pulse_animation, 0., freeze) : vec4(color + .5, freeze);
		}
		else
		{
	...
	//scale memory from ui data to function inputs
	float ui_scale  = 3.68;
	brush_size	= clamp((brush_size       - 2./43.) * 3.68, 0., 1.);
	blend_weight	= clamp((blend_weight   - 15.5/43.) * 3.68, 0., 1.);
	contour_weight	= clamp((contour_weight - 29.5/43.) * 3.68, 0., 1.);
	
	
	//create brush
	vec2 mouse_position  	= uv_to_screen(mouse);
	float mouse_angle	= vector_to_angle(-normalize(position-mouse_position));
	float mouse_distance 	= length(position-mouse_position);	

	//some noise for smoother results
	float noise		= time_position_hash(position)-.5;
	
	float brush 		= smoothstep((brush_size - mouse_distance * 3. - noise * pow(brush_size, 6.)), 0., 1.);
	brush			= clamp(1.-brush - pow(mouse_distance,.5), 0., 1.);
	
	//automata velocity (scales sampling kernel)
	float velocity = noise;
	if(freeze)
	{
		velocity    = brush * brush_size - noise;   
	}
	else
	{
		velocity    += .75 + brush_size;
	}

	
	//read the prior pixel to automata struct
	automata cell    	= read(texture2D(renderbuffer, uv));
	automata prior    	= cell;
	magic 			= magic && prior.angle != 0.;
	
	
	vec4 dx = dFdx(texture2D(renderbuffer, gl_FragCoord.xy/resolution.xy));
	vec4 dy = dFdy(texture2D(renderbuffer, gl_FragCoord.xy/resolution.xy));
	vec2 d	= normalize(vec2(length(dx),length(dy)));
    	//loop through neighbors and search for living cells
	vec3 color_sum = vec3(0.);
	for ( float i = 0.; i < 8.; i++ )
	{
        	//create offset uv
		float w			= mod(i, 2.) == 0. ? sqrt(2.) : 1.;
		vec2 uv_n       	= (gl_FragCoord.xy - neighbor_offset(float(i))* w * velocity - d * - .25)/resolution;
		uv_n			= clamp(uv_n, 0., 1. - 1./resolution.y);
		...
		//add color to sum
		color_sum		+= neighbor.paint * .125;
		
        	//if the neighbor is alive, check to see if it aligned to this cell
		if(neighbor.alive)
		{
			//generate sample field at correct frequency for the step function

			vec2 sample_position	= (mod(gl_FragCoord.xy*32.,25.)/25.)/8.;
			...
			float sample_angle 	= min(neighbor.angle, fract( neighbor.angle + .5 ));    
			bool axis		= sample_angle > .125 && sample_angle < .375;
			
			//grab the sample solution from the decision function (the step in the series for all paths at that coordinate)
			float sample		= axis ? sample_position.x : sample_position.y;
			
			//add it to pick between the two possible neighbor cells that the angle might enter
			float alignment		= mod(neighbor.angle + sample, 1.);  
			alignment		= bound(alignment);
			
			bool aligned	        = floor(alignment * 8.) == i;

	    		//collision handling : if a cell has already been accepted as aligned, roll the dice to see if we should swap in this new one 	
			if(cell.alive)
			{
				aligned = aligned && time_position_hash(position) > .125;
			}
	...
	//gather color info
	vec3 color_diff  = (abs(color - cell.paint) + abs(color_sum - cell.paint))*.5;
	float cdiff_sum  = dot(color_diff, vec3(1.));
	float angle_diff = prior.angle != 0. ? abs(prior.angle - cell.angle) : 0.;
		
	//add new cells
	if(!cell.alive && magic)
	{   
		cell 		= add(cell);
		cell.angle 	= fract(cell.angle + .5 * noise * (2.-contour_weight));
	} 
	
	
	//mix prior path with current
	cell.angle = prior.angle != 0. ? mix_angle(cell.angle, prior.angle, cdiff_sum * angle_diff +  contour_weight) : cell.angle;
	cell.angle =  prior.angle != 0. ? mix_angle(cell.angle, prior.angle-angle_diff, cdiff_sum * contour_weight) :  cell.angle;
	float mouse_influence = fract(mouse_angle + 4. * noise * mouse_distance + .25 * brush_size * angle_diff);
	cell.angle 	      = mix_angle(cell.angle, mouse_influence, brush * contour_weight);
	

	//mix colors  
	cell.paint = mix(cell.paint, color_sum, color_diff * blend_weight);							
	cell.paint = mix(cell.paint, color_sum, clamp(.00000001/angle_diff,0.,brush_size) * blend_weight);	
	cell.paint = mix(cell.paint, prior.paint, blend_weight);				
	...
	//clear at screen border
	result   *= screen_border(1.) ? vec4(1., 1., 1., 1.) : vec4(1., 1., 1., 1.);
	...
	result   =  write(cell);
    	...
    	//debug inputs
    	#ifdef DEBUG
	result = vec4(brush + step(abs(uv.x-vec3(brush_size, blend_weight, contour_weight)), vec3(.052)),0.);
    	#endif
	
	result	+= float(dx+dy)*.125;// mix(dx, dy, vec4(.5, .5, .5, .5))*32.;
	return result; 
}//sphinx


//this transforms the pixel to the struct
automata read(vec4 pixel)
{
	automata cell;
	cell.paint = pixel.xyz;
	cell.angle = pixel.w;
	return cell;
}


//writes the struct back to the pixel    
vec4 write(automata cell)
{
	vec4 result;
	result.xyz  = cell.paint;
	result.w    = cell.angle;
	return result;
}
...
{
	vec2 position 	     	= uv_to_screen(uv);
	float noise	 	= time_position_hash(uv);
	bool random_pixel   	= noise > .125;
	float brightness 	= dot(color, vec3(1.))/3.;
	float initial_angle 	= fract(brightness + noise * .0625);
	
	if(random_pixel)
	{
		cell.angle      = initial_angle;
		cell.paint      = color;
		cell.alive      = true;
	}
	else
...
//prevent automata angles from hitting zero
float bound(float angle)
{
...
//converts a 2d vector into an angle represented as a 0-1 value
float vector_to_angle(vec2 v)
{
	return fract(atan(v.x, v.y)/TAU) ;
}

//mixes angles and rotates properly across 0-1 
...
//a random 0-1 value for each pixel
float position_hash(vec2 uv)
{
...
//a random 0-1 value for each pixel at every timestep
float time_position_hash(vec2 uv)
{
	return position_hash(fract(uv-time) * rmat(time * 128.));
}


//center the uv coordinates onto the screen and correct the aspect ratio
vec2 uv_to_screen(vec2 uv)
{
	uv       = uv * 2. - 1.;
...
//returns true if the pixel is within the width of the screen border
bool screen_border(float width)
{
	return     gl_FragCoord.x > width 
	|| gl_FragCoord.y > width
	|| gl_FragCoord.x < resolution.x - width  
	|| gl_FragCoord.y < resolution.y - width;    
}
...
vec2 unpackCoord(float f) 
{
	...
	return coord * vec2(.00195312, 0.00347222);
}
...
void Tri(float pA, float pB, float pC, float pCol1, float pCol2)
{
    ...
    color = mix (color, triCol.rgb, triCol.a); 
}
