...
void main( void ) 
{
	...
	vec3 ch_color = hsv2rgb_smooth(vec3(time*0.4+uv.x*0.1+length(uv*0.05),0.6,1.0));

	//vec3 bg_color = vec3(_d*0.8*sin(time)*3.0, _d*0.0, _d*0.00);
	vec3 bg_color = vec3(0.1,_d*0.8*abs(sin(time+length(uv)*0.4))*0.1,0.1);
	bg_color = vec3(0.,0.,0.);
	
	
	// play DARK FOREST space crypto game https://discord.gg/EnGq2zJk
	
	uv.y += -1.+sin(time+uv.x*0.7)*0.5;
	ch_pos = ch_start;
			  	 nl2  C I A O _ D A R I A
