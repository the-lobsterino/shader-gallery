...
Sphere Sphere_ = Sphere(vec3(0.0,0.0,0.0), 0.5, vec3(.9,.1,.4), true);
Sphere Sphere_2 = Sphere(vec3(0.7, 0.7, 0.7), 0.3, vec3(0.5, 0.5, 1), false);
Sphere light = Sphere(10.0*vec3(-1.0, 1.0, 1.0), 0.1, vec3(0.5, 0.5, 1), false);
float light_intensity = 5000.0;
...
void plane_intersect(vec3 cur, vec3 ray, inout Intersect n)
{
	...
	if(0.001 <= t && t < 1000.0) {
		if(n.t > t || n.t < 0.0) {
			...
			if(mod(pos.x, 2.0) < 1.0 && mod(pos.z, 2.0) < 1.0 || mod(pos.x, 2.0) > 1.0 && mod(pos.z, 2.0) > 1.0) col = vec3(0.2,0.2,0.2);
...
{   
	vec2 pos = (gl_FragCoord.xy * 2.0 - resolution.xy) / min(resolution.x, resolution.y);
    	...
    	vec3 col = vec3(1.0);
    	vec3 cameraPos = vec3(2.-cos(time), 2.-sin(time), 12.0);
	// vec3 cameraPos = vec3(1., 1., 12.0);
	if(cam_move){
    		cameraPos = rotate(cameraPos, -mouse.y*2.0, vec3(1,0,0));
    		cameraPos = rotate(cameraPos, -mouse.x*2.0, vec3(0,1,0));
	}
	...
	if(obj == 0)
	{
		Intersect n = Intersect(-1.0, vec3(0.0), vec3(0.0), vec3(1), false);
		exec_intersects(cur, ray, n);		
		cur += ray * n.t;
		vec3 light_ray = normalize(lightDir(cur));
		Intersect shadow = Intersect(-1.0, vec3(0.0), vec3(0.0), vec3(1), false);
		...
		const int range = 5;
		const int range_half = 2;
		
		
		
		for(int j=0; j<range; j++){
			float diff_x = length(lightDir(cur))*cos(float(j-range_half)*angle/float(range_half));
			float diff_z =length(lightDir(cur))*sin(float(j-range_half)*angle/float(range_half));
			for(int i_=0; i_<range;i_++){
				vec3 light_dir = lightDir(cur);
				
				float diff_y = length(lightDir(cur))*sin(float(i_-range_half)*angle/float(range_half));
				diff_z += length(lightDir(cur))*cos(float(i_-range_half)*angle/float(range_half));
				if(!(j == range_half || i_ == range_half)){										
					light_dir.x += diff_x;	
					light_dir.y += diff_y;	
					light_dir.z += diff_z;	
					Intersect ii = Intersect(-1.0, vec3(0.0), vec3(0.0), vec3(1), false);
					exec_intersects(cur, normalize(light_dir), ii);
					if(ii.t >= 0.000001) {	
						intensity += 0.9;	
						shadow.t = -1.0;
					}
		...
		Intersect m = Intersect(-1.0, vec3(0.0), vec3(0.0), vec3(1), false);
		const int MAX_REF = 2;
		if(n.mirror){
