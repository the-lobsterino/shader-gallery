...
float fn(vec2 s)
{
	...
	return fract(t*dot(s,s));
}

#define tileSize 0.08/fn(surfacePosition)
#define charScale 1.2*fn(surfacePosition)

vec2 norm(vec2 pos) {
	return fract(pos * tileSize) * 2.0 - vec2(1.0, 1.0);
}
float rnd(float t, vec2 i) {
	float mag = fn(i)/length(i);
...
void main( void ) {
	vec2 scaledNormalCoords =  surfacePosition - norm(gl_FragCoord.xy) * charScale;
	vec2 charIndex = (gl_FragCoord.xy * tileSize);;
	
#if 0
	vec3 o = vec3(surfacePosition,fn(gl_FragCoord.xy));
	...
	if (loadIndex < 1.0) {
		...
		gl_FragColor = vec4(0.0, character0(1.0/scaledNormalCoords), 0.0, 1.0);
	}
#endif
}