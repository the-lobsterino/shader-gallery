...
vec3 Text(vec2 uv)
{
	...
	White _M _A _K _E _spc Red _D _O _N _A _L _D _spc Blue _D _R _U _M _P _F _spc Red _A _G _A _I _N _exc _exc _exc

	BEGIN_TEXT ( TX(11.0), res.y*0.5)
	print_pos += vec2(cos(time)*96., 0.6*sin(time)*48.);
	Orange _D _O _N _A _L _D _spc _D _R _U _M _P _F

	print_pos = vec2(mod(time * -50.0, res.x * 2.7) -320., 10.0 );
	Red _D _R _U _M _P _F _spc _W _I _N _S _spc _E _L _E _C _T _I _O _N _scl _spc _W _E _spc _A _L _L _spc _L _O _S _E _exc _exc _exc
