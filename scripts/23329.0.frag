...
Changes:
	...
	Changed background back in black ( http://www.youtube.com/watch?v=pAgnJDJN4VA )
*/
...
vec3 getSkyColor(vec2 cart){
	...
	return mix(vec3(.0, .0, .0), HASHES_COLOR, 3.*(length(cart)+.1)*rand(floor(cart.xy*100.))*acc/3.);
}
...
vec3 getColor(vec2 polar, vec2 cart){
	...
	vec3 skyColor = getSkyColor(cart);
	vec3 groundColor = mix(GROUND_COLOR*cos(cart.x*10.)*cos(cart.y*10.)*(.75+.25*cos(cart.x*100.)*cos(time*3.+cart.y*100.)), hsv2rgb(vec3(time*.1, 1., 1.))*cos(polar.x*400.) , inGround*(1.-inGroundBorder));
	polar.x -=height*TOWERS_HEIGHT;
	
	float block = rand(floor((polar.x)*20.)+heightA) ;
	vec3 towerColor = mix(vec3(.25+.5*sin((floor((polar.x)*20. ))*10.)), vec3(1., .9, 0.)*step( .9, block), step(.2, fract(index))*step(fract(index), .8)*step(.3, fract(polar.x*20.))*step(fract(polar.x*20.), .7));
	towerColor = mix(towerColor,  hsv2rgb(vec3(time*.1, 1., 1.)), inTower*(1.-inTowerBorder));
	inTower*=step( .15, fract(index))*step( fract(index), .85);
	return mix(mix(skyColor, towerColor, inTower), groundColor, inGround);
}
