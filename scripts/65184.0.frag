/*
 * Original shader from: https://www.shadertoy.com/view/WsBfWt
 */
...
#else
vec3 normal( in vec3 pos ) // for function f(p)
{
    const float eps = 0.0001; // or some other value
    const vec2 h = vec2(eps,0.);
    return normalize( vec3(map(pos+h.xyy).x - map(pos-h.xyy).x,
