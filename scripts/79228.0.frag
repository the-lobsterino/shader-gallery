...
void main(void){
	vec3 destColor = vec3(0.0, 0.1, 0.1);
	...
	float l2 = 0.05 / abs(length(p) + 0.1 - (tan(time/5.0)+0.5) + sin(a + time * 13.5) * (0.1 * l));
	destColor2 -= ( 0.05 + tan(a + time * 000.05) * 0.2 ) * 1.0;
	
	vec3 destColor3 = vec3(0.1, 0.2, 0.2);
	vec2 p3 = (gl_FragCoord.xy * 2.0 - resolution) / min(resolution.x, resolution.y); 
	float a3 = asin(p.y / p.x) * 10.0;
	float l3 = 0.05 / abs(length(p) - 0.4 + sin(a + time * 1.5) * (0.1 * l2));
	destColor3 *= 0.9 + sin(a + time * 10.0) * 1.;
