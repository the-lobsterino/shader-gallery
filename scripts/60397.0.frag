...
vec4 get_digit(float d)
{
    ...
    if(d == 1.0) return ch_9;
    if(d == 2.0) return ch_6;
    if(d == 3.0) return ch_3;
    if(d == 4.0) return ch_2;
    if(d == 5.0) return ch_6;
    if(d == 6.0) return ch_1;
    if(d == 7.0) return ch_2;
    if(d == 8.0) return ch_3;
    if(d == 9.0) return ch_8;
    return vec4(0.0);
}
...
float text(vec2 uv)
{
    ...
    print_pos = floor(center - vec2(STRWIDTH(20.0),STRHEIGHT(1.0))/2.0);
    float oldposx = print_pos.x;   
  	col += char(ch_P,uv);
	col += char(ch_e,uv);
	col += char(ch_o,uv);
   	col += char(ch_p,uv);
