...
float scene_2D( vec2 p )
{
    ...
    //p *= rot(time);
vec2 m = (1.875-mouse)*3.;
	m -= 5.0;
float zoom = pow( 2.0, 16.* ( 1. - m.x ) );
	p /= zoom;
    ...
    for ( int i = 0; i < 128; i++ )
    {
        float im = 2.*floor( ( mouse.x )*16.);
	float ii = float(i) + im;
	binary = cos( float( ii )*pi );
	size = vec2( 1.0, 1.0 )*pow( 2.0, float( -ii ) );
	col += ( 1./2. )*rectangle( (p), size )/16.; 
