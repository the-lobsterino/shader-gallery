...
float fbm(vec2 P, int octaveCount, float lacunarity, float gain)
{
    float sum = 0.0;
    float amp = 1.0;
    vec2 pp = P;
...
void main() {
    ...
    vec2 qq;
    vec2 r;
    float color = pattern2(p,qq,r,time * 0.05);
    ...
    vec4 c2 = c;
    c2 = c * -1.0;
    c = smoothstep(0.6, 0.9, c);
    c = c * 0.5 + c2 * 0.5;
    
    float alpha = c.z;
    vec4 bgcol = vec4(0.0, 0.0, 0.0, 1.0);
    c = vec4(c.x * 0.3, c.y * 0.3, c.z * 1.0, 1.0);
    c = c * alpha + bgcol * (1.0 - alpha);
