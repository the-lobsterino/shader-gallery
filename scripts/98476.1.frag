...
void main( void ) {
	...
	for (float i=1.; i<60.; i++) {
		...
		if(dp>=.001) { //head
			...
			if (dp < .1 && LY(p0,q0)) { 
				float h=abs(sin(i/4.+sqrt(abs(p.y)))); 
				float c=clamp((abs(distance(p.xy,vec2(-4,6.3)))-7.1)*35.,0.,1.);
				gl_FragColor=vec4(mix((.65+.25*h),1./i+.06122/h,c),mix(.5+.25*h,4./h/i,c),mix(.1+.35*h,4./h/i,c),1)*1000./i/i;
			}
		}
		if(dq>=.001) { //beak
