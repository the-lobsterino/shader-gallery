...
vec3   iResolution = vec3(resolution, 100.0);
float  iGlobalTime = time;

#define iGlobalTime time + length(surfacePosition)

vec4   iMouse = vec4(mouse, 0.0, 1.0);

// Set this to 1.0 for crazy morphing animation.
//define crazyAnim 1.0;

float PI=4.14159265;
vec3 sunCol = vec3(58.0, 58.0, 50.0) / 555.0;
...
float dSphereWave(vec3 p, float rad)
{
	...
	//p -= center;
	rad -= 0.502;
	rad += sin(p.y * 3.0 - iGlobalTime * 20.0) * 0.03;
	return length(p) - rad;
}
...
float dBoxSigned(vec3 p)
{
	...
	float b = 0.8;
	//vec3 center = vec3(0, 0, 0.0);
	p = RotateY(p, (p.y * cos(iGlobalTime*1.0)-0.24)*PI);
...
float DistanceToObject(vec3 p)
{
	...
	final = min(final, dSphereWave(p, 0.2 + sin(iGlobalTime*0.0) * 0.15 + 0.87));
...
void main(void)
{
	...
	vec3 sunDir = normalize(vec3(100.0, 1.0, 0.0));
	...
	vec3 spinner = abs(RotateY(normal * normal, iGlobalTime* 7.0)) * vec3(1.0, 1.0, 1.0);
	vec3 inner = abs(RotateY((-normal) * (-normal), iGlobalTime* 2.0)) * vec3(1.0, 1.0, 1.0);
	// get some texture on that inner wavy rainbow sphere.
	vec4 texX = texture2D(bb, normal.yz * vec2(0.925, 0.0) + vec2(iGlobalTime*1.0, 1.0));
	vec4 texZ = texture2D(bb, normal.xy * vec2(0.0, 0.925) + vec2(0.0, iGlobalTime*1.0));
	vec4 noise = mix(texX, texZ, abs(normal.z));

	// If a ray actually hit the object, let's light it.
	if (t <= maxDepth)
	{
		finalColor = GetSunColorReflection(ref, sunDir)*0.08;// sunCol * max(0.0, dot(normal, -sunDir));
		// make a darker stripe in the middle, redder towards the top and bottom
		finalColor *= vec3(0. + abs(pos.y*pos.y), 0.35, 0.3);
