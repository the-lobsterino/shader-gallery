...
void main()
{
	...
	vec2 uv = g / s;
	vec4 h = texture2D(buf, uv);
	g = -(gl_FragCoord.xy * 2.-s)/s.y*2.5;
	g.y += 1.;
	float altfield = section(g);
	float t = halfpi * 0.5;
	g = vec2(atan(g.x,g.y), pow(length(g), 1.5));
	...
	vec4 stream = texture2D_bicubic(buf,(uv-vec2(0.5))*zoom+vec2(0.5));
	gl_FragColor = color * vec4(length(a),length(b),length(c),1) * 0.001
			+ (h * 0.97 + step(h, vec4(2.,.0,.0,1)) * 0.01) * field
			+ stream * (1.0 - field);
	gl_FragColor.a = 1.;
}