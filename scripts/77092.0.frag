...
void main( void ) 
{
	...
	uv *= 29.0 + sin(time);     //  set zoom size
	if (REPEAT_SIGN)
	  uv = -100.0 + mod(.8*(uv-1.0),ch_space*vec2(16.,6.5));     //  set zoom size

	ch_pos = ch_start + vec2(sin(time),0.0);  // set start position

        _  S H A D E R T O Y  nl1 
         S T O P _ S T E A L I N G  nl2
	O U R _ S H A D E R S nl3
	T H A N K _ Y O U _ V E R Y _ M U C H
		
	vec3 color = mix(ch_color*(1.0+(sin(uv.x*0.25+2.2*time)*0.45)), bg_color, 1.0- (0.08 / d));  // shading
