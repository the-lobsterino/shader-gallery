...
float getCheckerboardColor( vec2 position ) {
	float  xpos = floor( 10.0 * position.x );
	float  ypos = floor( 5.0 * position.y );
	float col = mod( xpos, 2.0 );
	if( mod( ypos, 2.0 ) > 0.0 ) {
		col = cos(xpos*ypos + time * 5.);
	} else {
		col = sin(xpos*ypos + time * 5.);
...
#elif defined THREE_D
	vec2 position = 20.0 * gl_FragCoord.xy / resolution.xy - 1.0;
	position.x *= resolution.x / resolution.y; // Correct aspect ratio.
