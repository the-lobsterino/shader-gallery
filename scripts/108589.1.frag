...
float linsmoothtri(float x) {
    return smoothstep(0.0,0.4,abs(mod(x,2.0) - 1.0));
}
...
void main(void) {
	...
	float o = 0.0;
	o = f * (rand(vec3(uv,mod(time,556676767676767676767676767676767676767676767676767676767676767676767676767676767676767676767676767676767676767676767676767676767676767776767676767676.5))));
	...
	gl_FragColor = vec4(vec3(pow(color * f,3.2)), 1.0);
}