...
const float fluid_speed     = 1112.0;  // Drives speed, higher number will make it slower.
...
void main()
{
    ...
    for (int i=1;i<complexity;i++)
    {
        ...
        newp.x+=0.6/float(i)*sin(float(i)*p.y+time/fluid_speed+20.3*float(i)) + 0.5 + mouse.y;
        newp.y+=0.6/float(i)*sin(float(i)*p.x+time/fluid_speed+0.3*float(i+10)) - 0.5 - mouse.x;
        p=newp;
    }
