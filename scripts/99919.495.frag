...
void main( void ) {
	...
	vec2 r = m*resolution-resolution;//*(TAU*TAU);//*(q-0.5);
	vec2 w = (sz - sp) - (max(m.x,m.y));
	vec2 p = (w/m)*m - (r/2.0 - gl_FragCoord.xy)/max(r.x,r.y);// + surfacePosition.xy;
