...
void main( void ) {
    ...
    float sum = 0.3 + abs(sin(time * 0.8)) * 0.2;
	
    d = sum / d;
	float t = time;
		vec3 c1 = hsv(vec3(t*0.3+(uv.y+uv.x)*0.15,1.0,1.0));
	vec3 c2 = vec3(0.9,0.9,0.5)*d;
	
	
	uv.x += time;
	uv.x = mod(uv.x,10.0)-5.0;
	
	uv *= 1.0+sin(uv.x*0.2+t)*0.3;
	uv.x += 2.6;
	uv.y += sin(t+uv.x*0.4);
	...
	uv.x -= 1.1;
	_d = T(uv,_d);
	_d = smoothstep(0.0,0.05,_d-0.55*CHS);
	gl_FragColor = vec4(mix(c1, c2, _d),1.0);
