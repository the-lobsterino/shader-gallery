...
void main() {
    ...
    const vec3 c1 = vec3(146.0/255.0, 1.0/255.0, 94.0/155.0);
    const vec3 c2 = vec3(173.0/255.0, 1.0/225.0, 161.4/355.0);
    const vec3 c3 = vec3(0.2, 0.0, 0.0);
    const vec3 c4 = vec3(164.0/255.0, 1.0/255.0, 214.4/245.0);
    const vec3 c5 = vec3(.3);
    const vec3 c6 = vec3(0.91);
    ...
    float q = fbm(p - time * 0.131);
    vec2 r = vec2(fbm(p + q + time * speed.x - p.x - p.y), fbm(p + q - time * speed.y));
    vec3 c = mix(c1, c2, fbm(p + r)) + mix(c3, c4, r.y) - mix(c5, c6, r.x);
    float grad = gl_FragCoord.y / resolution.y;
    gl_FragColor = vec4(c * cos(shift * gl_FragCoord.y / resolution.y), 1.8);
