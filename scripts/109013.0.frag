...
const vec3 LINENCOLOR = vec3(200.,200.,0.)/256.;
...
vec3 star(vec2 v){
    ...
    color = redmask*vec3(1.,1.,1.)+(1.-redmask)*color;
    color = blackmask*vec3(0.5)+(1.-blackmask)*color;
    return color;
}
...
void main() {
    ...
    vec3 color = clothPatternPhongLighting(colorPattern(distort(st-vec2(0.3,0.0), 100., 0.002)), distort(st,1000.,0.002), 30.);
