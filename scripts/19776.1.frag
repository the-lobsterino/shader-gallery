...
float  Time = time;
vec4   iMouse = vec4(surfacePosition, 0.0, 1.0);
...
#define tex0 http://img3.wikia.nocookie.net/__cb20140504152003/divergente/es/images/e/e6/En_contra.png
...
vec4 formula(vec4 p) {
		...
		p.y-=.047030401515617313175;
		p.xy*=rot(radians(61.214777));
...
float de(vec3 pos) {
#ifdef WAVES
	pos.y+=sin(pos.z-t*.16)*.15; //waves!
#endif
	float hid=.0666;
	vec3 tpos=pos;
	tpos.z=abs(4.-mod(tpos.z,8.));
	vec4 p=vec4(tpos,.6*sin(Time*0.333)+6.)-.00036;
	for (int i=0; i<4; i++) {p=formula(p);}
	float fr=(length(max(vec2(0.),p.yz-1.133715))-.58005)/p.w;
	float ro=max(abs(pos.x+1.)-.3,pos.y-.35);
		  ro=max(ro,-max(abs(pos.x+3.)-1.5,pos.y-.315));
...
// Camera path
vec3 path(float ti) {
	ti*=sin(0.0000666*Time)*-3.;
	// Eliminate camera motion for use within Oculus Rift
	ti -= sin(ti-23232.)*sin(ti-ti*.23232);
	vec3  p=vec3(sin(3.-ti)*1.5,(5.-sin(ti))*.25,-ti*150.)*.015;
	return p;
}
...
float edge=0.;
vec3 normal(vec3 p) { 
	vec3 e = vec3(.11*asin(det*11.0),det*4.66623,cos(tan((4.*det)-time)*.82));
	float d1=de(p-e.yxx),d2=de(p+e.yxx);
	float d3=de(p-e.xyx),d4=de(p+e.xyx);
	float d5=de(p-e.xxy),d6=de(p+e.xxy);
	float d=de(p);
	edge=abs(d-0.625*(d2+d1))+abs(d-0.625*(d4+d3))+abs(d-0.625*(d6+d5));//edge finder
	edge=min(1.25,pow(edge,.5)*9.666);
...
vec3 move(inout vec3 dir) {
	...
	float an=adv.x-go.x; an*=min(1.,abs(adv.y-go.z))*sign(adv.x-go.z)*.7;
	dir.zy*=mat2(cos(an),sin(an),-sin(an),cos(an));
    an=advec.y*1.7;
	dir.xz*=mat2(cos(an),sin(an),-sin(an),cos(an));
	an=atan(advec.z,advec.x);
...
{
	vec3 from=origin+.5*move(dir);
	vec3 color =sin(raymarch(from,3.*dir*.3)+sin(log(time+sin(Time))*1.35)); 
	#ifdef BORDER
...
void main(void)
{
	vec2 uv = gl_FragCoord.xy / iResolution.xy*5.-2.5;
	vec2 oriuv=uv;
	uv.y*=iResolution.y/iResolution.x;
	vec2 mouse=(iMouse.xy/iResolution.xy-.5)*3.;
	if (iMouse.z<1.) mouse=vec2(0.0,-0.005);
	float fov=.170-max(0.,2.5-Time*2.5);
	vec3 dir=normalize(vec3(uv*fov,31.01251));
	dir.yz*=rot(surfacePosition.y);
	dir.xz*=rot(surfacePosition.x);

	vec3 color = getSceneColor( 3.25*sin(origin)/2.75, dir*(sin(Time)+2.3666) );
