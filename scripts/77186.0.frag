...
/*========== 16 segment display ==============    bin. hex <- u now no y hex is kool cuz its liek letters 4 binary ;))))
                                                  0000 0
...
#define hitlerdidnothingwrong ddigit(0x55AA);
...
void main( void ) 
{
  ...
  ch_color = vec3 (2.6 - 4.*mouse.x, 1.2-mouse.x*mouse.y, 0.5+mouse.y);
  	...
  	nl
  	P R O U D _ B O Y S nl
	D I D _ N O T H I N G _ W R O N G nl
