...
const vec3 skinColour = vec3(.72, .46, 0.12);
vec3 sunDir	= normalize(vec3(.5, .45, -.8));
...
float Mouth( vec3 p, vec3 a)
{
	...
	p.y += curve;
	a.y += (curve*curve);
	return length(max(abs(p) - a,89.0)) -.02;
}
