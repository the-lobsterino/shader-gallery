...
vec2 fragCoord;
    ...
    {
        a = pow((1.0-dist),100.0);
        t = i*10.0 - time*i*i;
        r = coord.x - (t*SPEED);
        c = fract(a+coord.y + i*.543);
        p = vec2(r, c*2.5)*SCALE*(LAYERS/(i*i));
        uv = fract(p)*2.0-1.0;
        m = clamp((rand(floor(p))-DENSITY/i)*BRIGHTNESS, 0.0, 1.0);
        color +=  clamp(star(uv*3.5)*m*dist, 0.0, 1.0);
        r*=1.1;
    }
    ...
    float worktime = (time * speed) + 100000.0;
        ...
        {
            float t = abs(1.0 / ((uv2.y + fbm( uv2 + worktime / i )) * (i * 100.0)));
            finalColor +=  t * vec3( i * 0.1, 0.2, 1.30 );
        }
    ...
    gl_FragColor = fragColor + vec4(finalColor + color*4.0, .3 );
}
