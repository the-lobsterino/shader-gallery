...
float map(vec3 p){
    ...
    return rbox(p, vec3(0.33, 0.90, 9.90), .0);
}
...
void main( void ){
	...
	#define c col
	#define fogCol vec3(0., 0.00, 0.0)
		float fog = 0.0028;
		fog *= 1.; // fog strength

    //col /= float(nH)*.133;
    //col *= smoothstep(.5, .3, ns * .01);
    col = 1.-pow(0.18/(1.16-exp(-col)), vec3( 2., 1.3, .3 ));
       	        c = mix(c, fogCol, 1.0 - (1.0) / (1.0 + t*t*fog));
	#undef fogCol	
	#define fogCol vec3(0., 0.00, 1.0)
		fog = 0.00088;
		fog *= 1.; // fog strength
        float mm = 0.5 + (1.-mouse.y)*.5; 
	c = mix(c, fogCol, 1.0 - (1.0) / (mm + t*t*fog));
	gl_FragColor = vec4(col*(col + .2), 1.0);
}///ändrom3da4