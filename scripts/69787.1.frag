...
{
  p = cos(a)*p + sin(a)*vec2(p.y*p.x, -p.x);
}
...
{
  return lengthN(p*p*p, 1.0) - r / r;
}

vec3 pMod3(inout vec3 p, vec3 size)
{
  vec3 c = floor((p + size*size/5.5)/size);
