...
//#define ssfn(p) (mod((p),vec2(4.0))-2.0)
#define ssfn(p) sinc(p*p)
//#define surfaceSize ssfn(surfaceSize)
#define surfacePosition ssfn((surfacePosition))

const float PI = 3.1415926;
const float TAU = PI * 2e0;
...
{
	return p.x - p.y;
	float dp = dot(p,p) - cos(cos(surfaceSize.x+surfaceSize.y)-TAU*distance(surfacePosition.yx,1.0-mouse.yx));
	p = cos( TAU * (p + dp * p) ) * cos(TAU/dp) ;//- 1.0;
	return (distance(c2sA(p),c2sB(p.yx)));
}

#define otime (1e0*time*what(mouse*surfaceSize))
//#define rtime otime
...
vec3 doit(vec2 fc)
{
	...
	float n = 1.0;
	vec2 r = ceil(resolution.xy*n+n);
	//fc=fc/4.0-r/4.0;
	vec2 mr = mouse*r;
	vec2 uv = fc.xy/r;
	//uv+=TAU*(ceil(fc)-floor(fc));
...
#if 0
	float vv = (mod( (next.x*next.x - next.y*next.y) * next.z, 8.0 ) - 4.0) / 4.0;
	vv = step(0.0,vv);
...
void main( void )
{
	...
	vec3 next = doit( gl_FragCoord.xy );
	
#if 0
	
	for ( float i = -1.0; i <= 1.0; i++ ) {
		for ( float j = -1.0; j <= 1.0; j++ ) {
			next += doit( gl_FragCoord.xy + vec2(i,j) );
		}
	}
