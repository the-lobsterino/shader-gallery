...
float xy_random() {
	return fract(sin(gl_FragCoord.xy.x*gl_FragCoord.xy.y*time/16.0)*999.0);		
}
...
void main(void)
{
	...
	float xyrandom = xy_random();
	float r = fract(xyrandom)* position.x + position.x * 0.6;
	float g = fract(xyrandom) * position.x + position.y * 0.2;
	float b = fract(xyrandom)*position.x + position.y * 0.4;
	
	vec3 col = vec3(pow(cos(v), cexp.x/r*time), pow(sin(v), cexp.y), pow(tan(v), cexp.z/b)/b*sin(time)) * 2.0;
