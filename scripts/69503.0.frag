...
float sdRackWheel2( in vec3 pos)
{
  return opS(sdTorus82(  pos-vec3(-2.0, 0.2, 0.0), vec2(0.20, 0.1)), 
             sdCylinder( opRep( vec3(atan(pos.x+2.0, pos.z)/6.2831 - 0.1*time, 
                                     pos.y, 
...
vec2 map( in vec3 pos )
{
  ...
  res = opU( res, vec2( sdRackWheel2   ( pos-vec3(0.2,0.0,0.5) ), 51.0 ) );	
  res = opU( res, vec2( sdRackWheel   (  pos-vec3(0.40,0.0,1.0) ), 51.0 ) );
  res = opU( res, vec2( sdRackWheel2   ( pos-vec3(0.60,0.0,1.5) ), 51.0 ) );	
  return res;
}
...
void main( void )
{
  ...
  // camera  
  float angle = ROTATE ? 0.0*time : 0.0;
  float rx = -0.5 + 3.2*cos(angle + 8.0*mo.x);
  float rz =  0.5 + 3.2*sin(angle + 8.0*mo.x);
