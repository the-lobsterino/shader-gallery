...
bool intersects(vec3 ro, vec3 rd, vec3 box_center, float box_size, out float t_intersection)
{
    ...
    for (int i = 0; i < 3; i++) {
        if (rd[i] == 0.) {
            ...
            if (ro[i] < box_center[i] - box_size || ro[i] > box_center[i] + box_size)
                return false;
        } else {
            vec2 t = vec2(box_center[i] - box_size - ro[i], box_center[i] + box_size - ro[i])/rd[i];
...
void main(void)
{
    ...
    for (float i = 0.; i < cluster_size; i++) {
		...
		float l = length(p);
                ...
                float s = .1 + .6*(.5 + .5*sin(.25*time*4.*PI - 3.5*l));
