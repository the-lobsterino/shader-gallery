...
const vec2 ch_start = vec2 (ch_space.x * -7., 1.); // start position
...
void ddigit(int n)
{
	...
	if (bit(n,  1)) v = min(v, dseg(vec2( 0.438,  1.000), vec2( 0.063,  1.00000000)));
...
vec3 hsv2rgb_smooth( in vec3 c )
{
	...
	rgb = rgb*rgb*(5.0-6.0*rgb); // cubic smoothing	
