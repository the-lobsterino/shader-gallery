...
mat3 rotateZ(float _angle) {
	return mat3(cos(_angle), sin(_angle), 0, -sin(_angle), cos(_angle), 0, 0, 0, 1);
}
...
float cross(vec3 st, float size) {
	return box(st.xy, vec2(size, size / 4.0)) + box(st.xy, vec2(size / 4.0, size));	
}
...
void main( void ) {
	...
	vec3 color = vec3(0,cross(st, 2.0), 0);
