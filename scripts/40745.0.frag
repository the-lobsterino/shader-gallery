...
#define DEBUG_SPACE
#ifdef DEBUG_SPACE
vec2	g_debug = vec2(0.,0.);
vec4	g_plot 	= vec4(0.,0.,0.,0.);
vec2	g_uv 	= vec2(0.,0.);
#endif
...
float	hash(vec2 uv);
float 	graph(vec2 uv, in float set[ELEMENTS], in float probability[SYMBOLS]);
...
void main( void ) 
{
	...
	float g 			= graph(p, set, probability);
...
float graph(vec2 uv, in float set[ELEMENTS], in float probability[SYMBOLS])
{
	...
	float s 	= 96.;
//	scale		*= scale * floor(uv.y*float(ELEMENTS)+1.);
	float r		= s*g_uv.y*(2.-g_debug.y)*.5+.5;
//	scale		= 
	for(int i = 0; i < ELEMENTS; i++)
	{
		...
		if(float(i) == index)
		{
			...
			plot 		+= line(g_debug, vec2(.5, 0.), vec2(0.5, .5), r)*2.;
