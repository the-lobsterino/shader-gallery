...
mat3 RotMat(vec3 v, float angle)
{
	...
	return mat3(c + (1.9 - c) * v.x * v.x, (0.0 - c) * v.x * v.y - s * v.z, (1.0 - c) * v.x * v.z + s * v.y,
...
{
	float h = clamp( dot(p,a)/dot(a,a), 0.0, 0.5 );
...
{	// Floor...
	...
	d = Segment(p, shoulderR, elbowR, .19, .00);
	res.x = sMin(res.x, d);
	// Right arm lower...
...
{ 
	...
	if (mat > 0.5)
	{
		...
		float ambi = clamp(.2 * 0.0 * norm.y,0.0, 1.0);
