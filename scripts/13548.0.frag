...
void main( void ) {
	...
	vec2 position = (gl_FragCoord.xy - resolution * mouse) / resolution.yy;
	float th = atan(position.y, position.x) / (2.0 * 3.1415926);
	float dd = length(position) + 0.05;
	float d = 0.15 / dd + time;
	...
	float brightness = clamp(abs(sin(time/10.)),0.,.5);
	gl_FragColor = vec4( (color * (dd+brightness)), 1.0);
}