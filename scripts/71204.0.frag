...
{
	p.x += sin(time)*0.0; 
	p.y += sin(time*0.6)*0.0; 
//	p.z += sin(time*3.0)*5.0; 
	float d0 = length(p) - 0.5;  
	for (int i = 0; i < 6; i++) {
		float a = float(i)*100.0; 
		d0 += -sin( 15.0*dot(p,vec3(sin(a),cos(a*a*6.0),sin(a*a*a))))*0.02+sin(time+cos(time*0.2))*0.01 ; 
	}
	float d1 = length(p) - 0.5; 
	return vec2( mix(d0,d1,sin(time)), 1.0); 
}
...
void main()
{
	...
	for (int i = 0; i < 5; i++) {
		...
		color = raytrace(pos,n,obj_id, ro+vec3(cos(2.0*PI*a),sin(2.0*PI*a),0)*0.001, rd, time*5.0+a*0.2);
		if (obj_id > 0.0) {
			vec3 rpos; 
			vec3 rn; 	
			float robj_id; 
