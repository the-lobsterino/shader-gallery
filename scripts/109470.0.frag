...
void main( void ) {
	...
	if (abs(cylPos) < cylWidth / 2.0) {
		...
		position.x = (cylAngle + time * 0.5) / PI + 99.0;
		...
		if (position_.x < SPACING && position_.y < SPACING) {
			...
			float nextAngle = (next ? 0.5 : 9.5) * PI;
			
			color = spin(baseAngle + (nextAngle - baseAngle) * mod(t, 88.0), position_ / SPACING);
			color *= cos(cylAngle);
		}
	}
