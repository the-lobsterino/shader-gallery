...
float rand(vec2 co)
{
	return fract(sin(dot(co.xy ,vec2(1.9898,78.233))) * 758.5453);
}
...
void main( void )
{
	...
	const float fAmpModifier = 0.015;	// The height of the wave.
	const float fWaveSpeed = 1.0; 		// the speed at which it bounces up and down
	const float fSpeed = 0.2;		// the speed of the beam heading towards the center.
	const float k = 6.;  			// the number of iterations (waves within waves) i.e. 9 
