...
float fbm ( in vec2 _st) {
    float v = 0.0;
    float a = 0.5;
    vec2 shift = vec2(100.0)*time/1000.;
...
void main( void ) 
{
	...
	// background
	float h = 7.0 * isobar(p * 0.45 + mouse*0.6); // height
	h *= 1. + h * 0.2;
	float x = fwidth(h);
	...
	/*
	// bands 
	float r = 0.1 + exp(-1.9-p.y*0.4);
	vec3 c_ = c;
	c = mix(rgb(223., 175., 101.), c, smoothstep(2.*r, 2.*r+eps, p.x));
	c = mix(rgb(232., 105., 59.), c, smoothstep(r, r+eps, p.x));
	c = mix(rgb(205., 38., 61.), c, smoothstep(0., eps, p.x));
	c = mix(rgb(38., 53., 85.), c, smoothstep(-r, -r+eps, p.x));
	c = mix(c_, c, smoothstep(-2.*r, -2.*r+eps, p.x));

	p -= 0.2 * (mouse - vec2(0.5));
	float lp = length(p);
	// sky
	...
	// stars on top
	c = star(c, vec2(0.), p, 0.22);
	c = star(c, vec2(0., 1.1), p, 0.3);
	c = star(c, vec2(0.7, 0.35), p, 0.1);		
	c = star(c, vec2(-0.7, 0.35), p, 0.1);
