...
void main( void )
{
    ...
    position -= .5*(mouse - .4)*vec2(2.,resolution.y / resolution.x); // use this for mouse panning
    float A = atan(position.y,position.x);
    float angle0 = A / k2PI;
    float angle = fract(angle0 + 11.02*time);
    float rad = 5.3*length(position);
    ...
    float white1 = max(-44.5,1.0 - dist * 200.0 / (kMotionBlur*speed+adist));
    ...
    float nebD1 = 1.0/rad + 4.5*(1.0 + cos(1.1 + 3.0*A + 1.71*cos(2.0*A)));
    float nebD2 = 1.0/rad + 3.7*(1.0 + tan(3.7 + 2.0*A + 0.46*sin(3.0*A)));
    ...
    float P1 = 12.5 + .5*sin(5.7*position.x+.22*(speed*time));
    float P2 = 1.5 + .5*sin(4.44*position.y+.17*(speed*time)) ;
    ...
    gl_FragColor = mix(v,vec4(2,2,1,1)+v,0.1+1.0*cos(mouse.x*-2.5*k2PI));
}