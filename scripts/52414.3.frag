...
#define MAX_DIST 220.
#define SURF_DIST 1.
#define OCEANS_LEVEL .75
#define PLANETCENTER vec3(-4, -3, 25)
#define PLANETRADIUS 6.66
#define MOONCENTER vec3(54, -50, 145)
#define MOONRADIUS 25.0
#define ATMOSPHERETHICKNESS 0.1
#define ATMOSPHEREDENSITY 4.0
#define LIGHTPOS vec3(20*3, 15*3, 10)*0.5
...
void main(void)
{
	...
	if(distance(resolution*vec2(.8,.6), gl_FragCoord.xy)<=1.5){gl_FragColor += vec4(cos(time*10.)*cos(time*10.), 0., 0., 0.)/2.;}
	if(distance(resolution*vec2(.72,.7), gl_FragCoord.xy)<=1.5){gl_FragColor += vec4(cos(time*10.-.2)*cos(time*10.-.2), 0., 0., 0.)/7.;}
	if(distance(resolution*vec2(.44,.7), gl_FragCoord.xy)<=1.5){gl_FragColor += vec4(cos(time*10.+2.)*cos(time*10.+2.), 0., 0., 0.)*2.;}
