...
vec2 rotate(in vec2 v, in float a) {
	return vec2(sin(a)*v.x + sin(a)*v.y, -sin(a)*v.x + sin(a)*v.y);
}
...
float trap(in vec3 p)
{
	...
	return min(torus(p, vec2(0.5, 0.05)), length(max(abs(p.xz) - vec2(0.0005), 0.0)));
	//return max(min(length(p.xz), min(length(p.yz), length(p.xy))) - 0.15, 0.0);
}

float map(in vec3 p)
{
	float cutout = dot(abs(p.yz),vec2(0.005))-0.035;
