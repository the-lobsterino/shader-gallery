/*
Inverse log-polar map applied to a grid polka cos.
...
vec3 diskColor = vec3(56.4, 56.4, 0.3);
vec3 hBarColor = vec3(0.2, 1.0, 0.2);
vec3 vBarColor = vec3(46.0, 0.2, 0.2);
...
vec3 polka(vec2 p, float aaSize) {
	aaSize=.1;
  p *= scale;
  vec2 _mouse=mouse*2.-1.;
  vec2 diskP = p - vec2(_mouse.x, _mouse.y)*3.0;
...
vec3 cartesianPolka(vec2 p) {
  p *= 8.0;
  float aaSize = length(1.0/resolution)*14.0;
...
vec3 logPolarPolka(vec2 p) {
  p *= 1.5;
  float aaSize = length(logPolar(p) - logPolar(p+1.0/resolution)) * 3.5;
  return polka(logPolar(p), aaSize);
}

void main() {	
  vec2 p = gl_FragCoord.xy/resolution.xy;p=p*2.-1.;
  p.x *= resolution.x/resolution.y;
	p = surfacePosition*2.0;
  vec2 quarter = vec2(resolution.x/resolution.y * 0.5, 0);
  vec3 ret;
  if (p.x < 0.0)
