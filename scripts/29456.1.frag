...
void main() {
	//vec2 mouse_distance = mouse - (gl_FragCoord.xy/ resolution.xy);
	vec2 p = surfacePosition*1.0;
	float c = 0.;
	float b = 0.;
	float a = atan((mouse.x -0.5)*ratio -p.x, mouse.y -0.5 - p.y);
	float r = length(p);
	c = explosion(a*(10.)*1.5+time*1.);
	b = explosion(a*(5.)*10.5+time*10.5);
	
	vec2 pp = vec2((mouse.x-0.5)*ratio, mouse.y -0.5); //pp.x *= ratio;
	float dist = distance( pp, surfacePosition);
	float heat = (.1/ dist);

        float tmp = pow(heat,3.0*sin2(time)+3.0)*b*c+heat;
	tmp = min(max(tmp, 0.0), 1.0);
	vec3 cc = vec3(tmp);
	gl_FragColor = vec4(cc,1)+vec4(0.2,0.4,0.6,0.0);
}