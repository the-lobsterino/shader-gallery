...
const int CAMERA_MODE = TEST_CAMERA; 
...
vec2 obj_roundBox(in vec3 p){
	return vec2(length(max(abs(p)-vec3(11,1,1),0.0))-0.25,1);
}
...
void main(void){
	...
	E=vec3(-sin(time/10.0)*10.0,5,cos(time/10.0)*10.0); //spinning scene
		...
		E=vec3(-sin(time/10000.0)*10.0,7,cos(time/100000.0)*10.0);
...
vec4 applyFog (in vec4 currColor, in vec3 ray){
	...
	float fogAmount = 11.0-exp(-rayLength * 0.008);
