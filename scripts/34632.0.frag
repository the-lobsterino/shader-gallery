...
const float SEA_SPEED = 1.8;const float SEA_FREQ = 0.16;
...
void main( void ) {
	...
	uv = vec2(sin(uv.y), cos(uv.y))*(sp.x/(1.+sin(-1.7+sp.y+.125/sp.x)));
	
    float time = time * 0.3 + mouse.x*0.01;
        
    // ray
    vec3 ang = vec3(sin(time*3.0)*0.1,sin(time)*0.2+0.3,time);  
	ang = vec3(0.0, 0.0, time);
    vec3 ori = vec3(0.0,3.5,time*5.0);
    vec3 dir = normalize(vec3(uv.xy,-2.0)); dir.z += length(uv) * 0.15;
    dir = getPrimaryRay(uv * 0.5 + 0.5);
    if(dir.y > 0.0) {

    // post
	gl_FragColor = vec4(pow(getSkyColor(dir),vec3(0.75)), 1.0);
    } else {
    // tracing
    vec3 p;
