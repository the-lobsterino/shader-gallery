...
float distanceFunction(vec3 ray) {
	float modt = mod(sin(cos((gl_FragCoord.x+(time*100.))/10.-time)/4.-cos((gl_FragCoord.x/gl_FragCoord.y/15.)+time/2.)+time*2.0)*10., 20.0)-10.0;
...
void main( void ) {
	...
	// lighting
	float pattern = ( abs(sin(PI*position.x*2000.+time*5.0)/2.0)+abs(cos(PI*position.y*2000.+time*5.0)/2.0) );
	vec3 color_background = vec3(0.)+vec3(0., 0., 0.)*(0.7+0.3);
	vec3 color_light = normalize(vec3(0,.6,1.));
	vec3 color_shadow = vec3(0., 0.3, 1.8);
