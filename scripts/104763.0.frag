...
{
    return exp( -(100.5*c.z)*mod(fract(time),mouse.x)*(100.0*c.z)*dot(p-c.xy,p-c.xy) );
}

 vec2 a = vec2( (0.556646250*(cos(time*1.13)-sin(time))+.192),0.10 );
 vec2 ba = vec2( -0.018203125+sin(time*.8), 2.10*sin(time*.3) );
...
{
    return 0.5 + 0.5*sin( x + vec3(1.0,1.15,2.0*fract(time)) );
}
