...
void main( void ) {
	...
	vec2 dist3 = gl_FragCoord.xy - point3;
     	float intensity3 = pow(62.0/(0.01+length(dist3)), 2.0);
	
	float alpha = 1.0;
	gl_FragColor = vec4( (color1*intensity1 + color2*intensity2 + color3*intensity3) * mod(gl_FragCoord.x*gl_FragCoord.y, 2.0), alpha);
