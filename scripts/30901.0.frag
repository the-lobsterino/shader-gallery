...
vec2 neighbor_offset(float i)
{
	float c = abs(i-2.);
	float s = abs(i-4.);
	float t = mod(i, 2.) == 0. ? 1. : sqrt(2.)/2.;
	return vec2(c > 1. ? c > 2. ? 1. : .0 : -1., s > 1. ? s > 2. ? -1. : .0 : 1.)*t;
}
...
void main()
{
	...
	for(float i = 1.; i < 128.; i++)
	{
		...
		scattering	+= max(pow(abs(2.5*dot(ndh, light)), 8.*delta)*.04, 0.);
		prior_range 	= range;
	...
	color		*= .38;
	vec4 buffer	= texture2D(renderbuffer, gl_FragCoord.xy/resolution.xy-vec2(0., 3.)/resolution);
	...
	for (int i = 0; i < 8; i++)
		...
		sum 			+= texture2D(renderbuffer, neighbor_uv);
	}
	
	buffer.xyz		= mix(sum.xyz/8.,  buffer.xyz, sum.xyz * .2);
	color		= mix(color, buffer.xyz+color*.1, .99)*.94;
