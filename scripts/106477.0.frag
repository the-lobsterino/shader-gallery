...
const int _VolumeSteps = 256;
const float _StepSize = 0.05; 
const float _Density = 0.01;
...
{
    float f;
    f = 0.005000*noise( p ); p = m*p*0.2002;
    f += 0.002500*noise( p ); p = m*p*0.2003;
    f += 0.001250*noise( p ); p = m*p*0.2001;
    f += 0.000625*noise( p );
...
float rect(vec3 p){
	float a = length(p.xy) - 0.01;
	float b = length(p.xz) - 0.2;
	float c = length(p.yz) - 0.2;
	float lk = min(a,b);
	lk = min(lk,c);
	return lk ;
}
...
{	
	float d = rect(p);	// distance to sphere
...
vec4 rayMarch(vec3 rayOrigin, vec3 rayStep, out vec3 pos)
{
	...
	float c = 1./ distance(pos, rayOrigin);
	return vec4(c,c,c,1.);
}
