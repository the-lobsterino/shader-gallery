...
#define beat sin(time/(60./149.)/2.)
#define time time*(60./149.)
...
void main(void){
    ...
    uv *= rot(sin(time*0.25)*2.);
    uv = kaleidoscope(uv, 3.+sin(time*0.1));
    uv *= rot(time*0.3+beat);
    uv.x += cos(time*0.1);
    uv = kaleidoscope(uv, 6.+cos(time*0.3));
    uv *= rot(time*0.35+beat);
    uv = kaleidoscope(uv, 12.+sin(time*0.25));
    uv *= rot(time*0.45+beat);
    uv.x += sin(time*0.3);
	vec3 color;
