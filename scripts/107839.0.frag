...
float trap(in vec3 p)
{
	...
	//return (sqrt(atan((length(p)-0.5))) )/20.0;
	return abs((tan((abs(min(tan(torus(vec3(p.x, mod(p.y,0.4)+0.2, p.z), vec2(0.1, 0.05))), max(sin(abs(p.x)+0.05), atan(abs(p.z)-0.5))))-0.05)))/1.0);
