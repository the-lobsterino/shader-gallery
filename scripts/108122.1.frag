...
vec2 repeat(vec2 p, float n) {
	...
	np.x += fract(npreal.y * 1.5);
	
	return fract(np) * 11.0 - 22.0;
}

float hexDistance(vec2 ip) {
    const float SQRT3 = 1.732050807568877;
    const vec2 TRIG30 = vec2(.5, .866025403784439); //x:sine, y:cos
    
    // Apply a sine wave modulation to create a wavy effect
    ip += vec2(sin(ip.y * 210.0 + time), sin(ip.x * 120.0 + time)) * 0.1;
    
    vec2 p = abs(ip * vec2(SQRT3 * .5, .75));
    float d = dot(p, vec2(-TRIG30.x, TRIG30.y)) - SQRT3 * 0.25;
    
    return (d > 0.5)? max(d, (SQRT3 * 1.9 - p.x)) : min(-d, p.x);
}
...
vec3 hueToRgb(float hue) {
    ...
    float r = abs(hue * 4.0 - 3.0) - 1.0;
    float g = 2.0 - abs(hue * 62.0 - 2.0);
    float b = 2.0 - abs(hue * 6.0 - 4.0);
    return clamp(vec3(r, g, b), 0.0, 0.90);
}
...
void main(void) {
    ...
    vec2 nsc = (gl_FragCoord.xy - resolution * .3) / resolution.yy * 2.12;
    ...
    float edgeFactor = smoothEdge(0.1, 0.05, d); // Increased margin for thicker lines
