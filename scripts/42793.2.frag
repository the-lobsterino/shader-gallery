...
vec3 Text(vec2 uv)
{
	...
	print_pos += vec2(cos(time)*50.,sin(time)*50.);
	
	RGB(sin(2.*time+uv.x*0.03)/2. +0.5, -sin(2.*time-uv.y*0.03)/2. +0.5, sin(2.*time+dot(cos(uv),sin(uv))/3.)/2. +0.5)
		_M _o _t _i _o _n _spc _T _r _a _i _l _s
...
void main( void )
{
	...
	vec4 orig = vec4(vec3(col), 1.0);
	float dist = pow(1.0 - length(gl_FragCoord.xy/resolution.xy - mouse.xy), 150.0);
	vec3 m = vec3(1.0, 1.0, 1.0) * dist;
	gl_FragColor = orig + 0.95 * prev + vec4(m, 1.0);
}