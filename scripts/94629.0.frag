...
void main()
{
	...
	vec2 uv = gl_FragCoord.xy / resolution.xy;
	    ...
	    float roto = 1.0 + 2.0 * sin(time);
	    float fv = gl_FragCoord.y /resolution.y;
	    float deg90 = 0.5 * 3.14159;
	    float theta = time;
	    float x[4];
	    for (int i = 0; i < 4; i++ ) {
		x[i] = 0.5+ 0.20* sin( roto * uv.y + roto + theta + deg90*float(i));
	    }
	   
	    for(int i = 0; i<4; i++) {
		x[i] =  (x[i] *resolution.x) ;
	    }
	    for(int i = 0; i<4; i++) {
		
		if ( gl_FragCoord.x >= x[i] && gl_FragCoord.x < x[int(mod(float(i)+1.0, 4.0))]) {
		    
		    float dx = x[int(mod(float(i)+1.0, 4.0))] - x[i];
		    float u = (gl_FragCoord.x - x[i]) / dx;
		    
		    float v = mod(gl_FragCoord.y, 128.0) / 128.0;
