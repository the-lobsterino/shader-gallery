...
vec2 map(vec3 p) {
    ...
    float md6 = 1.3;
    float td1 = 0.5, td2 = .06, td3 = 1.2;
    
    float p0 = 1.2;
	p += vec3(0.,2.4-sin(T*0.2),.0);
    vec4 z = vec4(p,4.4);
...
vec3 camera(vec3 lp, vec3 ro, vec2 uv) {
     vec3 cf = normalize(lp-ro),
          cr = normalize(cross(vec3(0.,0.,1.0),cf)),
...
void mainImage( out vec4 O, in vec2 F ){
    ...
    C = mix( C, fC, 1.-exp(-.01*d*d*d));
    // Output to screen
    C *=vec3(texture(iChannel0,uv)).x;
    O = vec4(pow(C, vec3(0.545)),1.0);
}
