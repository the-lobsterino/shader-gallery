...
{
	float f = clamp(x/s+.15, 0.0, 1.0);
	return ((fract(sin(x+time)*1e5))-.5)*f*(1.40-f)*s*0.3;
}
...
vec3 Wave(vec2 p)
{
	...
	vec2 p0 = vec2(x, Amp(x, s)); x++;
	vec2 p1 = vec2(x, Amp(x, s)); x++;
	...
	return vec3(mix(vec3((1.-b)*.5+.2)*.2, vec3(a0*.8,a0*.3,a0),(1.0-0.1/a1)));
}
