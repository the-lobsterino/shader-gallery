...
void ddigit(int n)
{
	...
	if (bit(n, 11)) v = min(v, dseg(vec2(-1.063,  0.063), vec2(-0.438,  0.938)));
...
mat2 rotate(float a)
{
	...
	float s = sin(a);
	return mat2(c, s, -s, c);
}eturn c.z * mix( vec3(1.0), rgb, c.y);
}
void main( void ) 
{
	...
	vec3 ch_color = hsv2rgb_smooth(vec3(time*0.9+uv.y*0.1,0.5,1.0));

	vec3 bg_color = vec3(_d*0.1, _d*0.1, _d*0.1);
	uv.x += 0.1+sin(time+uv.y*0.9)*0.9;
	ch_pos = ch_start;

	
	       G A Y _ P P L _ R _ C O O L;
