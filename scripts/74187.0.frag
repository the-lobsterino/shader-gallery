...
void main( void ) {
	vec3 palette[7];
	palette[0] = vec3(0.933, 0.604, 0.604);
	palette[1] = vec3(0.957, 0.647, 0.133);
	palette[2] = vec3(0.996, 0.914, 0.302);
	palette[3] = vec3(0.804, 0.898, 0.396);
	palette[4] = vec3(0.494, 0.827, 0.918);
	palette[5] = vec3(0.416, 0.592, 0.855);
	palette[6] = vec3(0.757, 0.612, 0.906);
	...
	float nAngle = (angle + PI) / (PI * 2.0);
	nAngle = fract(nAngle + time * 0.3);
