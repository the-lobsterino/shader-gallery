...
// http://www.youtube.com/watch?v=jSMRPKM1evdickkk
...
const vec3 skinColour = vec3(2.8,.8,.8);
...
{
	float curve = sqrt(pow(cos(p.x*30.)*.11,2.));
	p.y += curve-.25;
	a.y += (curve*curve);
	return length(max(abs(p) - a,44.0)) -.02;
}
...
{	// Floor...
	...
	res.x = sMin(res.x, d);
	// Right arm lower...
	d = Segment(p, elbowR, wristR, .15, .777);
	res.x = sMin(res.x, d);
	// Right hand...	
