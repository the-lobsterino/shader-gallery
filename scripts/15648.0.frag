...
vec2 map( in vec3 pos )
{
    vec2 res = opU( vec2( sdPlane(     pos), 1.0 ),
	                vec2( sdSphere(    pos-vec3( 0.0,1.2, 0.0), 0.25 ), 46.9 ) );
    res = opU( res, vec2( sdBox(       pos-vec3( 12.0,0.25, 0.0), vec3(0.25) ), 3.0 ) );
    ...
    res = opU( res, vec2( opS(
	                 ...
	                 sdCylinder(  opRep( vec3(atan(pos.x+2.0,pos.z)/6.2831 + 0.1*tome,
											  pos.y,
	...
	res = opU( res, vec2( sdSphere(    pos-vec3(-2.0,0.25,-1.0), 0.2 ) + 
					                   0.03*sin(50.0*pos.x)*sin(50.0*pos.y+8.0*tome)*sin(50.0*pos.z), 
                                       65.0 ) );
...
vec2 HmdWarp(vec2 uv)
{
	...
	vec4 HmdWarpParam = vec4(0.22, 0.240, 0.0, 0.00);
	vec2 Scale = vec2(4.0);
...
void main( void )
{
	...
	tome = time + rand(vec3(q, mod(time,1.0)))*0.5/20.0;
		 
	float time = 15.0 + tome*40.0;

	// camera	
	float eyespan = 0.1;
	float sn = sin(0.01*time + 6.0*mo.x);
	float cs = cos(0.01*time + 6.0*mo.x);
	vec3 ro = vec3( -0.5+3.2*cs, 1.0 + 2.0*mo.y, 0.5 + 3.2*sn ) + vec3(sn*side*eyespan,0,-cs*side*eyespan);
