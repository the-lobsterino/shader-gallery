...
float Mouth( vec3 p, vec3 a)
{
	...
	p.y += curve;
	a.y += (curve*curve);
	return length(max(abs(p) - a,0.0)) -.47;
}
...
void main( void )
{
	...
	hello =  (46.0+sin(m*100.0)) * .02 * (smoothstep(0.0, .01, m) - smoothstep(0.05, .1, m));
	...
	t = t*t*(3.0-2.0*t);
	vec3 origin = mix(vec3(9.0, 1.0, -1.2), vec3(-1.0, 1.0, -5.0), t);
	vec3 target = mix(vec3(0.0, 1.0, 4.0),  vec3( 56.0, -.4, 0.0), t);
