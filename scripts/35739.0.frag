...
vec3 nrand3( vec2 co )
{
   vec3 a = fract( cos( co.x*8.3e-3 + co.y )*vec3(1.3e5, 4.7e5, 2.7e5) );
   vec3 b = fract( sin( co.x*0.27e-3 + co.y )*vec3(8.1e5, 1.0e5, 0.1e5) );
   vec3 c = mix(a, b, 0.5);
   return c;
}
...
void main() {
   ...
   vec3 p = vec3(uvs / 8., 0) + vec3(1., -1.3, 0.);
   ...
   p += .2 * vec3( sin(time*0.10 / 9.), sin(time*0.10 / 7.),  sin(time / 11.));
   ...
   starcolor += vec4(pow(rnd2.y,27.0),pow(rnd2.x,10.0), pow(rnd2.x,5.0), pow(rnd2.y,10.0) );
