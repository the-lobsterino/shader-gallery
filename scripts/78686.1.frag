...
vec4 geo(vec3 p)
{
	float sphere1 = length(p - vec3(sin(iTime * 1.)*0.2,0.,1.5)) - 0.2 + sin(iTime * 3. + p.y * 9.) * 0.0999; //---'Sphere blob'
	float sphere2 = length(p - vec3(sin(iTime * 1.)*0.2,0.,1.5)) - 0.2 + sin(iTime * 3. + p.y * 9.) * 0.3; //---'Sphere blob'
	// float sphere2 = length(p - vec3(cos(iTime * 2.)*0.4,0.,1.5 + sin(iTime)*.3)) - 0.1; //---'Sphere blob'
	float plane = ( p.y + 0.35);
	float m = sphere1;
	m = min(m, sphere2);
	m = min(m, plane);
	return vec4(0.2,0.6,1.0, m);
}
