...
vec3 origin=vec3(-1.+sin(time*.09)*.4,.7+sin(time*.1)*.3,time);
float det=0.0;
...

{
	...
	dir.y-=.02;
	float sunsize=.5;//-max(0.,texture2D(iChannel0,vec2(.6,.2)).x-.4)*5.; // responsive sun size
	...
	col=mix(vec3(1.1,.9,.4),col,exp(-.01*totdist*totdist));// distant fading to sun color
...
void main(void)
{
	...
	#ifdef BORDER
	color=mix(vec3(0),color,pow(max(0.,.75-length(oriuv*oriuv*oriuv*vec2(1.05,1.1))),.5));
	#endif
