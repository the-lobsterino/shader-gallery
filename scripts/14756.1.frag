...
float fbm( vec3 p, vec3 n )
{
	...
	float x = sin(1.0); //texture2D( iChannel0, p.yz ).x;
	float y = sin(1.0); //texture2D( iChannel0, p.zx ).x;
	float z = sin(1.0); //texture2D( iChannel0, p.xy ).x;
...
vec3 calcColor( in vec3 pos, in vec3 nor, in float sid )
{
    ...
    if( sid<0.5 )
	{
		...
		float de = 20.0*length(length(pos.xz));
