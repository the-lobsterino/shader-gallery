...
vec3 ff(vec2 p){
    ...
    for(float i=0.;i<15.;i++)
      c+=(0.1+sin(i*sin(0.1)+vec3(0.,1.3,2.2)))*.2/length(q-vec2(sin(i),12.*sin(.3*time+i)));
    return c+vec3(mix(mod(floor(p.x*.2)+floor(p.y*2.2),2.),.001,gc));
}
vec3 ft(vec3 o,vec3 d){
    d.y*=.65+.1*sin(.5*time);
    float D=1./(d.y*d.y+d.z*d.z),
          a=(o.y*d.y+o.z*d.z+1.)*D,
          b=(o.y*o.y+o.z*o.z-30.)*D,
          t=-a-sqrt(a*a-b);
    o+=t*d;
...
void main(){
    ...
    vec3 cp=vec3(-time*1.+1.,1.6*sin(time*1.2),2.+2.*cos(time*.3)),
