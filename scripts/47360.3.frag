...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
   ...
   vec3 ro=vec3(1.5,-0.5,-1.0);
   vec3 rd=lookat(vec3(0.0,-0.5,0.0)-ro)*normalize(vec3(uv,1.0));
   fragColor=vec4(scene(ro,rd)*2.0,1.0);
	fragColor.rgb *= fragColor.rgb-vec3(-.5,1,1)+fragColor.r/3.;
	fragColor.rgb *= fragColor.rgb*0.1*(1.-0.4*sign(cos(time/2.2+cos(time/10.))+0.95)-0.55*sign(cos(time/3.3+cos(time/20.))+0.95));
}
