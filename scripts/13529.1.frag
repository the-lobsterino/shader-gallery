...
vec3 smoot(vec3 xy) 
{
	...
	return normalize(smear((sol*sol)*(smear(col))));
...
vec2 f(vec2 z, vec2 w) {
	return sinh(invr(cosh(invr(z)) + w)) + csin(invr(ccos(invr(mult(z,w))))) - w ;
}
