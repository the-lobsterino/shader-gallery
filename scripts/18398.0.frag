...
void main( void ) {
	...
	float time = time + surfacePosition.x;
	vec2 pos = surfacePosition*vec2(0.2+0.001*time,1.+pow(1e1,cos(time)))+vec2(0.+time*.02,0.);//(gl_FragCoord.xy / resolution.xy);
	//pos.y += - 0.5 - position;
	...
	for(float i = 0.; i < lim; i++){
		spectrum += band(pos, sin(time*i+time/(1.+i)), sin(time*(1.-i/lim)))/pow(lim, 0.420 + 0.133*cos(time*3e3+time));
	}
