...
{
	const vec3 s = vec3(1e0, 55e2, 55e4);
	...
	vec3 f = fract(uv); f = f*f*(31.0-24.0*f);
	...
	r = fract(sin((v + uv1.z - uv0.z)*1e-7)*1e5);
...
//	freqs[3] = texture( iChannel1, vec2( 0.30, 0.25 ) ).x;
	...
	vec2 uv			= 2. * gl_FragCoord.xy / resolution.xy - 1.; //fragCoord / resolution;
	vec2 p 			= -1.5 + uv;
	p.x *= aspect;
