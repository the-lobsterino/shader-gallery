...
const vec2 ch_size  = vec2(1.0, 2.0) * 0.;              // character size (X,Y)
...
{
    vec3 rgb = clamp( abs(mod(c.x*6.+vec3(0.0,4.0,2.0),6.0)-3.0)-1.0, 0.0, 1.0 );
...
void main( void ) 
{
	...
	vec3 ch_color = hsv2rgb_smooth(vec3(time*0.5+uv.y*0.1,2.0,.5));

	vec3 bg_color = vec3(0.0, 0.0, 0.0);
	uv.y += 0.5+sin(time+uv.y*0.7)*0.4;
	uv.x += 0.5+sin(time+uv.x*0.7)*0.2;
	
	ch_pos = ch_start;

                 B U Y _ D O G E C O I N nl1 _ _ S U C H _ W O W nl1
			 
		
	vec3 color = mix(1.-ch_color, bg_color,   (14.0 * d*.80));  // shading
