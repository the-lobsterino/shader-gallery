/*
 * Original shader from: https://www.shadertoy.com/view/MdfGDH
 */
...
{
    return exp( -(780.0*c.z)*mod(fract(time),mouse.x)*(100.0*c.z)*dot(p-c.xy,p-c.xy) );
}

 vec2 a = vec2( (0.556646250*(cos(time*1.13)-sin(time))+.192),5.10 );
 vec2 ba = vec2( -0.018203125
		*sin(time*.8), 2.10*sin(time*.3) );
//vec2 ba = vec2( -0.08203125+sin(time*.3), 1.0 );

const float k = 0.9899933157;
float brusha( in vec2 p, in vec3 c )
{
    vec2 pa = c.xy - a;
    vec2 q = a + ba*dot(pa,ba)*k;///dot(ba,ba);
    vec2 c2 = 2.0*q - c.xy;
    return mix( brushb( p, c ), .30, brushb(p,vec3(c2,c.z)) );
}

vec3 getCol( in float x )
{
    return 0.5 + 0.5*sin( x + vec3(1.0,1.15,1.0*fract(time)) );
}
...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
    ...
    col = mix( col, getCol(1.005), brusha(uv,vec3(0.62,0.83,0.98)) );
    ...
    col = mix( col, getCol(1.69)*sin(time*.3), brusha(uv,vec3(0.27,0.82,0.36)) );
    ...
    col = mix( col, getCol(13.00), brusha(uv,vec3(0.66,0.88,0.49)) );
    ...
    col = mix( col, getCol(1.09), brushb(uv,vec3(0.71,0.51,0.46)) );
    col = mix( col, getCol(2.955), brushb(uv,vec3(0.69,0.55,0.16)) );
