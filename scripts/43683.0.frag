...
float sca = 0.5 + 0.15*sin(time/10.0-10.0);
vec4 grow = vec4(1.0);
...
void main( void ) 
{
    ...
    float d= 4.5;
    vec3 ro = time*vec3(d*sin(an),1.0,d*cos(an));
    ...
    // create view ray
    vec3 rd = normalize( p.x*uu + p.y*vv + (1.0+mp.y*444.)*ww );

    //-----------------------------------------------------
    // render
    //-----------------------------------------------------
    
    vec3 col = vec3(0.07)*clamp(1.0-0.5*length(p),0.0,1.0);
