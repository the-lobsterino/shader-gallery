...
#define iGlobalTime (time/10.+.5*surfacePosition.y+.25*surfacePosition.x)
...
{
	// default ray dir
	vec3 dir = ray_dir( 33.0, iResolution.xy, gl_FragCoord.xy );
	
	// default ray origin
	vec3 eye = vec3( 0.0, 0.0,10.0 );

	// rotate camera
	mat3 rot = rotationXY( vec2( iGlobalTime * 0.05, iGlobalTime * 0.0125 ) );
	dir = rot * dir;
	eye = rot * eye;
