precision mediump float;uniform float time;uniform vec2 resolution;
vec3 roty(vec3 p,float a){return p*mat3(cos(a),0,-sin(a),0,1,0,sin(a),0,cos(a));}

float map(in vec3 p) {
	float res;vec3 c = p;
	for (int i = 0; i < 10; i++) {
		p =0.9*abs(p)/dot(p,p) -.7;
		p.yz= vec2(p.y*p.y-p.z*p.z,2.*p.y*p.z);
		res += exp(-30. * abs(dot(p,c)));}
	return res/2.0;}

vec3 raymarch(vec3 ro, vec3 rd){
	float t = 1.0;
	vec3 col;float c;
	for( int i=0; i<30; i++){
		t+=0.02*exp(-2.0*c);
		c = map(ro+t*rd);               
		col += vec3(c/2.0,c*c,c)/10.0;}    
	return col;}

void main(){
    vec2 p = (gl_FragCoord.xy-resolution/2.0)/(resolution.y); p = 18.*p;
    vec3 ro = roty(vec3(3.),time*0.1);
    vec3 uu = normalize( cross(ro,vec3(1.0,.0,0.) ) );
    vec3 vv = normalize( cross(uu,ro));
    vec3 rd = normalize( p.x*uu + p.y*vv -ro*0.3 );
    vec3 c= raymarch(ro,rd);
	//p *= 2.;
	c += raymarch(ro,rd * 0.1);
	c += raymarch(ro,rd * 0.2);	
	
    gl_FragColor = vec4(c,1.0);
}
//original code from https://www.shadertoy.com/view/MtX3Ws
//simplified edit: Robert 25.11.2015
// modified: Necip 12.04.20 