...
float text(vec2 uv, vec4 info)
{
	...
	col += print_number(1./info.z, uv);
    col += char(ch_spc,uv);
	col += print_number(1./info.w, uv);
    col += char(ch_spc,uv);
...
void main()
{
	...
	float ft = fract(time);
	float dt = min(abs(el_InfoColor.a-ft), abs(ft-el_InfoColor.w));
	if(gl_FragCoord.x < 4.){
		gl_FragColor = vec4(0,0,(el_InfoColor.z + dt)/2.,fract(time));
		return;
	}
		el_InfoColor.w = dt;
