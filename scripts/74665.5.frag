...
#define time sin(time*0.01/(surfaceSize.x*surfaceSize.y))*cos(surfacePosition.x*surfacePosition.y)
...
float fn(vec2 p)
{
	...
	float a = ceil(b) - ceil((gl_FragCoord.y * resolution.x + gl_FragCoord.x));
	//float m = (p.x*p.y);
...
void main(void)
{
  ...
  torus_r = TAU*time;//(resolution.y/resolution.x) * (M/N);
