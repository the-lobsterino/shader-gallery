...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
  vec2 st = fragCoord;
  vec3 color = vec3(0.);
  st = vec2(resolution.x + 25., resolution.y)*mouse + 15. - st;
  st = rotate2d((-1.)*PI*mod(time * 2., 2.))*st;
  float dist = sqrt(dot(st, st))/2.;
  float theta = (PI + atan(st.y, st.x))/(2. * PI);
  vec3 ringA = vec3(smoothstep(3., 5., dist));
...
//Now Loading
#define text() _spc _N _o _w  _nl _spc _L _o _a _d _i _n _g 
#define text1() _spc _N _o _w  _nl _spc _L _o _a _d _i _n _g _per
#define text2() _spc _N _o _w  _nl _spc _L _o _a _d _i _n _g _per _per
#define text3() _spc _N _o _w  _nl _spc _L _o _a _d _i _n _g _per _per _per
//Let's Start!!
#define text4() _L _e _t _apo _s _nl _spc _spc _spc _S _t _a _r _t _exc _exc
vec3 Text (vec2 uv) {
    ...
    uv.y += sin(time+uv.x*0.05)*5.;

    RGB(rgb.x, rgb.y, rgb.z) 

    float t1 = mod(time,20.); 
    if(t1<=15.){
        float t2 = fract(time);
        if(t2<=0.25){
		text();
...
void main( void ) {
    ...
    float y = step(mod(time,20.0)/15.,st.x);
