...
void main( void ) {
	...
	float a = 0.25 * time + sin(time * 0.4) * 0.7 + sin(time * 0.7) * 0.2;
	float zoom = 5.0 + sin(time * 0.3) * 1.5 + sin(0.5 * time) * 2.0;
	pos.x = mod((sin(a) * posOrig.x + cos(a) * posOrig.y) * zoom, 2.5) - 1.25;
	pos.y = mod((cos(a) * posOrig.x - sin(a) * posOrig.y) * zoom, 2.5) - 1.25;
