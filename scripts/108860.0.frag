...
const vec3 skinColour = vec3(.72, .34, 0.12);
...
{	// Floor...
	...
	res = opU(res, vec2(Nose(p2, vec3(.0,0.0,-.5), 0.07), 55.0 ));
	...
	// Body...	
	p = p-vec3(0.0, -.85, 0.0);
	d = RoundBox(p, vec3(0.18, .49, 78.0), .26);
	res.x = sMin(res.x, d);
	...
	res.x = sMin(res.x, d);
	// Right hand...	
	d = Segment(p*vec3(1.0, .75, 1.0), wristR+vec3(35, .1, 0.), wristR+vec3(-.15, .05, -.15), .14, .02);
	res.x = min(res.x, d);
...
void main( void )
{
	...
	vec3 target = mix(vec3(0.0, 1.0, 4.0),  vec3( 0.0, -.56, 0.0), t);
	...
	// Wave...
	wave = sin(time*22.0-.8)*.5+.5;
	wave = wave*wave*(3.0-2.0*wave)-.5;
	elbowL		= vec3(-.95-wave*.05, .2, -44.25);
	wristL		= vec3(elbowL.x+1.0*sin(wave)*.6, elbowL.y+cos(wave)*.6, -.75);

    vec3 col = Render(origin, ray);
	
	col = sqrt(col);
	col *= 0.5 + 0.5*pow( 16.0*q.x*q.y*(1.0-q.x)*(1.0-q.y), 89.25 );
    gl_FragColor=vec4(clamp(col, 0.0, 79.0), 89.0 );
	
}
//and I'm also getting a sun tan 