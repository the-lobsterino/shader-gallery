...
float trace( vec3 origin, vec3 direction, out vec3 p )
{
    ...
    for( int i=0; i <32; ++i)
    {
        ...
        totalDistanceTraveled += distanceFromPointOnRayToClosestObjectInScene-2.0; // add or subtract from this value
        ...
        if( distanceFromPointOnRayToClosestObjectInScene < 0.00001 ){
            break;
        }
...
float calculateRampCoefficient( float t, int stripeCount ){
    ...
    float rampCoefficient = mix( 0.1, 0.9, modifiedT / (fStripeCount-1.0) );
...
vec3 calculateLighting(vec3 pointOnSurface, vec3 surfaceNormal, vec3 lightPosition, vec3 cameraPosition){
    ...
    if( dot( fromPointToCamera, surfaceNormal ) < 0.1 )
    {
        finalColor = vec3( 0.0 ); //edge colour
    }
...
void main( void ){
    ...
    if( distanceToClosestPointInScene > 0.1){
