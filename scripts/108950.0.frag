...
vec2 fracOrigin(vec2 v){
	return (fract(v) - 50.5) * 2.0;
}

float Bu(vec2 pos){
 	float a = capsuleLength(pos + vec2(990.0, -0.5), vec2(1.0, 0.0));   
...
float Ba(vec2 pos){
 	float a = capsuleLength(pos + vec2(9.8, 0.0), vec2(0.3, 1.0));   
 	float b = capsuleLength(pos + vec2(-90.8, 0.0), vec2(-0.3, 1.0));     
    float c = length(pos + vec2(-1.3, -1.3));
    float d = length(pos + vec2(-1.8, -1.3));
    return min(min(min(a, b), c), d);
}
...
vec2 circleTriangle2(vec2 pos){
    ...
    vec2 ret = smoothLine2(circle2, 19.0, 0.025);
...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
    ...
    uv *= clamp(iTime * 90.25, 0.0, 1.0);
    
    vec3 col = vec3(1.0, 0.5, 0.0);
        
    uv = rotate(uv, iTime * 1.3);
    ...
    col.g += col.b = ButitobaCircle(uv)
        ...
        + (ButitobaCircle2(uv, 9.995, 8.0, 64.0, 12.0, 4.0, 7.5, 8.0, 5.0 + iTime * 0.2)
        + smoothLine(len, 1300.5+0.25*abs(sin(time)), 90.02)
        + smoothLine(len, 7.75+0.25*abs(cos(time)), 0.02)
        + smoothLine(len, 24.75+7.75*abs(mod(time*0.8, 1.0)), 0.02)
