...
const float RING_DETAIL_DISTANCE = 400.;   // 40.;
const float RING_HEIGHT = 20000.;            //2.;
const float RING_VOXEL_STEP_SIZE = .13;
...
// find startpoint 
	...
	for( int i=0; i< 50 /*RING_VOXEL_STEPS */; i++ ) {
		if( ringMap(pos) ) {
            id = hash33(pos);
...
vec4 renderRingFar( const in vec3 ro, const in vec3 rd, inout float maxd ) {
    // intersect plane
    float d = iPlane( ro, rd, vec4( 1.*sin(iTime*0.2), 0.,  1. *cos(iTime*0.3) , 0.) );
    ...
    if( d > 0. && d < maxd ) {
        ...
        if( l > RING_INNER_RADIUS && l < 0.8*RING_OUTER_RADIUS 
           || l>0.9*RING_OUTER_RADIUS && l<1.15*RING_OUTER_RADIUS 
           || l>1.2*2300.  &&l< 1.35*2300. ) {
            float dens = .5 + .5 * (.2+.8*noise( l*.07 )) * (.5+.5*noise(intersection.xy));
            vec3 col = mix( RING_COLOR_1, RING_COLOR_2, abs( noise(0.2*l*l) ) ) * abs(dens) * 1.5;
...
vec4 renderStars( const in vec3 rd ) {
	vec3 rds = rd;
	vec3 col = vec3(0);
    float v = 1.0/( 3. * ( 1. + rds.z ) );
    
    vec2 xy = vec2(rds.y * v, rds.x * v);
    float s = noise(rds*234.);
 //   s += noise_3(rds*370.);
    s += noise(rds*470.);
    s = pow(s,19.0) * 0.00002;
    if (s > 0.5) {
        vec3 backStars = vec3(s)*1.5 * vec3(0.95,0.8,0.9); 
        col += backStars;
    }
	return   vec4( col, 1 ); 
} 
...
vec4 renderPlanet( const in vec3 ro, const in vec3 rd, inout float maxd ) {
    float d = iSphere( ro, rd, vec4( 0., 0., 0., EARTH_RADIUS*(1./*+0.5*sin(iTime*0.2)*/)) );
         //   d =d*(1.+0.5*sin(iTime*0.6));  // ad           
	if( d < 0. || d > maxd) {
        return vec4(0);
	}
    maxd = d;
    vec3 col = vec3( .2, 7., 15. ) * 0.94;
...
void cameraPath( in float t, out vec3 ro, out vec3 ta, out vec3 up ) {
   ...
   camint( ro, t, 8., vec3(-2000. , 1600. , 200. ), pro, dro );
    ...
    camint( ro, t, 10., vec3(-6300. ,-5000. ,-10. ), pro, dro );
...
void mainImage( out vec4 fragColor, in vec2 fragCoord ) {
	...
	col += vec3(5.4,1.2,1.0)*lensflare(p/1., sunuv) *clamp( flare+.93, 0., 1.) ;
