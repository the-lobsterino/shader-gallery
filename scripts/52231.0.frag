...
const vec3 kAmbientColor = vec3(0.276, 0.1, 0.1);
...
float message(vec3 p)
{
	...
	p.z += 0.5+sin(time*1.2+p.x*0.5)*0.75;
	p.z += 1.0;
	float t1 = fract(iTime*0.35) * TAU;
	p.y -= sin(t1+p.x*0.55)*0.6;
	vec2 uv = p.xy + vec2(2.0, -1.5);
	d = X4(uv,d); uv.x -= cw;
	d = X2(uv,d); uv.x -= cw;
	d = X4(uv,d); uv.x -= cw;
	d = X3(uv,d);
...
{   
    ...
    return opUnion(RayResult(c, kMaterialBlue), RayResult(b, kMaterialGreen));
}
