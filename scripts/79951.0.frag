// extra changes by robux icon 144M username xahviprz
...
void main( void ) {
    ...
    camera.y = 1.3*cos((camera.x=0.100)*(camera.z=time * 5.0));
    camera.x -= sin(time) + 3.0;

    for( float depth=.0; depth<21.; depth+=.07 ) {
    ...
    gl_FragColor = vec4(background,3.8);
