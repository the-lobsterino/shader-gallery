...
void main()
{
    ...
    for (int i=1;i<complexity;i++)
    {
        ...
        newp.x+=0.8/float(i)*sin(float(i)*p.y+time/fluid_speed+20.3*float(i)) + 0.5 + .1*(mouse.y);
        newp.y+=0.8/float(i)*sin(float(i)*p.x+time/fluid_speed+0.3*float(i+10)) - 0.5 -.1*( mouse.x);
        p=newp;
    }
