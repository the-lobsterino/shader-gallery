...
void main( void ) {
	...
	vec3 gColor = vec3(0.0, 0.5, 0.9);
	vec3 bColor = vec3(0.7, 0.5, 0.1) * ((cos(time)+1.125) * 8.0);
	vec3 yColor = vec3(0.1, 0.0, 0.0);

	vec2 position = ( gl_FragCoord.xy / resolution.xy ) / 4.0;
	position = gl_FragCoord.xy * 3.0 - resolution;
	...
	gl_FragColor = backColor + vec4(color, 1.0);
