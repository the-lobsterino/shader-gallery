...
void main(void)
{
    ...
    float dd = 0.2*sin(01.*time);
    float ss = 1.0 + clamp(1.0-r,0.0,1.0)*dd;
    r *= ss;
    vec3 col = vec3( 0.5, 0.5, 0.8 );
    float f = fbm( 5.0*p );
    col = mix( col, vec3(0.3,0.5,0.7), f );
    col = mix( col, vec3(0.9,0.6,0.2), 1.0-smoothstep(0.2,0.6,r) );
    a += 0.05*fbm( 20.0*p );
    f = smoothstep( 0.1, 1.0, fbm( vec2(32.0*a,15.0*r) ) );
    col = mix( col, vec3(1.0,1.0,1.0), f );
    f = smoothstep( 0.0, 0.9, fbm( vec2(15.0*a,10.0*r) ) );
    col *= 1.0-0.5*f;
    ...
    f = smoothstep( 0.69, 0.82, r );
    col = mix( col, vec3(1.00,1.0,1.0), f );
