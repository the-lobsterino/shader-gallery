...
#define NORMAL_OFFS 0.1

//Scene settings
#define HAZE_COLOR vec3(.2,.3, .4)
...
vec3 Shade(MarchResult hit, vec3 direction, vec3 camera)
{
    ...
    //Lighting
    float ambient = 1.41;
    float diffuse = 1.0 * -dot(reflect( direction, hit.normal), hit.normal);
    float specular = .5 * max(.5, dot(camera*direction, (reflect( direction, hit.normal))));
    
    color *= vec3(ambient + diffuse + pow(specular, 5.));
