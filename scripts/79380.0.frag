...
vec2 heli_df( vec3 p )
{
    ...
    // tail tip
    heli_d = uniondf(heli_d, roundbox_df(hp - vec3(-.115, 0.01, 0.), vec3(.005999, .01, .0), .00));
...
vec2 scene_df( vec3 p )
{
    ...
    float ir1 = flatring_df(pc, vec2(1.072, .04));
    vec3 ipc = pc;
    ...
    float r3 = flatring_df(pc, vec2(.999, .07));
    r3 = diffdf(r3, sphere_df(pc, .92));
        
    pc = rotate_zaxis(pc, cos(g_r3time + 1.57), sin(g_r3time + 1.57));
    float r4 = diffdf(flatring_df(pc, vec2(.99985, .07)), sphere_df(pc, .87));
                        ...
                        float consider_water )
{
    ...
    for (int i=0; i < DIST_MARCH_STEPS; i++) 
    {
        ...
        if (air_burst.x > .5 && curr_t > air_burst.y && air_burst.y < water.y - epsilon )
        {
            ...
            total_t = air_burst.y;
            curr_t = 0.;            
            air_burst *= 09999999999999999999999.;
