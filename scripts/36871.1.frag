...
#define CHAR_SIZE vec2(6, 9)
#define CHAR_SPACING vec2(6, 9)

vec2 scale = vec2 (2.7, 4.0);
...
#define Blue text_color = vec3(0,0,1);
...
#define White text_color = vec3(1,1,1);
...
vec3 Text(vec2 uv)
{
	...
	Red _T _R _U _M _P _spc  White _D _E _F _E _A _T _S _spc Blue _H _I _L _L _A _R _Y _spc _exc _exc _exc

	BEGIN_TEXT ( TX(11.0), res.y*0.5)
	print_pos += vec2(cos(time)*100., 0.6*sin(time)*50.);
	Red _P _R _E _S _I _D _E _N _T _spc _T _R _U _M _P

	BEGIN_TEXT ( TX(11.0), res.y*0.5)
	print_pos += vec2(sin(time)*100., 0.6*cos(time)*50.);
	White _P _R _E _S _I _D _E _N _T _spc _T _R _U _M _P

	BEGIN_TEXT ( TX(11.0), res.y*0.5)
	print_pos += vec2(cos(-time)*100., 0.6*sin(-time)*50.);
	Blue _P _R _E _S _I _D _E _N _T _spc _T _R _U _M _P

	print_pos = vec2(mod(time * -50.0, res.x * 2.7) -320., 10.0 );
	Red _T _R _U _M _P _spc _W _I _N _S _spc _L _A _N _D _S _L _I _D _E _spc _2 _0 _1 _6 _spc _P _R _E _S _I _D _E _N _T _I _A _L _spc _E _L _E _C _T _I _O _N _spc _exc _exc _exc
...
void main( void )
{
	...
	p*=100.0; p.x += resolution.x*0.2;p.y += resolution.y*0.1;
	...
	col *= (1.0 - distance(mod(p,vec2(1.0)), vec2(0.65)))*1.2;
