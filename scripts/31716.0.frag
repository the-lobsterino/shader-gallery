...
// Oh, did i/t say Trump or Satan, I thought those were the same...
...
vec3 processColor(vec3 p)
{
	...
	if (ground<d) { col = vec3(diff+spec*0.3)*vec3(0.6,0.33,3.0); d = ground; }
...
{	
	...
	letters = min(letters,S(p.xy-vec2(-1.00,0.2))+cos(time*5.1+3.0)*0.03);
