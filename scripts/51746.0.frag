...
void main(void)
{
	...
	vec3 p1=(vec3(gl_FragCoord.xy*fBias,0.0)*2.0-1.0)*fScale;
	vec3 p2=normalize(vec3(1.0,0.0,0.0));

	vec3 s= -vec3(1.0,1.0,1.0) * 0.1;

	float s1=meteor(p1 + vec3(s.x, 0.0, 0.0)) / s.x;
	float s2=meteor(p1 + vec3(0.0, s.y, 0.0)) / s.y;
	float s3=meteor(p1 + vec3(0.0, 0.0, s.z)) / s.z;
