...
vec4 fake_textureCube( float v, vec3 n ) {
	...
	float l = length(n);
	return vec4( abs(vec3(n.xyz)) / (l), 1.0 );
}
...
float sphereRadius = 6.0; //6.*sin(time); // radius of sphere before tex displace
...
float hex(vec2 p, int i)
{
    ...
    // 3. => hexagon
    // 4. => brain pattern
	float v = mod(pi.x + pi.y, 3. );//pattern);
...
vec4 scn(vec4 col, vec3 ro, vec3 rd)
{
	...
	return fake_textureCube(iChannel0, rd);
...
float st = -sb - sqrt(abs(sd));
	...
	if (!(sd < 0.0 || st < 0.0)) {
		...
		const int godrayIter = 4;
		const float godrayIntensity = 0.1;
		const float godrayPrecision = 0.1;
		
		float gr = -4.0;
			
		ro += rd*st;
		float rlen = length(ro);//6.928203-length(ro);
			
		rd *= godrayPrecision;
		for (int i = 0; i < godrayIter; i++) {
			    
		     if (hex(uvs(ro)*100.,0) < .01) 
		    	gr-=3.9;//max(1.0,1./rlen);
		       ro += rd*1.5;//abs(sin(float(i/16)+time));
		       rlen = length(ro); // 6.928203-length(ro);
		       //if (( rlen > 1.)) break;
		}
