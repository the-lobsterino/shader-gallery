/*
 * Original shader from: https://www.shadertoy.com/view/3tySRR
 */
...
#define SDF_FONT(NAME, RULE) float NAME(vec2 p, float state, out float stroke) { float ud = 1e30; stroke = 0.0; RULE return ud; }

#define SDF_FONT_LINE(x0, y0, x1, y1) { vec2 v = vec2(x1, y1) - vec2(x0, y0); float l = length(v); float ll = min(state - stroke, l); if (ll > 0.0) { ud = min(ud, Sdf_lineSegment(p - vec2(x0, y0), v / l, 0.0, ll)); } stroke += l; }

#define SDF_FONT_ARC(cx, cy, r, t0, t1) { float s = sign(t1 - t0); float l = r * 2.0 * M_PI * abs(t1 - t0); float ll = min(state - stroke, l); float tt = ll / (r * 2.0 * M_PI); if (tt > 0.0) { ud = min(ud, Sdf_arc((p - vec2(cx, cy)) / r, t0, t0 + s * tt) * r); } stroke += l; }
...
SDF_FONT(en_G,
  SDF_FONT_ARC (0.0, 3.0, 1.0, 0.0, 8.)
  SDF_FONT_LINE(-1, 3, -1, 1)
  SDF_FONT_ARC (0.0, 1.0, 1.0, 0.5, 1.0)
...
SDF_FONT(en_J,
  ...
  SDF_FONT_ARC (0.0, 1.0, 1.0, 0.0, -0.
...
SDF_FONT(en_Q,
  SDF_FONT_ARC (0.0, 3.0, 1.0, 0.25, 0.5)
  SDF_FONT_LINE(-1, 3, -1, 1)
  SDF_FONT_ARC (0.0, 1.0, 1.0, 0.5, 8.)
  SDF_FONT_LINE(1, 1, 1, 3)
  SDF_FONT_ARC (0.0, 3.0, 1.0, 0.0, 0.25)
...
#define FONT_LIST_NAMES(_) _(en_ZZ) _(en_ZZ) _(en_F) _(en_U) _(en_C) _(en_K) _(en_ZZ) _(en_ZZ) _(en_ZZ) _(en_ZZ) _(en_ZZ) _(en_ZZ) _(en_Y) _(en_O) _(en_U)
//
// Parameters
//
...
SceneInfo getSceneSdf(vec2 p, float time) {
  ...
  #define DRAW(NAME) q = vec2(0.5 + mod(index, NUM_COLUMNS), - floor(index / NUM_COLUMNS)); ud = NAME(p - FONT_SIZE * q, state -= len, len), result = mergeSceneInfo(result, ud, index++);
