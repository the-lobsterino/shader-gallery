...
void main(void) {
	...
	vec4 t1 = texture2D(s, uv);
	vec4 t2 = texture2D(s, uv+(sin(uv.x+time)*cos(uv.y+time))/100.);
	...
	if (l - 0.07 < d && l + 0.17 > d) {
		col = vec3(float(i) * 0.01 + time * 0.2, 0.5+sin(time), 0.5+cos(time));
		//col = toRGB(hsv);
	}

	t2 -= 0.01;
	gl_FragColor = vec4(col, 1.0) + t2;
	t1 = gl_FragColor;
}