...
vec3 bkg(vec2 v, float t){
   return v.xyx*cos(t+vec3(1,2,3)*2.)*.5+.5;   
}
...
vec3 lights(vec3 r){
		vec3 lv = vec3(.2, cos(1.4),-.7);
  vec3 lv2 = vec3(.9, cos(1.2),-.7);
  vec3 lv3 = vec3(-.1, -.01,-.1); 
  vec3 lc = vec3(0, .5,.4);
  vec3 lc2 = vec3(.7,.7,0.);  
  vec3 lc3 = vec3(.0,.4,.1);  
  vec3 l = light(r,lv,lc, 322.);
  vec3 l2 = light(r,lv2,lc2, 422.);
  vec3 l3 = light(r,lv2,lc3, 2.);  
  vec3 lf = vec3(-.01,.0,-1.);
  vec3 cf = vec3(.4,.6,1.);
  vec3 f = fresnel(r,lf,cf);
  vec3 ll = vec3(.5,.0,.7)*.2+.8*(l+l2+l3);
  return ll+f*.75;
...
vec3 trace(vec3 o, vec3 d, vec2 uv){
 ...
 float m = (1.-dist*1.3)*.4;
  ...
  r = o+d*t;
		dist = map(r);
		if(dist < epsilon){
	   return lights(r)+(cos(m*4.+time)*.4+.3)*bkg(uv+m,time);// + vec3(.4,.6,1)*m*.7;
		}
		t+= dist*dist_margin;
	}
	return vec3(.4,.6,1)*m*.9; //(cos(time*3.+3.+m*6.)*.3+.3);  //bkg(uv,time);
}
