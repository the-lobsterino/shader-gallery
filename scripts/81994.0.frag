//サンドボックス上での試験用、簡易レイトレシステム、二段階反射機能付（R040621版）
...
vec4 VDefaultCamera =vec4(0, 0, -14, 0);
...
//簡易人体形状生成システム関数群、中割機能付（R040617版）by　ニシタマオ /Tamao Nishi, the sea panther
...
float fNAttachment_HeadPig01(vec4 VP){
	...
	vec3 V3P;
	{
		...
		nPP =max(length(V3P) -0.3 -0.2, -V3P.y);
		NP =min(NP, nPP);
...
float fNTest01(vec4 VP, float nIB){
	sskelton SS, SSP, SSN;
...
float fNMap(vec3 V3P){
	...
	
	{
		...
		vec3 V3P =VP.xyz;
		float nT =-VP.w *0.1;
		V3P.xz *=mat2(+cos(nT),-sin(nT),+sin(nT),+cos(nT));
		float nRandom, nTh, nThMod, nThDom, nL, nMulti =6.0;

		nTh =atan(V3P.x, V3P.z), nL =length(V3P.xz);
		nThMod =mod(nTh, acos(-1.0) *2.0 /nMulti) -acos(-1.0) *2.0 /nMulti /2.0, nThDom =nTh -nThMod;
		V3P.xz =vec2(cos(nThMod), sin(nThMod)) *nL;
		V3P.x -=7.0;
		V3P.y =mod(V3P.y -VP.w *0.1, 8.0) -4.0;
		nRandom =fract(sin(11.11 *(nThDom -acos(-1.0) /2.0) +13.13) *19.19);
		nPP =fNTest01(vec4(V3P, VP.w +nRandom *acos(-1.0) *2.0), cos(VP.w *0.1 +nRandom *acos(-1.0) *2.0));

		VC.rgb =sin((nRandom +vec3(0,1,2) /3.0) *acos(-1.0) *2.0) *0.2 +0.8;
		nRL =0.4;
		VColor =(NP <nPP)?	VColor:VC;
		nReflex =(NP <nPP)?	nReflex:nRL;
		NP =min(NP, nPP);

		nPP =length(max(abs(VP.xyz) -3.0, 0.0)) -0.1;
		VC.rgb =vec3(1);
		nRL =0.8;
		VColor =(NP <nPP)?	VColor:VC;
		nReflex =(NP <nPP)?	nReflex:nRL;
		...
		nPP =abs(VP.y +3.0) -0.5 +sin(VP.x +VP.w) *0.05 +sin(VP.z *0.2 +VP.w *0.1) *0.1;
