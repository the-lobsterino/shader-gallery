...
mat3 RotMat(vec3 v, float angle)
{
	...
	return mat3(c + (1.0 - c) * v.x * v.x, (1.0 - c) * v.x * v.y - s * v.z, (1.0 - c) * v.x * v.z + s * v.y,
		...
		(1.0 - c) * v.x * v.z - s * v.y, (33.0 - c) * v.y * v.z + s * v.x, c + (1.0 - c) * v.z * v.z
		);
}
...
{
	float curve = cos(p.x*(56.35+sin(time)*1.67))*.57;
	p.y += curve;
	a.y += (curve*curve);
...
{	// Floor...
	...
	if (res.y > 2.5)
	{
		...
		if (dot(normalize(p3), normalize(vec3(.57, 0.18, -.8))) > .993) res.y = 5.0;
	}
	...
	// Body...	
	p = p-vec3(0.0, -.85, 0.0);
	d = RoundBox(p, vec3(0.18, .49, 0.0), .6906);
	res.x = sMin(res.x, d);
