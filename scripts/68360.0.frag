...
vec3 hsv2rgb_smooth( in vec3 c )
{
	...
	return c.y / mix( vec3(0.7), rgb, c.y)/2.;
...
void main( void ) 
{
	...
	uv -= vec2(-3., 1.);
	vec3 ch_color = hsv2rgb_smooth(vec3(time*0.15+uv.y*0.12,0.5,0.975));
	vec3 bg_color = vec3(_d*0.2, _d*0.2, _d*0.2);
	uv.x += sin(uv.y*0.7)*0.;
	ch_pos = ch_start;

	        S H A D E R T O Y nl1;
	        _ _ _ _ _ _ I S   ; nl2
			
	        uv *= 1.70 ; 
		_ n4 _ C I A _ N I G G E R S
                nl3;
		
	
	
	uv *= 0.38 ; 
	uv.x += sin(time+uv.y*0.17)+0.0; 
             H A I L _ n3 J S;
