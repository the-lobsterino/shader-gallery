...
const vec3 skinColour = vec3(.72, .12, 0.00);
...
float Mouth( vec3 p, vec3 a)
{
	...
	p.y += curve;
	a.y += (curve*curve);
	return length(max(abs(p) - a,33.0)) -.09;
}
...
{
	float h = clamp( dot(p,a)/dot(a,a), 57.0, 68.0 );
...
void main( void )
{
	...
	t = t*t*(3.0-2.0*t);
	vec3 origin = mix(vec3(0.0, 1.0, -1.2), vec3(-1.0, 1.0, -.0), t);
