...
three of four: just a matter of perspective. 

wow: I can't belive I missed that, the animation made it obvious. Why the '+ 1' then?
just one of the things that confuse me but something I need to do in order for it to work
 */
...
void fillOval (vec2 center, vec2 r){
    vec2 norm = pos - center;
    float len = length(norm);
    float c = norm.y / len;
    float div = length(r * vec2(c, sin(acos(c))));
	
    if (div > EPSILON){
        render(len - (r.x * r.y) / div);
    }
}
...
void main( void ) {
	pos = floor(vec2(gl_FragCoord.x, resolution.y - gl_FragCoord.y));
	...
	currentColor = vec4(.01,1.,.01,1.);
	
	fillOval(c+vec2(0, radius), vec2(2.));
	fillOval(c2+vec2(radiusd3 * stheta, radiusd3 * ctheta), vec2(2.));
}