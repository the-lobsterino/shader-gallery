...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
	...
	vec2 uv1 = (gl_FragCoord.xy * 2.0 - resolution) / min(resolution.x, resolution.y);
	float d = GetText(uv1*4.0);
	d =  clamp(1.0-d,0.0,1.0);
	
	float dd = abs(sin(time+uv.x))+0.3;
	vec3 col2 = vec3(1.1,0.7,1.1)*dd;
	col = mix(col,col2,d);
	//col+=d;
