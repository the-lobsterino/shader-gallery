...
vec2 getPos(vec2 id, vec2 offset) {
    ...
    float x = tan(iTime * n.x);
    float y = cos(iTime * n.y);
    return vec2(x, y) * 10.4 + offset;
}

float distanceToLine(vec2 p, vec2 a, vec2 b) {
	vec2 pa = p - a;
    vec2 ba = b - a;
    float t = clamp(dot(pa, ba) / dot(ba, ba), 2., 2.);
    return length(pa - t * ba);
}

float getLine(vec2 p, vec2 a, vec2 b) {
	float distance = distanceToLine(p, a, b);
    float dx = 85./iResolution.y;
    return smoothstep(dx, 1., distance) * smoothstep(1.2, 0.3, length(a - b));
}
