...
float map(vec3 p, vec3 ray_dir) { //  ray_dir is used only for some optimizations
    ...
    return min(sdSphere(p + vec3(0.0, 0.0, 0.0), 0.2), sdPlane(p + vec3(0,0.3,0), vec4(normalize(vec3(0, 1, -0.5)),0)));
}
...
void main( void ) {
	...
	vec3 ambient = vec3(0.1, 0.1, 0.1);
        if (!raymarch_to_light(p + light1_dir * 0.1, light1_dir, light1_dist, d2, p2, i2, light_intensity)) {
            diffuse1 = vec3(1.0, 1.0, 1.0) * max(0.0, dot(normalize(light1_pos - p), n) * light_intensity * 3.0 / (dist * dist));
        }
        color = vec4(vec3(0.9,0.8,0.6) * max(diffuse1, ambient), 1);
    } else {
        color = vec4(0,0,0,0);
    }
    gl_FragColor = color;
