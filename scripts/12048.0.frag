...
{
	float d = 0.;
	vec3 col = vec3(0.3, .07+0.07*(.5+sin(gl_FragCoord.y*3.14159*1.1+time*2.0)) + sin(gl_FragCoord.y*.01+time+2.5)*0.05, 0.1);
	
	caret = caret_origin;

	// Gallery reset! I had code bookmarked for later use. Shit.
	d += I(r()); space(); 
	d += H(r()); add(); d += A(r()); add(); d += D(r()); space(); d += C(r());  add(); d += O(r()); add(); d += D(r()); add(); d += E(r());
	newline();
	
	d += B(r()); add(); d += O(r()); add(); d += O(r()); add(); d += K(r()); add(); d += M(r()); add(); d += A(r());  add(); d += R(r()); add(); d += K(r());
	add(); d += E(r()); add(); d += D(r());
	newline();
	newline();
	
	d += S(r()); add(); d += H(r()); add(); d += I(r()); add(); d += T(r());
	
	d = clamp(d* (.75+sin(gl_FragCoord.x*PI*.5-time*4.3)*.5), 0.0, 1.0);
      
	col += vec3(d*.9, d*.2, d*.2);
	vec2 xy = gl_FragCoord.xy / resolution.xy;
	//col *= vec3(.8, .0, .0) + 0.5*pow(100.0*xy.x*xy.y*(1.0-xy.x)*(1.0-xy.y), .4 );	
