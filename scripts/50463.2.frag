...
void main( void ) {
	...
	vec3 c =  (sample((gl_FragCoord.xy + vec2( 0.25,  0.25)) * invResolution) 
		+   sample((gl_FragCoord.xy + vec2(-0.25, 0.25)) * invResolution) 
		+   sample((gl_FragCoord.xy + vec2( 0.25, -0.25)) * invResolution) 
		+   sample((gl_FragCoord.xy + vec2(-0.25, -0.25)) * invResolution)) / 4.0;
	#else
	vec3 c = sample(st);
	#endif
