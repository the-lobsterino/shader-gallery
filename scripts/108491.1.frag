...
void horizon(vec2 p, vec2 a, vec2 b){ //horizon between a, b with a.x<b.x in domain of p
    ...
    float d = length(ab-p);
	amp += -.5*smoothstep(0.*fwidth(d),.0,d);
    if ((p.x>a.x) && (p.x<b.x) && (ab.y<p.y)) amp -= .3+.25*d; //sky
    amp += step(a.y,.140)*.05/length(p-vec2(a.x+.1,1.5*a.y)); //lights
}
...
void mainImage( out vec4 fragColor, in vec2 fragCoord ){
    float animTime = mod(2.*20.9999*iTime,2.*PI); //...buggy without mod at large iTime, ?overflow
    float animFrame = floor((iTime*600.)/1.);
	vec3 col = vec3(0.5);
	amp = 0.;
