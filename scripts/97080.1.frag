/*R041201作例　サーカス　by ニシタマオ　THE SEA PANTHER*/
/*サンドボックス上での試験用、簡易レイトレシステム、反射・屈折、ぼかし影、カラーパレット機能付（R041201版）*/
/*直近の改修：色彩微調整*/
...
sface fSFSet_Default(void){
	...
	SF.VColor =vec4(1);
	SF.nDistance =1e+6;
	SF.V3Real_Reflect_Refract =vec3(1,0.5,0.5);
	SF.nRefrax =0.75;
	return SF;
}
...
float fNMap(vec3 V3P){
	...
	if(true){
		vec4 VPi =VP;
		float nX_Mod =mod(VPi.x, 12.0) -6.0, nX_Dom =VPi.x -nX_Mod;
		VPi.x =nX_Mod;
		VPi.w =sin(VPi.w *2.0 +nX_Dom) *4.0;
		VPi.z +=VPi.w *2.0;
...
sface fSFEffect_Before(sface SF, smarch SM){
	...
	if(SF.ID_Object ==1)		VColor.rgb =sin(vec3(0,1,2) /3.0 *acos(-1.0) *2.0 +VP.x *0.1) *0.2 +0.8;
	if(SF.ID_Object ==101)	VColor.rgb =sign(sin(VP.x) *sin(VP.z)) *0.1 +vec3(1);
	if(SF.ID_Object ==102)	VColor.rgb =sin(vec3(0,1,2) /3.0 *acos(-1.0) *2.0 +VP.w) *0.1 +0.9;
