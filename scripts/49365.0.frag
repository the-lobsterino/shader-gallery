...
// better comments added to source by Ded
...
// Color matrixes
// RGB Color Lines (base, spatial, power, brightness)
// Final Line (r, g, b, all)
...
const mat4 sun1 = mat4(
	1, 1.2, 3, 3,
	1, 1.2, 3, 3,
	1, .1, 11, .003,
	1, 1, 1, .08
);
const mat4 sun2 = mat4(
	1, .1, 3, 3,
	1, .11, 3, .03,
	1, .1, 3, 3,
	1, 1, 1, .04
...
void main() {
	...
	_h M lf_h A lf_h G lf_h A
	
	ch_pos = vec2(-22.5, 12.);
	I _ W A S T E D _ A N _ H O U R _ O N _ T H I S
	
	
	ch_pos = vec2(-20.5, -12.);
	M A K E _ A N I M E _ G R E A T _ A G A I N
	
	nl 
	
	float g = 1e6;
	float cp = 2.5 + 1.8 * sin(time * .47);
	g = min(g, 1.0 * cp * abs(length(uv/10.)-.7 + .025*sin(time)));
	g = min(g, .15 * cp * abs(length(uv/01.)-7. + .45 * sin(time *.39)));
	
	float gs = 5.;
	float gp = 2.5 + .8 * sin(time * .3);
	vec2 f = vec2(fract(uv.x / gs), fract(uv.y / gs));
	f = min(1.0 - f, f);
	
	g = min(g, gp * abs(5. * f.x / sin(uv.y / 3.14 )));
	g = min(g, gp * abs(5. * f.y / sin(uv.x / 3.14 )));
	
	float sun = 1000.;
	sun = .1 * min(sun, length(uv));
	sun = sun * sun;
	...
	float sp = 1.0;
	if (uv.y > 0.0) { sp = 1.0 +uv.y * 1.50; }
	
	float sp2 = 1.0;
	if (uv.y < 0.0) { sp2 = 1.0-uv.y * 1.0; }
	
	vec3 textc = cc(uv, text);
	vec3 gridc = cc(vec2(uv.x*3.,0), grid);
	vec3 sunc1 = cc(vec2(sunuv), sun1) * sp;
	vec3 sunc2 = cc(vec2(sunuv), sun2) * sp2;
	
	vec3 color = vec3(0, 0, 0);
	
	color += 1. * dblend(gridc, g, vec2(1.9, .004));
	
	color += 1.1 * dblend(sunc1, sun, vec2(1.5 + .05 * sin(time), .0001));
	color += .41 * dblend(sunc2, sun, vec2(1.45 + .15 * sin(time*1.3), 1e-4));
	
	// Core
	color -= 15. * dblend(textc, d, vec2(19.2, 1e-88));
	// Body
	color += 1. * dblend(textc, d, vec2(13.3, 1e-88));
	// Glow
	color += 1. * dblend(textc, d, vec2(1e88, .14));
