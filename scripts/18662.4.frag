...
void main()
	...
	vec2 oldpos = (old.a < 0.1) ? vec2(0.5) : old.rg;
	float oldroll = (old.a < 0.1) ? 0.5 : old.b;
	...
	if (int(gl_FragCoord.y) == 0)
	{
		...
		col.b = mix(0.5, fract(oldroll + (0.001 * (pow(7., 5.0 * abs(ms.x))) * sign(ms.x))), abs(ms.x * 2.0)); //fixme: this is sloppy
	}
	}
