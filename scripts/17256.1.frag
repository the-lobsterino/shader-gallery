...
// Removed the "music beat" effects - now it's just a plain ol' tunnel :-)
// TODO: De-minify/beautify the code.
...
float PI=3.14159;
vec3 ff(vec2 p){
    vec2 q=vec2(sin(.03*p.x),4.*p.y);
    vec3 c=vec3(0);
    for(float i=0.;i<10.;i++)
      c+=(1.+sin(i*sin(time)+vec3(0.,1.3,5.2)))*.2/length(q-vec2(sin(i),12.*sin(.3*time+i)));
    return c*.4+vec3(mod(floor(p.x*.2)+floor(p.y*2.2),2.));
}
