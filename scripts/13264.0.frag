...
void main( void ) {
	...
	if (dot(p,p) < 0.5 && abs(atan(p.y,p.x)) > abs(fract(fasttime*2.6)-0.5)*1.7 ) {
		gl_FragColor = vec4(1.,1.,0.,1.);
	} else {
		float det = p.x;
		p.x = fract(p.x+0.5+fasttime)-0.5;
		p *= 0.6;
	
		vec2 t = p;
		t.x += sin(fasttime)/45.0;
		float f = step(sqrt(t.x*t.x - t.y*t.y*t.y), 0.1);
		f *= step(length(t), 0.3) * step(-det, 0.0);
		
		float s = step(abs(p.x+cos(p.y*9.0 + fasttime)/45.0), 0.01);
