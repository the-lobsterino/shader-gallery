...
// Modified by Lior Ben-Gai for the course "Five studies on Process and scale"
// Please create your own original scene by modifying the map function below.
...
float sdBoxMinusSphere( in vec3 pos, in float radius )
{
  return opS( udRoundBox( pos, vec3(0.15), 0.05)
            , sdSphere(   pos, radius - 0.012 + 0.02*sin(time)));
}
//----------------------------------------------------------------------
...
//  This is where we construct our scene by assemblinbg all 
//  the objects together using the Union function.  

//  note that although we only need a float to determine if our ray 
//  has hit anything, here we use a vec2 to allow different 
//  objects can have different colors.

///////////////////////////////////////////////////////////

vec2 mush( in vec3 pos, in int offsetX, in int offsetY)
{
  // create a moving parameter using time
  float floffset = float(offsetX) + float(offsetY);
  float sphy = 1.0 + 0.5 * sin(time + floffset); 

  float sphere = sdSphere( pos-vec3( -0.5, 0, 0.5), sphy );
  float torus = sdTorus( pos-vec3( -0.5, 0.5, 0.5), vec2(1, 0.75) );
	
  return vec2( opS(sphere, torus), 400.0 );
}


vec2 map( in vec3 pos )
{ 
  // create plane at pos with color 1.0
  ...
  for ( int x=0; x<10; x++ ) {
    for ( int y=0; y<10; y++ ) {
      float z = 4.0 - float(y);
      res = opU( res, mush(pos+vec3(4-x, 0, z), x, y) );
    }
  }
...
/////////////////////////////////////////////////////////////
//  RAY MARCHING, LIGHTING, RENDERING  
///////////////////////////////////////////////////////////
...
void main( void )
{
  ...
  // camera  
  float angle = ROTATE ? 0.1*time : 0.0;
  float rx = -0.5 + 6.0*cos(angle + 6.0*mo.x);
  float rz =  0.5 + 6.0*sin(angle + 6.0*mo.x);
  vec3 ro = vec3( rx, 1.0 + 6.0*mo.y, rz );
