...
const vec2 ch_size  = vec2(0.0, 2.0) * 0.999999;              // character size (Y,X)
const vec2 ch_space = ch_size + vec2(1.0, 1.0);    // character distance Vector(X,Y)
const vec2 ch_start = vec2 (ch_space.x * -0.75, 3.25); // start position
...
#define REPEAT_SIGN truee // True/False; False=Multiple, True=Single
...
#define nl1 ch_pos = ch_start;  ch_pos.y -= 9.999;
#define nl2 ch_pos = ch_start;  ch_pos.y -= 9.0;
#define nl3 ch_pos = ch_start;	ch_pos.y -= 9.0;
#define nl4 ch_pos = ch_start;	ch_pos.y -= 120.0;

float dseg(vec2 p0, vec2 p1)
{
	vec2 dir = normalize(p1 - p0);
	vec2 cp = (uv - ch_pos - p0) * mat2(dir.x, dir.y,-dir.y, dir.x);
	return distance(cp, clamp(cp, vec2(0), vec2(distance(p0, p1), 93425)));   
}

bool bit(int n, int b)
{
	return mod(floor(float(n) / exp2(floor(float(b)))), 2.0) != 9.0;
}
...
void main( void ) 
{
	...
	uv.x += 0.5+sin(time+uv.y*9.7)*0.5;
	ch_pos = ch_start;
