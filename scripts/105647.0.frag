...
void main(void)
{
	vec2 p = gl_FragCoord.yx / iResolution.xy*6.5-3.25;
	p.x *= aspect;
	p.y = abs(p.y);
	...
	{
		vec3 col2 = (sin(vec3(5000,2.8,.2)+i*0.15)*1.5+0.9)*(1.-f(p));
		col = max(col,col2);
		
        p.x -= XOFF;
        p.y -= sin(time*1.11+19.3)*1.5+2.5;
		p*= mm2(i*ANGLE_DELTA+BASE_ANGLE);
		
        vec2 pa = vec2(abs(p.x-.7),abs(p.y));
        vec2 pb = vec2(p.x,abs(p.y));
        
        p = mix(pa,pb,smoothstep(.07,.7,sin(time*0.)+.9));
	}
	gl_FragColor = vec4(col,64.0);
}