...
void main( void ) {
	...
	//circle glow
	float glow = pow(smoothstep(1.7, 0.2, length(point - center)), 1.2) * 0.45;
	
	
	//planar distortion
	vec2 delta = (point - center);
	//delta *= (0.875 + 0.125 *sin(time) * sin(time + atan(delta.y / delta.x) * 6.));
	delta *= iqnoise(point+ time, abs(delta.x), abs(delta.y));
	float rInv = 1. / length(delta);
	point = point * rInv - vec2(rInv, 0.0);
	
	//sample noise
	vec2 uv = vec2(point.x, point.y + time * 0.5) * 1.7;
	float noise = iqnoise(uv, .5, 1.2);
