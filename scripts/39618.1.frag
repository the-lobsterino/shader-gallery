...
float eyeSphere(vec3 p){
    float localtime = time/3.0;
    p = mat3( cos(localtime), sin(localtime), 0, -sin(localtime), cos(localtime),0 ,0,0,1)*p;

    vec3 p_1 = p + vec3(0.15,-0.07,-2.0);
    vec3 p_2 = p + vec3(-0.15,-0.07,-2.0);
    return min(length(p_1) - 0.02, length(p_2) - 0.02);
}
