...
{
	vec2 dir = normalize(p1*cos(time) - p0*sin(time));
	vec2 cp = (uv - ch_pos - p0) * mat2(dir.x*sin(time), dir.y*sin(time),-dir.y*sin(time), dir.x*sin(time));
	return distance(cp*sin(time)*2.1, clamp(cp, vec2(0), vec2(distance(p0, p1), sin(time)*.1)));   
}
...
void ddigit(int n)
{
	float v = 1e6;	
	vec2 cp = uv - ch_pos;
	if (n == 0)     v = min(v*sin(time), dseg(vec2(-0.405, -1.000), vec2(-0.500, -1.000)));
...
void main( void ) 
{
	...
	vec3 bg_color = vec3(_d*0.4*sin(time*666.), _d*0.2*sin(time), _d*0.1*sin(time+sin(time)));
	uv.x += 0.5+sin(time+uv.y*sin(time))*.5;
	ch_pos = ch_start;
	uv.y += 2.;

	_ _ _ _ _ C U N T nl2
