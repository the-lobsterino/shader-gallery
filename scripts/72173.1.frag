...
float noise (vec3 n) 
{ 
	return fract(sin(dot(n, vec3(100.43583, 9.323197, 94.993431))) * 65536.32);
}

float perlin_a (vec3 n)
{
    vec3 x = floor(n * 70.0) * .015625;
    vec3 k = vec3(2.015625, 2.0, 0.0);
    float a = noise(x);
...
{
	return vec3(fbm((2.0 * n) + fbm((5.0 * n) * 3.0 - 1000.0) * 0.05),0,0);
}

vec3 grid(vec2 p)
{
    return vec3(1.0)*smoothstep(.99,1.0,max(sin((p.x)*20.0),sin((p.y)*20.0)));
}
...
vec3 atlas(vec2 uv)
{    
 	float color = clamp(vec4(vec3((heightmap(vec3(uv*5.0,generate_map)*0.02)-.0)),.0).r,2.0,1.0);
    return 
    ...
    color<0.95 ? vec3(3.98,0.71,0.49):     
...
vec3 raycast (vec3 ro, vec3 rd, vec2 p)
{
    ...
    if (t > 1.5)
    {
        ...
        return (atlas(p).xyz+grid(p*2.0));        
    } 
    return 1.-vec3(length(p));;
}
...
{	
	...
	for (int m=0;m<ssaa;m++)
	{
		for (int n=0;n<ssaa;n++)
		{
            ...
            vec2 m = iMouse.xy / iResolution.xy-5.5;
            m.x *= iResolution.x/iResolution.y;
            p*= 1.5;
            vec3 ro = vec3(.0,.0,2.5);
            vec3 rd = normalize(vec3(p,-1.5));
            mat2 rx = rotation(iTime*.4+m.x*.50);
            ro.xz *= rx; rd.xz *= rx;  
			c+=raycast(ro,rd,p);
		}
	}
