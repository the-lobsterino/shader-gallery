...
#define RAY_STEPS (64+int((sin(time))*50.0))
...
float de(vec3 pos) {
	...
	float d=min(fr,ro) /* hack-> */ +sin(time*2.34567) * 0.06 - 0.06;
	return d;
}
...

{
	...
	//if (totdist>25.) col=(backg+cc.xyz); // hit background
...
void main(void)
{
    ...
    //color = vec3(color.r + color.g + color.b, 0.0, 0.0) * 0.33;
