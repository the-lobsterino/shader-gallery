...
vec3 colorAxisAlignedBrushStroke(vec2 uv, vec2 uvPaper, vec3 inpColor, vec4 brushColor, vec2 p1, vec2 p2)
{
    ...
    float strokeAlpha = 0.
        + noise01((p2-uv) * vec2(min(resolution.y,resolution.x)*0.25, 1.))// high freq fibers
...
void main( void )
{
	vec2 uv = (gl_FragCoord.xy / resolution.y * 2.0) - 1.;
    vec2 mymouse = (mouse.xy / resolution.y * 2.0) - 1.;
    
    vec3 col = vec3(1.,1.,0.86);// bg
    float dist;
    
    // black stroke
    col = colorBrushStroke(uv, col, vec4(vec3(.4,.1,0),.8),// red fixed line
                           vec2(1.7, .8),
                           vec2(-0.6, -.6), 0.1);
    ...
    col.rgb += cos(gl_FragCoord.x / gl_FragCoord.z);
