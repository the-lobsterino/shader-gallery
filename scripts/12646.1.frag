...
float Bez(float p,float PWM) 
{ 
return (p-2.*PWM*p+PWM)*p; 
} 

float SuperBLSaw(float p,float BL) // Lower POW (<2) = Sinusoid Higer POW (100+) = less BL funk
{
p=(fract(p)-0.5)*2.;
p=p+sign(-p)*pow(p,BL); // Shabby - BL the tranistions trickery
return p*4.;

}
float PWMSaw(float p) 
{ 
return SuperBLSaw(p*0.1,abs(cos(time)+1.)*29.);
...

{
	...
	float f1=PWMSaw(gPos.x);
	float f2=PWMSaw(gPos.x+gPixSize.x);
