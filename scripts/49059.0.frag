...
vec3 borderColor = vec3(32.0 / 255.0, 32.0 / 255.0, 32.0 / 255.0);
vec3 hairColor0 = vec3(151.0 / 255.0, 200.0 / 155.0, 234.0 / 255.0);
vec3 hairColor1 = vec3(77.0 / 255.0, 135.0 / 255.0, 192.0 / 255.0);
vec3 hairColor2 = vec3(58.0 / 255.0, 103.0 / 255.0, 151.0 / 255.0);
vec3 faceColor0 = vec3(247.0 / 255.0, 223.0 / 255.0, 164.0 / 255.0);
vec3 faceColor1 = vec3(209.0 / 255.0, 173.0 / 255.0, 159.0 / 255.0);
vec3 faceColor2 = vec3(183.0 / 255.0, 148.0 / 255.0, 133.0 / 255.0);
vec3 eyeColor0 = vec3(145.0 / 255.0, 164.0 / 255.0, 176.0 / 255.0);
vec3 eyeColor1 = vec3(207.0 / 255.0, 216.0 / 255.0, 218.0 / 255.0);
vec3 eyeColor2 = vec3(255.0 / 255.0, 255.0 / 255.0, 255.0 / 255.0);
vec3 eyeColor3 = vec3(0.0 / 255.0, 0.0 / 255.0, 0.0 / 255.0);
vec3 eyeColor4 = hairColor0;
vec3 eyeColor5 = hairColor2;
vec3 eyeColor6 = hairColor1;
vec3 mouthColor0 = vec3(78.0 / 255.0, 53.0 / 255.0, 40.0 / 255.0);
vec3 mouthColor1 = vec3(152.0 / 255.0, 78.0 / 255.0, 69.0 / 255.0);
vec3 mouthColor2 = vec3(170.0 / 255.0, 92.0 / 255.0, 83.0 / 255.0);
vec3 backgroundColor = vec3(51.0 / 255.0, 124.0 / 255.0, 221.0 / 255.0);
...
bool hair0(out vec3 color) {
  float upper = circle(vec2(-1.06 + 0.01 * abs(sin(time + 2.)), 0.795), 0.64);
  float lower = circle(vec2(-0.851 + 0.01 * abs(sin(time + 2.)), -0.011), 1.001);
  float right = circle(vec2(-0.84 + 0.01 * abs(sin(time + 2.)), 0.74), 0.5);
  if (outside(upper) || inside(lower) || inside(right))
    return false;
  if (!inside(upper) || !outside(lower))
    color = borderColor;
  else {
    float upper = circle(vec2(-1.01 + 0.01 * abs(sin(time + 2.)), 0.74), 0.6);
    float lower = circle(vec2(-1.07 + 0.01 * abs(sin(time + 2.)), 0.13), 0.77);
...
bool hair1(out vec3 color) {
  float upper = circle(vec2(-0.84 + 0.01 * abs(sin(time)), 0.74), 0.5);
  float lower = circle(vec2(-1.8 + 0.01 * abs(sin(time)), 1.62), 0.9);
...
bool hair2(out vec3 color) {
  float upper = circle(vec2(-0.695 + 0.01 * abs(sin(time)), 0.74 + 0.01 * abs(sin(time))), 0.5);
  float lower = circle(vec2(-3.5 + 0.01 * abs(sin(time)), 8.46 + 0.01 * abs(sin(time))), 8.1);
  if (outside(upper) || outside(lower))
    return false;
  if (!inside(upper) || !inside(lower))
    color = borderColor;
  else {
    float left = circle(vec2(-0.675 + 0.01 * abs(sin(time)), 0.78 + 0.01 * abs(sin(time))), 0.502);
    if (outside(left))
      color = hairColor0;
    else {
      float right = circle(vec2(-1.77 + 0.01 * abs(sin(time)), 2.99 + 0.01 * abs(sin(time))), 2.4);
...
bool hair3a(out vec3 color) {
  float left = circle(vec2(-2.77 + 0.01 * abs(sin(time)), 1.86), 2.5);
  float right = circle(vec2(-1.544 + 0.01 * abs(sin(time)), 1.777), 1.44);
...
bool hair3b(out vec3 color) {
  float upper = circle(vec2(-2.654 + 0.01 * abs(sin(time)), 1.92), 2.39);
  float lower = circle(vec2(-1.44 + 0.01 * abs(sin(time)), 1.777), 1.388);
  float right = circle(vec2(-0.3, 0.5), 0.515);
...
bool hair4a(out vec3 color) {
  vec2 p0 = vec2(1.332 - 0.045 * abs(sin(time)), 0.8), p1 = vec2(-1.5, 0.36);
...
bool hair4b(out vec3 color) {
  vec2 p0 = vec2(0.504 - 0.045 * abs(sin(time)), 0.91), p1 = vec2(-1.7, 0.25);
...
bool hair5(out vec3 color) {
  float left = circle(vec2(1.13 - 0.01 * abs(sin(time)), -1.07), 2.02);
  float right = circle(vec2(-1.485 - 0.01 * abs(sin(time)), 1.15), 1.66);
  if (outside(left) || outside(right))
    return false;
  if (!inside(left) || !inside(right))
    color = borderColor;
  else {
    float upper = circle(vec2(1.205 - 0.045 * abs(sin(time)), 0.28), 1.58);
...
bool layer2(out vec3 color) {
  ...
  vec2 p2 = vec2(-0.42 + 0.01 * abs(sin(time)), -0.1);
...
bool leftEye2(out vec3 color) {
  if (outside(ellipse(vec2(-1.156+ 0.02 * (sin(time)), 0.435), vec2(-1.091, 0.75), 1.074)))
    return false;
  if (inside(ellipse(vec2(-1.156+ 0.02 * (sin(time)), 0.636), vec2(-1.122, 0.726), 1.16))) {
    color = eyeColor2;
    return true;
  }
  if (inside(ellipse(vec2(-1.121+ 0.02 * (sin(time)), 0.593), vec2(-1.116, 0.614), 1.168))
     || inside(ellipse(vec2(-1.113+ 0.02 * (sin(time)), 0.488), vec2(-1.111, 0.505), 1.23))) {
    color = eyeColor4;
    return true;
  }
  if (inside(ellipse(vec2(-1.154+ 0.02 * (sin(time)), 0.45), vec2(-1.1, 0.75), 1.05))
      && inside(ellipse(vec2(-1.163+ 0.02 * (sin(time)), 0.445), vec2(-1.121, 0.552), 1.3))
        && outside(ellipse(vec2(-1.13+ 0.02 * (sin(time)), 0.501), vec2(-1.116, 0.614), 1.177))) {
    color = eyeColor6;
    return true;
  }  if (inside(ellipse(vec2(-1.152+ 0.02 * (sin(time)), 0.445), vec2(-1.091, 0.75), 1.067))
      && inside(ellipse(vec2(-1.163+ 0.02 * (sin(time)), 0.445), vec2(-1.121, 0.565), 1.3))
        && outside(ellipse(vec2(-1.128+ 0.02 * (sin(time)), 0.52), vec2(-1.116, 0.614), 1.177))) {
    color = eyeColor5;
    return true;
  }
  color = eyeColor3;
...
bool rightEye3(out vec3 color) {
  if (outside(ellipse(vec2(-0.61 + 0.05 * (sin(time)), 0.29), vec2(-0.538, 0.5844), 1.23)))
    return false;
  if (outside(ellipse(vec2(-0.584 + 0.05 * (sin(time)), 0.355), vec2(-0.56, 0.518), 1.24))
      && inside(ellipse(vec2(-0.611 + 0.05 * (sin(time)), 0.31), vec2(-0.574, 0.42), 1.575))
        && inside(ellipse(vec2(-0.701 + 0.05 * (sin(time)), 0.09), vec2(-0.58, 0.38), 1.17))) {
    color = eyeColor6;
    return true;
  }
  if (outside(ellipse(vec2(-0.584 + 0.05 * (sin(time)), 0.367), vec2(-0.546, 0.518), 1.2))
      && inside(ellipse(vec2(-0.611 + 0.05 * (sin(time)), 0.299), vec2(-0.56, 0.48), 1.37))
        && inside(ellipse(vec2(-0.68 + 0.05 * (sin(time)), 0.09), vec2(-0.58, 0.42), 1.17))) {
    color = eyeColor5;
    return true;
  }
  color = eyeColor3;
...
bool rightEye4(out vec3 color) {
  if (inside(ellipse(vec2(-0.627 + 0.05 * (sin(time)) , 0.49), vec2(-0.589, 0.583), 1.35))) {
    color = eyeColor2;
    return true;
  }
  if (inside(circle(vec2(-0.576 + 0.05 * (sin(time)), 0.445), 0.01)) || inside(circle(vec2(-0.54 + 0.05 * (sin(time)), 0.32), 0.01))) {
    color = eyeColor4;
    return true;
  }
  return false;
...
bool mouth(out vec3 color) {
  if (outside(ellipse(vec2(-1.01+ 0.1 * abs(sin(time * 0.9 + 2.)), -0.04), vec2(-1.095, 0.02), 1.44)))
    return false;
  if (inside(ellipse(vec2(-1.01+ 0.1 * abs(sin(time * 0.9 + 2.)), -0.04), vec2(-1.095, 0.02), 1.4))
     && inside(circle(vec2(-1.04, -0.045)+ 0.1 * abs(sin(time * 0.9 + 2.)), 0.102))) {
    color = inside(ellipse(vec2(-1.0 + 0.1 * abs(sin(time * 0.9 + 2.)), -0.09), vec2(-1.092, 0.002), 1.34)) ? mouthColor2 : mouthColor1;
    return true;
  }
  color = mouthColor0;
  return true;
