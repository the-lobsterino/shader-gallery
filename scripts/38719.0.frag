...
void main( void ) {
	...
	if (length(position-mouse) < 0.01) {
		float rnd1 = mod(fract(sin(dot(position + 0.001, vec2(14.998,78.33))) * 4358.5453), 1.0);
		if (rnd1 > 0.5) {
			gl_FragColor = live;
		} else {
			gl_FragColor = blue;
		...
		float sum = 0.;
		sum += texture2D(backbuffer, position + pixel * vec2(0., 0.)).g;
