...
float noise (in vec2 st) {
    ...
    float a = random(i);
    float b = random(i + vec2(1.0, 0.10));
...
void main( void ) {
	...
	float time_factor = 9999.0;
	float time_ = time/((rndm/4.0)+time_factor);

	float time_step = posterize(time_, .0001);
	// resets every second
	float sub_time = (time_)-time_step;
	
	float motion_power = 6.0;
	float eased_time = ease(sub_time,motion_power);
	
	// changing pos	
	float current_x = random(vec2(time_step+y_step));
	float next_x = random(vec2(16.0+time_step+y_step));
	...
	if(in_rect == .0){
		...
		color.r = noise_val(rect_x, 3.0, 2.0, 2.0, 0.3, 4.0, 01.3);
		//color.r = color.r + noise_val(rect_x, 1.0, 5.0, 9.0, 0.3, 4.0, 0.56);
		
		color.g = noise_val(rect_x, 3.0, 1.0, 2.0, 0.3, 4.0, 0.4);
		
		color.b = noise_val(rect_x, 2.0, 2.0, 2.0, 0.3, 4.0, 01.31);
	...
	if(in_rect == 1.0){
		...
		color.g = noise_val(rect_x, 3.0, 1.0, 2.99, 0.3, 4.0, 0.4);
