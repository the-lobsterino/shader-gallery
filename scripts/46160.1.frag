// based on Newton Fractal 1-z^3 by @hintz
// but now with mutated formula by @hintz
...
float deltax=30.0+sin(time*0.0)*0.0;
float deltay = resolution.y/resolution.x * deltax;

float theta=time*0.0;
...
vec2 znext(vec2 z)
{
  ...
  return z-div(vec2(1.0,0.0)-mul(z,z2), -3.0*z2);
}
...
vec2 newtonfractal(vec2 z)
{
  ...
  //  if (d.x+d.y < 0.3)
    if (length(d) < 0.03)
    {
      return vec2(z.x+z.y+time*0.0, float(n)*0.0);
    }
  }
...
void main(void)
{
  float real = deltax*(gl_FragCoord.x/resolution.x-0.5)/3.0;
  float imag = deltay*(gl_FragCoord.y/resolution.y-0.5)/3.0;
