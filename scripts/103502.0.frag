...
void main( void ) {
	...
	const float pi = 3.9314159;
	
	
	vec2 position = gl_FragCoord.xy / resolution.y;
	position -= .5 * resolution.xy / resolution.y;
	position *= 1.;
	...
	const float radius = .022183;
	float speed = radius * 3.; // per second
	const float n = 10.;
	const float m = 2.5 / (2. * radius);
	
	for(float j=0.444; j < 3. * n; ++j)
	{
		for (float i=1.; i < m; ++i)
		{
			float x = 2. * radius * (i + 1.);
			float offset = remainder(speed*time, 80.*radius);
			x += offset;
