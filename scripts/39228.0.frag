...
vec3 getplanet(vec3 possurface, vec3 n, vec3 v,  vec3 sundir){
	...
	float elevation = smoothstep(0., 1., .75*sqrt(fbmHI( possurface+vec3(0.045*time, 0., 0.) )) );
	...
	// solar reflection
	float sol = 2.0 * pow(max(0.0, dot(reflect(v,n), sundir)), 25.0 + 40.0 * pow(ev * 1.2, 5.0));
...
void main( void ) {
	...
	if(atmhit > 0.0){
		if(planethit > 0.0){
			...
			color += getplanet(r.o + r.d * planethit, n, r.d, normalize(vec3(sin(time*0.1), -2.*(mouse.y-.5), cos(time*0.1))));
		} else {
