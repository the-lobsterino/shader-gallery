...
{	
    ...
    return sdBox(p+vec3(0,sin(1.6*time),0),vec3(20.0, 0.05, 1.2))-noise(8.*p+3.*bounce)+.4;
}
...
// 	bouncy cam every 10 seconds
    float wobble=0.;//(fract(.1*(time-1.))>=0.9)?fract(-time)*0.1*sin(30.*time):0.;
