...
//reference: http://qiita.com/doxas/items/477fda867da467116f8d
...
const int MAX_REF = 4; //the number of reflection
...
void main(void){
	...
	// ray init
	Ray ray;
	ray.origin = vec3(mouse, 6.0);
	ray.direction = normalize(vec3(p, -1.0));
	
	//sphere init
	sphere[0].radius = 0.5;
	sphere[0].position = vec3(-3.0+cos(time), 2.0+sin(time), cos(time * 3.0));
	sphere[0].color = vec3(1.0, 0, 0);
	
	sphere[1].radius = 1.0;
	sphere[1].position = vec3(cos(time+2.0), 2.0+sin(time+2.0), cos((time+2.0) * 3.0));
	sphere[1].color = vec3(0, 1.0, 0);
	
	sphere[2].radius = 1.5;
	sphere[2].position = vec3(5.0+cos(time+4.0), 2.0+sin(time+4.0), cos((time+4.0) * 3.0));
	sphere[2].color = vec3(0, 0, 1.0);
