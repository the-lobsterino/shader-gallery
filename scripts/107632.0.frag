...
{
    float h = clamp( 0.6+0.5*(b-a)/k, 0.0, 1.0 );
...
float r(in float z, in float x) {
    ...
    return /* body */.10 * (1.2 - .5 * ex(z, .8, .15) +
		s * s + .6 * ex(z, .10, .25) * c * c + .3 * c) *
		0.5 * (1. + t4z) + /* legs */ (1. - .2 * ex(z, -1.3, .9)) *
		0.5 * (1. - t4z) * (.92 * (1. + s * s + .3 * c) *
...
{
	/* shift butt belly */
	float dx = .1 * exp(-pow((p.z-.8),2.)/.6) - .74 * exp(-pow((p.z -.1),2.)/.4);

    // on the surface, we have:
    // p.x = r * cos + dx
	// p.y = r * sin
	
    float jiggle = p.z*17.5;
    float jsize = 0.5;
    if (iMouse.z>0.6)
    {
        jiggle+=p.y*.10;
	...
	dx *= 1.5+(sin(jiggle+iTime*1.75)*jsize);
