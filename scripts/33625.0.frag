...
#define darkmatter 0.1400
#define distfading 0.60
#define saturation 0.800


#define transverseSpeed zoom*(.0)
...
float field(in vec3 p) {
	...
	for (int i = 0; i < 16; ++i) {
		...
		tw += w;
		prev = mag;
		//if ( fract(time*100.0*sin(time*0.1)) > 0.99 ) break;
	}
...
void main()
{
	...
	uv /= 0.8*(1.0-(-5.9)*dot(uv,uv)); //*sin(time*0.1+0.5*sin(time*0.05));
	uv.x = pow(3.0,uv.x);
	uv.y = pow(3.0,uv.y);
