...
// Twist: 
vec3 opTwist( vec3 p, float angle, float amt )
{
  float  c = cos(amt*p.y + angle);
  float  s = sin(amt*p.y + angle);
  mat2   m = mat2(c, -s, s, c);
...
{
  return 0.5*sdTorus( opTwist(pos,angle, 10.0), vec2(0.20, 0.05));
}
//----------------------------------------------------------------------
...
vec2 map( in vec3 pos )
{
  ...
  res = opU(res,vec2(SphereSponge(opTwist((pos - vec3(0.0, 0.7, 0.0)), time*.01+pos.y*4., .0)), color));
  return res;
}
//----------------------------------------------------------------------
