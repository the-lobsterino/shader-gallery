/// ändrom3da - rainbow ring of circles
/// (without for loop) 
...
vec2 nCircles( vec2 u, float r, float num )  // ring
{
    float theta = atan( u.x, u.y ) + tau/2.;
    float nt = num/tau;
    float circleRadius = 3.*r*sin( .5/nt );
    ...
    return vec2 ( o, ( pn/tau + .5 )*float( repeatColors ) );
}
...
void main( void )
{
    ...
    // draw circles
    vec2 nCircles1 = nCircles( u, 1., circleNum );
    c += nCircles1.x*hsv2rgb( vec3( nCircles1.y, 1., 1. )  );
