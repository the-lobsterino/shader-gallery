...
void main( void ) {
	...
	float intensity = tan(gl_FragCoord.y) * tan(gl_FragCoord.x) * 1234.0/circle(mouse.x *resolution.x, mouse.y * resolution.y, 100.0);
	float blueness = 0.3 + 1000. * length(mouse - position) / length(resolution);
	float redness = 1. - blueness;
	gl_FragColor = intensity * vec4(redness, 5., blueness, 1.);
