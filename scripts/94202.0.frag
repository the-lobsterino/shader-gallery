...
vec3 bgColor = vec3(3.0, 0, 2.99);
vec3 rectColor = vec3(0.9, 0.2, 0.6);

const float noiseIntensity = 2.4;
const float noiseDefinition = 1.2;
const vec2 glowPos = vec2(-1., 1.);

const float total = 55.;
const float minSize = .56;
const float maxSize = 1.04;
const float yDistribution = 2.5;
...
float fbm(in vec2 uv)
{
    ...
    float f  = 0.5009*noise(uv); uv = m*uv;
...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
    ...
    for (float i=0.; i<total; i++){
        ...
        pos.y = sin(index*rnd*9999.+velY) * yDistribution;
