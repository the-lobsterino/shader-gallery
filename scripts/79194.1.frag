...
const float streetDistance = 0.3;
const vec3 streetColor = vec3(1.0, 0.0, 1.0);

const float fogDensity = 0.06;
const float fogDistance = 6.0;
const vec3 fogColor = vec3(1, 0.37, 0.4);
...
float hash1(vec2 p2) {
    ...
    p2 += dot(p2.yx, p2.xy + vec2(22.5351, 14.3137));
...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
    vec2 m = vec2(speed * iTime, 0.8);
    m *= tau * vec2(1.0, 0.25);
    
    vec3 center = vec3(0.1 * iTime, 0.1, 0.1);
    float dist = 35.0;
