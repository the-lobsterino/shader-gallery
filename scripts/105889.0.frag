...
void main()
{
	...
	float n1 = fbm(vec3((uv.xy * 12.0) + (time * 0.1),0.0));
	
	float n2 = fbm(vec3((uv.yx * 8.0) - (time * 0.03),0.0));
	float n21 = fbm(vec3((uv.yx * 16.0) - (time * 0.03),0.0));

	
	float edge1 = (abs(sin(time * 0.1)) * 0.05) + 0.45;
	float edge2 = mix(0.5, 0.5 + (sin(time * 30.0) * 0.01), pulse);
