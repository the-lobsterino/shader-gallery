/*
 * Original shader from: https://www.shadertoy.com/view/ssByzt
 */
...
void mainImage(out vec4 FragColor,vec2 FragCoord){
	...
	vec2 xa2=vec2(C2, -S2);
	vec2 ya2=vec2(S2, C2);
	vec2 shift = vec2( 0.03, 0.1);
	vec2 shift2 = vec2( -0.03, -0.22);
	float Z = 0.2 + mspt.y*0.1;
	float m = 0.99+sin(iTime*0.3)*0.1;
	for ( int i = 0; i < l; i++ ){
		float r = dot(v,v);
		float r2 = dot(vv,vv);
		if ( r > 1.0 )
