...
void main( void )
	...
	float lp = length( p*7. );
	p *= rot( lp + time*time/3. );
	float theta = atan( p.y, p.x );
        c += step( sin( theta*7. + sin( time/4. + 33.*p.x - 5.*cos( 5.*p.y*theta - 2.*time ) ) + 55.5 ), 0. );
	c *= circle( p, 1. );
        c *= hueShift( theta/tau )*vec3( 1. );
	gl_FragColor = vec4( c, 1. ); }////ändrom3da again lol let's accelerate a bit...