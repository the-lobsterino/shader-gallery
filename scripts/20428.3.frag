...
void main( void ) 
{
	vec2 uv		= surfacePosition*1.1+0.5;//gl_FragCoord.xy/resolution.xy;
	uv.x = uv.x * 0.5 + 0.25;
	ray r		= view(uv);
...
vec2 map(in vec3 position)
{
	vec3 rotation		= vec3(cos(pow(time, 0.1))*.03, sin(pow(time, 0.100123456789))*.03, 0.1) * (time*0.6+position.y*1.0);	
...
surface shade(in ray r, in surface s,  in material m, in light l)
{
	...
	float occlusion		= ambient_occlusion(r.position, s.normal);
    	float shadow		= shadow(r.position, l.direction);
