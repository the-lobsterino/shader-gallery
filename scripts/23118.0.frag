...
//Object A (tunnel)
float oa(vec3 q) { 
    vec3 r = mod(q,vec3(3.,3.,3.)) - 0.5 * vec3(3.,3.,3.);
    return length(r) - 0.5;
}   

//Object B (ribbon)
float ob(vec3 q) { return length(max(abs(q-vec3(cos(q.z*1.5)*.3,-.5+cos(q.z)*.2,.0))-vec3(.125,.02,time+3.),vec3(.0))); }
...
void main(void)
{
    ...
    vec4 c=vec4(1.0);
    vec3 org=vec3(0.,0.,time),
         dir=normalize(vec3(1.2*p.x,p.y,1.0)),
         pp;
    ...
    float d = .0;
    vec3 q = org;
    for(int i=0;i<64;i++) { d=o(q); q+=d*dir; }
