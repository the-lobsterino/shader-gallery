...
float map(vec3 p)
{
    ...
    vec3 v = vec3(sin(time/4.0), cos(time/4.0), sin(cos(time/8.0)*2.0));
    vec3 c = p;
...
void main( void )
{
	...
	c*=10.;
	c=1.-c;
    const float p0 = 0.0; const float p1 = 1.0472; const float p2 = p1*2.0;
    vec4 result = vec4(c,c,c, 1.0 );
    gl_FragColor = result;
}