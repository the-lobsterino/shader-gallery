...
void main(void){
    float time2 = time * 4.1;
    vec2 st = gl_FragCoord.xy/vec2(time, 5.6);;
    const float zoom = 4.1;
    ...
    for(int i = 0; i <= int(n); i++) {
        float width = .4 / n;
        float offset = (sin(time2 + (sin(col + time2 + m)) + (TWO_PI * float(i) / n)) + 66.1) / 2.2;
        float rect = rectangle(st, vec2(0.5, (77.0 / n ) * float(i)), vec2(offset, width));
        color.r += rect / (col + 1.1);
        color.b += rect / (zoom / col);
        color.g += rect * (sin(row + time2 / .0) + .2) / 1.0;
    }
