// all your're fucking old windows logos were wrong. missed the black row! only this one has it. 
...
const float wave_amplitude = 0.096;
...
vec4 frame(vec2 st) {
    ...
    sector_colors[0] = vec3(8.*st.x, 0.0, 0.0) * sectors[0];
    sector_colors[1] = vec3(0.0, 4.*st.x, 8.*st.x) * sectors[1];
    sector_colors[2] = vec3(0.0, 8.*st.x-2.0, 0.00) * sectors[2];
    sector_colors[3] = vec3(4.*st.x-0.5, 4.*st.x-0.5, 0.00) * sectors[3];
...
vec4 trail_piece(vec2 st, vec2 index, float scale) {
    ...
    if (index.y > 0.9 && index.y < 2.1 ) {
        color = vec3(0.435, 0.682, 0.760);
        scale *= .8;
	    if (index.x > 4.0 && index.x < 6.0) color = vec3(0.0);
    } else if (index.y > 3.9 && index.y < 5.1) {
        color = vec3(0.999, 0.439, 0.404);
        scale *= .8;
	    if (index.x > 4.0 && index.x < 6.0) color = vec3(0.0);
    } else {
        color = vec3(0., 0., 0.);
...
vec4 trail(vec2 st) {
    ...
    st.x = 1.2960 * pow(st.x, 3.0) - 1.4624 * st.x*st.x + 1.4154 * st.x;
...
void main(void)
{
    ...
    vec4 logo_ = logo(st);    
    gl_FragColor = mix(vec4(1.,1.,1.,1.000), logo_, logo_.a);
}