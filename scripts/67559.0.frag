...
const float overallSpeed = 7.2;
const float gridSmoothWidth = 0.10;
const float axisWidth = 10.15;
const float majorLineWidth = 1.025;
const float minorLineWidth = 0.0125;
const float majorLineFrequency = 3.0;
const float minorLineFrequency = 1.0;
const vec4 gridColor = vec4(0.5);
const float scale = 5.0;
const vec4 lineColor = vec4(1.25, .3, 1.0, 1.0);
const float minLineWidth = 0.02;
const float maxLineWidth = 0.5;
const float lineSpeed = 1.0 * overallSpeed;
const float lineAmplitude = 3.5;
...
{
    return   drawCrispLine(0.6, axisWidth, axis)
...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
    ...
    space.y += random(space.x * warpFrequency + iTime * warpSpeed) * warpAmplitude * (1.5 + horizontalFade);
