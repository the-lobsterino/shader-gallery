...
//#define SLOW_FACTOR SCALED_NOISE
...
//#define mouse vec2(time,1.0-time)
...
void FabriceNeyret2_mainImage( out vec4 O, vec2 u )
{
    ...
    float y = .5;
    float z = (time); // / SLOW_FACTOR);//*(TAU*TAU);
    float l = 0., d = l;
    y += -4.*M.y, z = -4.*M.x;
    D.xz *= rot(y); D.xy *= rot(z);
    p.xz *= rot(y); p.xy *= rot(z);
    ...
    for (int i = 0; (i < 32); i++)
    {
	d = SCENE_SDF( p );//transform(p, time ) );
        if ( hit = d < 1e-3 ) break;
	if ( (d > 5.9) ) break;
        p += d*D;
	D *= 0.95;
    ...
    //O.xyz = hit ? p :  vec3(1,0,1); // hmm coords
    //O.xyz = hit ? transform( p, time ) :  vec3(1,0,1);
    //O.xyz = hit ? transform(n, time)  :  vec3(1,0,1); // hmm coords
    //O.xyz = hit ? texture(iChannel0, n.xzy) : vec3(1,0,1);
    //O.xyz = hit ? texture(iChannel0, reflect(D,n).xzy) :  vec3(1,0,1);
    p = transform(p, time);
    O.xyz = hit ? scene( p, p ) :  vec3(1,0,1);
...
void main(void)
{
   ...
   float z = 4.0 + sin(time);//(mouse.x * mouse.y) / spdp + time*time;
#else
   float z = 2.0;
#endif
...
#if ANIMATE
   vec3 colorC = mix(colorA,colorB,abs(sin(time))); //(colorA + colorB) * 0.5;
#else
   ...
   vec4 o;
   FabriceNeyret2_mainImage( o, sp );
   float alpha = o.w;// * (1.4-color.b);
   colorC = mix( o.xyz, colorC, 1.0-alpha );
   //color = vec3(nfc.xy*color.r,100.0);
#endif
