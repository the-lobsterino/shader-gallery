...
#define time (500.0+time*sin(time+cos(time*0.2))*0.1)

const float position = 0.0;
float scale = 21.0 * fract(time);
...
void main( void ) {
	...
	surfacePosition = surfacePosition.yx*vec2(-6,1.5)+vec2(4.,0);
	vec3 color = vec3(01.5, 0.5, 1.0);
	color = color == vec3(01.0)? vec3(0.5, 0.5, 1.0) : color;
	...
	gl_FragColor.grb = mix( gl_FragColor.gbr, vec3( dot( vec3( 0.3, 0., 0.11 ), gl_FragColor.gbr ) ), sin(time)*step(sin(time*2.0),fract(time)) ).grb;
