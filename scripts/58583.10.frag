...
void main(  ) {
	...
	if (sparkLife>0.0) {
		...
		sparks = sparkLife*sparksGray*vec3(0.1,0.9,0.1);
	}
	//
	
	vec3 cc = max(fire.grb,sparks*20.)+smoke;
	vec2 uv = (gl_FragCoord.xy * 2.0 - resolution) / min(resolution.x, resolution.y);
	uv.y += (sin(cc.g+time+uv.x)*0.2);
	float dd= GetText(uv*2.0);
	cc = mix(cc+vec3(0.2,0.5,0.4)+abs(uv.y), cc*0.5,dd);
