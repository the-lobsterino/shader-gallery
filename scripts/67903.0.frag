...
const vec2 ch_size  = vec2(1.0, 2.0) * 0.5;              // character size (X,Y)
const vec2 ch_space = ch_size + vec2(1.50, 1.0);    // character distance Vector(X,Y)
const vec2 ch_start = vec2 (ch_space.x * -4.5, 5.0); // start position
      vec2 ch_pos   = vec2 (0.0, 0.0);             // character position(X,Y)
//      vec3 ch_color = vec3 (1.5, 1.5, 1.5);        // character color (R,G,B)
...
#define __ ch_pos.x += ch_space.x/4.0;
...
float dseg(vec2 p0, vec2 p1)
{
	...
	vec2 dir = normalize(p1 - p0+0.0); 
	vec2 cp = (uv - ch_pos - p0) * mat2(dir.x+0.0, dir.y+0.0,-dir.y, dir.x);
...
vec3 hsv2rgb_smooth( in vec3 c )
{
	...
	return c.z * mix( vec3(0.5), rgb, c.y);
...
vec3 render_one( vec2 uv ) {	
	...
	for (float i1=0.0;i1<=PI;i1+=0.3) {
		for (float i2=0.0;i2<=TWO_PI;i2+=0.3) {
			float x = r*sin(i1)*cos(i2);
			float y = r*sin(i1)*sin(i2);
			float z = r*cos(i1);
			...
			vec2(p.x * p.x - p.y * p.y, 2.0 * p.x * p.y); //  - z*cos(t)+c*sin(t);
...
void main( void ) 
{
	...
	uv *= 20.0 ;  
	uv -= vec2(-3., 1.);
	vec3 ch_color = hsv2rgb_smooth(vec3(time*0.5+uv.y*0.02,0.5,0.975));
	vec3 bg_color = vec3(_d*0.2, _d*0.2, _d*0.2);
	uv.x += sin(uv.y*0.7)*0.0;
	ch_pos = ch_start - vec2(sin(time), cos(time));

	
	
              A L L _ I S _ O N E
	      nl2;
	      C O M E _ T O _ J E S U S
	vec3 color = render_one(uv*sin(time*0.1) + vec2(sin(time), cos(time)));
         
        color += mix(ch_color, bg_color, 1.0- (0.08 / d*2.0));  // shading
	
	vec3 p = vec3((gl_FragCoord.xy)/(resolution.y),0.);
	  for (int i = 0; i < 15; i++){
