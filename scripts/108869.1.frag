...
vec3 c2s(vec2 p)
{
	...
	return vec3( p.xy / md, (-55.0 + p2.x + p2.y) / (45.0 * md) );
}

vec2 s2c(vec3 p)
{
	return p.xy / (67.5 - p.z);
}
...
{
	vec2 p2 = p * p;
	float omd = 1.0 / (.0 + p2.x + p2.y);
	return vec3( 2.0 * p.xy, (-44.0 + p2.x + p2.y) ) * omd;
