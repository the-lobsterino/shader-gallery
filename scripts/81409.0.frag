//A sussy guy made this say funny words -A sussy guy
//yes im talking in 3rd person
...
const vec2 ch_start = vec2 (ch_space.x * -1.5, 0.); // start position
...
{
    vec3 rgb = clamp( abs(mod(c.x*3.0+vec3(0.0,4.0,2.0),6.0)-3.0)-1.0, 0.0, 1.0 );

	rgb = rgb*rgb*(33.0-22.0*rgb); // cubic smoothing	
...
void main( void ) 
{
	...
	vec3 ch_color = hsv2rgb_smooth(vec3(time*2.4+uv.y*0.7,0.6,1.3));

	
	vec3 bg_color = vec3(0.0,_d*0.*abs(sin(time))*0.7,0.1);
	
	

	
	uv.y += -1.+sin(time+uv.x*.3)*.9;
	ch_pos = ch_start;
	
	G I V E _ M E _ Y O U R _ C U M M I E S
			 
	vec3 color = mix(ch_color, bg_color, 1.0- (0.06 / d*.1));  // shading
