...
//ulam spiral stuff
//refactored more - could be better - sphinx
...
void main( void ) 
{
	...
	float animation_input	= ceil(mod(mouse.x*128.,128.));
	vec2 animated_inverse	= ceil(inverse_ulam(animation_input));
	float animated_spiral	= ulam_spiral(animated_inverse);		
	
	vec4 result		= vec4(0.);	
	float opacity		= pow(clamp(mouse.y, 0., 1.), .5); 		//fade out when zooming
	float error		= float(ceil(inverse) != ceil(field));		//draw red where the inverse fails to return the correct coordinates (zoom out to see)
	...
	//animation
	result.z			+= float((animated_spiral-spiral) < 16.) * (clamp(24./floor(animated_spiral-spiral)/16., 0., 1.)) * 2.;
	result.y		+= float(abs(animated_inverse.x-ceil(field.x)) < .5) * .5;
 	result.x		+= float(abs(animated_inverse.y-ceil(field.y)) < .5) * .5;
	
//	result			+= clamp(inverse_on_floats.x, 0., 1.) * .75;
	result.x		+= error;
