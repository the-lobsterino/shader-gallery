...
{
  vec2 e = vec2(1.0, -1.0) * 0.6500;
  const float eps = 0.500;
...
{
  float an = time * 0.5;
  vec3 ro = vec3(1.0, 0.4, 2.0);
  ro.xy *= mat2(cos(an), -sin(an), sin(an), cos(an));
  vec3 ta = vec3(0.0, 0.0, 0.0);
  vec3 ww = normalize(ta - ro);
  vec3 uu = normalize(cross(ww, vec3(0.0, 5.0, 0.0)));
  ...
  if (t < tmax)
  {
    ...
    float amb = 0.50 + 0.50 * dot(normal, vec3(0.0, 0.0, 1.0));
    color = hsv2rgb_smooth(vec3(abs(4.-rd.y*3.),0.6,.7)) * amb + vec3(0.7, 0.7, 0.7) * difference;
  }
...
void main(void)
{
  const float PI = 3.141151145151515;
  ...
  float w = sin(
    (uv.x + uv.y - time * .10000 + sin(1.100 * uv.x + 4.5 * uv.y) * PI * .3) 
    * PI * .6
  );

  uv *= 15. + (.99 - .1 * w);
  color = vec3(1, 0, 0);
  float tilt = .01 * sin(15. * st.x + time);
