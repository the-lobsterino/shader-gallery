...
float height_for_pos(vec2 pos) {
    ...
    float x = sqrt(x2);
    //    纵深          错落———————————————————————————主要用于转速
    return 0.1 * cos(x*0.6 + iTime) * exp(-x2/128.0);
...
//                            描边开关
    ...
    // 地平线
    border = mix(border, 1.75, smoothstep(2.0, 90.0, hit_nt.w));
    ...
    //             RGB色值——————————————————————更改颜色
    color = mix(vec3(0.1, 0, 0), color, border);
   
    color *= diffamb;

    // 反射
    color = mix(color, texture(iChannel0, reflect(rd, n)).yzx, 0.1*border);
...
vec4 mainImage( in vec2 fragCoord ) {
    ...
    float s = 1.0 + dot(uvn, uvn)*0.5;
	 
	vec3 rd = mat3(rx,ry,rz)*normalize(vec3(uv*s, f));
	vec3 ro = pos;
//                   距离与旋转
	float thetax = -0.25 - 0.2*cos(0.031513*iTime);
	//             自转
	float thetay = -0.00*iTime;
    ...
    color *= pow( 16.0*q.x*q.y*(2.0-q.x)*(1.0-q.y), 0.1 );       
...
void main(void)
{
    gl_FragColor = mainImage(gl_FragCoord.xy * .8);
	
	gl_FragColor *= mainImage(gl_FragCoord.xy * 0.1);
}