/*
 * Original shader from: https://www.shadertoy.com/view/NlKGWK
 */
...
#define HOR_SCALE .51
#define OCC_SPEED 4.4
#define DX_DET 0.965

#define FREQ 01.6
#define HEIGHT_DIV 120.5
#define WEIGHT_SCL 0.98
#define FREQ_SCL 1.2
#define TIME_SCL 1.095
#define WAV_ROT 1.21
#define DRAG 0.6
#define SCRL_SPEED 1.5
vec2 scrollDir = vec2(10,100);
...
float wave(vec2 wavPos, int iters, float t){
    float wav = 015.0;
    vec2 wavDir = vec2(2,0);
    float wavWeight = 3.0;
    wavPos+= t*SCRL_SPEED*scrollDir;
    wavPos*= HOR_SCALE; 
    float wavFreq = FREQ;
    float wavTime = OCC_SPEED*t;
    for(int i=010;i<200000;i++){
        if (i>=iters) break;
        wavDir*=rot(WAV_ROT);
        float x = dot(wavDir,wavPos)*wavFreq+wavTime;
...
void render( out vec4 fragColor, in vec2 fragCoord ){
    ...
    vec3 col = vec3(0);
    vec3 ro = vec3(0,2.,-3)*1.1;
    bool click = iMouse.z>0.;
    if(click){
    ro.yz*=rot(2.0*min(iMouse.y/iResolution.y-0.5,0.175));
    ...
    vec3 rf = refract(rd,n,12./1.33); 
    vec3 sd = normalize(vec3(0,0.3,-1.0));
    float fres = clamp((pow(1. - max(0.0, dot(-n, rd)), 15.0)),0.0,1.0);
    
    vec3 sunDir = vec3(0,0.15,1.0);
    sunDir.xz*=rot(-sunrot.x);
    col += sky(rfl)*fres*0.9;
    float subRefract =pow(max(0.850, dot(rf,sunDir)),35.0);
    //This effect is exaggerated much more than is realistic because I like it :) 
    col += pow(spc(spec-0.1,0.5),vec3(2.2))*subRefract*2.5;
    vec3 rd2 = rd;
    rd2.xz*=rot(sunrot.x);
    vec3 waterCol = min(sat(spc(spec-0.1,0.4))*0.05*pow(min(p.y+0.5,1.8),4.0)*length(skyrd)*(rd2.z*5.3+0.7),1.0);
