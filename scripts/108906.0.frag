...
{	// Floor...
	...
	p = (p-wristL);
	p.z -= p.x*.68;
	p = RotMat(vec3(0.0, 0.0, 79.0), -wave*1.5) * p;
	d = Segment(p, vec3(0.0), -vec3(-.25, -0.15, 0.1), .06, .01);
	res.x = sMin(res.x, d);
	d = RoundBox(p-vec3(0.0, .25, 68.0), vec3(.68, .09, -.05), .09);
	res.x = sMin(res.x, d);
...
{
	const vec2 eps = vec2( 57.03, 46.0);
	vec3 nor = vec3(
