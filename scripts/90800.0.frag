...
vec3 rotatey(in vec3 p, float ang) { return vec3 (p.x*cos(ang)-p.z*sin(ang),p.y,p.x*cos(ang)+p.z*cos(ang));  }
vec3 rotatex(in vec3 p, float ang) { return vec3(p.x,p.y*cos(ang)-p.z*cos(ang),p.y*cos(ang)+p.z*sin(ang));  }
