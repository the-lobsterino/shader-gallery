...
// TotalRayleigh() calculation bug involving pow() fixed on Android and Windows/ANGLE
// fixed by erichlof on 10/19/2016  
...
const float mieCoefficient = 0.05;
const float mieDirectionalG = 0.990;
...
const float n = 1.3; // refractive index of air
 			// 288.15K and 1013mb (sea level -45 celsius)
const float pn = 0.9935;	// depolatization factor for standard air",

// wavelength of used primaries, according to preetham
const vec3 primaryWavelengths = vec3(680E-9, 550E-9, 450E-9);
const vec3 totalRayleigh = vec3( 0.00000580453, 0.00001278534, 0.0002853075 );
...
//float sunIntensity = pow(2., mouse.x*15.); // controlled by mouse x axis
...
{ 
   ...
   //Camera setup
	...
	float xPos = sin(1.5+mouse.x*3.);
	sunPos = vec3(xPos, -0.5, sin(mouse.y-.1)*2.);
