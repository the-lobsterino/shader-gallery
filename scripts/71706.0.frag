// author --> cskonopka
// title ---> "interwovenrelationshipsOfStressAndIntuition"
// link ----> http://glslsandbox.com/e#71696.1
...
void main() {
    ...
    float y3 = sin(cos((st.y)*(.10)))-sin(st.x*0.12);
    ...
    vec3 colorA = vec3((y*y2)+(y3*(sin(time*.0013))));
    colorA = (1.0)*colorA*vec3(.90,0.920,(100.0+(sin(y3+(time*0.0012)))));

    float y4 = cos(cos((st.y-st.x)*(sin(time*0.001913))*cos(0.43*st.x+st.y))) * cos(cos((-st.y+st.x)*(sin(time*-.001913))*sin(time*.00821)*cos(0.3)));
    ...
    colorMix = mix(colorA, colorB, ((sin(((y6-y2*(st.y-st.x))*-0.174)-sin(((-y6-(y2+st.y))*0.74))+sin((-cos(pct2*-0.92)))))));

    gl_FragColor = vec4(colorMix,5.0);
}