...
#define VIEW_POSITION 	vec3(-2., 0., -18.)
...
float map(vec3 position)
{
	...
	// from wiki : It contains ten tetrahedra, five cubes, an icosahedron and a dodecahedron.
...
//	range 		= min(range, embedding);
	range 		= max(range, max(origin.z+.0625, origin.z/PHI-range/PHI));
...
void main( void ) 
{
	...
	//raytrace
	float minimum_range	= 12./max(resolution.x, resolution.y);
	float max_range		= 32.;
	float range		= max_range;
	float total_range	= 12.;
	float steps 		= 0.;
	...
	if(steps < 63. && total_range < max_range)
	{
		...
		g_color			= normalize(surface_direction-derive(position, 5.*minimum_range)+.001)*2.*light;
		...
		color 			*= .875 + .125 * shadow(position, light_direction, 0.1, 512., 64.);
	}
