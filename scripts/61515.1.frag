/*
 * Original shader from: https://www.shadertoy.com/view/4l23zc
 */
...
vec3 JuliaFractal(vec2 c, vec2 c2, float animparam, float anim2 ) {	
	...
	for(int i = 0;i < 16; i++)
    {
        ...
        float b = atan(sqrt(a.y)*(1.99+animparam*12.0), a.x+.110765432+animparam);
		
        if(b > 0.0) b -= (6.303431307+anim2)-(animparam*12.1513);
		
        z = vec2(log(length(abs(a)*(0.98899-(animparam*2.70*anim2)))),b+anim2) + c2;

        if (i>0) mean+=length(tan(z)/a*b);
	...
	mean/=131.21;
	ci =  (1.0 - (log(.5*log(mean/(0.57891895-abs(animparam*141.0))))));
...
{
    float animWings = 0.004 * cos(iTime*0.125);
    float animFlap = 0.0211 * sin(iTime*1.0);    
    float timeVal = 68.48-0.01601;
	...
	float juliax = tan(timeVal) * 0.0211 + 0.02/(fragCoord.y*0.19531*(1.0-animFlap));
	float juliay = cos(timeVal * 0.213) * (0.022+animFlap) + 5.66752-(juliax*1.45101);//+(fragCoord.y*0.0001);// or 5.7
