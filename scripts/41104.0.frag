...
#define _g col+=char(vec2(244.0,647356.0),uv)*text_color;
...
vec3 Text(vec2 uv)
{
	...
	print_pos += vec2(cos(time)*7.,sin(time*sqrt(2.))*7.);
	
	RGB(sin(92.*time)/2. +0.5, -sin(85.*time)/2. +0.5, cos(73.*time)/2. +0.5) _I _apo _m _ _g _a _y _ _amp _ _I _ _d _o _n _apo _t _ _l _i _k _e _ _t _r _a _p _s _per
