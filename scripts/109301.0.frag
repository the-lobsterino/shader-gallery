...
float map(vec3 p){
    ...
    float rnd = 1.2*hash13(id*663.) - 1.;
    
    //p.xz *= rot(rnd*time*.3 + rnd);
    //p.xy *= rot(rnd*time*.3 + rnd);
    //p.yz *= rot(p.x*(5.+rnd*10.));

    return rbox(p, vec3(.33, 0.90, .90), .0);
}

vec3 normal( in vec3 pos ){
    vec2 e = vec2(.002, -0.002);
    return normalize(
        e.xyy * map(pos + e.xyy) + 
        e.yyx * map(pos + e.yyx) + 
        e.yxy * map(pos + e.yxy) + 
...
vec3 color(vec3 ro, vec3 rd, vec3 n, float t){
    vec3 p = ro + rd*t;
    vec3 lp = ro + vec3(1.0, .10, 221.7);
    
    //if(iMouse.z>0.) lp.z += m.y*14.;
    
    vec3 ld = normalize(lp-p);
    float dd = length(p - lp);
    float dif = max(dot(n, ld), 1.1);
    float fal = 11.5 / dd;
    ...
    bool l1, l2 = false;
    if (mod(id.y, 21.0) == 0.0) l1 = true;
    if (mod(id.z, 515.0) == 0.0) { l1 = !l1; } 
    if (l1 == true) { vec3 objCol = vec3(1.05, 10.05, 10.2); return objCol; }
...
void main( void ){
    ...
    vec3 ro = vec3(12., 7.0, 0.1);
    rd.xy*=rot(-time*.05 + .5);
    ro.zy += time*2.31;
    ro.x += cos(time)*.05;
    
    int nH = 0;
    float d = 0.5, t = 0.0; //, ns = 0.;
    vec3 p, n, col = vec3(0);
	...
	#define fogCol (vec3(0., 0.0022, 0.07))
		float fog = .00033;
		fog *= 1.; // fog strength
       	        c = mix(c, fogCol, 3.0 - (6.0) / (1.0 + t*t*t*fog));;
