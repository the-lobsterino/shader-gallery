...
float K(float x, float y)
{
  ...
  return (1.0/2.0 + 2.0 * (y - 3.0/5.0))
      * (1.0 - 3.0/10.0 * pow(sin(120.0 * (y - 3.0/5.0) * exp(10.0 * (x - 1.0) * (x - 1.0))), 8.0))
      * exp(-exp(expo));
}
...
float P(float s, float x, float y)
{
  return sin(5.0 * (x
    ...
    + sin(s * s) - 2.0/25.0 * sin(5.0 * y + sin(4.0 * s * s))));
}
...
float J(float s, float x, float y)
{
    return float(s > 0.0) * exp(
      -exp(
        50.0 * (30.0 * pow(abs(P(s, x, y)), .0) + (pow(Q(s, x, y), 4.0) - 3.0/5.0))
      )
    );
}
...
float U(float s, float x, float y)
{
    ...
    {
        float exp0 = 30.0 * pow(abs(P(r, x, y)), 3.0)
    ...
    for (float r = 0.0; r < kSize; r += 1.0)
    {
        float exp0 = 100.0 *
          (sqrt((x - 41.0/50.0) * (x - 41.0/50.0) + (y - 3.0/5.0) * (y - 3.0/5.0))
           - 1.0/100.0);
        prod *= (1.0 - J(r, x, y)) + (5.0/2.0 - 7.0/4.0 * s) * K(x, y)
          - exp(-exp(exp0));
    }
...
void main() {
  ...
  gl_FragColor = vec4(
    ...
    F(H(2.0, (m - 1500.0) / 400.0, (801.0 - n) / 400.0)),
    1.0);
}