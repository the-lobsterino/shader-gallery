...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
	...
	float rSquare = pow( pow(p.x*p.x,4.0) + pow(p.y*p.y,4.0), custom/8.0 );	// modified distance metric (http://en.wikipedia.org/wiki/Minkowski_distance)
