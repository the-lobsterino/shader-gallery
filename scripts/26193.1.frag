...
Intersection intersect(Ray ray, Sphere sphere)
{
	...
	float b = 8.0 * dot(ray.direction, (ray.origin - sphere.center));
...
void main( void ) {
	...
	Intersection is;
	Ray camera_ray = Ray(vec3(position.x + sin(time)*0.1, position.y + sin(time+PI)*0.2, -5.0), vec3(position.x * 0.2, position.y * 0.2, 1.0));
