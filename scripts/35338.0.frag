...
// originale here : https://www.shadertoy.com/view/XtBXRm
...
void main()
{
	...
	vec3 cv=vec3(0,0,0); //Change camere view here
    	vec2 uv = (gl_FragCoord.xy*2.-si)/min(si.x, si.y);
	...
	if (d.x<DPrec.y)
		...
		f.rgb = vec3(pow(rnd.y,10.0));
	}
	float a = atan(uv.x,uv.y);
	gl_FragColor = f + (.3+.1*sin(a*10.+time))/length(uv);
}
