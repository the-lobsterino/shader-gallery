...
void main(void) {
	...
	const int numVertices = 35;
	float cylinderRadius = 0.36;
	...
	for (int j=0; j<numRings; j++) {
		for (int i=0; i<numVertices; i++) {
			...
			// make a flower-like thing by varying the radius over the angle
			float radius = cylinderRadius * (1. + 0.4 * cos(float(i)*pi2*0.14 + float(j) * time*3.));
			
			vertices[j * numVertices + i] = vec4(radius*cos(0. + float(i) * step), -.3 + float(j) * (.35 / float(numRings)), radius*sin(0. + float(i) * step), 1.);
