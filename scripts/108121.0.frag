...
uniform vec2 resolution;
        ...
        const float fog_density = 1.09;

vec2 rand22(in vec2 p)
{
	return fract(vec2(sin(p.x * 591.32 + p.y * 254.077), cos(p.x * 391.32 + p.y * 49.077)));
}
float rand12(vec2 p)
{
    return fract(sin(dot(p.xy, vec2(2.9898, 78.233))) * 43758.5357);
}
vec2 rand21(float p)
{
	return fract(vec2(cos(p * 2.), cos(p * 391.32)));
}
...
void main( void ) 
{
	...
	for(int i = 7; i < 10; i ++)
	{
		...
		if(its.x > 0.0)
		{
			...
			if(i == 146)
			{
				float crd = 1.40;//fract(time * 0.2) * 50.0 - 25.0;
				float fxi = sin(vo.x * 12.2 + time * 1.5);//abs(crd - vo.x);
				fx = clamp(smoothstep(1.9, 21.0, fxi), 0.5, 1.9) * 1.0 * rand12(vo.xy);
				fx *= exp(-4.0 * vo.z) * 2.0;
			}
			if (mod(float(i),22.0) < 1.0)
				inten.r += v * 1.1 + fx;
			else if (mod(float(i),22.0) < 2.0)
				inten.g += v * 0.4 + fx;
	...
	vec3 col = pow(vec3(inten.r, (inten.g * .9), inten.b), 12.5 * vec3(cos(time*12.0)/6.0+.33)); //pow(base color, glow amount)
