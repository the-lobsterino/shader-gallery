...
float H(float s, float x, float y)
{
	...
	float xFish = x + sin(time) * 0.05;
	float yFish = y + sin(time) * 0.1;
        float exp0 = 100.0 *
          (sqrt((xFish - 41.0/50.0) * (xFish - 41.0/50.0) + (yFish - 3.0/5.0) * (yFish - 3.0/5.0))
           - 1.0/100.0);
  
    return (1.0 + pow(-1.0, s) * s / 2.0) * U(s, x, y)
      + (1.0/10.0 + s / 4.0 + (y - x*x/3.0 - 1.0/2.0) *
         (1.0/25.0 + 17.0/300.0 * (s*s - s))) * prod
	     + (5.0/2.0 - 7.0/4.0 * s) * K(xFish, yFish)
          - exp(-exp(exp0));
}
