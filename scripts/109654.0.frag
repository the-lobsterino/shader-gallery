...
mat3 rotM(vec3 axis,float angle){
  ...
  axis = normalize(axis);
	...
	oc * axis.x * axis.y + axis.z * s, oc * axis.y * axis.x + c, oc * axis.y * axis.z - axis.x * s,
...
{
  float da = sdBox(p,vec3(9100.,1.,1.));
...
float map(vec3 p){
	...
	float dc = tan(time*rand(index)*4.-rand(index)*0.4);
	...
	d = min(d,sdCapsule(q,vec3(2.,0.,h+dc),vec3(-c.x,0.,noise(left)+dleft),0.01));
...
map(p + vec3(d, 1.5, 0.0)) - map(p + vec3(-d, 0.0, 0.0)),
...
gl_FragColor = mix(gl_FragColor, (texture2D(backbuffer, gl_FragCoord.xy / resolution.xy) + texture2D(backbuffer, gl_FragCoord.xy / resolution.xy + vec2(2.001, 0.001)) +texture2D(backbuffer, gl_FragCoord.xy / resolution.xy + vec2(-0.001, -0.001))) / 3.0, 0.5);
}