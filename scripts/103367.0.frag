...
Sphere lightSourceVolume = Sphere(20., vec3(0., 0., 0.), vec3(12.), vec3(0.), DIFF);
Sphere sphere(int i) {
	if (i--==0) return	Sphere(1e5, vec3(-1e5-49., 0, 0),	vec3(0.), vec3(.75, .25, .25), DIFF);
	if (i--==0) return	Sphere(1e5, vec3( 1e5+50., 0, 0),	vec3(0.), vec3(.25, .25, .75), DIFF);
	if (i--==0) return	Sphere(1e5, vec3(0., 0, -1e5),		vec3(0.), vec3(.75), DIFF);
	if (i--==0) return	Sphere(1e5, vec3(0., 0,  1e5+81.6),	vec3(0.), vec3(0.), DIFF);
	if (i--==0) return	Sphere(1e5, vec3(0., -1e5-40.8, 0),		vec3(0.), vec3(.75), DIFF);
	if (i--==0) return	Sphere(1e5, vec3(0.,  1e5+40.8, 0),	vec3(0.), vec3(.75), DIFF);
	if (i--==0) return	Sphere(16.5, vec3(-23., -24.3, 34.6), 		vec3(0.), vec3(1.), SPEC);
	if (i--==0) return	Sphere(16.5, vec3(23., -24.3, -3.6), 		vec3(0.), vec3(.7, 1., .9), REFR);
	return 				Sphere(600., vec3(0., 640.43, 0),	vec3(12.), vec3(0.), DIFF);
}
...
void main(void) {
	...
	vec3 color = vec3(0.);
	vec3 castDir = normalize(vec3(uv, 1. / tan(radians(90. / 2.))));
	castDir *= rotationMatrix;
	for (int i = 0; i < SAMPLES; ++i)
		color += radiance(Ray(camPos, castDir));
	float gamma = 2.2;
	color = (color / float(SAMPLES) + pow(previous.rgb, vec3(gamma)) * 29.) / 30.;
	gl_FragColor = vec4(pow(clamp(color, 0., 1.), vec3(1./gamma)), 1.);
}
