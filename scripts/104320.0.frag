...
#define text() _spc _T _h _o _r _i _u _m _spc _C _l _i _e _n _t
...
vec3 Text(vec2 uv) {
    ...
    vec3 colorA = vec3(0, 1, 0.894);
    vec3 colorB = vec3(0.529, 0.027, 0.631);
	
    vec2 position = (gl_FragCoord.xy / resolution.xy);
	
    float pct = abs(sin(time + position.x + position.y));
    vec3 rgb = vec3(mix(colorA, colorB, pct));

    vec3 col = vec3(0.0);

    vec2 center_pos = vec2(res.x/2.0 - STRWIDTH(15.5)/2.0,res.y/2.0 - STRHEIGHT(1.0)/2.0);
    if (center_pos.x == -1.) {
...
vec3 explosion(vec2 uv, vec2 p, float seed, float t) {
    ...
    for(float i=0.; i<15.; i+=.7) {
        ...
        sparkle = pow(sparkle, pow(en.x, 2.)*50.)*mix(0.01, .01, en.y*n.y);
      
    	//size += sparkle*B(.6, 1., .1, t);
        size += sparkle*B(en.x, en.y, en.z, t)/2.;
        
        col += rgb*light(uv, pos, size);
    }
...
void main( void ) {
    ...
    vec3 col = pixel*1.9+0.075;
    col *= (1.-distance(mod(uv,vec2(1.)),vec2(0.5)))*0.8;
	
    for(float i=0.; i<1.; i++) {
	float time = time/6.;
    	float et = time+i*1234.45235;
        float id = floor(et);
        et -= id;
