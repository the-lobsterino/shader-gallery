...
void main( void ) 
{
	...
	float dy = 0.000+gl_FragCoord.y-resolution.y/2.0;
	float len = sqrt(dx*dx+dy*dy);
	float ds = len/longest;
	float md = time*2.0;
	
	float ang = 3.0*atan(dy,(len+dx));
	ang += pow(len, 2.5)*5.5;
	
	float red = (452.0 - sin(ang + md*3.141592*2.0) * 127.0)*(1.0-ds);
	float green = (18.0 - cos(ang + md*3.141592*2.0) * 127.0)*(1.0-ds);
	float blue = (28.0 + sin(ang  + md*3.141592*2.0) * 127.0)*(1.0-ds);
