...
const float mosaic      = 22.; // mosaic (even integers only)
const float spriteSize  = .271; // sprite size (0. to ~.5)
const float offsetSize  = .094;  // size of noise for mask and pattern offsets
const float patternSize = .2513;  // size of noise for pattern
const float patternOffs = 2.17; // amount pattern can be perturbed
const float maskOffs    = .4312;  // amount mask can be perturbed

// ~changeables


#define patternOffs fract(time)
//#define patternSize fract(time)
...
void mainImage( out vec4 fo, in vec2 fc ) {
    ...
    p = surfacePosition; // zoom/pan support
