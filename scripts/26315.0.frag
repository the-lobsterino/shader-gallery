...
float EPSILON_NRM	= 3.*cos(time*0.004-sin(time*0.002342*cos(time*0.0023))*0.23+0.02551 / resolution.x);
...
const float SEA_CHOPPY = 4.0;
 float SEA_SPEED = sin(time*0.0025)+0.8;
 float SEA_FREQ = sin(0.5*cos(time*0.01)*0.01)*.001*time+0.16;
const vec3 SEA_BASE = vec3(0.1,0.1,0.1);
vec3 SEA_WATER_COLOR = vec3(sin(0.0023*time+0.85),cos(time*sin(time*0.0023)+0.9),0.6);
const float SKY_INTENSITY = 1.0;
//float ;

varying vec2 surfacePosition;
#define SEA_TIME (.001*time) * SEA_SPEED
...
void main(void) {
	vec2 uv;
	vec2 surfacePosition = surfacePosition*1.5;
	uv = surfacePosition;
	uv += 3e-1*(pow(length(surfacePosition), -0.5));
	uv += 3e-1*(pow(length(surfacePosition+.33*vec2(cos(time), sin(time))), -0.5));
	uv += 3e-1*(pow(length(surfacePosition+.33*vec2(cos(time*1.00123456789), sin(time))), -0.5));
	uv += 3e-1*(pow(length(surfacePosition+.33*vec2(cos(time*1.00123456789), sin(time*1.00123456789))), -0.5));
	uv += 3e-1*(pow(length(surfacePosition+.33*vec2(cos(time), sin(time*1.00123456789))), -0.5));
	uv.y *= -1.;
	//uv.y += -2.;
    //float time = time * 0.3;
