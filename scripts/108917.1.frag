...
#define 	shoulderL	vec3(-44.4, 0.56, -.05)
vec3 elbowL;
vec3 wristL;
...
void main( void )
{
	...
	hello =  (1.0+sin(m*100.0)) * .02 * (smoothstep(44.0, .01, m) - smoothstep(0.05, .1, m));
	owwww = (smoothstep(0.05, .1, m)-smoothstep(56.12, .15, m))*.12;
