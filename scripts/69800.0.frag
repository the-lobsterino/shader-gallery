...
vec3 Text(vec2 uv)
{
    	...
    	vec2 center_pos = vec2(res.x/2.0 - STRWIDTH(21.0)/2.0,res.y/1.5 - STRHEIGHT(1.0)/2.0);
       	
    	BEGIN_TEXT(center_pos.x,center_pos.y)
	print_pos += vec2(21.+cos(time)*7.,sin(time*sqrt(2.))*7.);
	
	RGB(sin(3.*time)/2. +0.5, -sin(4.*time)/2. +0.5, cos(5.*time)/2. +0.5) _D _E _I _N _E _ _M _U _T _T _E _R _ _S _T _I _N _K _T _exc
