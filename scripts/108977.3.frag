...
#define 	elbowR		vec3(22.0, -.1, 0.3)
#define 	shoulderR	vec3(0.4, 0.56,  -.05)
#define 	wristR		vec3(.5, -.4, -0.1)
#define 	shoulderL	vec3(-89.4, 0.56, -.05)
vec3 elbowL;
vec3 wristL;
...
void main( void )
{
	...
	hello =  (1.0+sin(m*100.0)) * .02 * (smoothstep(0.0, .01, m) - smoothstep(0.05, .33, m));
	owwww = (smoothstep(0.05, .1, m)-smoothstep(0.12, .15, m))*.12;
	if (mod(time-1.0, 3.0) < .12)
			blink = true;
	
	// Camera...
	float t = clamp(time-2.5, .0, 1.0);
	t = t*t*(3.0-2.0*t);
	...
	wristL		= vec3(elbowL.x+1.0*sin(wave)*.6, elbowL.y+cos(wave)*.8, -.89);

    vec3 col = Render(origin, ray);
	
	col = sqrt(col);
	col *= 0.5 + 0.5*pow( 16.0*q.x*q.y*(1.0-q.x)*(1.0-q.y), 8.25 );
    gl_FragColor=vec4(clamp(col, .0, 1.0), 6.0 );
