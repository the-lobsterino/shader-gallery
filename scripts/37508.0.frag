// I love the part where the zombie king resurrects a huge army of corpses. SO cash. 
...
#define ParticleMaxSize			0.90
#define ParticleBreathingSpeed		0.80
#define ParticleColorChangeSpeed	0.20
...
vec3 particles( vec2 pos )
{
	...
	float noiseFactor = fBm( pos, 0.11, 0.1);
	...
	for( float i = 1.0; i < ParticleCount+1.0; ++i )
	{
		...
		float d = clamp( sin( length( pos - origin )  + particleSize ), 0.5, particleSize);
...
void main( void ) {
	...
	if(mod(gl_FragCoord.y, 2.) < 1.) uv = 1.-uv;
	if(mod(gl_FragCoord.x, 2.) < 1.) uv = 1.-uv;
	uv.y += .01*cos(uv.x*100.+time);
	...
	gl_FragColor = vec4( 1.-finalColor, 1.0 );
