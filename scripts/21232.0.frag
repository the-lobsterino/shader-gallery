...
void main( void ) {
	...
	gl_FragColor.rgb = vec3(0.0, 0.4, 0.6)+vec3(1.0, 0.5, 0.6) * (pow(dist, -0.9) * 0.04);
	for (float ray = 0.0; ray < 10.0; ray += 1.0) {
		//float rayang = rand(5234, ray)*6.2+time*5.0*(rand(2534, ray)-rand(3545, ray));
		float rayang = rand(5234, ray)*6.2+(time*0.05)*10.0*(rand(2546, ray)-rand(5785, ray))-(rand(3545, ray)-rand(5467, ray));
