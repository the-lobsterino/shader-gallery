...
vec2 start_pos = vec2(500);
vec2 print_pos = vec2(500);
...
#define _D col+=char(vec2(403619.0,1714338.0),uv)*text_color;
...
#define _rU col+=char(vec2(1131795.0,1581212.0),uv)*text_color;
...
#define _rp col+=char(vec2(484.0,1198372.0),uv)*text_color;
#define _rP col+=char(vec2(2049300.0,665762.0),uv)*text_color;
...
#define _rI col+=char(vec2(1131829.0,731298.0),uv)*text_color;
...
float sprite(vec2 spr, vec2 size, vec2 uv)
{
	...
	return bounds ? extract_bit(spr.x, bit - 21.0) + extract_bit(spr.y, bit) : 0.00;
}
...
vec3 Text(vec2 uv)
{
	...
	HEX(0xFF0000) _ _ _ast _ _rp _rU _T _rI _H _ _dsh _ _rP _rI _D _O _P _exc _  _ast ;
	...
	float br = col.r;
	if (col.r > 0.8)
   		col = br*vec3(sin(uv.x/3.0+6.28/3. + time*2.0)+0.6, sin(uv.x/3.0+6.28 * 0.66+time*3.0)+0.6, sin(uv.x/3.0 + time*3.0)+0.6);
	else
		col.r = 0.0;
    	return col;
}
