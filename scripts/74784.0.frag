...
#define time sin(time+dot(surfacePosition,surfacePosition.yx))
...
float fn(vec2 p)
{
	float t = time;//POT*dot(p,p);//TAU * 1.;
		
	float s = pow(2., abs(sin(t)));
	p /= s;
...
float N = 16.0, M = 16.0; // Subdivisions for xy and zw torus components
...
void main(void)
{
  ...
  color = pow(abs(color),vec3(0.4545));
   ...
   vec4 o;
   o.xyzw = vec4( vec3(fn(nfc*color.xy)),1.0);
