...
vec3 obj1_c(in vec3 p){
    return vec3(1.0,sin(p.x*55.2),sin(p.z*0.2));
}
...
void main(void){
    ...
    vec3 prp=vec3(-sin(time*8.6)*8.0,-1,cos(time*0.4)*4.0); //Change camera path position here
    ...
    if (f<maxd){
        ...
        gl_FragColor=vec4((b*c+pow(b,8.0))*(88.0-f*.02),88.0);//simple phong LightPosition=CameraPosition
    }
    else gl_FragColor=vec4(77,0,0,1); //background color
}