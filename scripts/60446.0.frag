...
vec2 dSpongeBob(vec3 p) {
    ...
    // nose
    q = p;
    rotate(q.zy, -0.5*p.z*p.z);
    s = dSegment(q + vec3(0.0, -0.6, 0.0), vec3(0), vec3(0.0, 0.0, .6));
    b = min(b, s.x - 0.035);

    // penis
    q = p;
    rotate(q.zy, -0.9*p.z*p.z);
    s = dSegment(q + vec3(0.0, 0.15, 0.0), vec3(0), vec3(0.0, -0.1, .515));
    b = min(b, s.x - 0.035);

    // balls
    q = p;
    rotate(q.zy, 0.3*p.z*p.z);
    rotate(q.xy, p.y*p.y);
    s = dSegment(q + vec3(0.0, 0.15, 0.0), vec3(0), vec3(0.0, -0.1, .3));
    b = min(b, s.x - 0.035);

    // brows
    q = p;
    rotate(q.zx, 0.5*p.x*p.x);
    e = dBox(q + vec3(0.2, -1.1, -0.27), vec3(0.2, 0.05, 0.01));
    if(e < b) res = 2.0;
    b = min(b, e);
