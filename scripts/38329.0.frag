...
float opDisplace(vec3 p) {
	...
	vec3 q = p+vec3(0,.9,0);
	float w = 0.2;
	float d1 = sdBox(q, vec3(w, .1, w));
	
	#define stack() {w *= 0.9-0.2*mouse.x;q.y -= 0.2+0.1*mouse.y;d1 = opU(d1, sdBox(q, vec3(w, .1, w)));}
...
vec3 shade(vec3 position, vec3 rayDir, vec3 normal, int steps) {
	...
	vec3 directColor;
	if(visible) directColor = vec3(mix(diffuse, specular, mix1))*spin(position);
