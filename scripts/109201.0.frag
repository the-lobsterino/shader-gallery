...
void main(void){
  ...
  vec3 prp=vec3(cos(my)*cos(mx),sin(my),cos(my)*sin(mx))*2.0; //Trackball style camera pos - Change camera path position here
  ...
  if (f<maxd){
    ...
    gl_FragColor=vec4((b*c+pow(b,8.0))*(44.0-f*.01),1.0);//simple phong LightPosition=CameraPosition
  }
