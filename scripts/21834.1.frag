...
vec3 GetLightCol()
{
	return vec3(10.0, 12.0, 14.0) * 10.0 * abs(sin(time * 2.75));
}
...
void ApplyAtmosphere(inout vec3 col, const in C_Ray ray, const in C_HitInfo intersection, const in vec3 vPos, const in vec3 vLightDir, const in float fLightDistance)
{
	#ifdef ENABLE_FOG
	// fog
	float fFogAmount = exp(intersection.fDistance * -kFogDensity * 3.0);
	vec3 cFog = GetSkyGradient(ray.vDir);
	float visibility = GetShadow(vPos, vLightDir, fLightDistance) * 3.0;
...
{                                                           
	...
	else
	{
		...
		#ifdef ENABLE_REFLECTIONS    
		{
			...
			float fRandom = sin(47.5341564);
			C_Ray reflectRay;
			reflectRay.vDir = reflect(ray.vDir, vNormal);
			reflectRay.vOrigin = intersection.vPos + reflectRay.vDir * fSepration * fRandom;
															      
			cReflection = GetSceneColourSimple(reflectRay) * 1.0 * fRandom * fRandom;                                                                         
		}
		#else
