...
void main( void ) {
	...
	p = p*1.5;//(7. - 6.*mouse.y);  // zoom? right click for that
	p /= dot(p,p);
	//p = p/2.5-vec2(0.625,-0.5);
	float a = angle;
	float s = sin(a+0.03*time);
	float c = cos(a-0.05*time);
	vec3 col = vec3(time/20.0 + sin(time/2000.)*cos(time/2000.));
	for (int i = 0; i < 200; i++) {
		if (dot(p, p) > 4.0) {
		    col *= 4.0; 
		    break;
	...
	for(float iy = -1.; iy < 1.1; iy += 1.0001){
		...
		float relevance = (-0.05)/(1.+dot(sample, gl_FragColor));
