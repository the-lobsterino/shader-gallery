...
{
    vec3 rgb = clamp( abs(mod(c.x*10.0+vec3(0.0,4.0,2.0),6.0)-3.0)-1.0, 0.0, 1.0 );
...
void main( void ) 
{
	...
	vec3 ch_color = hsv2rgb_smooth(vec3(time*0.4+uv.y*10.1,0.5,1.0));

	
	vec3 bg_color = vec3(0.,_d*0.*abs(sin(time))*0.,0.);
	...
	_ Z A H U V E _ K A Z _ T H E H A R P I S _ A F U K N U T
