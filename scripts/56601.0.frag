/*
 * Original shader from: https://www.shadertoy.com/view/wlf3WX
 */
...
float mHead(vec3 p) {
    ...
    p = pp*0.95;
	float a = -0.3;
	p.xz = vec2(p.x*cos(a)-p.z*sin(a), p.z*cos(a)+p.x*sin(a));
    p += vec3(-.405,.12,.10);
    pR(p.xy, -.12);
    pR(p.xz, .35);
    pR(p.yz, -.3);
    ...
    p = pe;
    pR(p.xz, 0.2);
    ridge = ellip(p.zy - vec2(0.01,-0.03), vec2(.045,.055));
...
float map(vec3 p) {
    ...
    float scale = 1.;
    float s = .2;
    float ry = 0.0*mouse.x*-7.+sin(time*0.3)*0.4;//mix(sstep(sstep(sstep(mod(iTime * s, 1.)))), mod(iTime * s, 1.), .3) * PI * 2.;
    float rx = sin(iTime * 4.33) * sin(iTime * 4.33+1.) * .2;
