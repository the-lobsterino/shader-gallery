...
// @gabriel_xiiii better sky and colors. less fov.
#ifdef GL_ES
...
vec3 terrain_clr(in vec3 p){
	...
	vec3 clr_a = vec3(5.4, 0.5, 0.2);
	vec3 clr_b = vec3(0.3, 5.3, 0.3);
	float m = fract(p.y*4.0);
	float g = fbm_3oct(p.xz * 49.0);
	clr_b += g;
...
void main(void){
 //Camera animation
 vec3 U=vec3(0,2.0,0);//Camera Up Vector
	float speed = time*-2.3;
	float my =  sin(time/100.0); // mouse.y*4.0;
	float camy = -1.0+my;
	float tary = -2.0+my;
 ...
 //vec3 E=vec3(mouse.x*0.0, 4, mouse.y*0.0); //Camera location; Change camera path position here
 ...
 vec3 B=cross(A, C);
 vec3 M=(E+C);
  vec2 vPos=2.0*gl_FragCoord.xy/resolution.xy - 0.60; // (2*Sx-1) where Sx = x in screen space (between 0 and 1)
 ...
 //Raymarching
 const vec3 e=vec3(11.0,1.9,11.9);
 ...
 for(int i=0;i<100;i++){
  ...
  if (f<MAX_DEPTH){
   if (s.y==0.0)
     c=terrain_clr(p);
vec3 cc = vec3(0.9, c.g, c.b);
	
	float m = smoothstep(7.0, 6.0, f);
	 c = mix(c,cc, 2.0-m);
   ...
   gl_FragColor=vec4(b*c*(2.0-f*0.3),1.0);//simple phong LightPosition=CameraPosition
 }
	else {
		float invy = 0.6-vPos.y;
		float r = invy*10.8;
		float g = 0.5 + invy*0.7;
		vec3 sky_clr = vec3(r,0.5,0.7);
		gl_FragColor=vec4( sky_clr,0.2); //background color
	}
