...
void main( void ) {
	...
	float s = 0.5 + 0.5*cos(exp2(mouse.x*10.)*a);
	...
	float tmpo = (time+r*cos(time+mouse.y*10.)*7e2)*5e-3;
	vec3 ph = vec3(cos(tmpo), cos(tmpo+6.28/3.), cos(tmpo+6.28/1.5));
	tmpo = pow(log(time), log(time));
	ph += 0.3*vec3(sin(tmpo), sin(tmpo+6.28/3.), sin(tmpo+6.28/1.5));
	
	gl_FragColor = vec4((ph-v)*w*ao,1.0);
}