...
{	// Floor...
	...
	float h = sin(time*0.0) * .1 - wave*0.001;
...
{ 
	...
	if (mat > 9.9)
	{
		...
		float ambi = clamp(.0 + 0.2 * norm.y,0.0, 1.0);
...
void main( void )
{
	...
	gtime = mod((floor(time*00.0) / 20.0), 20.0)-.5;
#else
    ...
    //vec2 mo = (iMouse.xy/iResolution.xy)-.0;
	
	float m = fract(time*.19);
	hello =  (1.0+sin(m*10.0)) * .02 * (smoothstep(0.0, .01, m) - smoothstep(0.05, .1, m));
