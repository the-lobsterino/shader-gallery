...
float crv(float a, float b) {
    float tr = 3.5 * max(fract(a-b), fract(b+a-b)) - 1.;
    tr *= tr * (3. - 2. * tr);
...
float h21 (vec2 a) {
    return fract(sin(dot(a.xy, vec2(9.8, 84.2))) * 43758.5453123);
}
...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
  ...
  //  float s2 = clamp(2. * r * crv(10., 0.5 * a / pi + 3. * r + iTime),0.,1.);
    ...
    vec3 e = vec3(10.);
    vec3 col = s * pal(s + 10.1 * iTime, e, e, e, vec3(0.,0.33,0.66));
