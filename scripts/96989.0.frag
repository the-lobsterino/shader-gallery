...
/*人体形状生成機能、中割機能付（R041130版）by ニシタマオ*/
...
sskelton fSSRSet_WitchBear01(sskelton SS){
	float nD =acos(-1.0) /180.0, nS =sin(SS.VP.w), nSL =sin(SS.VP.w +acos(-1.0) /180.0 *30.0);
	SS.VR_Cntr;
	SS.VR_Body =vec4(-45,	0,	0,	0);
	SS.VR_Shld =vec4(0,	0,	0,	0);
	SS.VR_Neck =vec4(-30,	0,	0,	0);
	SS.VR_Head =vec4(-15,	0,	0,	0);
	SS.VR_ArRU =vec4(+30,	0,	-15,	1);
	SS.VR_ArLU =vec4(+30,	0,	+15,	1);
	SS.VR_ArRL =vec4(0,	0,	0,		1);
	SS.VR_ArLL =vec4(0,	0,	0,		1);
	SS.VR_LeRU =vec4(-15,	0,	+15,	0);
	SS.VR_LeLU =vec4(-15,	0,	-15,	0);
...
sskelton fSSRSet_CycleBear01(sskelton SS){
	...
	SS.VR_Head =vec4(-15,	0,	0,	1);
	SS.VR_ArRU =vec4(+45,	0,	0,	1);
	SS.VR_ArLU =vec4(+45,	0,	0,	1);
...
float fNOutfit_WitchBear01(sskelton SS){
	float NP =fNOutfit_Bear01(SS);
...

	{
		...
		V3P.y +=1.0;
		float nPi;
		nPi =max(length(V3P.xy) -0.25, abs(V3P.z) -6.0);
		NP =min(NP, nPi);
		V3P.z -=6.0;
		nPi =length(V3P.xy) -1.25 +abs(V3P.z) *0.25;
		NP =min(NP, nPi);
	}

	{
		...
		V3P.y -=0.5;
		float nPi;
		nPi =max(length(V3P.xz) -2.0, abs(V3P.y) -0.1);
		NP =min(NP, nPi);
		nPi =length(V3P.xz) -0.75 +V3P.y *0.25;
...
float fNOutfit_CycleBear01(sskelton SS){
	float NP =fNOutfit_Bear01(SS);
...
float fNObject_WitchBear01(vec4 VP){
	VP.xz *= -1.0;
	sskelton SS;
	SS.VP =VP;
	SS =fSSPSet_Bear01( SS);
	SS =fSSRSet_WitchBear01( SS);
	SS =fSSkeltonMake( SS);

	float NP, nPP;
	NP =fNMuscle_Animal01(SS);

	nPP =fNOutfit_WitchBear01(SS);
	NP =min(NP, nPP);

	return NP;
}

float fNObject_CycleBear01(vec4 VP){
	sskelton SS;
	SS.VP =VP;
	SS =fSSPSet_Bear01( SS);
	SS =fSSRSet_CycleBear01( SS);
	SS =fSSkeltonMake( SS);

	float NP, nPP;
	NP =fNMuscle_Animal01(SS);

	nPP =fNOutfit_CycleBear01(SS);
	NP =min(NP, nPP);
...
vec3 fV3PGame_Airplane01(float nTime, float nSeed){
	...
	V3P.y +=16.0;
	V3P.zx +=vec2(4,1) *nTime -vec2(8,0);	//移動
	return V3P;
}
...
//			SFi =fSFObject_Car01(VPi);
			...
			VPi.z *=-1.0;
			float nPi;
			nPi =fNObject_WalkBear01(VPi);
//			nPi =fNObject_CycleBear01(VPi);
			SFi.nDistance =nPi;
...
sface fSFEffect_Before(smarch SM, sface SF){
	...
	if(ID_Object ==103)	VColor.rgb =sin(SF.VP.xyz +VP.w +sin(SF.VP.yzx +VP.w *2.0 +sin(SF.VP.zxy +VP.w *3.0))) *0.2 +0.8;
