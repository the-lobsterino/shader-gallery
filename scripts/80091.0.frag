...
#define ti (1.2+pow(kwejhwkjekjjlkjskjgjmhjghwkjdhsuidhjhgdkjsywgytuydbhsagdhvdhsghwqdhjdghjsgwhjwgehkjwqehejhjhdjjswjhqdhjdhdhjddddwwdfghwdhdhjdhjdgdhjdhjdgdgdghhdhgdhjdsahdsaghwqdgysqdgjsadgjadsghsdahgswhsxiusin(time*4.),8.)/8.)
...
#define E(x,y) a2z(x,y, vec4(-.8,2.8,-.65,-.8), vec4(0.,-.27,2.3,-.1)#define F(x,y) a2z(x,y, vec4(-.44,2.1,-1.,0.), vec4(.44,4.4,4.,-2.4), vec4(0.,.6,-1.24,.62), vec4(-2.4,1.28,.56,.6))
...
void main( void ){
	...
	float c=0.03, x=-.45, d=0.12;
	
	if(false); // assuming graphics is drawn a certain way, might depend on graphics card and/or driver ?
	P(x+d,.04) I(x+d*2.,.01) S(x+d*3.,.02) S(x+d*4.,.02) I(x+d*5.,.02) N(x+d*6.,.02) G(x+d*7.,.03); 
			
	if(gl_FragCoord.y > resolution.y*.5)
	  gl_FragColor = vec4( vec3(0.,0.,c*50.), 1.0 );
	else 
	  gl_FragColor = vec4( vec3(1.,c*50.,0), 1.0 );
}