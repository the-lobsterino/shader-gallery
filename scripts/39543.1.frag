...
float fix(float x) {return x<.002 ? x : 100000.0;}
vec3 drawCircle(vec2 uv, vec3 color0, vec3 color, float r, vec2 p, float w)
{
	return vec3(color * w /  fix(abs(distance(uv, p) - r))) + color0;
}

vec3 drawPoint(vec2 uv, vec3 color0, vec3 color, float r, vec2 p)
{
	return vec3(color * r /  fix(distance(uv, p))) + color0;
}
