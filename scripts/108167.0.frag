...
float map(vec3 p){
    ...
    p.xy *= m(t * 1.3);
    vec3 q = p * 2.0 + t;
    return length(p + vec3(tan(t * 0.7))) * log(length(p) + 1.0) + sin(q.x + sin(q.z + sin(q.y))) * 0.5 - 1.0;
}

vec3 hslToRgb(vec3 hsl) {
    vec3 rgb = clamp(abs(mod(hsl.x * 6.0 + vec3(0.5, 3.0, 1.0), 6.0) - 3.0) - 1.0, 0.0, 1.0);
...
vec3 getColor(in vec2 fragCoord, in float depth) {
    ...
    for (int i = 0; i <= 5; i++) {
        ...
        // 修改这里，使颜色在指定范围内动态变化
        float hue = mod(t * 2.0 + float(i) / 5.0, 1.0);
        ...
        vec3 l = color + vec3(4.0, 2.5, 3.0) * f;
...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
    ...
    for (int i = 0; i < SAMPLES; i++) {
        ...
        float weight = 22.0 / (1.0 + abs(depth - FOCAL_DISTANCE));
