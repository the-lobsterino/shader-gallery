...
//transforms 2D equirectangular depth to 3D position
vec3 equirectangular_ray(vec2 uv)
{
	const float pi  = 4. * atan(1.);
	uv.x            = fract(uv.x + 0.);
	vec2 theta    = (uv * 2. - 1.) * vec2(TAU, TAU * .5);
	return vec3(cos(theta.y) * cos(theta.x), sin(theta.y), cos(theta.y) * sin(theta.x));
}
...
void main( void ) 
{
	...
	float proj_transform 		= cos(time+uv.y)*.5+.5;
	...
	vec3 rect_origin 		= vec3(0., 0., 0.);	
	origin 				= mix(origin * vec3(1., 1., 1.), rect_origin, proj_transform);
	...
	vec3 rect_direction		= equirectangular_ray(uv*.5*aspect-.25);
	direction 			= mix(direction, rect_direction, proj_transform);
	...
	{				
		...
		#define REFLECTION
		#ifdef REFLECTION
		ray reflection 		= emit(r.position , reflect(r.direction, r.gradient.xyz),  .0001);
		...
		if(r.material.z == 1.)
		{
			...
			refraction.direction 	= -refract(r.direction, r.gradient.xyz,  .35);
			refraction 		= emit(refraction.origin, refraction.direction, r.distance*.5);
		
			if(refraction.intersection)
			{				
				float depth		= r.distance;
				refraction.origin 	= refraction.position + r.gradient.xyz * r.distance * 8.;
				refraction.direction 	= refract(refraction.direction, -refraction.gradient.xyz,  .38);
				r.color 		+= abs(dot(normalize(LIGHT_POSITION-refraction.position), refraction.direction.xyz))*.35;		


				refraction		= emit(refraction.origin, refraction.direction, .0002);
...
ray shade(inout ray r)
{
	...
	float ndl   		= max(0.0001, dot(surface_direction, light_direction));
	float ndv   		= max(dot(surface_direction, r.direction), -.999);	
