...
void main(void) {
	...
	// And again dithering instead of overlaying! :p
    float d = distance((gl_FragCoord.xy/resolution.xy), vec2(0.5,0.5));
    vec3 texCol = tex(uv);
	texCol*= smoothstep(1.0,0.0,d);
    vec3 col = floor(texCol + (floor(texCol*2.2)*mod(gl_FragCoord.x +gl_FragCoord.y,2.))); //Glorious dithering! :D
    vec4 finalColor = vec4(col*length(p),1);
