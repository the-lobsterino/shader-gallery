...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
  ...
  float mx=mouse.x*3.14*2.0;
  float my=mouse.y*3.14/2.01;
  vec3 ro=vec3(cos(my)*cos(mx),sin(my),cos(my)*sin(mx))*9.0; //Trackball style camera pos
	//vec3 ro = vec3( 8.0*cos(mx), 6.0, 8.0*sin(mx) );
//	vec3 ro = vec3( -8.0, 6.0, 8.0 );
	vec3 ta = vec3( 0.0, 3.5, 0. );
/*
