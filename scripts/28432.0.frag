...
vec3 raymarch( in vec3 ro, vec3 rd, vec2 tminmax )
{
    float t = tminmax.x;
    //float dt = .02;
    float dt = .33 - 1.0*sin(time*0.0) + dot(ro,rd)/100.;//animated
    vec3 col= vec3(0.);
    float c = 0.;
    for( int i=0; i<64; i++ )
	{
...
void main()
{
    ...
    vec2 m = vec2(10.0) * mouse.xy ;
    //if( mouse.z>0.0 )m = mouse.xy/resolution.xy*3.14;
    m-=.5;

    // camera

    vec3 ro = zoom*vec3(10.0);
    ro.yz*=rot(m.y);
    ro.xz*=rot(m.x); // (0.0*time);
    vec3 ta = vec3( sin(time)*10.0 , 0.0, 0.0 );
