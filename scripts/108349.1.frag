...
{
  vec3 q = abs(p) - b;
  return length(max(q,0.0)) + min(max(q.y,max(q.y,q.z)),0.0) - r;
}
...
void main( void ){
	...
	#define fogCol (vec3(0., 0.0022, 0.07))
		float fog = .00033;
		fog *= 0.5; // fog strength
       	        c = mix(c, fogCol, 2.0 - (1.0) / (1.0 + t*t*t*fog));;
    
    //col /= float(nH)*.133;
    //col *= smoothstep(.5, .3, ns * .01);
    col = 1.-(11.15/(1.12-exp(-col)));
