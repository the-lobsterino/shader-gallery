...
const vec2 ch_start = vec2 (ch_space.x * -7., 2.); // start position
      vec2 ch_pos   = vec2 (0.0, 0.0);             // character position(X,Y)
      vec3 ch_color = vec3 (0.5, 0.5, 0.5);        // character color (R,G,B)
...
void main( void ) 
{
	...
	if (REPEAT_SIGN)
	  uv = -14.0 + mod(1.8*(uv-1.0),ch_space*vec2(16.,7.5));     //  set zoom size 4

	ch_pos = ch_start ;  // set start position  3

	T W O _ M I L L I O N _
		D O L L A R S _ nl1
		E A C H _
		D A Y _
		I S _
		A L S O _ nl2
		G R E A T _ T H O U G H
	s_dot
		
		vec3 color = mix(ch_color, bg_color, 1.0- (0.38 / d));  // shading
	gl_FragColor = vec4(color*1.001, 1.0);
