	...
	if(SET == 0){
		...
		for(int i=0; i<maxIter; i++)
		{
			...
			if(length(z)>maxRadius)
			{
				...
				vec3 color;
				if(i<1)
					color += mix(mandelColor1, mandelColor2, x);
				else if(i<2)
					color += mix(mandelColor2, mandelColor3, x);
				else if(i<3)
					color += mix(mandelColor3, mandelColor4, x);
				else if(i<4)
					color += mix(mandelColor4, mandelColor1, x);
				gl_FragColor.rgb += color/mandelScale;
				break;
			}
		}
	}else if(SET == 1){
