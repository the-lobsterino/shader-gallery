...
void main( void ) {
	...
	for (int i = 0; i < iter; i++) {
		...
		p /= l*l*0.2;
		p.xy = vec2(atan(c.x*c.y)*p.x - cos(c.x)*p.y, asin(c.x)*p.y-acos(c.x)*p.x);
		p.xz = vec2(atan(c.x*c.y)*p.x - asin(c.y)*p.z, asin(c.y)*p.z-acos(c.y)*p.x);
		p = abs(mod((p), n)-n/2.0);
