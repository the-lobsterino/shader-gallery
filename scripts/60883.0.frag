...
const float color_intensity = 5.8;
...
void main()
{
  ...
  for(int i=1;i<complexity;i++)
  {
    ...
    newp.x+=0.8/float(i)*sin(float(i)*p.y+time/fluid_speed+0.3*float(i)) + 0.5; // + mouse.y/mouse_factor+mouse_offset;
    newp.y+=0.99/float(i)*sin(float(i)*p.x+time/fluid_speed+0.3*float(i+10)) - 0.5; // - mouse.x/mouse_factor+mouse_offset;
    p=newp;
  }
