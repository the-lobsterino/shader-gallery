...
float getMomentum(vec2 uv, float angle){
	...
	float m0 = texture2D(bb, uv).a;
	float part = (3.1415 * 2.0) / 16.0;
	mat2 rot = mat2(cos(part), -sin(part), sin(part), cos(part));
	vec3 last = vec3(pshuf.x, texture2D(bb, uv + pshuf * pixel).a, pshuf.y);
	pshuf = rot * pshuf;
	vec3 force = vec3(0.0);
	float prs = 0.0;
	float velocity = 0.0;
	for(int i=0;i<16;i++){
		vec3 p = vec3(pshuf.x, texture2D(bb, uv + pshuf * pixel).a, pshuf.y);
		prs += p.y;
	...
	last = vec3(pshuf.x, texture2D(bb, uv + pshuf * pixel).a, pshuf.y);
	pshuf = rot * pshuf;
	force /= 16.0;
	prs /= 16.0;
	for(int i=0;i<16;i++){
...
void main( void ) {
	...
	momentum += ((cos(time * 1.0) * cos(time * 7.9873) * cos(time * 1.45)) )  * (1.0 - smoothstep(0.0, 0.1, distance(mouse2, position)));
	//float phase = cos(time * 6.0)  * 1.0 - smoothstep(0.0, 0.1, distance(mouse, position));
	
	gl_FragColor = vec4(normal.x, 0.0, normal.z, momentum);
