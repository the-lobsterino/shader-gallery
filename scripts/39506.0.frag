...
{
	float h = (sin(p.x) + sin(p.z)) * 0.2;
	return max(p.y - pos - h, max(abs(p.x), abs(p.z)) - 9.9);
}
...
void main( void ) {
	...
	vec3 origin = vec3(sin(time * 0.3) * 20.0, 0.0, -cos(time * 0.3) * 20.0);
	...
	if (dt.x < 1.0)
	{
		...
		vec2 shadow = rayCast(marchPos + normal * 0.1, lightDir);
