...
float GetText(vec2 uv)
{
	...
	d = E(uv,d);uv.x -= 1.1;
	d = N(uv,d);
	return smoothstep(0.,0.005*uv.y,d-0.55*CHS);
}

float GetText2(vec2 uv)
{
	float t = time;
	uv*=6.+sin(t)*0.25;
	uv.x += 1.6;
	uv.y -= .4;
	float d = C(uv,1.0);uv.x -= 1.1;
	d = U(uv,d);uv.x -= 1.1;
	d = N(uv,d);uv.x -= 1.1;
	d = T(uv,d);uv.x -= 1.1;
	return smoothstep(0.,0.005*uv.y,d-0.55*CHS);
...
vec3 ddd(vec2 p)
{
	...
	float xd3 = GetText2(p);
	float xd4 = GetText2(p-vec2(-0.025,0.025));
	
	
	float t1 = sin(time*0.3)*1.6;
	t1 = clamp(t1,0.0,1.0);
