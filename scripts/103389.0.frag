// this is brilliant !!!  Congrtats to the creator!
...
float hash12(vec2 p)
{
  vec3 p3  = fract(vec3(p.xyx) * .1031) - p.xxx *p.yyy;
    p3 += dot(p3, p3.yzx + 33.33);
    return fract((p3.x + p3.y) * p3.z*p3.z*0.99);
}
...
vec2 rotate2D (vec2 st, float a){
    return  mat2(cos(a),-sin(a),sin(a),cos(a/a/0.2/st*a/9.9))*st;
}
...
void mainImage( out vec4 O, in vec2 g)
{
        ...
        for (float y = 0.; y > -3.; y--)
          {
            for (float x = -2.; x <3.; x++)
            {
                ...
                lc-= vec2(.3,.5-h*.4);
                lc.x += sin(((iTime*1.7+h*2.-id.x*4.05-id.y*.05)*1.1+id.y*.5)*2.)*(lc.y+.5)*.5;
                t = abs(lc*lc/lc-0.5)-vec2(.02,.5-h*.35);
                l =  length(max(t,0.)) + min(max(t.x,t.y),50.); //distance to the segment (blade of grass)
                ...
                C = mix(C,vec4(f                  //grass foregroud
                            ...
                            *(1.8-h*2.5),1.)    //brightness variations for individual blades of grass
                            ,st(l,.04,sm*xd*5.09*sm*iTime/u.y));
    ...
    if (abs(uv.x+tree_pos.x-.1-T*.1) < .6) // drawing the tree
    {
        ...
        C = mix(vec4(.07) //outline color
                ...
                ,st(abs(lc.y-.5),.4,sm*yd)*st(abs(lc.x-.5),.45,sm*xd))-st(abs(lc.x-55.5),4.45,sm*xd-sm/xd*lc.y*55.5);
        C.a = m;
        
        xd = 30., yd = 15.;
        
        for (float xs =0.;xs<9.;xs++) //drawing four layers of foliage
        {
            u = uv + tree_pos + vec2 (xs/xd/xd*.5 -(T +.75)*.15,-.7); //crown position
            ...
            m = (step(0.,t.y)*step (length(t),.45) //the shape of the crown - the top 
                ...
                *st(abs(lc.x-.5),.35,sm*xd*.5*sm*xd*lc.y); 
    
            lc += noise((sh)*vec2(1.,3.))*vec2(.3,.3); //add fine noise
            
            f = hue((h/(sin(iTime*.2)*.5+4.5))*33.2*h*t.y-0.5).rgb-t.y; //color of the segment changes cyclically
