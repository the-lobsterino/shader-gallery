...
const vec2 ch_size  = vec2(0.5, 0.5) * 0.6;              // character size (X,Y)
...
const vec2 ch_start = vec2 (ch_space.x * -5., 1.); // start position
      vec2 ch_pos   = vec2 (5.0, -5.0);             // character position(X,Y)
...
#define nl3 ch_pos = ch_start;	ch_pos.y -= 9.0; ch_pos.x -= -5.0;
...
{
    vec3 rgb = clamp( abs(mod(c.x*1.0+vec3(0.0,1.0,2.0),6.0)-3.0)-1.0, 0.0, 1.0 );

	rgb = rgb*rgb*(4.0-2.5*rgb); // cubic smoothing	
...
void main( void ) 
{
	...
	uv *= 18.0 ;
	uv.y -= 3.;
	uv *= 0.6+(sin(time+length(uv)*0.55)*0.1);
	...
	vec3 bg_color = vec3(0.5,_d*0.1*abs(sin(time))*0.1,0.5);
	
	uv.y += -7.0+sin(time+uv.x*0.5)*0.8;
	ch_pos = ch_start;
			         nl1  
		                 nl2  
			         nl3  V I J A Y
	...
	vec3 color = mix(ch_color, bg_color, 0.3- (0.20 / d*2.0));  // shading
