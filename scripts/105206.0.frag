...
{
  return max( length6(p.xz) - h.x, acos(p.y) - h.z );
}
...
{
  vec3 r1, r2;
  float sphy = 0.35 + 0.1 * sin(time);
  vec3 sp = pos - vec3( 1.0, sphy, 0.0);
  vec2 res = vec2( sdPlane(     pos), 1.0 ); 
  res = opU( res, vec2( sdSphere(    sp, 0.25 ), 46.9 ) );
  res = opU( res, vec2( sdBlob(      pos-vec3( 0.0, 0.25, 0.0), -0.5 - 0.45*sin(time) ), 246.9 ) );
  ...
  res = opU( res, vec2( udRoundBox(  pos-vec3( 1.0, 0.25, 1.0), vec3(0.12), 0.1 ), 41.0 ) );
                                     r1 = rotateX (pos - vec3( 0.0, 0.25, 1.0), sin(time)*0.3);
                                     r1 = rotateY (r1, time*2.0);
  res = opU( res, vec2( sdTorus(     r1, vec2(0.20, 0.05) ), 25.0 ) );
                                     r1 = vec3(-1.0, 0.2, 0.0) + rotateY (vec3(-0.15, 0.0, -0.15), time);
                                     r2 = vec3(-1.0, 0.2, 0.0) + rotateY (vec3(-0.15, 0.0, +0.15), time);
  res = opU( res, vec2( sdCapsule(   pos, r1, r2, 0.1  ), 31.9 ) );
  res = opU( res, vec2( sdTriPrism(  pos-vec3(-1.0, 0.25, -1.0), 0.1*sin(time) + 0.25, 0.05 ), 43.5 ) );
                                     float h1 = 0.2 + 0.06 * sin(time);
  res = opU( res, vec2( sdCylinder(  pos-vec3( 1.0, 0.30, -1.0), vec2(0.1, h1) ), 8.0 ) );
  res = opU( res, vec2( sdCone(      pos-vec3( 0.0, 0.50, -1.0), vec3(0.8, 0.5+0.1*sin(time), 0.3) ), 55.0 ) );
  res = opU( res, vec2( sdTorus82(   pos-vec3( 0.0, 0.25, 2.0), vec2(0.20, 0.05) ), 50.0 ) );
                                     r1 = rotateY (pos - vec3(-1.0, 0.25, 2.0), time*0.25);
  res = opU( res, vec2( sdTorus88(   r1, vec2(0.20, 0.05) ), 43.0 ) );
                                     r1 = rotateY (pos - vec3(1.0, 0.30, 2.0), time*0.25);
  res = opU( res, vec2( sdCylinder6( r1, vec3(0.1, 0.05, 0.25) ), 12.0 ) );
                                     r1 = rotateX (pos - vec3(-1.0, 0.25, 1.0), time*0.5);
  ...
  res = opU( res, vec2( sdTwistedTorus( pos - vec3(-2.0, 0.25, 2.0), 6.0*sin(time) ), 46.7 ) );
  res = opU( res, vec2( sdWaveSphere  ( pos - vec3(-2.0, 0.25, -1.0), 0.2 , 10, sin(time)*0.2), 880 ) );
