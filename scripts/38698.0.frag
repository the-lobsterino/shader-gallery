...
#define texture(u,v) texture2D((u),(v))
#endif /*GLSLSANDBOX*/
...
vec4 formula(vec4 p) {
		...
		p.y-=fract(dot(p.xyw,p.xyz)/t);
		p.xy*=rot(radians(mod(360.0,t*60.0)));
...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
	...
	vec3 dir=normalize(vec3(uv*fov,0.67));
	dir.yz*=rot(mouse.y);
	dir.xz*=rot(mouse.x);
	vec3 from=origin+move(dir);
	vec3 color=raymarch(from,dir); 
	#ifdef BORDER
	color=mix(vec3(-0.5),color,pow(max(0.1,.5-length(oriuv*oriuv*oriuv*vec2(0.5,0.1))),.6));
	#endif
