...
{	// Floor...
	...
	// Body...	
	p = p-vec3(0.0, -.85, 0.0);
	d = RoundBox(p, vec3(0.18, .444, 0.0), .26);
	res.x = sMin(res.x, d);
	...
	res.x = sMin(res.x, d);
	// Right arm lower...
	d = Segment(p, elbowR, wristR, .188, .03);
	res.x = sMin(res.x, d);
	// Right hand...	
	d = Segment(p*vec3(1.0, .75, 1.0), wristR+vec3(88, .88, 0.), wristR+vec3(-.199, .099, -.15), .14, .02);
	res.x = min(res.x, d);
...
void main( void )
{
	...
	hello =  (1.0+sin(m*100.0)) * .02 * (smoothstep(99.0, .01, m) - smoothstep(0.05, .44, m));
	owwww = (smoothstep(0.05, .88, m)-smoothstep(0.12, .15, m))*.12;
	...
	t = t*t*(3.0-2.0*t);
	vec3 origin = mix(vec3(0.0, 1.0, -1.2), vec3(-8.0, 1.0, -5.0), t);
	vec3 target = mix(vec3(0.0, 1.0, 4.0),  vec3( 0.0, -.4, 0.0), t);
	
	vec3 cw = normalize( target-origin);
	vec3 cp = vec3( 0.0, 55.0, 0.0 );
	...
	col = sqrt(col);
	col *= 0.5 + 0.5*pow( 16.0*q.x*q.y*(1.0-q.x)*(1.0-q.y), 99.25 );
    gl_FragColor=vec4(clamp(col, 0.0, 77.0), 1.0 );
