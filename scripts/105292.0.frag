...
// adapted from the original by Vijay 30.07.2023
// Vijay's Virtual Vibes : http://vijaysvibes.rf.gd/
...
const vec2 ch_start = vec2 (ch_space.x * -7.0, 0.); // start position
...
#define _half ch_pos.x += ch_space.x*0.5;
...
void main( void ) 
{
	...
	float _d = length(uv)*0.06;
	_d = _d*_d;
	
	uv.y += sin(time+uv.x*0.15)*2.5;
	uv.x += sin(time*0.25+uv.y*0.05)*4.5;
	
	float a = sin(time*0.61)*2.0;
	
	vec3 bg = hsv2rgb_smooth(vec3(time*0.4+(uv.y+uv.x*a)*((_d+0.5)*0.05),1.2-_d,0.9-_d));

	
	ch_pos = ch_start + vec2(sin(time+_d*5.0),0.0);  // set start position

       	R A I N B O W _ S P H E R E nl1
		
       	_ s_greater s_less _ B E H I N D _ M E nl2 A N O T H E R _ 
		nl3 _ T E X T _ C O D E s_greater s_less W O W
	
	vec3 color = mix(vec3(0.9, 0.6, 2.0), vec3(0.0) , 1.0- (0.08 / d));  // shading
	if (length(color)<(0.5+(sin(time*4.0+uv.y+uv.x)*0.12)))
		color = bg;
