...
{
	p *= font_size;
	float inkNess = 0., nearLine, strokeWidth = 0.06;
	nearLine = minimum_distance(from,to,p +(  sin(p.x + time))  ); 
	inkNess += smoothstep(0.7, 1.5, 1.- 12.*(nearLine - strokeWidth)); // ugly still
	inkNess += smoothstep(0.2, 2., 1.2- (nearLine  + 5. * strokeWidth)); // glow
	return inkNess;
}
...
{
	return ( vec2( (letterspace.x / 1.5) * .65 , 1.0-((letterspace.y / 2.) * .95) ));
}
...
d += R(r()); add();
d += D(r()); add();
d += E(r()); add();newline();newline();newline();
d += G(r()); add();
	d += U(r()); add();d += Y(r()); add();
	d = clamp(d* (.75+sin(gl_FragCoord.x*3.1456+time*.03)*.5), 0.0, 1.0);
