...
#define sunsHiNE void main( void ) { m = ncenter( mouse*res ); AAimage( c ); c = tonEmap( c ); c = gammacorRect( c ); OUT = vec4( c, 1. ); }
...
vec3	firePalette(float i) { float T = 1400. + 2023. * i; vec3 L = vec3(7.4, 5.6, 4.4); L = pow(L, vec3(5.))*(exp(1.43876719683e5/(T*L)) - 1. ); return 1. - exp(-5e8/L); }
 
vec3 Image0( vec2 p )	// this is the main function in regards to the sun
	{ float ct, st; p *= rot( time );
	float blade = clamp( pow( sin( atan( p.y + ct,p.x + st)*BLADES)+BIAS, SHARPNESS ), 0., 1. );
	vec3 c = mix( vec3( .34, .5, 1. ), vec3( 5., .5, 1. ), ( p.y + 1. ) * .5 );
	c += (vec3(.95, .65, .30) * 1. / distance(vec2(0.), p + vec2(st, ct))*.075);
	c += vec3(.95, .75, 12.30) * min(1., blade *.7) * (1. / distance( vec2( 0., 0. ), p)*.075 );
	c = 1. - exp( 3./-pow( c, vec3( 0.5, 1.67, 1.5 ) ) ); return c; }

sunsHiNE //by ändrom3da4 