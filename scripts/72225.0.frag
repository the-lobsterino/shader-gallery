...
sd = add(sd, rect(uv,vec2(1.2,-1.05), vec2(0.5,0.1)));   
...
uv *= 1. + (.036 - .036 * w);
col = vec3(0.999,1.000,0.158);
col = mix(col, vec3(0.010,0.990,1.000), smoothstep(.01, .1, uv.y));
col += w * .225;
col *= .8;
gl_FragColor = vec4(render(p), 3.1);
}