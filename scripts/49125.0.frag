...
#define ENABLE_FOG
#define ENABLE_SPECULARhhhhhiiii
...
{          
	...
	vec3 vSphereDomain = DomainRepeatXZ(vPos, vec2(2.0, 3.0));
	vSphereDomain=RotateX(vSphereDomain,time*0.5);
//        vSphereDomain=RotateY(vSphereDomain,time*0.5);
	vec4 vDistSphere = vec4( GetDistanceXYTorus(vSphereDomain,0.3,0.15), 2.0, vSphereDomain.xy);
...
vec3 GetLightPos()
{
	vec3 vLightPos = vec3(0.0, 4.0, 3.0);
	#ifdef ENABLE_MONTE_CARLO        
...
{           
	...
	float fSpecIntensity = (fSpecPower + 1.0) * 0.125;
