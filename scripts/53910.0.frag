...
float n3D(vec3 p){
    ...
    h.xy = mix(mix(42.*sin(-time*0.000123)*h.xz, 23.*sin(-time*0.0000623)*h.yw, mod(p.y, (p.y+sin(time*0.0009)))), mix(h.xz, h.yw, p.y), sin(time*0.000023));
    
    // Interpolating along Z, and returning the 3D noise value.
    return mix(sin(time*2.0)*h.x, cos(time*0.25)*h.y, sin(time*0.98)*p.z); // Range: [0, 1].
...
void mainImage( out vec4 fragColor, in vec2 fragCoord ){
    ...
    vec2 p = uv*(7.*sin(time*0.0234)+3.5) + vec2(0, .5) + vec2(1, 0)*(iTime+13.*sin(time*.666));
    ...
    vec3 col = vec3(cos(time*.96), sin(time*.2792), sin(time*.7788));
    float pat = clamp(sin((time*p.x + -time*p.y)*128.) + .0, 0., 1.);
    ...
    if(hash21(floor(p))>.9) charCol = vec3(cos(time*1.2), sin(time*.03)+0.5, tan(time*3.2)+.01);
