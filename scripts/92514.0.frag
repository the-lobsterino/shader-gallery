...
void main() {
	...
	d = min(d,T(uv2-vec2(-2,0.5)));
	d = min(d,O(uv2-vec2(-1.5,0.5)));
	d = min(d,O(uv2-vec2(-1,0.5)));
	d = min(d,K(uv2-vec2(-0.5,0.5)));
	d = min(d,E(uv2-vec2(-0.0,0.5)));
	d = min(d,E(uv2-vec2(0.5,0.5)));
	...
	vec3 rgb = hsv2rgb(vec3(position.x + position.y + (time / 5.0), 0.7, 1));
