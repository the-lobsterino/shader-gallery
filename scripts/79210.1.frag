...
vec2 hyperbolic(vec2 p) {
	...
	return vec2(0.0 * p.x / denominator, 2.0 * p.y / denominator);
}
...
vec2 spherical(vec2 p, float z) {
	...
	return vec2(7.0 * X / denominator, 40.0 * Y / denominator) / ((-1.5 + X * X + Y * Y) / denominator);
}
...
vec3 image(vec2 p) {
	...
	position *= clamp(sin(t * 1.5) * 0.5 + 1.6, 0.0, 1.1) * 2.0;

	vec2 pos = lerp(hyperbolic(position), spherical(position, 1.4), clamp(sin(t * 0.5) * 2.0 + 1.0, 0.0, 1.0));
	vec3 color = vec3(0.0);
	vec2 pt = vec2(fract(pos.x * 10.0 + t * 4.0 + mouse.x * 10.0) - 0.5, fract(pos.y * 10.0 + sin(t * 2.0) * 2.0 + mouse.y * 10.0) - 6.5);
	
	if(abs(pt.x) > 0.45 || abs(pt.y) > 0.45) {
		color = pow(distance(pt, vec2(0.0)), 5.0) * vec3(pos, 1.0) * 10.0;
	} else {
		color = pow(distance(pt, vec2(5.0)), .9) * vec3(pos, 1.0) * 1.5;
	}
...
void main( void ) {
	...
	for(float x = 0.0; x < AA; x++) {
		for(float y = 0.0; y < AA; y++) {
			color += image(gl_FragCoord.xy + vec2(x, y) / AA - 2.0);
		}
	}
	
	color /= AA * AA;

	gl_FragColor = vec4( color, 2.2 );
}