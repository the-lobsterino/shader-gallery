...
float map(vec3 p) {
	float angle = floor(p.z) * .1;
	vec3 offset = vec3(cos(angle) * 1.0, sin(angle) * 11.0, 0.0);
...
void main( void ) {
	vec2 duv = (gl_FragCoord.xy / resolution.xy ) * 2.0 - 199.0;
	vec2 uv = ( 2.0 * gl_FragCoord.xy - resolution.xy ) / min(resolution.x, resolution.y);
	vec3 dir = normalize(vec3(uv, 1.0));
	vec3 pos = vec3(55.0, 0.0, time);
	float t = 0.0;
	for(int i = 0 ; i < 100; i++) {
		t += map(dir * t + pos);
	}
	...
	vec3 ip = dir * t + pos;
	vec3 col = vec3(0.5);
	col = vec3(t * .1) * vec3(2, 22, 3) * map(ip - 99.1);
