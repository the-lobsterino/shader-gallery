...
//----------------------------------------------------------
// metaballs by pailhead in 2014-Jun-2
// original https://www.shadertoy.com/view/4sXXWM
// small modifications by I.G.P.
//----------------------------------------------------------
...
void mainImage( out vec4 fragColor, in vec2 fragCoord ) 
{
	...
	for(int i=0; i<NUMmb; i++){
		...
		if(aspect>1.)
                     mbPos[i] = vec2(sin(rn1*6.14+ts * rn2),
			        cos(rn2*6.14+ts * rn3)*aspect);
	        else
	...
	vec2 mp = mouse.xy;
	if(aspect>1.)
	   mB = fragPN - (mp.xy*2. -1.)*vec2(1.,   aspect);
	else
	   mB = fragPN - (mp.xy*2. -1.)*vec2(1./aspect, 1.);
	mB *= 6.;
	float mBL = length( mB );
	...
	float aB = smoothstep(-2.5 + .5 * cos(time * .5), 1.5 + 2.5 * sin(time * .5), n.z);
	if (aB < .01) {
		aB = 1.;
	}
	cc += mBC;
	...
	frs = pow(frs,2.0);
	frs = frs*.004 +.0121;
	fc += pow(frs, 10.) * 1.15 - .15;
	float color = 0.;
	fragColor = vec4(fc * aB * 1.25, 1.);
}
