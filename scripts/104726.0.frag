// From: http://xdpixel.com/category/ray-marching/


// Ray Marching Tutorial (With Toon Shading)
// By: Brandon Fogertydd
...
vec3 calculateLighting(vec3 pointOnSurface, vec3 surfaceNormal, vec3 lightPosition, vec3 cameraPosition)
{
    ...
    diffuseStrength = calculateRampCoefficient( diffuseStrength, 2 );
    vec3 diffuseColor = diffuseStrength * vec3( 0.3, .6, 0.1 );
    ...
    if( dot( fromPointToCamera, surfaceNormal ) < 0.07 )
    {
        finalColor = vec3( 0 );
    }
...
void main( void ) 
{
    ...
    vec2 uv = ( gl_FragCoord.xy / resolution.xy ) * 4.0 - 2.0;
    ...
    vec3 finalColor = vec3(0.,0.,0.09);
    if( distanceToClosestPointInScene > 0.0 )
    {
        // Move our light around on both the x and y axis.
        float lx = mix( -2.4, 2.4, sin(time * 0.23) * 0.3 + 0.5);
        float ly = mix( -2.4, 2.4, sin(time * 0.11) * 0.3 + 0.5);
        vec3 lightPosition = vec3( lx, ly, -8.5 );
