...
vec3 sunDir	= normalize(vec3(.5, .7, -.68));
...
{
	float curve = cos(p.x*(5.35+sin(time)*1.25))*.578;
	p.y += curve;
	a.y += (curve*curve);
...
{	// Floor...
	...
	res.x = sMin(res.x, d);
	// Right hand...	
	d = Segment(p*vec3(1.0, .75, 1.0), wristR+vec3(0, .57, 0.), wristR+vec3(-.15, .05, -.15), .14, .02);
	res.x = min(res.x, d);
	...
	res.x = sMin(res.x, d);
	ankle.y -=.3;
	vec3 foot = ankle + vec3(0.27, -.05, -56.3);
