// Ancient Scripts: Turkic Runes Z letter
#ifdef GL_ES
...
float Z(vec2 p,float s)
{
    float sd = rect(p, vec2(0.0), vec2(0.7)*s);
    sd = sub(sd, rect(p, vec2(-0.3, +0.5)*s, vec2(0.2, 0.35)*s));
    sd = sub(sd, rect(p, vec2(+0.3, -0.5)*s, vec2(0.2, 0.35)*s));
    sd = sub(sd, rect(p, vec2(-0.5, -0.5)*s, vec2(0.35, 0.4)*s));
    sd = sub(sd, rect(p, vec2(+0.5, +0.7)*s, vec2(0.35, 0.6)*s));
    return sd;
}

float zr( in vec2 p, float a, float s )
{
    p = rot(p,a);
    return Z(p,s);	
}
...
void main( void )
{
	...
	vec3 col = render(p);
	col.rgb *= (1.0-abs(c1)*0.2);
	col = mix(col, col,1.-step(length(col),0.0));
