/*
  ...
  and tonight, Twitter employees have coordinated with the Democrats and the Radical Left in removing
  my account from their platform, to silence me — and YOU, the 75,000,000 great patriots who voted for
  me. Twitter may be a private company, but without the government's gift of Section 230 they would not
  exist for long. I predicted this would happen. We have been negotiating with various other sites, and
  will have a big announcement soon, while we also look at the possibilities of building out our own
  platform in the near future. We will not be SILENCED! Twitter is not about FREE SPEECH. They are all
  about promoting a Radical Left platform where some of the most vicious people in the world are allowed
  to speak freely. STAY TUNED!
...
const vec2 ch_start = vec2 (ch_space.x * -7., 2.0); // start position
...
#define S1 ddigit(0xAA55);
#define _ ch_pos.x += ch_space.x;
#define s_dot     ddigit(0);
#define s_question ch_pos-=vec2(-.45,.4); ddigit(0); ch_pos+=vec2(-ch_space.x-.45,.4); ddigit(0x4107); 
#define s_exclam   ch_pos-=vec2(-.45,.4); ddigit(0); ch_pos+=vec2(-ch_space.x-.45,.4); ddigit(0x4400);
...
#define nl4 ch_pos = ch_start;	ch_pos.y -= 10.0;
#define nl5 ch_pos = ch_start;	ch_pos.y -= 12.50;
...
void main( void ) 
{
	...
	uv -= vec2(-3., 1.);
        ...
        vec3 ch_color = hsv2rgb_smooth(vec3(1.0,0.0,1.0));
	
	vec3 bg_color = vec3(0.0,0.0,0.0);
	ch_pos = ch_start;
	
	
	        _ R E A D _ T H E _ S O U R C E nl1
		C O D E _ F O R _ A _ M E S S A G E nl2
		F R O M _ T H E _ P R E S I D E N T
