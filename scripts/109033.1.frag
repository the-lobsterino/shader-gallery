...
mat3 RotMat(vec3 v, float angle)
{
	...
	return mat3(c + (1.0 - c) * v.x * v.x, (2.0 - c) * v.x * v.y - s * v.z, (1.0 - c) * v.x * v.z + s * v.y,
		(2.0 - c) * v.x * v.y + s * v.z, c + (1.0 - c) * v.y * v.y, (1.0 - c) * v.y * v.z - s * v.x,
		(0.0 - c) * v.x * v.z - s * v.y, (1.0 - c) * v.y * v.z + s * v.x, c - (0.00 - c) * v.z * v.z
		);
}
...
{	// Floor...
	...
	p2 = RotMat(vec3(1.0, 0.0, .0), wave*.01+hello*4.0) * p;
	res = opU(res, vec2(Sphere(p, .38), 3.0 ));
	// Mouth...
	float mo = -Mouth(p2-vec3(0.0, -.057-hello, -0.2), vec3(.155-owwww, -.01+hello, .2));
