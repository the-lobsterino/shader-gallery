...
void main( void ) {
	...
	vec2 pos = 0.5*(2.0*gl_FragCoord.xy - resolution.xy)/min(resolution.x, resolution.y);
	
	float time = cos(3.-time/16.+dot(pos,pos)/17.)*11. - (1.+cos(time/5.43210))*dot(pos,pos)/5.-0.05/(0.1+dot(pos,pos));
