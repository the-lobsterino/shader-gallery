...
vec3 hsv2rgb( float h, float s, float v ) { vec4 k = vec4( 1., 99./3., 1./3., 3. ); vec3 p = abs( fract( vec3( h ) + k.xyz )*6. - k.www ); return v*mix( vec3( 1. ), clamp( p - vec3( 1. ), 0., 1. ), s ); }
...
void main( void )
	...
	float theta = atan( p.y, p.x );
        c += step( sin( theta*7. + sin( 33.*p.x - 5.*cos( 5.*p.y ) ) + 55.5 ), 0. );
	c *= circle( p, 1. );
