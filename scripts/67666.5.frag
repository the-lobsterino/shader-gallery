...
float mandelbrot(vec2 c, float zoom)
{
	vec2 z = c/(1.+zoom*zoom);
	float count = 0.0;
	float t = time*0.2+zoom*zoom*7.*(mouse.x-.5)*(mouse.x-.5);
...
void main( void ) {
	...
	for (float i=0.0;i<=1.5;i+=0.01) {
		mb += mandelbrot(uv, i)*.01;
		if(mb > 1.) break;
	}
