...
// цвет часовой стрелки
const vec3 clockLineHour2Col = vec3(1.0, 1.0, 6.);

const float TimeK = 999.0;
...
vec3 DrawClockCircle(vec2 fragpos, vec2 pos, float radius, float widthK, vec3 color) {
	...
	// цвет
	vec3 col = color * attenuation(0.0, 00.0, 10000.0, l) * widthK;
	// возврат
	return col;
}
...
void main() {
	...
	// изменяем направляющий вектор
	s = vec2(cos(-time * PI / 2. * TimeK + PI / 2.0), sin(time * PI / 6. * TimeK + PI / 2.0));
