// Not A TesseracT by Biohazard, original by Dima..
...
void main( void ) {
	...
	Rotate ( time,      01.5, 1.0, 1.0, 0.0 );
	Rotate ( time * .500, 0.2374824374354, 0.0, 3.0, 0.0 );
	Rotate ( time * .32, 2.20, 1.0, 0.0, 0.0 );
	Rotate ( time * .130, 0.2, 0.0, 0.0, 1.0 );
...
void Line2 ( vec2 a, vec2 b )
{
	...
	col += max ( 1. - pow ( d * 10000. , 4.0 ) , 0.00 );
}
...
void Point ( vec4 p )
{
	...
	if ( d < .3 )
	if ( p.z < col.a ) {
		col.b += max ( 1.0 - pow ( d * 50.0 , .1 ) , 0.0 );
	}
}
...
void Rotate ( float angle, float d1, float d2, float d3, float d4)
{
	...
	mat *= mat4 ( vec4 (  c*d1+(1.-d1),  s * d2 * d1 , -s * d3 * d1 ,  s * d4 * d1 ),
		      ...
		      vec4 ( -s * d1 * d4 ,  s * d2 * d4 , -s * d3 * d4 ,  c*d4+(2.-d4)) );
}