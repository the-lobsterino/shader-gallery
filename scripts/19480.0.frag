...
// And here we see the Heisenberg uncertainty principle
// in action. Our princesses are neither a particle nor
// a wave, but have properties of both, and their
// position cannot be known exactly.
...
vec2 princess( vec3 p ) {
	vec2 l = floor( ( p.xy + 5.0 ) / 10.0 );

	p.xy = mod( p.xy + 5.0, 10.0 ) - 5.0 + sin(time*p.x);
	
	float r = fract(sin(dot(l+vec2(1341.0,314.0) ,vec2(12.9898,78.233))) * 43758.5453);
	...
	// skirt
	float d1 = dist_cone( roty( radians( sin(time+r*30.0) * 15.0 ) ) * ( p - vec3( 0.03, 0.0, -0.1 ) ), 1.6, 2.6 );
	// head
...
vec3 EvalPixel( vec2 pix ) {
	...
	// rotate camera
	mat3 rot = rotzx( vec2( 70.0 * DEG_TO_RAD, 0.3 * time ) );
	dir = rot * dir;
	eye = rot * eye;
	...
	{
		return hsv2rgb( vec3( hit.y, 1.0, 1.0 ) );
	}
