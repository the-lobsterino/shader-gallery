...
void main( void ) 
{
	...
	float hit_target = (
		...
		;
	vec2 _uv = uv;
	uv.x = .1+.8*fract(3.*uv.x);
	border_pixel_width *= 2.;
	hit_target = max(hit_target, (
		float(false
		|| (abs(uv.x-0.33)*2. < border_pixel_width/resolution.x)
		|| (abs(uv.x-0.67)*2. < border_pixel_width/resolution.x)
		))*1.1);
	...
	{
		if(total_range > 30.*hit_target && hit_target > 0. && mouse.x > 0.5){
			gl_FragColor = vec4(0.2)/(hit_target*hit_target*hit_target*hit_target)+texture2D(backbuffer, (gl_FragCoord.xy+vec2(1,0))/resolution)*.5+texture2D(backbuffer, (gl_FragCoord.xy-vec2(1,0))/resolution)*.5;
			return;
		}
