...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
    ...
    vec3 color = vec3(0.0);
    color += auroraLayer(uv, 0.05, 0.3, vec3(0.0, 0.2, 0.2));
    color += auroraLayer(uv, 0.1, 0.4, vec3(0.1, 0.5, 0.5));
    color += auroraLayer(uv, 0.15, 0.3, vec3(0.2, 0.1, 0.4));
    color += auroraLayer(uv, 0.07, 0.2, vec3(0.2, 0.1, 0.5));
    
    vec3 skyColor1 = vec3(0.3, 0.11, 0.3);
    vec3 skyColor2 = vec3(0.3, 0.2, 0.36);
    // Add a gradient to simulate the night sky
    color += skyColor2 * (1.0 - smoothstep(0.0, 4.0, uv.y));
    color += skyColor1 * (1.0 - smoothstep(0.0, 8.0, uv.y));
