...
float segment(vec2 position, vec2 a, vec2 b, float width)
{
	return distance(position, project(position, a, b))-width;
}


#//this draws a countour line at the zero distance of the function
...
#//function map - this picks which function to display based on where the mouse is 
float map(vec2 position)
{
	...
	fields[5]		= segment(position, vec2(0.), vec2(1., 0.) * rmat(time), .0125);
	fields[6]		= fractal(position, mouse.y, .25, .125);
...
{
	vec2 offset = vec2(1./resolution.x, 0.);
...
void main( void ) 
{
	...
	result.xyz		= field < 0. ? vec3(normal, .75) : result.xyz;
