...
void main( void ) {
	...
	if (time < 0.05) {
		...
		if ((resolution.x - gl_FragCoord.x) < 1. && gl_FragCoord.y < 1.) {
	...
	vec3 worldPos = (texture2D(backbuffer, vec2(1, 0)).xyz - 0.5) * 16.;
	...
	if (gl_FragCoord.y < 1. && (resolution.x - gl_FragCoord.x) < 1.) {
		...
		vec3 encodedPos = worldPos / 16.;
		gl_FragColor = vec4(clamp(encodedPos + 0.5, 0., 1.), 1.);
		return;
	}
