...
float prng(in vec2 seed) {
	seed = fract (seed * vec2 (500.3983, 5.4427));
...
float noiseStack(vec3 pos,int octaves,float falloff){
	...
	return (1.0+noise)/5.0;
}

vec2 noiseStackUV(vec3 pos,int octaves,float falloff,float diff){
	float displaceA = noiseStack(pos,octaves,falloff);
	float displaceB = noiseStack(pos+vec3(39084.293,4230.21,52305.19),octaves,falloff);
...
void main(  ) {
	...
	coordScaled.y = dot(coordScaled,coordScaled)*0.1;
	coordScaled.x = dot(coordScaled,coordScaled)*0.2;
	
	
	vec3 position = vec3(coordScaled,0.0) + vec3(1223.0,6434.0,8425.0);
	vec3 flow = vec3(4.1*(0.5-xpart)*pow(ypartClippedn,4.0),-2.0*xfuel*pow(ypartClippedn,640.0),0.0);
	vec3 timing = realTime*vec3(0.0,-1.7,1.1) + flow;
	//
	vec3 displacePos = vec3(1.0,0.5,1.0)*2.4*position+realTime*vec3(0.01,-00.7,1.3);
	...
	float noise = noiseStack(noiseCoord,30,0.4);
	//
	...
	//
	// smoke
	float smokeNoise = 1.5+snoise(0.4*position+timing*vec3(11.0,1.0,0.2))/12.0;
	...
	if (sparkLife>0.0) {
		...
		float sparkRadians = 9990.0*sparkRandom*2.0*PI + 2.0*time;
		...
		sparks = sparkLife*sparksGray*vec3(.3,.3,2.0);
	}
	//
	gl_FragColor = vec4(max(fire.brr,sparks*20.)+smoke,1.0);
}