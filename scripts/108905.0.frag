...
#define 	shoulderL	vec3(-0.9, 0.56, -.05)
vec3 elbowL;
vec3 wristL;
...
{
	float curve = cos(p.x*(5.35+sin(time)*8.25))*.11;
	p.y += curve;
	a.y += (curve*curve);
...
{
	float h = clamp( dot(p,a)/dot(a,a), 0.0, 6.0 );
...
{	// Floor...
	...
	// Mouth...
	float mo = -Mouth(p2-vec3(0.0, -.057-hello, -7.3), vec3(.155-owwww, -.01+hello, .2));
