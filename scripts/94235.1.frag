...
/*ここまで*/
vec3 fV3Game_Airplane01P(void){
	vec3 V3P;
	V3P.xy +=VMouse.xy *vec2(-1, +1) *64.0;
	V3P.z +=8.0;
	return V3P;
}

mat3 fM3Game_Airplane01R(void){
	mat3 M3R =mat3(1,0,0,0,1,0,0,0,1);
	M3R *=fM3Rotate(vec3(-4.0 *VMouse.y, 0, 0));
	M3R *=fM3Rotate(vec3( 0,-4.0 *VMouse.x, 0));
	M3R *=fM3Rotate(vec3( 0, 0,+5.0 *VMouse.x));
	return M3R;
}

vec4 fVGame_Airplane01(vec4 VP){
	VP.xyz +=fV3Game_Airplane01P();
	VP.xyz *=fM3Game_Airplane01R();
	return VP;
}

float fNGame_Airplane01(vec4 VP){
...
vec3 fV3Game_Drone01P(void){
	...
	V3P.z +=32.0;
	V3P.y +=8.0;
	V3P.xyz +=sin(vec3(3,2,1) *nTime *0.5) *vec3(16,16, 8);
	return V3P;
}

mat3 fM3Game_Drone01R(void){
	mat3 M3R;
	M3R =fM3Rotate(sin(nTime *vec3(1.3,1.1,0)) *vec3(0.1, 0.2, 0));
	return M3R;
}
...
float fNMap(vec3 V3P){
	...
	if(true){	/*飛行機*/
		vec4 VPi =fVGame_Airplane01(VP);

		float nPi =fNGame_Airplane01(VPi);
...
//		float nPi =fNGame_Airplane01(VPi);
	...
	if(true){	/*飛行機の機関銃*/
		...
		vec4 VPi =fVGame_Airplane01(VP);
		if(VPi.z >0.0){
			float nSignX =sign(VPi.x);
			VPi.x =abs(VPi.x) -4.0 *nE;
			VPi.y +=1.0 *nE;
			VPi.z =mod(VPi.z -32.0 *(VPi.w +nSignX *0.1), 8.0) -4.0;
	...
	if(true){	/*ドローンの機関銃*/
		...
		if(VPi.z <0.0){
			VPi.y +=6.0 *nE;
			VPi.xyz *=fM3Game_GunDrone01R();
			VPi.w +=sign(VPi.x) *0.1 +sign(VPi.y) *0.2;
			VPi.xy =abs(VPi.xy) -0.5 *nE +VPi.z *0.1;
			VPi.z =mod(VPi.z +50.0 *VPi.w, 32.0) -16.0;
	...
	if(iSQ ==0){	/*建物*/
		...
		VPi.xz +=VRandom.xz *8.0;
		float nPi =length(max(abs(VPi.xyz) -vec3(2,8.0 +VRandom.y *16.0,2), 0.0)) -0.1;
	...
	if(true){	/*飛行機の機関銃*/
		if(ID_Object !=11){
			...
			if(VPi.z >0.0){
				VPi.x =abs(VPi.x) -4.0 *nE;
				VPi.y +=1.0 *nE;
				float nL =length(VPi.xy);
				if(ID_Object ==12){
					VColor.rgb +=vec3(1,0.25,0) /nL /nL *8.0 *(sin(nL *2.0 -VP.w *20.0) *0.5 +0.5);
		...
		if(ID_Object !=12){
			...
			if(VPi.z <0.0){
				...
				if(ID_Object ==11){
					...
					VColor.rgb +=(sin(vec3(0,1,2) /3.0 *acos(-1.0) *2.0 +VPi.z *0.1 +VPi.w *10.0) *0.5 +0.5) /nL /nL;
				}
