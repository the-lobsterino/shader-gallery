...
float rnd(vec4 v) { return fract(dot(v,v)+4e4*sin(dot(v,vec4(13.46,41.74,-73.36,14.24))+17.34)); }
...
void mainImage( out vec4 fragColor,  vec2 fragCoord )
{
    ...
    if(i < steps)
    {
        ...
        vec3 color = .5+.1*sin(val*vec3(39.896,57.3225,48.25))*0.5+0.5;
	    ...
	    float d = distance(base, fro);
	    float th = 2.;
	    if(d > th){
		    fragColor += 0.1*vec4(pow(d/th, 2.));
	    }
    }
}
...
void main(void)
{
	...
	vec2 rho = 1. / resolution;
    vec2 uv = gl_FragCoord.xy * rho;
    vec4 back = texture2D(backbuffer, uv);
	back = min(back, 
	  0.25 * texture2D(backbuffer, (uv+rho*vec2(1,1)))
	+ 0.25 * texture2D(backbuffer, (uv+rho*vec2(-1,1)))
	+ 0.25 * texture2D(backbuffer, (uv+rho*vec2(1,-1)))
	+ 0.25 * texture2D(backbuffer, (uv+rho*vec2(-1,-1)))
	);
    ...
    gl_FragColor = max(gl_FragColor, back + (2./256.)*sign(gl_FragColor-back));
    gl_FragColor.a = 1.0;
}