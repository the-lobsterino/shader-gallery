#ifdef GL_ES
// Note from WolfJR i am not gay
// Note from WolfJR (Creator): WHAT THE! WHY DID U DO THIS AAA!! Guys i swear i SAID  im gay!!! They just changed it but...
// Why? If you will continue doing this then... 🙂🔫 (Joke)
//
// Note From WolfJR (Very angry i am): In a few days i changed the text to thank god right? well... i didnt notice that "Guys i swear i SAID im gay!!!" YOU LITE-
// RALLY TROLLED ME YOU MOTHER- (Gasps) ok... calm down... if you will continue this... then... we will make something. Everyone that sees this shader pls edit-
// all the text by me saying random words. (SWEARS ARE NOT ALLOWED) like: im gay, Im totally not a furry, and more... (Swears not allowed becuz im a 10 year old-
// kid.) 
...
const vec2 ch_size  = vec2(3.0, 1.0) * 0.6;              // character size (X,Y)
...
#define nl2 ch_pos = ch_start;  ch_pos.y -= 6.1;
...
void main( void ) 
{
	...
	vec3 bg_color = vec3(0.3,_d*0.8*abs(sin(time))*0.7,0.1);
	
	

	
	uv.y += -1.+sin(time+uv.x*0.9999)*0.9;
	ch_pos = ch_start;
	
	M I _ P A N _ S U _ S U _ S U M
