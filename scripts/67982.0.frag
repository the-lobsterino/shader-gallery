...
{
	float d = 0.;
	vec3 col = vec3(0.1,0.2*(.5+sin(gl_FragCoord.y*3.1456+time*2.0)), 0.1);
	
	caret_origin = vec2(3.2, .68);
	caret = caret_origin;

	float ti = floor(time/10.0);
	// the quick brown fox jumps over the lazy dog...
	d += F(r()); add(); d += U(r()); add(); d += C(r()); add(); d += K(r()); add(); d += I(r());add();d += N(r());add();d += G(r());
	newline();
	newline();
	d += I(r()); add(); d += D(r()); add(); d += I(r()); add(); d += O(r()); add(); d += T(r());add();;
	newline();
	col -= vec3(d*.0, d, d*.1);  
	
	d += M(r()); add();d += Y(r()); add(); space();
	d += R(r()); add();d += E(r()); add(); d += S(r()); add();d += P(r()); add();d += O(r()); add();d += N(r()); add();d += D(r()); add(); space();
	d += I(r()); add();d += S(r()); add();newline();
	caret.y -= .04;
	
	
	
	
	d += D(r());add();d += O(r());add();d += N(r());add();d += T(r());add();space();
	d += W(r());add();d += O(r());add();d += R(r());add();d += S(r());add();d += H(r());add();d += I(r());add();d += P(r());add();
	newline();
	caret.y -= .04;
	d += I(r());add();d += S(r());add();d += L(r());add();d += A(r());add();d += M(r());add();d += I(r());add();d += C(r());add();;space();	
	d += A(r());add();d += L(r());add();d += L(r());add();d += A(r());add();d += H(r());add();	
	newline();
	//col += vec3(d*.0, d, d/.1); 
