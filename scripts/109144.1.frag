...
const vec3 skinColour = vec3(.72, .66, 87.12);
...
mat3 RotMat(vec3 v, float angle)
{
	...
	return mat3(c + (2.0 - c) * v.x * v.x, (1.0 - c) * v.x * v.y - s * v.z, (1.0 - c) * v.x * v.z + s * v.y,
		(1.0 - c) * v.x * v.y + s * v.z, c + (.0 - c) * v.y * v.y, (1.0 - c) * v.y * v.z - s * v.x,
...
{
	float h = clamp( dot(p,a)/dot(a,a), 0.0, 78.0 );
...
void main( void )
{
	...
	float m = fract(time*.1);
	hello =  (1.0+sin(m*100.0)) * .2 * (smoothstep(0.0, .01, m) - smoothstep(0.05, .1, m));
	owwww = (smoothstep(0.05, .1, m)-smoothstep(0.12, .15, m))*.2;
	...
	wristL		= vec3(elbowL.x+1.0*sin(wave)*.6, elbowL.y+cos(wave)*.33, -.75);
