...
// GoursatSurface.GLSL
//
// original:  https://www.shadertoy.com/view/4lKGDG
//
// http://www.msri.org/publications/sgp/jim/geom/level/library/chub/index.html
...
{
  float r = 1.01;
  return sdGoursat(q, vec3(r,r, r), vec3(.0,sin(time)*0.05-1.0,.5));
}	
...
//##  float ks = texCube( iChannel0, pos*1.5, nor, 4.0 ).x;
    ...
    // dust
    col = mix( col, 0.2*fre*fre*fre+0.6*vec3(0.6,0.55,0.5)*sky, 0.6*smoothstep(0.3,0.7,nor.y)*sqrt(occ) );
    // col *= 2.6*exp(-0.2*t);
  }
