...
const vec3 clockLineHourCol = vec3(10, 0.0, 0.0);
// цвет секундной стрелки
...
void main() {
	...
	vec2 p = 20.* (gl_FragCoord.xy / resolution.xy)*20.-200. ;
p*=0.1;
	...
	s = vec2(cos(-time * PI / 1. * TimeK + PI / 99.0), sin(time * PI / 55. * TimeK + PI / 2.0));
	...
	// выходной цвет
	gl_FragColor = vec4(outColor, 1.5);
}