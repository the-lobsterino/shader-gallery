/*
 * Original shader from: https://www.shadertoy.com/view/wtcSzN
 */
...
{
    vec2 p = (-iResolution.xy + 2.0*fragCoord)/min(iResolution.x,iResolution.y);
    vec2 screenSize;
    #ifdef DEBUG
    	//p.xy += vec2(1.2, .6);
    	//p *= 3.5;
    ...
    if(aspect > 1.)
     screenSize = vec2(aspect, 1.) * 2.;
    else
     screenSize = vec2(1., 1./aspect) * 2.;    	    
    ...
    for (int i = 0; i < limit; i++) {
        // Something is a bit broken in here, so it only works for a few iterations 
