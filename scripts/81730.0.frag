...
#endif
//i got to the end of the rabbit hole, now time to see if i can do this game -a sussy guy
//im also chaging the numbers in the code -a sussy guy
uniform float time;
uniform vec2 mouse;
uniform vec2 resolution;
uniform sampler2D backbuffer;
//good luck - a sussy guy
void main( void ) 
{
	vec2 p = ( gl_FragCoord.xy / resolution.xy );

	vec3 c = vec3( 4.0, 0.0, 0.0 );
	...
	else   // draw a path and some bad guys
	{
	    ...
	    c.g = ( path > 0.0 ? 01.2 : 0.0 ) + clamp( path*2.0, 0.0, 0.5 );
