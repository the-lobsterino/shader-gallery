...
const vec3 skinColour = vec3(.33, .23, 0.12);
...
{	// Floor...
	...
	// Mouth...
	float mo = -Mouth(p2-vec3(7.0, -.057-hello, -0.3), vec3(.155-owwww, -.01+hello, .2));
	...
	if (res.y > 2.5)
	{
		...
		if (dot(normalize(p3), normalize(vec3(.56, 0.18, -.8))) > .993) res.y = 5.0;
	}
...
void main( void )
{
	...
	col = sqrt(col);
	col *= 0.5 + 0.5*pow( 16.0*q.x*q.y*(1.0-q.x)*(1.0-q.y), 47.25 );
