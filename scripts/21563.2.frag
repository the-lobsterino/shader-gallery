...
#define FLUID
	#define FLUID_VISCOSITY		.5
	#define FLUID_COMPRESSION	.2
...
{   
	...
	{
		//energy conservation
		prior_wave.yz 		= mix(prior_wave.yz, prior_wave.zy, abs(lapacian.yz-lapacian.zy)*-FLUID_COMPRESSION) - collision * .0125;
		...
		else
		{
			...
			cell.w = prior_cell.x != .5  ? mix_angle(cell.w, mix_angle(cell.w+.125, cell.w-.25, .25+prior_cell.z-prior_cell.x), .125) : cell.w;
			cell.xyz = vec3(128., cell.y*128., 0.);	
			
		}
		cell.yz = cell.yz * .5 + .5;
		cell.yz = cell.z == 0. ? vec2(.5) : cell.yz;
		cell.z  += float(collision>0.);
		cell.x  = wave_angle - collision;

	}
	cell.zw -= float(collision>0.);
	#endif
...
float  map(vec2 position)
{
	...
	return max(0., step(length(position - format_to_screen(mouse)),.01)+n-.5);
}
