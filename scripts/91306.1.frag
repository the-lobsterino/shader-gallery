...
float de(vec3 p) {
    return deMandelbulb(p, 4.0*mouse.y+mouse.x);
}
...
vec3 background(vec2 st) {
    return mix(vec3(0.5), vec3(0.1), length(st) * .9);
}

void main(void) {
    vec2 st = (2.0 * gl_FragCoord.xy - resolution) / min(resolution.x, resolution.y);

    vec3 ro = vec3(cos(time), .7*sin(time*.4), 2.5);
    vec3 ta = vec3(0.0);
    vec3 z = normalize(ta - ro);
    vec3 up = vec3(mouse.x*3., 2.0*-mouse.x, mouse.y*3.);
