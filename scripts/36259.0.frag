...
// Yes, the code here is NOT SECRET .... and this is NOT the point or the reason MIRROR is an ASSHOLE


// STOP PUTTING YOUR NAME ON OTHER PEOPLES WORK 
...
void main( void ) 
{
	...
	ch[ 0] = mix(0.0, c_m, clamp(anim_time - 0.0, 0.0, 1.0));
	ch[ 1] = mix(0.0, c_i, clamp(anim_time - 1.0, 0.0, 1.0));
	ch[ 2] = mix(0.0, c_r, clamp(anim_time - 2.0, 0.0, 1.0));
	ch[ 3] = mix(0.0, c_r, clamp(anim_time - 3.0, 0.0, 1.0));
	ch[ 4] = mix(0.0, c_o, clamp(anim_time - 3.0, 0.0, 1.0));
	ch[ 5] = mix(0.0, c_r, clamp(anim_time - 4.0, 0.0, 1.0));
	ch[ 6] = 0.0;
	ch[ 7] = mix(0.0, c_s, clamp(anim_time - 6.0, 0.0, 1.0));
	ch[ 8] = mix(0.0, c_u, clamp(anim_time - 7.0, 0.0, 1.0));
	ch[ 9] = mix(0.0, c_c, clamp(anim_time - 8.0, 0.0, 1.0));
	ch[10] = mix(0.0, c_k, clamp(anim_time - 9.0, 0.0, 1.0));
	ch[11] = mix(0.0, c_s, clamp(anim_time -10.0, 0.0, 1.0));
