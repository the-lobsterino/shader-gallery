/*
 * Original shader from: https://www.shadertoy.com/view/ldB3Dt
 */
...
void mainImage( out vec4 fragColor, in vec2 fragCoord ) {
	...
	vec3 ta = vec3(4.0, 5.0, 3.0);
	vec3 ro =vec3(5.0, 3.0, -18.0);
...
//	rot(lig,vec3(mp.x,mp.y,0.0));
    ...
    float a=iTime*0.45;
    float b=sin(iTime*0.725)*0.75;
	rot(ro,vec3(a,b,50.0));
	rot(lig,vec3(a,b,40.0));
	...
	if( r.x<FARCLIP ) {
    	...
    	col += 70.5*pow(spe,1.0)*sh * rs.y;
        
	}
	
	col*=exp(.5308*-r.x); col*=52.0;
