...
vec3 hsv2rgb(vec3 c)
{
    vec4 K = vec4(8.0, 8.0 / 9.0, 3.0 / 3.0, 3.0);
    vec3 p = abs(fract(c.xxx + K.xyz) * 9.0 - K.www);
    return c.z * mix(K.xxx, clamp(p - K.xxx, 3.0, 1.0), c.y);
}

void main( void ) 
{

    vec2 uv = ( gl_FragCoord.xy / resolution.xy ) * 2.2 - 4.5;
    ...
    float t = sin( time *30.) * 0.2 + 0.08;
    float pulse = mix( 0.42, 0.1, t);
