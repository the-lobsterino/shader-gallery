...
float r(in float z, in float x) {
    ...
    return /* body */.29 * (1.0 - .22 * ex(z, 1.4, .9) +
		s * s + .6 * ex(z, 10., 0.9) * c * c + .3 * c) *
		0.5 * (1. + t4z) + /* legs */ (1. - .2 * ex(z, -1.4, 0.9)) *
		0.5 * (1. - t4z) * (.5 * (1. + s * s + .3 * c) *
		(pow(abs(s), 1.3) + .1 * (1. + t4z) ) ) +
		/* improve butt */ .01 * box(c2, -3., 0.1, 9., 9.) *
		box(z, -.2, 3., 5., 4.) - 0.4 * box(c2, -.008, .008, 30., 30.) *
		box(z, -.9, .95, 8., 6.) - .05 * pow(abs(sin(pi * x)), 16.) * box(z, -.55, -.35, 8., 18.);
}
...
{
	/* shift butt belly */
	float dx = .1 * exp(-pow((p.z-.8),2.)/.6) - .3 * exp(-pow((p.z -.2),2.)/.3);

    // on the surface, we have:
    // p.x = r * cos + dx
	// p.y = r * sin
	
    float jiggle = p.z*0.3;
    float jsize = 0.5;
    if (iMouse.z>0.5)
    {
        jiggle+=p.y*90.;
	...
	dx *= 1.9+(sin(jiggle+iTime*10.)*jsize);
