...
#define time sin(time+TAU*(dot(surfacePosition.xy,surfacePosition.yx)))
...
//const float TAU = 256.0 * PI;

#define Duh (mouse.y*mouse.x)/(surfaceSize.x*surfaceSize.y)
#define TAU (Duh*2.0*PI)
...
vec3 getcolor(vec3 p3) {
  ...
  uv -= 0.5; // Centre cell
	...
	uv*=surfacePosition/time;
	return vec3(uv,fn((uv.yx)));
...
//  for (int i = -AA; i < AA; i++) {
//     for (int j = -AA; j < AA; j++) {
       vec2 uv = surfacePosition*(2.0*(gl_FragCoord.xy) - resolution.xy)/resolution.y;
