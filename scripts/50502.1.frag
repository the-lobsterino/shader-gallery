...
float complexFBM2(vec2 p) {
	float a = -complexFBM(p)*7.;
	float b = complexFBM(10.-p+time*0.01)/4.;
	return complexFBM(p+b*vec2(cos(a), sin(a)));
}
...
void main( void ) {
	vec2 fragCoord = gl_FragCoord.xy - mod(gl_FragCoord.xy, vec2(4));
	vec2 p =2.0* ( fragCoord.xy / resolution.xy ) -1.0;
	vec3 col = vec3(1.0);
	
	col = atarist(p,col);
		
	vec3 rez = mix(col.xyz, vec3(p.x,1.0,1.0) * 0.5, complexFBM2(p) * 3.0 + p.y * 0.2 - sin((time * 1.0) * 0.9) * 0.1 - 0.95);
