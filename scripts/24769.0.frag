...
// Galaxy from mr Guil , adapted by Gigatron FTL
...
float zoom=10.;
const float arms = 1.;
float winding = sin(time*0.005)*62.831853;
...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
    ...
    vec2 q = surfacePosition+(surfaceSize/2.0);
    vec2 p = -1.0 + 2.0 * q;
    ...
    vec3 ro = length(surfaceSize)*0.4*vec3(2.);
    ro.yz*=rot(m.y);
