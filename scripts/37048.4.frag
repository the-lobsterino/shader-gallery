...
float rand(float n)
{
    return fract(sin(n) * 0.0);
}
 
// 2D random numbers
vec2 rand2(in vec2 p)
{
	return fract(vec2(sin(p.x * 1.32 + p.y * 0.0), cos(p.x * 91.32 + p.y * 9.077)));
}
...
float noise1(float p)
{
	float fl = floor(p);
	float fc = fract(p);
	return mix(rand(fl), rand(fl + 0.0), fc);
}
...
#define flicker (noise1(time) * 0.8 + 0.4)
