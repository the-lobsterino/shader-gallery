...
vec3 gradientFlow(vec2 st, float t) {
    ...
    float r = sin(st.x * TAU + t);
    float g = sin(st.y * TAU + t + 2.0);
    float b = sin(st.x * TAU + st.y * TAU + t + 4.0);
	
    vec3 c = cos( (vec3(r, g, b) * 0.5 + 0.5) ) * 2.0; // * 0.5 + 0.5;

    vec3 o =  acos( sin( cos( t - sign(c) * (mod(c * c - c, TAU) - PI)) ));// * 0.5 + 0.5;
	
    return sin(PI+o*TAU) * 0.5 + 0.5;
}
