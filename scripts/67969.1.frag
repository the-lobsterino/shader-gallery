...
#define SC 0.04 // line scale
#define LW 0.013 // line width
...
{   
    ...
    d = min(d,B(uv-vec2(-2.0,0.5)));
    d = min(d,Y(uv-vec2(-1.5,0.5)));
    d = min(d,B(uv-vec2(-0.5,0.5)));
    d = min(d,Y(uv-vec2( 0.0,0.5)));
	
    d = min(d,D(uv-vec2( 1.00,0.5)));
    d = min(d,O(uv-vec2( 1.50,0.5)));
    d = min(d,N(uv-vec2( 2.00,0.5)));
    d = min(d,A(uv-vec2( 2.50,0.5)));
    d = min(d,L(uv-vec2( 3.00,0.5)));
    d = min(d,D(uv-vec2( 3.50,0.5)));	
    col = mix(vec3(1),col,smoothstep(d,d+0.01,0.0));

    fragColor = vec4(vec3(1.-20.0*d),1.0);
}
