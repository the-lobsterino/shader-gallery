...
float wave(float t) { return .7 - .5 * cos(0.5/sin(2.0*sin(cos(t)))); }
...
void main( void )
{
	vec2 c = vec2(.15-1.2*wave(time/7.2347890), -0.25-.5*wave(time/19.1345897));
	vec2 c2 = 3.25 * vec2(sin(time/18.175453), sin(time/16.2));
	
	float sinq = (2.*sin(time/255.4))+0.1;
