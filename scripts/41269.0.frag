...
#define SPECULAR_DISTANCE_FACTOR 2.0
#define BUBBLES 0.1
...
//add by zbb
in vec2 breakerTexCoords1;
in vec4 refrectColor;
#endif
...
//add by zbb
varying vec2 breakerTexCoords1;
varying vec4 refrectColor;
#endif
#ifdef PROPELLER_WASH
...
void main()
{
	...
	// VRE by LXY: ÁÙÊ±½â¾ö½ü´¦ÏòÔ¶´¦¹ý¶ÉÌ«ÁÁÎÊÌâ
    //float reflectivity = max(0.0,min(1.0,r+(1.0-r)*pow((1.0-dot(nNorm,reflection)) * fresnelScale , 4.0)));
    //float reflectivity = max(0.0,min(0.6,r+(1.0-r)*pow((1.0-dot(nNorm,reflection)) * fresnelScale *0.8, 6.0)));
    float fresnel = clamp( reflection, 0.0, 1.0 ) + 1.0;
    float reflectivity = pow(fresnel, -8.0) * 0.7;
...
#endif
    //add by zbb: refraction color
    //trit_refractColor = refrectColor.rgb;
    ...
    vec3 breakerTex = texture(trit_breakerTex, breakerTexCoords).xyz;
        //add by zbb
    vec3 breakerTex1 = texture2(trit_breakerTex, breakerTexCoords1).xyz;
    breakerTex += breakerTex1;
#else
    vec3 breakerTex = texture2D(trit_breakerTex, breakerTexCoords).xyz;
    //add by zbb
    vec3 breakerTex1 = texture2D(trit_breakerTex, breakerTexCoords1).xyz;
    breakerTex += breakerTex1;
    breakerTex *= 0.4;
#endif
	...
	// VRE Changed by LXY
	toneMappedColor.a *= oe_ocean_alpha;

	// transparency = 0.5;
	// toneMappedColor.x = alpha;//smoothstep( 0, 1 ,transparency);
	// toneMappedColor.yz = toneMappedColor.xx;
	// toneMappedColor.a = 1;	

    //VRE Change by zbb nNorm
    //toneMappedColor.xyz = normalNoise;
