...
vec2 map( in vec3 pos )
{
    ...
    res = opU( res, vec2( sdCapsule(   pos,vec3(-1.3,0.90,-0.1), vec3(-1.0,0.20,0.2), 0.1  ), 31.9 ) );
	res = opU( res, vec2( sdTriPrism(  pos-vec3(-1.0,0.25,-1.0), vec2(0.25,0.05) ),43.5 ) );
	res = opU( res, vec2( sdCylinder(  pos-vec3( 1.0,0.30,-1.0), vec2(0.1,0.2) ), 8.0 ) );
	res = opU( res, vec2( sdCone(      pos-vec3( 0.0,0.90,-1.0), vec3(0.8,0.6,0.3) ), 55.0 ) );
	res = opU( res, vec2( sdTorus82(   pos-vec3( 0.0,10.25, 2.0), vec2(0.20,0.05) ),50.0 ) );
	res = opU( res, vec2( sdTorus88(   pos-vec3(-1.0,0.50, 2.0), vec2(0.20,0.05) ),43.0 ) );
...
{ 
    ...
    if( m>-0.5 )
    {
        ...
        brdf += 0.40*dom*vec3(0.90,0.70,1.00)*occ;
        brdf += 0.30*bac*vec3(0.25,0.25,0.25)*occ;
        brdf += 0.40*fre*vec3(1.90,1.00,1.00)*occ;
		brdf += 0.02;
		col = col*brdf;
