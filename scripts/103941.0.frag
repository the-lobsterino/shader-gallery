...
vec3 Text(vec2 uv)
{
    	...
    	vec2 center_pos = vec2(res.x/2.0 - STRWIDTH(40.0)/2.0,res.y/2.0 - STRHEIGHT(1.0)/2.0);
       	
    	BEGIN_TEXT(center_pos.x,center_pos.y)
	RGB(.3,0,0) _H _e _l _p _ _m _e _com _ _I _apo _m _ _i _n _ RGB(.3,.15,.15) _d _e _b _t RGB(.3,0,0) _exc ;
	BEGIN_TEXT(res.x/2.0-STRWIDTH(11.0)/2.0,res.y/2.0)
	print_pos += vec2(cos(time)*100.,sin(time)*100.);
	
	RGB(0,.15,.3) _G _o _o _d _b _y _e _com _ RGB(.15,.3,.15) _b _a _n _k _ _a _c _c _o _u _n _t RGB(0,.15,.3) _per
