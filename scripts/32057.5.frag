...
// todo : clear text and do greetings and credits in a new text.... display more texts with a clear...  
	...
	#define NOPE dist = min(dist, ddigit(float(sin(time) * 100.0) , uv - offs));offs.x += ch_space.x;
...
void main( void ) {
	...
	nd =sin(2.17*pos.x + (i*0.1+sin(+time)*0.4) + time)*0.5 + pos.x;
  ...
  gl_FragColor = cbuff + dbuff;
	...
	uv -= aspect / 2.0;
	uv *= 22.0;  ///  DIST.  do GL_Translate for zoom  ;-)
	
	float dist = 1e6;
	
	//Glitch fade-in animation
	float anim_time = clamp(time * 0.25, 0.0, 1.0) * 46.0;
	

	
	//Printing and spacing
	vec2 ch_size = vec2(1.0, 1.0);
	vec2 ch_space = ch_size + vec2(0.20,0.20);
	///////POS / Move text//////
	float posx = 13.5+atan(uv.x)*cos(time)*1.0; // Start place...  change for moving... Sceen: 16  <->  -16
	float posy = 0.7+atan(uv.x+uv.y)*sin(time*.8)*.96; // Start place...  change for moving (SIN make it jump!)... Sceen: 12  <->  -12
	///////////////////////
	vec2 offs = vec2(-ch_space.x * posx,posy);  

	F U N _ W I T H _ G L S L _ F E E L T H E _ M A G I C
			
	//Shading
	vec3 color = vec3(0.0);
	
	color = mix(vec3(cos(time)+.4,0.1,0.3), vec3(0.0,0.0,0.0), smoothstep(0.0, 0.0, dist) - (0.10 / dist));
