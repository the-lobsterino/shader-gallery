...
void main( void ) {
	...
	vec2 spiral_uv = spiralzoom(moebius,vec2(0.),6.,0.6,1.8,-vec2(0.5,0.5)*time);
	gl_FragColor = vec4(1.-border(spiral_uv,0.9));


//	gl_FragColor = mix(gl_FragColor, vec4(1.), vec4(0.3));	// fog of f war, console style! harr harr
	gl_FragColor.a = 1.;
}