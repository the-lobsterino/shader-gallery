...
{
	float d = 0.;
	vec3 col = vec3(0.1, .07+0.07*(.5+sin(gl_FragCoord.y*3.14159*1.1+time1*2.0)) 
                                        + sin(gl_FragCoord.y*.01+time+2.5)*0.05, 0.1);
	font_size = 20. + sin(time)*3.0;
	font_spacing = 1. / font_size;
	...
	d += A(r()); add(); d += Y(r()); 
	newline();
	d += T(r()); add(); d += U(r()); add(); d += N(r()); add();
	d += E(r()); add(); d += D(r());  d += STOP(r());
//	d = clamp(d* (.75+sin(gl_FragCoord.x*PI*.5-time*4.3)*.5), 0.0, 1.0);
      
	col += vec3(d*.75, d*.75, d*.05);
