...
vec2 pattern(vec2 p)
{
	...
	d = p.x;
	g += pow(clamp(1.0 - abs(d), 0.0, 1.0), 10.0);
	d = p.y;
	g += pow(clamp(1.0 - abs(d), 0.0, 1.0), 5.0);
	d = p.x - 1.0;
	g += pow(clamp(1.0 - abs(d), 0.0, 1.0), 10.0);
	d = p.y - 1.0;
	g += pow(clamp(1.0 - abs(d), 0.0, 1.0), 10.0);
...
float scene(in vec3 p)
{
	...
	#if 1
	//p *= rotatey(p, 0.5*time)*0.000022; 
	//p *= rotatex(p, 2.03*time)*0.0031; 
...
void main( void ) {
	...
	vec2 p = 1.750*( gl_FragCoord.xy / resolution.xy )-1.0;
	vec3 col = vec3(p.y*0.25)*vec3(0.75,0.85,1.0);
	
	p.x *= resolution.x/resolution.y; 
	
	vec3 ro = vec3(2.,-0.69,1.0-time);
	...
	if (dist < 100.0 && d < 3.91) {
		...
		vec3 tex = getpat(0.95*pos); 

		
		float fres = clamp(dot(n,-rd), 0.0, 1.0); 
		col = tex*vec3(1,1,1)*mix(vec3(1,1,1)*1.0, vec3(1,1,1)*0.07, fres); 
