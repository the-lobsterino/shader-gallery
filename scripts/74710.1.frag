...
#define time sin(time+surfacePosition.x*surfacePosition.y/(surfaceSize.x*surfaceSize.y))
...
float fn(vec2 p)
{
	...
	float a = abs(b + (gl_FragCoord.y * resolution.x + gl_FragCoord.x));
	//float m = (p.x*p.y);
...
void main(void)
{
  ...
  #define AA 2
       ...
       vec2 uv = surfacePosition.xy*nfc;
