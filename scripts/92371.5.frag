...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
    ...
    vec2 uv = surfacePosition;//(fragCoord.xy - 0.5 * resolution.xy) / resolution.y;

	float t0 = surfaceSize.x * surfaceSize.y;
	float t = t0*1e-5;
	float dp = dot(uv,uv);
	float ft = fract(t0-t+dp) * 2.0 - 1.0;
	uv /= cos(dp+ft);
