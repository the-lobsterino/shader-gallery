...
//uh, can you teach me how to code -a sussy guy
//i dont know what i did or what im doing

// ändrom3da here... ok, email me at yaguare44@yahoo.de
...
#define SPEED               1.20
#define TAIL                01.0  // betwenn 0.0 and 1.0
#define STAR_DENSITY        0.00
#define RAINBOWSTAR_RATIO   100.0    // every 10th star is a "rainbow star"
#define TIMEWAVE             1.0
#define COLORBAR              1
#define MAX_DIST 45000.0
#define DIFFERENCE vec2(600.0, 600.0)
...
#define TAU TAUi + 1.5                 // omg did this just happen ? 
...
void main(void)
{
	//vec3  camPos = vec3(1.55, 0.15*(time)*144., 13.*sin(0.44*time));   // movement
	vec3  camPos = vec3(1.55, 33.*sin(0.1*time),2.*sin(.1*time)+0.5*time);   // movement
	camPos.zx *= rot(.25*time);
	
	// zoommiii
	float zoomi = (1.0/1.0);
	zoomi = mix(0.6, .3, sin(time));
	
	// fragment position
	vec2 p = (zoomi)*(gl_FragCoord.xy * 2.0 - resolution) / min(resolution.x, resolution.y);
