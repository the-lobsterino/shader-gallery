/*
 * Original shader from: https://www.shadertoy.com/view/WdGSDd
 * Dark 2020 by HussNuss
 */
...
vec3 red = vec3(0.05, 0.01, 0.11);
vec3 yellow = vec3(0.05, 0.01, 0.11); // 248, 181, 51
vec3 green = vec3(0.05, 0.01, 0.11);  //1, 162, 88
vec3 blue = vec3(0.05, 0.01, 0.11);   //5, 142, 197

// 2020 with colors and shadows (premultiplied rgba)
vec4 yearCol(vec2 uv)
{
    float angle = sin(iTime) * 0.02;
    
    vec4 date = twoCol(uv, red, vec2(-3., 0.0), angle);
    date = premulBlend(zeroCol(uv, yellow, vec2(-1., 0.0), angle), date);
    date = premulBlend(twoCol(uv, green, vec2(1., 0.0), angle), date);
    date = premulBlend(zeroCol(uv, blue, vec2(3.0, 0.0), angle), date);
...
vec3 bokeh(vec2 uv)
{
    ...
    vec3 chroma = mix(vec3(0.0, 0.0, 0), vec3(0, 0, 0), uv.y * 0.1 + 0.4 + intensity * 0.4);
...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
    ...
    //add a bit of light
    dateCol.rgb -= uv.y * 0.04 * dateCol.a;
    bg.rgb -= uv.y * 0.005;
