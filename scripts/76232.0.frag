...
void main( void ) 
{
	...
	if (REPEAT_SIGN)
	  uv = -.02 / mod(3.8*(uv*asin(uv/uv)-41.0),ch_space-ch_space/vec2(16.,6.5));     //  set zoom size

	ch_pos = ch_start + vec2(sin(time),0.0);  // set start position

        _ _ S H A D E R T O Y  nl1 
	B A N N E D _ M E _ F O R   nl2
	_ _ _ _ S H I T _ P O S T I N G 	nl3
		
	vec3 color = mix(ch_color*sin(-ch_color)/(41.0+(sin(uv.y-304.25+2.2*time)/01.45)), bg_color*-bg_color*5.0, 1.0- (0.08 / d));  // shading
	gl_FragColor = vec4(color, 1.0/uv.x);
}
