/*
 * Original shader from: https://www.shadertoy.com/view/mltGRB
 */
...
void mainImage( out vec4 fragColor, in vec2 fragCoord ) {
    ...
    float time = iTime * 0.09; // bteitler: Animation is based on time, but allows you to scrub the animation based on mouse movement
   ...
   // vec3 ang = vec3(sin(time*3.0)*0.1,sin(time)*0.2+0.3,time);
    float roll = PI + sin(iTime)/1040.0 + cos(iTime/2.0)/5040.0 ;
    float pitch = PI*1.021 + (sin(iTime/2.0)+ cos(iTime))/4000.0 
