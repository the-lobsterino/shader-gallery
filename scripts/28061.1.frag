...
void main( void ) {
  ...
  gl_FragColor = cbuff + dbuff;
	...
	///////POS / Move text//////
	float posx = 13.5+sin(uv.x)*sin(time)*.3; // Start place...  change for moving... Sceen: 16  <->  -16
	float posy = 0.7+sin(uv.x+uv.y)*sin(time*.5)*.36; // Start place...  change for moving (SIN make it jump!)... Sceen: 12  <->  -12
	///////////////////////
	vec2 offs = vec2(-ch_space.x * posx,posy);  

_ _ _ _ _ _ _ W I M S E Y _ _ R U L E S _
		
	//Shading
	vec3 color = vec3(0.0);
	
	color = mix(vec3(sin(time)+.4,0.1,0.3), vec3(0.0,0.0,0.0), smoothstep(0.0, 0.0, dist) - (0.10 / dist));
