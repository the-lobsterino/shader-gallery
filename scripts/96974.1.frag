/*
 *  ändrom3d△ - a little bit of star experiment
 */

//// options
#define glowColor1   vec3( 0.0, 0.0, 0.0 )
#define glowColor2   vec3( 0.0, 0.0, 0.1 )
#define speed        8.0
...
float sineN( float x, int N )   // sinewave that only "waves" only every "Nth" period starting at 0.
{
    float o = ( sin( x - tau/4. ) + 1. )/2.;
    o *= step( mod( x, float( N )*tau ), tau );  // well, it works.
    return o;
}
...
float star( vec2 u, float size )
{
    float o;
    o += step( size , u.x*u.y ); //c = clamp( c, 0., 1. )
    u *= rot( tau/4. );
    o += step( size, u.x*u.y );
    return 1.-o;
}
...
void main( void )
{
    ...
    /// star
    float t = time;
	float ss = sineN( t*3.0, 13 );
    o += star( u/(ss*4.), 1./1024. )* ss;	
	u += vec2( 1.5, 0.75);
	u += vec2( 0.2, 0.3);
   t -= -0.5;
	float ss2 = sineN( t*3.0, 13 );
    o += star( u/(ss2*1.8), 1./1024. )* ss2;	
