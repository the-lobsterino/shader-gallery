...
vec3 render(vec3 ro, vec3 rd) {
	...
	if(pos.y > -0.99) {
		...
		vec3 mat = mix(vec3(1), vec3(0.6), noise(30.0*a));
		mat = mix(mat, vec3(0.7, 0.4, 0.1), 1.0 - f);
		mat = mix(mat, vec3(0.0), (1.0 - f)*fbm(5.0*pos));
...
void main( void ) {
	...
	float time = 0.3*time;
	vec3 ro = vec3(3.5*cos(time), 2, -3.5*sin(time));
