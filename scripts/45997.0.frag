...
float field(in vec3 p) {	
	...
	for (int i = 0; i < 6; ++i) {
		float mag = dot(p, p);
		p = abs(p) / mag + vec3(-.9, -.1 + 0.1*sin(time*0.7 + 1.0), -1.1+0.3*cos(time*0.3));
...
void main() {   
	...
	backCol2.b = 0.1*mix(backCol2.g, backCol2.b, 0.8);
	backCol2.g = 0.0;
	backCol2.bg = mix(backCol2.gb, backCol2.bg, 0.478*(cos(time*0.8) + 0.9));	
