...
vec3 sunDir	= normalize(vec3(.5, 0.0, -9.8));
...
{ 
	...
	if (mat > 0.5)
	{
		...
		col = mix(col, vec3(.9), min(dis*dis*.001, 3.0));
	}
...
void main( void )
{
	...
	gtime = mod((floor(time*4.0) / 20.0), 20.0)-.5;
#else
	...
	hello =  (1.0+sin(m*100.0)) * .02 * (smoothstep(33.0, .01, m) - smoothstep(0.05, .1, m));
	owwww = (smoothstep(0.05, .1, m)-smoothstep(0.12, .15, m))*.12;
	if (mod(time-1.0, 3.0) < .12)
			blink = true;
	
	// Camera...
	float t = clamp(time-22.5, 0.0, 1.0);
	t = t*t*(3.0-2.0*t);
	vec3 origin = mix(vec3(0.0, 1.0, -1.2), vec3(-.0, 1.0, -33.0), t);
	vec3 target = mix(vec3(0.0, 1.0, 4.0),  vec3( .0, -.8, 9.0), t);
	...
	wristL		= vec3(elbowL.x+1.0*sin(wave)*.6, elbowL.y+cos(wave)*.9, -.75);

    vec3 col = Render(origin, ray);
	
	col = sqrt(col);
	col *= 0.5 + 0.5*pow( 16.0*q.x*q.y*(1.0-q.x)*(8.0-q.y), 9.25 );
