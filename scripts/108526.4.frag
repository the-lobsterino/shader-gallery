...
vec2 Running( vec2 p)
{
	...
	runnerHeight = -.5 + .05*min(.9, cos(runCycle*2.0+0.5));
	...
	vec2 h = Segment( vec2(0.4,-0.1-0.01-headX*.5), vec2(0.42+headX,-.9), p );
	...
	if (d > 0.005)
	{
		...
		d2 = h.x - 0.03 + h.y*0.001;
		d = min(d, d2 );
		knee = Rotate(vec2(0.4,-.55), vec2(.4,-0.35), leg1+.0);
		h = Segment(vec2(0.4,-.35), knee, p );
