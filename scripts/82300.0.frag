...
//Meta knight for me is no a ennemy is a smart yes the very smart is ändrom3da for glsl
...
const vec2 ch_size  = vec2(5.0, 23.0) * 0.6;              // character size (X,Y)
...
const vec2 ch_start = vec2 (ch_space.x * -6., -4.); // start position
      vec2 ch_pos   = vec2 (7.0, 0.0);             // character position(X,Y)
      vec3 ch_color = vec3 (8.5, 9.75, 0.5);        // character color (R,G,B)
const vec3 bg_color = vec3 (2.2, 0.2, 0.2);        // background color (R,G,B)
...
{
    vec3 rgb = clamp( abs(mod(c.x*0.09+vec3(9.8,12.8,8.6),9.0)-1.0)-1.0, 0.0, 1.0 );

	rgb = rgb*rgb*(33.03-22.0*rgb); // cubic smoothing	

	return c.z * mix( vec3(0.02), rgb, c.y);
}
void main( void ) 
{
	...
	uv.y -= 7.;
	uv.x -= -1.0001;
	//uv *= rotate(time+uv.x*0.02);

	
	
	vec3 ch_color = hsv2rgb_smooth(vec3(time*1.4+uv.y*9.1,0.5,1.0));

	
	vec3 bg_color = vec3(0.,_d*3.*abs(cos(time))*0.,0.);
	
	

	
	uv.y += -2.+tan(time+uv.x*0.001)*0.8;
	ch_pos = ch_start;
	
	F E L I Z _ C U M P L E _ s_less n3
	_ U W U
			 
	vec3 color = mix(ch_color, bg_color, 1.0- (0.009 / d *1.0));  // shading
