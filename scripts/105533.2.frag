...
float tri(vec2 pos)
{
	pos+=(2.0*gl_FragCoord.xy - resolution.xy)/max(resolution.x, resolution.y) / 1.0;
	p0 = gl_FragCoord.xy/resolution.xy;
	//pos *= 1.-exp(-sin(time))+.5;
	pos*=rot;
...
void main( void ) {
	...
	c = 1. - exp(-c);
 	c = 1. - exp( -c );
	c = .24*c + .8073*texture2D(bb, p0).xyz;   
	gl_FragColor = vec4( c, 1.0 );
}//welljustanotherändrom3da4twist