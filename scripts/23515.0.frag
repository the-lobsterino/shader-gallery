...
inkNess += smoothstep(1., 1., 1.- 14.*(nearLine - STROKEWIDTH)); // ugly still
...
d += H(r()); add(); d += E(r()); add(); d += L(r()); add();d += L(r()); add();d += O(r()); add();  space(); d += F(r()); add();d += A(r()); add();d += G(r()); add(); d +=G(r()); add();d += O(r()); add();d+=T(r());
newline();
d += I(r()); space(); add();d += A(r()); add();d += M(r()); add(); space(); d += C(r()); add();d += O(r()); add();d += O(r()); add(); d +=L(r()); add(); d +=EX(r());
newline();
;
newline();

d = clamp(d* (.5+sin(gl_FragCoord.x*PI*.5-time*4.3)*.5), 0.0, 1.0);
      
    col += vec3(d*.5, d, d*.85);
vec2 xy = gl_FragCoord.xy / resolution.xy;
col *= vec3(.2, .4, .3) + 0.5*pow(100.0*xy.x*xy.y*(1.0-xy.x)*(1.0-xy.y), .4 );
