...
{
    	p.x = abs(p.x  + 0.1*sin( 5.0*length(p.zy + vec2(1.0,1.0)))) - (s.x) / 220.0;
	p.y = abs(p.y ) - (s.y) / 4.0;
	p.z = abs(p.z) - (s.z) / 4.0;
    return min(max(max(p.x,p.y),p.z),0.0) + length(max(p,0.0));
}
float scene(vec3 p)
{
	p = rotatey(p, 1.3*time);
	p = rotatex(p, 1.2*time);
	p = rotatez(p, 0.4*time);
	float d0 = sdBox(p, vec3(01.5));
	return d0;
