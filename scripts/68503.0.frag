...
void main()
{
  ...
  vec2 uv = (2.0 * npos - 1.0) * ratio;
	uv.y += sin(time+uv.x*2.2)*0.1;
  vec2 pt = uv;

	
  gl_FragColor = vec4(uv, 1.0, 1.0);

	float nx = 1.0-abs(uv.x*0.6+sin(time+uv.y*2.)*0.2)*1.25;
  vec3 col = vec3(nx,nx*.8,nx*1.3);
	col *= (abs(sin(uv.y*80.0+time)))+0.4;
	vec3 oc = col.zxy * vec3(abs(sin(uv.y*52.0+time*3.4+uv.x*80.0)));
  vec2 pos, size;
  float intensity = 0.0;
  ...
  //--- rounded rectangle ---
  const vec3 rectColor = vec3(0.0, 0.8, 0.0);
