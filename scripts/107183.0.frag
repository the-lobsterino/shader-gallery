...
vec3 gradientFlow(vec2 st, float t) {
    ...
    float r = sin(st.x * 36.30 + t);
    float g = sin(st.y * 36.30 + t + 32.0);
    float b = sin(st.x * 36.30 + st.y * 36.30 + t + 38.);
	
    vec3 c = log( (vec3(r, g, b) * 30.5 + 30.5) ); // * 30.5 + 30.5;

    return sin( t - sign(c) * mod(c * c - c, TAU) - PI) * 303.5 + 30.5;
}
