...
#define OMEGA 0.52 	// <0.8 upto GPU
#define RADIUS 240. // 
#define DECAY .985 // max.0.98
#define GRIDON true // 
#define GRID2ON true //
#define GRID2RES 8 //
...
float noise(vec2 n) 
{
    ...
    float h1 = mix(r(fn),           r(fn+vec2(11,0)), sn.x);
...
void main()
{
	...
	float map = perlin(gl_FragCoord.xy+vec2(0., time));
        green*=float(map>.18 && map<.19);
    }
    green+=lg2;
    /**   */
    float grid, grid2, grid3;
    grid = circleA(cen, RADIUS+1., .015);
    if(GRIDON)	/** the grid  */
    {
        grid+=circleA(cen, RADIUS/2., .01);
	grid+=circleA(cen, RADIUS*3./4., .01);
        grid+=circleA(cen, RADIUS/4., .01);
        for(int i=0; i<8; i++)
        {
            float phase = 2.*PI/8.*float(i);
            vec3 p0 = cen+RADIUS/4.*vec3(cos(phase), sin(phase), 0.0);
            vec3 p1 = cen+RADIUS*vec3(cos(phase), sin(phase), 0.0);
            grid2+=segment(p0, p1, .75, false);
	    /** removed peripherial grid **/
            p0 = cen+RADIUS/2.*vec3(cos(phase+PI/8.), sin(phase+PI/8.), 0.0);
            p1 = cen+RADIUS*vec3(cos(phase+PI/8.), sin(phase+PI/8.), 0.001);		
            grid2+=segment(p0, p1, .75, false);
            
        }
    }
    if(GRID2ON) /** the raster grid */
    {
	for(int i=0; i<=GRID2RES; i++)
	{
            float xp = cen.x-RADIUS+float(i)*RADIUS/float(GRID2RES)*2.;
            float yp = cen.y-RADIUS+float(i)*RADIUS/float(GRID2RES)*2.;	
	    grid3+=float(gl_FragCoord.x>xp && gl_FragCoord.x<xp+1.);
            grid3+=float(gl_FragCoord.y>yp && gl_FragCoord.y<yp+1.);
	}
        grid3*=float(length(gl_FragCoord.xy-cen.xy)-RADIUS<0.);
    }
    outColor = vec4(0., grid3+green, grid+grid2, 1.0);
