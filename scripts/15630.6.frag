...
vec3 smear(vec3 xyz)
{
	...
	vec4 xy = texture2D(backbuffer, fcn);
	...
	//return vec3(0.,0.,0.);
	return ((vec4( ((xp+xn+yp+yn+xpyp+xpyn+xnyp+xnyn+xy)/4.) ).xyz)+xyz)/2.5;
}
void main(void) {
	...
	if (f < maxd) { // cube
		...
		gl_FragColor=vec4(smear(vec3((refract(col,p,0.33)*b + pow(b, 16.))*(1. - f*f*.01))), 1.);
		...
		gl_FragColor=mix(vec4(vec3(a),1.0), vec4(c,1.0), 0.05);
		gl_FragColor.rgb = smear(gl_FragColor.rgb);
	}
}
