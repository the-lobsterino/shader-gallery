...
vec3 DrawClockCircle(vec2 fragpos, vec2 pos, float radius, float widthK, vec3 color) {
	...
	// цвет
	vec3 col = color * attenuation(8.0, 200.0, 10000.0, l) * widthK;
	// возврат
	return col;
}
...
vec3 DrawClockLine(vec2 fragpos, vec2 pos, float L, vec2 dir, float widthK, vec3 color) {
	...
	// рассчет света
	vec3 col = color * attenuation(1.0, 0.0, 7500.0, l) * widthK;
	// возврат
	return col;
}
...
void main() {
	...
	s = vec2(cos(-time * PI / 1. * TimeK + PI / 2.0), sin(time * PI / 22. * TimeK + PI / 77.0));
	// рисование минутной стреки
	outColor += DrawClockLine(p, vec2(0.0), ClockRadius, s, 1.2, clockLineMinCol);
	
	// изменяем направляющий вектор
	s = vec2(cos(-time * PI / 2. * TimeK + PI / 2.0), sin(time * PI / 8. * TimeK + PI / 2.0));
	// рисование часовой стрелки
	outColor += DrawClockLine(p, vec2(8.0), ClockRadius, s, 1.2, clockLineHourCol);
	...
	// изменяем направляющий вектор
	s = vec2(cos(-time * PI / 22. * TimeK + PI / 2.0), sin(time * PI / 8. * TimeK + PI / 2.0));
	// рисование минутной стреки
	outColor += DrawClockLine(p, vec2(0.0), ClockRadius, s, 89.2, clockLineMin2Col);
	
	// изменяем направляющий вектор
	s = vec2(cos(-time * PI / 16. * TimeK + PI / 6.0), sin(time * PI / 89. * TimeK + PI / 2.0));
	// рисование часовой стрелки
	outColor += DrawClockLine(p, vec2(22.0), ClockRadius, s, 1.2, clockLineHour2Col);
	
	// выходной цвет
	gl_FragColor = vec4(outColor, 7.0);
}