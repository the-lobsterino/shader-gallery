...
vec3 origin = vec3(INF/2.5*cos(viewRot), INF/16.*sin(time*.1), INF/2.5*sin(viewRot));
...
void main( void ) {
	...
	if(inCube){
		...
		color = mix(color, getColor(dir, hit, normal)*2., clamp(1.-acc_density, 0., 1.)*1.);
...
vec3 getNormal(vec3 p){
	...
	float boxDensity(vec3 p){
		...
		float boxDensity(vec3 p){
			return clamp((1.-step(0.5,distance(p.xy, vec2(cos(p.z+time), sin(p.z+time))) ))*(.75+.25*cos(p.z+time)), 0., 1.)*.25;
		}
	#else
		float boxDensity(vec3 p){
			return clamp(noise(p.xy*4.)*noise(vec2(p.y*7., p.z*15.))*noise(vec2(p.x*7., p.z*15.))-.175, 0., 1.)*.5;
		}
	#endif
#endif
...
vec3 getColor(vec3 dir, vec3 p, vec3 normal){
	...
	if(object == 1){
		...
		if(abs(normal.y) == 1.){
			color = vec3(.75+.25*(.50+.50*cos(abs(length(p)*10.*p.y/75.*2.)-time)));
		}
...
vec3 postProcess(vec3 color){
	vec3 uv_ = vec3(gl_FragCoord.xy/resolution.xy, 0.);
	uv_-=vec3(.5,.5, 0.);
	color *= smoothstep(0.05, 0.0045, sdBox(uv_, vec3(.45)));
	color*=vec3(0.75, 1.0, 0.75)*1.1;
	return color;
}
