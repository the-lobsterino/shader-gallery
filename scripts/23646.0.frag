...
// (The camera code was taken from an example by @mrdoob)
// (Bloom + Vignette by @AlteredQ)
...
void powN1(inout vec3 z, float r, inout float dr) {
	...
	//z = zr*vec3(cos(theta)*cos(phi), cos(phi)*sin(theta), sin(theta));
	z = zr*0.9*vec3(sin(phi), cos(phi)*sin(theta), cos(phi)*cos(theta));
	//z = zr*vec3(sin(theta),cos(phi),cos(phi));
}
