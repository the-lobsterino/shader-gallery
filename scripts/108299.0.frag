...
vec4 snow(vec2 uv, float scale) {
    ...
    if (w < 0.4) return vec4(0.0); // Возвращаем полностью прозрачный цвет для "прозрачных" частиц
    uv += time / scale;
    uv.y += time * -0.1 / scale; // Уменьшаем скорость падения снежинок
    uv.x += sin(uv.y + time * 0.1) / scale;
    uv *= scale;
    vec2 s = floor(uv), f = fract(uv), p;
    float k = 3.0, d;
    p = 0.5 + 0.25 * sin(11.0 * fract(sin((s + p + scale) * mat2(9, 3, 6, 5)) * 2.0)) - f; // Уменьшаем амплитуду движения
    d = length(p);
    k = min(d, k);
    k = smoothstep(0.0, k, sin(f.x + f.y) * 0.02);
...
void main(void) {
   ...
   // finalColor += snow(uv, 2.0);
    finalColor += snow(uv, 0.6);
