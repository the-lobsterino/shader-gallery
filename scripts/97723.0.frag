...
vec2 print_pos = vec2(10);
vec2 print_pos_pre_move = vec2(10);
vec3 text_color = vec3(10);
...
vec3 Text(vec2 uv)
{
	...
	HEX(0x00FF7F) _ _ _K _E _S _S _I _E _ _I _S _ _C _O _O _L
	BEGIN_TEXT(res.x/2.0-STRWIDTH(1.0)/2.0,res.y/2.0)
...
void main( void )
{
	...
	vec3 col = pixel*0.9+0.21;
	col *= (1.-distance(mod(uv,vec2(1.0)),vec2(0.65)))*1.2;
	
	gl_FragColor = vec4(vec3(col), 100000);
