...
vec4 map(in vec3 p) {
    ...
    float wv = (tan(pi.x*1.72+iTime*1.2) + tan(pi.z*2.27+iTime*1.2))*.25;
    vec2 swv = vec2(.2*tan(pi.x*1.72+iTime*2.5),.2*tan(pi.x*1.72+iTime*2.5));
     float cap = min(
...
vec3 get_color(float m, vec2 bi){
    ...
    if(m==2.) {
    	float md = mod(bi.x,88.);
    	mate = md<.1 ? vec3(.02) : vec3(.22);
    }
