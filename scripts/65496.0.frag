...
{   
	...
	float h = 1024.*hash(fract(time*.1));
	uv = floor(uv*h)/h;
	...
	//sample further out to accellerate incoming particles that approach the mouse
	float velocity_field 	= (length((uv-mouse)*vec2(resolution.x/resolution.y, 1.)*-31.));
	velocity_field 		= 1.;//max(1., 1. + velocity_field);
