...
// FUCK YOU FUCKING PIECE OF SHIT DUMB FAGGOT!
...
{
	float curve = cos(p.x*(5.5+sin(time)*1.25))*.11;
	p.y += curve;
	a.y += (curve*curve);
...
{	// Floor...
	...
	res.x = sMin(res.x, d);
	// Right hand...	
	d = Segment(p*vec3(1.0, .95, 1.0), wristR+vec3(0, .1, 0.), wristR+vec3(-.15, .05, -.15), .14, .02);
	res.x = min(res.x, d);
	...
	p = (p-wristL);
	p.z -= p.x*.5;
	p = RotMat(vec3(8.0, 0.0, 1.0), -wave*1.5) * p;
	d = Segment(p, vec3(0.0), -vec3(-.95, -0.15, 0.1), .06, .01);
	res.x = sMin(res.x, d);
	...
	res.x = sMin(res.x, d);
	ankle.y -=.3;
	vec3 foot = ankle + vec3(0.27, +.05, +0.0);
	d = Segment(p, ankle, foot, .25, .05);
	d = max((ankle.y-p.y), d);
	res.x = sMin(res.x, d);
	
	//A FUCKING DICK BITCH YOU FUCKING SON OF A BTICHES COCKSUCKERS MOTHER FUCKERS.
	d = Segment(p, vec3(0.,-1.,-0.), vec3(0.,-0.0+(wave/20.),-1.), .1, .01);
	res.x = sMin(res.x, d);
