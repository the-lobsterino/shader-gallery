// It started yesterday. Frame on, frame off. No driver change.
// AMD R9380 2GB/Chrome/Windows 10
// Update: making a new shader fixes it... but only for the new shader.
// Update 2: changing the resolution appears to fix the flickering when viewing any shader.


// 060520 Ncp
// A word is a symbol for a hole world
...
const vec2 ch_start = vec2 (ch_space.x * -6., 2.); // start position
      ...
      vec3 ch_color = vec3 (0.0, 3.0, 5.);        // character color (R,G,B)
...
#define one ddigit(0x0018);
#define two ddigit(0x1177);
#define three ddigit(0x9167);
#define four ddigit(0x1189);
#define five ddigit(0x09E6);
#define six ddigit(0x11F8);
#define seven ddigit(0x0087);
#define eight ddigit(0x11FF);
#define nine ddigit(0x11EF);
...
void ddigit(int n)
{
	...
	if (bit(n,  4)) v = min(v, dseg(vec2(-0.500, -0.063), vec2(-0.500, -0.8)));
	if (bit(n,  5)) v = min(v, dseg(vec2(-0.438, -1.000), vec2(-0.063, -1.000)));
	if (bit(n,  6)) v = min(v, dseg(vec2( 0.063, -1.000), vec2( 0.438, -1.000)));
	if (bit(n,  7)) v = min(v, dseg(vec2( 0.500, -0.938), vec2( 0.500, -0.03)));
	if (bit(n,  8)) v = min(v, dseg(vec2( 0.063,  0.000), vec2( 0.438, -0.000)));
	if (bit(n,  9)) v = min(v, dseg(vec2( 0.063,  0.063), vec2( 0.438,  0.938)));
	if (bit(n, 10)) v = min(v, dseg(vec2( 0.000,  0.063), vec2( 0.000,  0.7)));
	if (bit(n, 11)) v = min(v, dseg(vec2(-0.063,  0.063), vec2(-0.438,  0.8)));
	if (bit(n, 12)) v = min(v, dseg(vec2(-0.438,  0.000), vec2(-0.063, -0.00)));
	if (bit(n, 13)) v = min(v, dseg(vec2(-0.063, -0.063), vec2(-0.438, -0.8)));
	if (bit(n, 14)) v = min(v, dseg(vec2( 0.000, -0.938), vec2( 0.000, -0.03)));
	if (bit(n, 15)) v = min(v, dseg(vec2( 0.063, -0.063), vec2( 0.438, -0.8)));
	ch_pos.x += ch_space.x;
	d = min(d, v);
}
...
void main( void ) 
{
	...
	uv *= 27.0 + sin(time);     //  set zoom size
	uv.x += 5.;
	if (REPEAT_SIGN)
	  uv = -1.0 + mod(1.8*(uv-1.0),ch_space*vec2(6.,6.5));     //  set zoom size
	uv.x += 5.;

	ch_pos = ch_start + vec2(sin(time),0.0);  // set start position

        _ _ _ _ _ _ P O J D U _ L E J A T 
nl1	_ _ _ _ _ _ _ one two three four five six seven eight nine O

		
	vec3 color = mix(ch_color*(1.0+(sin(uv.x*0.25+.2*time)*0.45)), bg_color, 1.0- (0.08 / d));  // shading
	
	color -= 1.5+.5*(cos(uv.x+time)+cos(uv.y+time));
