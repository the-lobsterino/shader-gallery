...
void main( void ) {
	...
	vec2 position = ( gl_FragCoord.xy / resolution.xy ) + mouse * 0.0;
	...
	float equation2 = 0.0;
	//float size = 8.0;
	float size = 4.0 * sin(time)*0.4 + 4.0;
	...
	gl_FragColor = vec4( vec3((color1+color4)/4.0,(color2+color5)/21.0, (color3+color6)/23.0 ), 1.0 );
