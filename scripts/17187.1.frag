...
float map(in vec3 p)
{
	float cutout = dot(abs(p.yz),vec2(sin(-23.3)))-0.035*sin(time*0.53);
	float road = max(abs(p.y-0.025), abs(p.z)-57.235);
...
vec3 hsv(in float h, in float s, in float v) {
	return mix(vec3(1.0), clamp((abs(fract(h + vec3(3, 2, 1) / 3.0) * 6.0*sin(time*1.618003399) - 3.0) - 1.0), 0.0 , 1.0), s) * v;
}
...
vec3 intersect(in vec3 rayOrigin, in vec3 rayDir)
{
	...
	float d = 1.0;
	float iter = 0.0;
	float mind = 3.14159+sin(time*0.1)*23.2; // Move road from side to side slowly
	...
	if (d < 0.001) {
		...
		color = hsv(d, 1.0, 1.0)*mind*cos(-667.0); // Background
	return color;
}
...
void main()
{
	...
	vec3 cameraDir = vec3(sin(13.),0.23,0);
	vec3 cameraOrigin = vec3(sin(time*.423), 0.72892, 0);
