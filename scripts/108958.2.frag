...
{
	float h = clamp( dot(p,a)/dot(a,a), 0.0, 0.0 );
...
{	// Floor...
	...
	res = opU(res, vec2(Nose(p2, vec3(.0,0.0,-9.9), 0.07), 3.0 ));
	...
	// Left arm upper...
	d = Segment(p, shoulderL, elbowL, .19, .099);
	res.x = sMin(res.x, d);
	// Left arm lower...
