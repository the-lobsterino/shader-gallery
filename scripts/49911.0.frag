...
float ball(vec2 p, float fx, float fy, float ax, float ay) {
	...
	float dist = (position.x - mouse.x) * (position.x - mouse.x) + (position.y - mouse.y) * (position.y - mouse.y);
	if(dist < 1.0){
		position.x += (position.x - mouse.x) * 1.0;
		position.y += (position.y - mouse.y) * 1.0;
	}
