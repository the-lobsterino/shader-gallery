/*
 * Original shader from: https://www.shadertoy.com/view/NtSXRc
 */
...
vec2 mp( vec3 p ,float ga){ 
  ...
  pp.xy=vec2(abs(abs(pp.x)-18.2)-(1.+4.*b),abs(pp.y));    //CLONE MAIN POS A COUPLE OF TIMES
  pp.z=mod(pp.z,10.)-1.; //INFINITE MODULO MAIN POS TO REPEAT FOLDABLE BIT ALONG Z 
  rp=pp; //ROTATE POSITION BASED ON MAIN POSITION
  rp.y-=8.*b+1.;  //SHIFT UP AND DOWN
  ...
  for(int i=0;i<3;i++){ //FRACTAL GREEBLE BULLSHIT
    ...
    gr=min(gr,clamp(sin(gp.x*5.),-.95,.95)*.5*sca.x); //ADD ITERATION GREEBLE TO PREVIOUS GREEBLE DETAIL FOR GREEBLE ON FLOOR
