...
//		color	=	pow(color,1.1);
		...
		a = vec3(p.y) + sin(p.x*(i*0.5*vec3(1.0,1.0-sin(p.x)*0.02,1.0-sin(p.x)*0.05))+time*1.+sin(time*(0.0001 + i*0.0001))*5.0+noise)*0.0005*i-0.05*sin(time+i*0.2);
		
		color += sin(max(vec3(0.0),pow(vec3(1.0)-abs(a),vec3(800.,600.,100.))*0.5+pow(vec3(1.0)-abs(a),vec3(50.,45.,40.))*0.35));
		
		
		if(a.x < 0.0){
			color += vec3(0.6-i*0.1,-1.5+i*0.2,1.0+i*0.05)*(0.1-pow(a.x,5.)*5.)*0.1;
