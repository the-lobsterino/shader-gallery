...
float mandelbulb(vec3 pos){
	...
	if(sin(transitionTime)>0. && sin(transitionTime)<=0.5){
        ...
        Power=2.+sin(transitionTime)*114.;	
	}else if(sin(transitionTime)<=-0.25){
        Power=4.;
	}
	...
	//When length(pos)>4.,escape the loop     
        ...
        float theta = acos(pos.y/r1) ;//+ sin(t2); 
	...
	//Distance estimator?
        ...
        phi = phi*Power+1.-mouse.y/mouse.x*6.5;//
	...
	//pos = (vec3(sin(theta)*cos(phi),sin(theta)*cos(phi),cos(theta))*zr)+inputPos;  
        ...
        pos = (vec3(sin(theta*2.)*cos(phi), sin(phi)*sin(theta), cos(theta))*zr)+inputPos; 
...
//    gl_FragColor = vec4(vec3(trace(ray, rayDir)),0.0);	
	...
	float background_strips = 11.2;//0.04; //adding color and strips to the background
	...
	//🔵🟣 purple-blue
	//vec4 result = vec4( 1.0-vec3(0.01*c, 2.0*c, 0.5*c) - vec3(0.025, 0.1, 0.)*count*0.4, 1.0 ); 
	    //⚪️🔵🟡
	    //vec4 result = vec4( 1.0-vec3(background_strips*c, 2.0*c, 0.000005*c*-0.3) - vec3((0.025, 0.1, 0.25)*count*brightness*0.13), 1.0 ); 
    vec4 result = vec4( 1.0-vec3(background_strips*c, 2.0*c, 0.5*c) - vec3(0.025, 0.1, 0.25)*count*brightness, 1.0 ); 
    gl_FragColor = result;
}