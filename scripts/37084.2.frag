...
float RaySphereIntersection(in Ray ray, in Sphere sphere)
{
    ...
    float discrimnant = b * b - 4.0 * c;
    if (discrimnant < 0.0) return -1.0;
    float q = b < 0.0 ? ((-b - sqrt(discrimnant))/2.0) : ((-b + sqrt(discrimnant))/2.0);
    float t0 = q;
    float t1 = c / q;
    if (t0 > t1) {
        float temp = t0;
...
void main( void ) {
	...
	float hitdist = RaySphereIntersection(Ray(camera, dir), Sphere(spherepos, sphereradius));
