...
vec4 inverseSF( in vec3 p, const in float n )
{
    ...
    vec2  F   = vec2(floor(Fk+.5),floor(Fk*PHI+.5));
...
vec3 calcNormal( in vec3 pos, in float ep )
{
    ...
    {
        vec3 e = 0.5773*(2.0*vec3(i==0||i==3,i>=2,i==1||i==3)-1.0);
...
void main( void )
{
	...
	fragCoord -= iResolution * 0.5;
	fragCoord *= 0.3;
	fragCoord.y += iResolution.y * 0.35;
	fragCoord += iResolution * 0.5;
	
    float an = (iTime-10.0)*0.05;
    
    // camera	
    vec3 ro = vec3( 0.0, -4.5*sin(an), -4.5*cos(an) );
