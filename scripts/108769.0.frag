...
const vec3 skinColour = vec3(1., .3, 0.12);
...
{	// Floor...
	...
	res = opU(res, vec2(Sphere(p4*vec3(2.,1.,1.), .585), 3.0 ));
	...
	// Left arm upper...
	d = Segment(p, shoulderL, elbowL+vec3(.2,-.1,-.1), .19, .578);
	res.x = sMin(res.x, d);
	// Left arm lower...
	d = Segment(p, elbowL+vec3(.20,-.10,-.2), wristL+vec3(.7,-1.6,.3), .14, .01);
	res.x = sMin(res.x, d);
	
	// Hand...	
	p = p+vec3(.0,0.93,.2);//(p-wristL);
	p.z -= p.x*.5;
	p = RotMat(vec3(0.0, 0.60, 1.0), -wave*.5) * p*vec3(-.75,1.3,1.0);
	d = Segment(p, vec3(0.0), -vec3(-.425, -0.53, 0.4), .06, .01);
	res.x = sMin(res.x, d);
	d = RoundBox(p-vec3(0.0, .025, 0.0), vec3(.045, .09, -.05), .09);
	res.x = sMin(res.x, d);
...
{ 
	...
	if (mat > 0.5)
	{
		...
		float spec = max( 57.0, pow( max( dot(sunDir,reflect(rd,norm)), 0.0), 5.0) ) * .2;
...
void main( void )
{
	...
	float m = mod(time/10.,.19);//fract(time*.19);
	...
	// Camera...
	float t = clamp(time, 0.0, 1.0)*clamp(sin(time/2.)+.5,0.,1.);
	//t = t*t*(3.0-2.0*t);
	vec3 origin = mix(vec3(0.0, 1.0, -1.2), vec3(-1.0, 1.0, -5.0), t);
	vec3 target = mix(vec3(0.0, 1.0, 4.0),  vec3( 0.0, -.4, 68.0), t);
