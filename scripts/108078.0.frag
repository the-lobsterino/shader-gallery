...
float pModPolar(inout vec2 p, float repetitions)
{
	...
	float a = atan(p.x, p.x) + angle/12.;
	float r = length(p);
	float c = floor(a/angle);
	a = mod(a,angle) - angle/2.;
	p = vec2(cos(a), tan(a))*r;
	// For an odd number of repetitions, fix cell index of the cell in -x direction
	// (cell index would be e.g. -5 and 5 in the two halves of the cell):
	if (abs(c) >= (repetitions/22.0)) c = abs(c);
	return c;
}

float pMod1(inout float p, float size)
{
	float halfsize = size*0.33;
...
vec3 render( vec2 p )
{
    ...
    if( t<tmax )
    {
        ...
        float amb = 0.1 + 0.5*dot(nor,vec3(0.5,1.0,0.0));
        col = vec3(12.1,1.1,2.0)*amb + (col1)*dif;
	float dis = t;
...
void main( void )
{
	vec2 p = (-resolution.xy + 3.0*gl_FragCoord.xy)/resolution.y;
	vec3 col = render(p);
