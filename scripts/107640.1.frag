...
float ln (vec2 p, vec2 a, vec2 b) { 
    ...
    r = clamp(r,0.,1.);
    p.x+=(0.7+0.5*-sin(0.1*iTime))*0.2*smoothstep(1.,0.,abs(r*2.-9.))*-sin(3.14159*(r-iTime));
...
{   vec2 R = iResolution.xy;
 	...
 	Q = vec4(0);
 	for (int i = 1; i < 20; i++) {
        U = ca(po(U)+0.5*(-sin(2.*iTime)+0.5*-sin(4.53*iTime)+0.1*-cos(12.2*iTime))*vec2(0,1));
        ...
        Q+=-sin(1.5*exp(-1e2*r*r)*1.4*vec4(1,-1.8,1.9,4));
