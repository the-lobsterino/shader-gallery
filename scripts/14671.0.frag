...
vec3 color(vec3 space){
	...
	for(int i=1;i<complexity+1;i++)
	{
		...
		space.x+=ee*(cos(s.z*(ii*pi))+cos(s.y*(ii*hfpi))+(sin(s.x*(ee*sqpi)))+(sin(s.z*(ee*hfpi))));
		space.y+=ee*(sin(s.z*(ii*pi))+cos(s.x*(ii*hfpi))+(cos(s.y*(ee*sqpi)))+(cos(s.z*(ee*hfpi))));
...
void main(){
	...
	gl_FragColor=vec4(color(pi*vec3(surfacePosition,(time/pisq+sin(time/picu)*cos(time/pipi)))), 1.0);
}
