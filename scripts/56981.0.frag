...
const vec2 ch_size  = vec2(1.0, 2.0) * 0.3;              // character size (X,Y)
const vec2 ch_space = ch_size + vec2(1.0, 1.0);    // character distance Vector(X,Y)
const vec2 ch_start = vec2 (ch_space.x * -11., 1.); // start position
...
void main( void ) 
{
	...
	uv *= 20.0 +sin(time)*4.0;
	
	uv *= rotate(sin(time*2.0)*0.2);
	...
	ch_pos = ch_start;
	           ...
	           Y O _ C H E C K _ O U T _ M Y _ P E N I S
