...
F f(T a)
{
    ...
    {
        F H=c*(t*.002);
        a.xy=mat2(sin(H),-cos(H),sin(H),cos(H))*a.xy;
        F W=pow(1.6,c*.7);
...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
	...
	T P=T(atan(-t*.204),cos(t*.04),0.)*-.35+v.yyx*.8,
		j=normalize(v.yxx-P),
		S=cross(j,v.xyx),
		U=cross(S,j),
    ...
    d=pow(l*.9,2.);
	fragColor=vec4((dot(-w,g(p,1E-4))*min(1.-d*.01,0.)+L(d+.2))*((d+L((1.-dot(g(p,1E-4),g(p,1E-3)))*8.+.1)*pow(.7,mod(t,9.)))*T(.2,.3,.6)+T(.1))*(1.-pow(1E-7,t*.04))*(1.3-length(u)*.7),1.);
}
