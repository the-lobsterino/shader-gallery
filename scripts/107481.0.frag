...
vec3 gradientFlow(vec2 st, float t) {
    ...
    float b = sin(st.x * 6.0 + st.y * 6.0 + t + 4.0);
	
    vec3 c = sin( st.x*st.y * (vec3(r, g, b) * 0.5 + 0.5) ); // * 0.5 + 0.5;
...
void main() {
    ...
    vec3 color = mix( colorA, colorB, dot(colorA,-colorB) );
