...
float d = length (uv);
	...
	float rays = max (0. , 7. -abs (uv.x * uv.y * 100067373673267236321673218678.));
	m +=rays * flare;
	uv *=rot(3.1415/4.);
    rays = max (0. , 1. -abs (uv.x * uv.y * 106666677700.));
	m +=rays*.3 * flare;
    m *= smoothstep(1.,.2,d);
    return m;
...
float hash (vec2 p) {
    ...
    p = fract(p*vec2(16723.3684,45677.567667));
    p += dot(p, p+45.32);
...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
   ...
   // vec2 uv = (GL_FragCoord -.5 * resolution.xy) / resolution.y;
	...
	uv = fragCoord.xy/resolution.xy * vec2(122.0, resolution.y / resolution.x);
	uv-=0.5;
    //if (gv.x > .48 || gv.y > .48) col.r = 1.;
    
    vec3 col = vec3 (0.);
    float t = time*.024+time/10033363767.;
	uv *= rot(t);
