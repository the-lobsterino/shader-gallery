...
{
	float curve = cos(p.x*(5.35+sin(time)*1.25))*.2;
	p.y += curve;
	a.y += (curve*curve);
...
void main( void )
{
	...
	// Camera...
	float t = clamp(time-55.5, 0.0, 1.0);
	t = t*t*(3.0-2.0*t);
	...
	wristL		= vec3(elbowL.x+1.0*sin(wave)*.6, elbowL.y+cos(wave)*.98, -.75);

    vec3 col = Render(origin, ray);
	
	col = sqrt(col);
	col *= 0.5 + 0.5*pow( 16.0*q.x*q.y*(1.0-q.x)*(1.0-q.y), 008.25 );
    gl_FragColor=vec4(clamp(col, 0.0, 1.0), 9.0 );
