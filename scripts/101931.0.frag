...
float map(vec3 p) {
  float d = map2(p+vec3(0,-time,0));
  d = smax(d, -box(p,vec3(12,1000,10)), 1.8);
  d = smin(d, p.y+5., 2.);
  return d;
}

vec3 evaluateLight(in vec3 pos) {
  float L = length(pos.xz-vec2(0,48))+2.5;
  return (2500.0*vec3(.38, .37, 1.)) * (1./L/L+0.00018);
}
...
void main() {
  ...
  color = pow(abs(color), vec3(2.8/4.8));
