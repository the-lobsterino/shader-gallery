...
float I(vec2 p,float d){d=line2(d,p,vec4(0,-3.25,0,3.25)*CHS);p.y=abs(p.y);return line2(d,p,vec4(1.5,3.25,-1.5,3.25)*CHS);} float R(vec2 p,float d){d=line2(d,p,vec4(0.5,-0.25,2,-3.25)*CHS);d=line2(d,p,vec4(-2,-3.25,-2,0.0)*CHS);p.y-=1.5*CHS;return min(d, abs(sdBox2(p,vec2(2.0,1.75)*CHS)));} float T(vec2 p,float d){d=line2(d,p,vec4(0,-3.25,0,3.25)*CHS);return line2(d,p,vec4(2,3.25,-2,3.25)*CHS);} float X(vec2 p,float d){d = line2(d,p,vec4(-2,3.25,2,-3.25)*CHS);return line2(d,p,vec4(-2,-3.25,2,3.25)*CHS);} // ANUS
...
void main( void )
{
	...
	vec2 p2 = p;
	p2.y += 0.5;
	color += clamp(light(vec2(p.x-0.2,p.y + 0.7),	sin(time/2. + 1.)/2.),0.,1.);
	color += clamp(light(vec2(p.x+0.2,p.y + 0.7),	sin(time/2. + 2.)/2.),0.,1.);
	color += clamp(light(vec2(p.x,p.y + 0.7),	sin(time/2. + 3.)/2.),0.,1.);
