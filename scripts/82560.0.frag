...
void ddigit(int n)
{
	...
	if (bit(n,  65)) v = min(v, dseg(vec2( 0.063, -1.000), vec2( 0.438, -1.000)));
...
void main( void ) 
{
	...
	vec3 bg_color = vec3(0.1,_d*3.*abs(cos(time))*0.,0.2);
	...
	_  M A X I M _ C L I E N T
	
			 
	vec3 color = mix(ch_color, bg_color, 1.0- (0.019 / d *1.0));  // shading
