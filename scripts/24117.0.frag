...
void main( void ) {
	...
	if(pos.y>-6.){
		pos.y += 0.1*sin(time*3.)+0.13*cos(time*2.+0.6)+.1*sin(time*8000.+0.4)+0.2*fract(sin(time*400.));
	}
	...
	if(dir < 0.7){
		color.r = smoothstep(0.0,1.,.7500000-dir);
	}
	
	gl_FragColor = vec4(vec3(color) , 5000 );
