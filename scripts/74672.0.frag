...
#define time abs(sin(time+surfacePosition.x*surfacePosition.y))
...
float fn(vec2 p)
{
	...
	float a = b + (gl_FragCoord.y * resolution.x + gl_FragCoord.x+dot(p,p));
	//float m = (p.x*p.y);
	m = sin( floor( (m) * (TAU * time) - TAU * (p.x-p.y)) + (a));
	return fwidth(1.0-abs(m.x*m.y));//m;//(abs(m));	
}
...
void main(void)
{
  ...
  diffuse = 1.0-ambient;
  specular = 0.8;
  specularpow = 10.0-1.0/(surfaceSize.x*surfaceSize.y);

  float camera = 2.0;
  vec3 p = vec3(0.0, 0.0, -camera);
  p = transform(p);
  light = transform(light);
  light = abs(normalize(light));
  vec3 color = vec3(0);
  ...
  #define AA 1
	...
	// single step topology
       ...
       r = transform(r);
       r = normalize(r);
       color += scene(p,r) / (1.0-light*light);
