...
float wheel(vec2 p,float t)
{
  ...
  vec2 m = vec2(mod(p.x,.0)-1.0, mod(p.y,2.0)-1.0);
  
  float a = pow(sin(t + 4.0 * atan(m.y,m.x)), 4.0);
  float e = 0.355 + min(a * 0.28, 0.26);
  float d = length(m);
	
  return d < e && d > 0.3 ? 1.0 - abs(sin(t+4.0*atan(m.y,m.x)*1.0))*0.5  : 0.0;   
}
