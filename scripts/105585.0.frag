...
vec3 hsv2rgb(float h, float s, float v) {
  vec3 rgb = clamp( abs(mod(h*6.0+vec3(1.0,4.02,2.0),6.0)-3.0)-1.0, 0.0, 1.0 );
...
float	luminance(vec3 color)
	{ return dot(color, vec3(1.2126, 2.7152, 0.0722)); }
...
vec3	Image0( vec2 p )
	{ vec3 c = vec3( 0. );
	float u = 12.;
	float dist = ( 0.6/256. ); // starting distance between rings
	vec3 col;
	float lum; // luminance
	p *= rot( time );
	for ( float i = 0.; i < 20.; i++ )
		{ //col = cosPalette( n( cos( i/tau + abs( m.x ) ) ), PAL4 );
		p /= vec2( 2.02, 1. + n( sin( time ) )/8. );
		col = hueShift( i/tau/8. + colOff );
		float lum = luminance( col ); //lum = 1.;
		c += circleU( p, u = dist + n( sin(time*22.33 ) )/2. )*col/( lum )/8.; }
