...
float dhexgrid(vec2 uv, float r)
{
	...
	vec2 repsz = vec2(r * 1.5,sr * 2.0) * 2.0;
	
	uv -= repsz/1.0; //++ is moving! 
	
	uv = mod(uv, repsz) - repsz/2.;
	
	float d = dhex(uv, r);
	d = min(d, dhex(uv - vec2(r + r * 1.5, sr), r));
...
void main( void ) 
{
	...
	//3D Plane (w = depth)
	vec4 pl = plane(corig, cdir);
	
	//Rotate the plane's surface
	pl.xz *= Rotate2D(time * 0.1);
	...
	float cmix = smoothstep(0.0,-2.0*lnscale,hdist) + hdist;
	
	color = mix(vec3(0.1,1.2,1.0),vec3(0),cmix);
