...
vec3 Text(vec2 uv)
{
	...
	print_pos += vec2(0.+cos(time)*7.,sin(time*sqrt(2.))*7.);
	
	RGB(sin(3.*time)/2. +0.5, -sin(4.*time)/2. +0.5, cos(5.*time)/2. +0.5) _I _F _ _Y _O _U _ _S _E _E _ _T _R _A _N _N _Y _ _C _O _C _K _exc
	print_pos += vec2(-140.+cos(time)*7.,sin(time*sqrt(2.))*7.);
	RGB(sin(3.*time)/2. +0.5, -sin(5.*time)/2. +0.5, cos(5.*time)/2. +0.5) _Y _O _U _ _W _A _N _T _ _T _R _A _N _N _Y _ _C _O _C _K _exc
