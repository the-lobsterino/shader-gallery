...
void main() {
	...
	float rx = position.x + sin(angle * (freq - 1.5)) * step * (resolution.x / 4096.0);
	float ry = position.y + cos(angle * (freq - 1.0)) * step * (resolution.y / 4096.0);
	vec2 rv = vec2(rx, ry);
	float r  = distance(rv, center);
	
	float rr = rrrr * min(resolution.x, resolution.y) / .6;
