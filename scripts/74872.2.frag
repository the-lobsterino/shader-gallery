...
#define time fn(surfacePosition.xy)
//#define time fwidth((surfaceSize.x*surfaceSize.y)*dot(surfacePosition,surfacePosition))
//#define time (dot(surfaceSize,surfaceSize)/dot(surfacePosition,surfacePosition))
//#define time time*fwidth(sin(time)+cos(time))/(surfaceSize.x*surfaceSize.y)
//#define time fwidth(sin(time))
...
{
	return bpt_xy2rgb3dot(pos,dot(pos,pos));
}
...
float N = 8.0, M = 8.0; // Subdivisions for xy and zw torus components
...
bool solve(vec3 p0, vec3 r, float tmin, inout Result result) {
  ...
  result.p = p; result.n = n; result.color = basecolor; //applylighting(basecolor,p,n,r);
  return true;
}
...
void main(void)
{
  ...
  float camera = 0.0;
  vec3 p = vec3(mouse * 2.0 - 1.0, -camera);
	p = normalize(p);
  p = transform(p);
  ...
  vec2 uv = light.xy;
   ...
   vec2 nfc = ((2.0*(gl_FragCoord.xy) - resolution.xy)/resolution.y);
       ...
       uv = surfacePosition.xy*nfc;//((2.0*(gl_FragCoord.xy) - resolution.xy)/resolution.y);
...
//	r = sin( r * (2.0-dot(r,r)) );
  ...
  for (int i = -AA; i < AA; i++) {
     for (int j = -AA; j < AA; j++) {
       uv = surfacePosition.xy*((2.0*(gl_FragCoord.xy+vec2(i,j)/float(AA*AA)) - resolution.xy)/resolution.y);
       vec3 r = normalize(vec3(uv, 2.0));
       r = transform(r);
       r = normalize(r);
       color += scene(p,r);
	...
	color = mod(color,vec3(2.0))/2.0;
  //color /= float((3*AA)*(3*AA));
...
#if 0
   vec4 o;
   FabriceNeyret2_mainImage( o, nfc/color.xy * cos(color.z*PI) ); //*surfacePosition.xy*surfaceSize ); //color.xy / surfaceSize.xy ); //(uv-color.xy)*(resolution.xy) );
   float alpha = o.w;// * (1.0-color.b);
   color = mix( o.xyz, color, 1.0-alpha );
   //color = vec3(nfc.xy*color.b,0.0);
#endif
	
	
  //if (alert) color = vec3(1.0*step(fract(time),0.5),0,0);
