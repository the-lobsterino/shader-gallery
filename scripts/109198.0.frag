...
const float numSteps = 111.0; // Number of color steps.
...
void main(void) {
    ...
    vec3 color = vec3(0.2, 0.2, 0.2); // Dark gray background.
    ...
    if (uv.y < 0.25) {
        color = vec3(colorIntensity, colorIntensity, colorIntensity); // Gray
    } else if (uv.y >= 0.25 && uv.y < 0.5) {
        color = vec3(colorIntensity, 0.0, 0.0); // Red
    } else if (uv.y >= 0.5 && uv.y < 0.75) {
        color = vec3(0.0, colorIntensity, 0.0); // Green
    } else if (uv.y >= 0.75) {
