...
void main( void ) {
	...
	//noise1
	vec3 vv = snoiseVec3(vec3(position.x * 3.0, position.y * 3.0+time*0.6, time));
	
	float gray = length(vv.rgb);
	...
	vec3 fragColor = vec3(dFdy(gray)) * 20.0;
	
	gl_FragColor = vec4( fragColor / col, 1.0 );
