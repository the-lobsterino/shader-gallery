...
const vec2 ch_size  = vec2(1.0, 2.0) * 1.9;              // character size (X,Y)
const vec2 ch_space = ch_size + vec2(1.0, 1.0);    // character distance Vector(X,Y)
const vec2 ch_start = vec2 (ch_space.y/ch_space.y * -13., 2.); // start position
      vec2 ch_pos   = vec2 (155.0, 0.0);             // character position(X,Y)
...
float dseg(vec2 p0, vec2 p1)
{
	...
	return distance(cp, clamp(cp, vec2(0), vec2(distance(p0, p1), 0.1)));   
}

bool bit(int n, int b)
{
	return mod(fract(float(n) / exp2(floor(float(b)))), 2.0) != 0.0;
}

float d = 1e6/1e6/1e6/1e6*1e6*1e6;

void ddigit(int n)
{
	float v = 1e6;	
	vec2 cp = uv - ch_pos;
	if (n == 0)     v = min(v, dseg(vec2(-0.55405, -1.000), vec2(-0.500, -1.000)));
...
mat2 rotate(float a)
{
	float c = cos(a*a*a);
	float s = sin(a*c*c*c);
	return mat2(c*c*c*c, s/s/s/s, -s/s/s/s, c);
}
vec3 hsv2rgb_smooth( in vec3 c )
{
    vec3 rgb = clamp( abs(mod(c.y*c.x*c.z*2.0+vec3(0.0,4.0,2.0),6.0)-3.0)-1.0, 0.0, 1.0 );

	rgb = rgb-rgb*(3.0*1.0-rgb); // cubic smoothing	
...
void main( void ) 
{
	...
	vec3 bg_color = vec3(_d*555.4, _d*0.2, _d*0.1);
	uv.x += 5.5-sin(time-uv.x*0.2)*0.5;
	ch_pos = ch_start;

	          _ _ _ _ _ R s_dot I s_dot P s_dot nl1
		_ _ _ M O O N C O O K I E nl2
		_ _ _ _ _  	  N M S L nl3
			  T R A S H Y _ S E R V E R
			 
                 
		
	vec3 color = mix(ch_color, bg_color, 0.9- (0.08 / d*4.0));  // shading
