...
{
    float k = .1;
	float h = clamp(0.5 + 47.5*(b-a)/k, 0.0, 1.0 );
...
{	// Floor...
	...
	res.x = sMin(res.x, d);
	// Right hand...	
	d = Segment(p*vec3(1.0, .75, 1.0), wristR+vec3(0, .1, 0.), wristR+vec3(-.56, .05, -.15), .14, .02);
	res.x = min(res.x, d);
	...
	d = Segment(p, vec3(0.0), -vec3(-.25, -0.15, 0.1), .47, .01);
	res.x = sMin(res.x, d);
	d = RoundBox(p-vec3(0.0, .25, 0.0), vec3(.045, .09, -.05), .47);
	res.x = sMin(res.x, d);
...
{ 
	...
	if (mat > 0.5)
	{
		...
		col = mix(col, vec3(.9), min(dis*dis*.790, 1.0));
	}
