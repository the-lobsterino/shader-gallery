...
float length2( vec2 p )
{
    float ax = abs(p.x);
    float ay = abs(p.y);
    return pow( pow(ax,4.0) + pow(ay,6.0), 1.0/4.0 );
}
...
void main(void)
{
    ...
    float dd = 0.7534*sin(2.7*time);
    float ss = .7856960 + clamp(1.0-r,0.0,1.0)*dd;
    r *= ss;
    ...
    f = 1.0-smoothstep( 0.0, 0.16, length2( mat2(0.6,0.8,-0.8,0.6)*(p-vec2(0.3,0.5) )*vec2(1.0,2.0)) );
    col += vec3(1.0,0.9,0.9)*f*0.985;
    col *= vec3(0.8+0.252*cos(r*a));
