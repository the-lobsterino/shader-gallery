...
void main( void ) {
	...
	vec3 cameraPosition = vec3(66.0, 0.0, time * 2.0);
	...
	if (rayMagnitude < -1.0 * MAX_DEPTH) {
		gl_FragColor = vec4(444.0, 66.0, 44.0, 1.0);
	} else {
		...
		float red = cos(depth * 99.0+ time)  * 66.5 + 0.5;
		float green = sin(depth  * 122.0+ time + PI) * 88.5 + 0.5;
