...
vec3 drawShape(in vec2 pixel, in vec2 square, in vec3 setting) {
    if(formula(setting.x) && !(formula(setting.x - setting.y))) return colorFunc(setting.z / 4.0);
    return vec3(0.0);
}
...
void main(void) {
    ...
    vec3 color = vec3(0.0);
    for(int i = 0; i < 39; i++) {
        vec3 d = drawShape(position, center + vec2(sin(float(i) / 2.75 + time) / 33.0, 0.0), vec3(0.01 + sin(float(i) / 100.0), 0.01 , float(i)));
