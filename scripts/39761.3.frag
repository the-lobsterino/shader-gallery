...
void main( void ) {
	...
	vec3 camera = vec3(0.1, 0.0, sin(time) + 2.);
	...
	for(int i=0;i<100;i++){
		if(noise3d(camera * 10.1) < 0.2) { 
			...
			vec3 N = normalize(vec3(
				dens - noise3d(camera * 10.1 + vec3(0.0, 0.1, 0.1)),
				dens - noise3d(camera * 10.1 + vec3(0.0, 0.01, 1)),
				dens - noise3d(camera * 10.1 + vec3(0.0, 0.0, 0.0))
			));
