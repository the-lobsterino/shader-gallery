...
const int   complexity      = 830;    // More points of color.
const float mouse_factor    = 36.0;  // Makes it more/less jumpy.
const float mouse_offset    = 5.0;   // Drives complexity in the amount of curls/cuves.  Zero is a single whirlpool.
const float fluid_speed     = 53.0;  // Drives speed, higher number will make it slower.
...
{ 
  ...
  for(int i=1;i<complexity;i++) {
    ...
    newp.x+=1.5/float(i)*sin(float(i)*p.y+time/fluid_speed+0.2*float(i)) + 0.2; // + mouse.y/mouse_factor+mouse_offset;
  ...
  vec3 col=vec3(color_intensity*sin(32.0*p.x)+color_intensity,color_intensity*sin(3.0*p.y)+color_intensity,color_intensity*sin(p.x+p.y)+color_intensity);
