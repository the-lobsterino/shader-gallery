// IT'S SPOOKY SEASON. NEEDED TO UPDATE THIS ONE.

/*
 kjlknojuh8jimkojibyu8jimkg6h7unj uyiu87n9mijkmnjhuinjmknkjnuuh.  jni.     * Original shader from: https://www.shadertoy.com/view/3lc3zS
 */
...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
    ...
    //black of face
    float eye = sdEllipse(rot(uv - center, -0.5 + 0.05*sin(iTime)) + vec2(0.14, 0.05), vec2(0.017, 0.045));
    float eyeC = sdEllipse(rot(uv - center, -0.5 + 0.05*sin(iTime)) + vec2(0.14, 0.05), vec2(0.035, 0.045));
    // mouth
    ...
    float eyeSpec = sdSphere(rot(uv - center, -0.25 + 0.06*sin(iTime)) + vec2(0.138, -0.009), 0.015);

    //nose
    float nose = sdTri(70.*(uv - center - vec2(0., 0.008 -0.005*sin(iTime))));

    vec4 layer0 = render(d, vec3(0.16), stroke);

    vec4 layer0_5 = blockRender(earsInner, 0.8*vec3(0.99, 0.6, 0.7), stroke);

      //eye yellow
      vec4 layer1 = render(eyeC, vec3(.9,.95,.0), stroke);
    //eye black
    vec4 layer1_5 = render(bF, vec3(0.), stroke);
    //eye spec
    vec4 layer2 = blockRender(eyeSpec, vec3(1.), stroke);

    //itty bitty kitty nose
    vec4 layer3 = blockRender(nose, vec3(0.), stroke);

    //BG KITTIES

    //uv.x = center.x - abs(uv.x - center.x);
    vec2 bUV = vec2(length(Ouv - center), abs(0.5*atan((Ouv.x - center.x)/(Ouv.y - center.y))))+cos(time/10.);

    //vec2 bUV = Ouv+sin(time/10.);
    ...
    vec4 bg = 1.3*vec4(.7,.3,.05,1);
    vec4 bgKitties = blockRender(bgKitty, vec3(0.), stroke);
    ...
    fragColor.rgb = mix(fragColor.rgb, layer1_5.rgb, layer1_5.a);
