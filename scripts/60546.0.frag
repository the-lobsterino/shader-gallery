...
void main( void ) {
	...
	vec2 uv = vec2(((position.x-0.5)*(resolution.x/resolution.y))+0.5,position.y);
	...
	float stars = clamp(pow(noise((uv.x+2.)*(uv.y+2.)),200.),0.001,1.); //Weird glitch clamp when setting min to 0. Also blinking for some rare reason.
