...
void ddigit(int n)
{
	...
	if (bit(n,  3)) v = min(v, dseg(vec2(-0.500,  0.937), vec2(-0.500,  0.092)));
...
void main( void ) 
{
	...
	vec3 ch_color = hsv2rgb_smooth(vec3(-time*0.4-0.041*sqrt(dot(uv,uv)),0.5,1.0));
