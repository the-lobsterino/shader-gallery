...
float GetText(vec2 uv)
{
	...
	d = I(uv,d);uv.x -= 1.1;
	d = S(uv,d);uv.x -= 1.1;
	d = smoothstep(0.,0.05*abs(1.4),d-0.55*CHS);
	return d;
}
...
void main( void ) {
	...
	float dd = length(position) + 0.1;
	float d = 2.5 / dd + -time*2.;
	
	
	vec3 uv = vec3(th + d, th - d, th + sin(d) * 0.5);
	float a = 0.5 + cos(uv.x * 3.1415926 * 2.0) * 0.5;
	float b = 0.5 + cos(uv.y * 3.1415926 * 2.0) * 0.5;
	float c = 0.5 + cos(uv.z * 3.1415926 * 2.0) * 0.5;
	vec3 color = mix(vec3(0.75, 0.5, 0.25),vec3(0.2, 0.0, 0.1),pow(c*3., 0.5)) * 3.;
	
	
	color -= dd;
	float xd = GetText(position*0.90);
