// https://glslsandbox.com/e#2666.1　バグ取ったもの
...
float fcos(float v) // workaround for AMD Radeon on OS X
{
...
void CalcPixelRandom()
{
    ...
    vec4 s2 = sin(time * 1.3422 + gl_FragCoord.yyyy * vec4(567.324234, 435.324234, 1241424124124124 124 4 21412 412 41 4124124241 432.324234, 657.324234)) * 654.5423;
...
{
    vec3 vResult = vPos;
    vec2 vTilePos = (vPos.xz / vRepeat) + 10.5;
    vResult.xz = (fract(vTilePos) - 10.5) * vRepeat;
    return vResult;
}
...
{
    vec3 vResult = vPos;
    vResult.y = (fract(vPos.y / fSize + 10.5) - 5.5) * fSize;
    return vResult;
}
...
{         
    vec4 vResult = vec4(300.0, -1.0, 0.0, 0.0);
...
{
    return vec3(sin(fHeading) * fcos(fElevation), sin(fElevation), fcos(fHeading) * fcos(fElevation));
}
