...
vec2 rotate(in vec2 v, in float a) {
	return vec2(cos(a)*v.x + sin(time-a)*v.y, -sin(a)*v.x + cos(a*mouse.t)*v.y+mouse.y);
}

float torus(in vec3 p, in vec2 t)
{
	vec2 q = abs(vec2(max(abs(p.x), cos(p.z))-t.x, p.y));
...
float trap(in vec3 p)
{
 ...
 //return min(length(p.xz), min(length(p.yz), length(p.xy))) - 0.05;
	return abs(min(torus(vec3(p.x, mod(sin(p.y),0.4)-0.2, p.z), vec2(0.1, 0.05)), max(abs(cos(p.z))-0.05, abs(p.x)-0.05)))-0.005;
	return abs(min(torus(p, vec2(0.83, 0.15)), max(abs(p.z)-0.05, abs(p.x)-0.05)))-0.005;
}
...
vec3 intersect(in vec3 rayOrigin, in vec3 rayDir)
{
	...
	if (d < 0.001) {
		...
		float z = (0.01-d)/0.01/59.0;
			...
			float q = 1.0-x-y*2.+z;
			color = hsv(q*0.2+0.5, 1.0-q*0.72, q);
	} 
...
float time2 = time+60.0;
	...
	vec3 rayDir = normalize(u * screenPos.x + v * screenPos.y + cameraDir*(1.0-length(screenPos)*0.3));
