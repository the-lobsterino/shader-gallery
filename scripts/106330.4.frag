...
vec4 androm3da (vec2 p) {
	p = z*.8*( gl_FragCoord.xy - res/2. )/min( res.x, res.y );
	float lp = .5*length( p*7. )*cos( time/128. ) ;
	float speEDO = sin( time/30. );
	p *= rot( lp + time*( speEDO ) );
	float theta = atan( p.y, p.x );
        c2 += step( sin( theta*7. + sin( time/4. + 33.*p.x - 5.*cos( 5.*p.y*theta - 2.*time ) ) + 55.5 ), 0. );
	c2 *= circle( p, 1. );
        c2 *= hueShift( theta/tau )*vec3( 1. );
	return vec4( c2/1.25*length( p ), 1. ); }////ändrom3da lysergic speed
...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
	...
	uv.x -= .05;
	ret += vec4(m(uv), 1.);		
	if( ret.xy == vec2( 0. ) ) ret += androm3da(uv);
	fragColor = ret;
}
