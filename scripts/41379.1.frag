...
vec3 char(vec2 ch, vec2 uv)
{
	...
	print_pos.y += tan(2.0/print_pos.x-time*0.2);
	return vec3(px);
}
...
vec3 Text(vec2 uv)
{
    	...
    	vec2 center_pos = vec2(res.x/2.0 - STRWIDTH(15.0)/2.0,res.y/2.0 - STRHEIGHT(1.0)/2.0);
       	
    	BEGIN_TEXT(center_pos.x -30.0,center_pos.y - 20.0)
	HEX(0xFF00DD) _ _ _K _O _O _D _I _ _b _y _ _K _E _K _E _P _L _A _S _T _E _R _I;
	...
	HEX(0xFF00DD) _ _ _M _U _S _A _ _b _y _ _Y _Z _I;
	BEGIN_TEXT(res.x/2.0-STRWIDTH(10.0)/2.0,res.y/2.0)
	print_pos += vec2(cos(time)*96.,sin(time)*96.);
