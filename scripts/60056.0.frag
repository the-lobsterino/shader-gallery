...
vec3 Text(vec2 uv)
{
	...
	print_pos += vec2(20.+cos(time*6.)*20.,sin(time*2.)*70.);
	
	RGB(sin(2.*time+uv.x*0.03)/2. +0.5, -sin(4.*time-uv.y*0.03)/2. +0.5, cos(50.*time+dot(cos(uv),sin(uv))/3.)/2. +0.5)
		_O _b _A _m _A _ _x _ _S _h _R _e _K _ _m print_pos -= vec2(1., -1.25+max(0., cos(30.*time)))*6.; _l
