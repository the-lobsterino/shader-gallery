...
// цвет обода часов
const vec3 clockCol = vec3(1.0, 0.0, 0.0);
// цвет секундной стрелки
const vec3 clockLineSecCol = vec3(0.0, 2.0, 1.0);
// цвет минутной стрелки
const vec3 clockLineMinCol = vec3(1.0, 1.0, 0.0);
// цвет часовой стрелки
const vec3 clockLineHourCol = vec3(1.0, 0.0, 1.0);

const float TimeK = 100.0;
...
vec3 DrawClockCircle(vec2 fragpos, vec2 pos, float radius, float widthK, vec3 color) {
	...
	// цвет
	vec3 col = color * attenuation(1.0, 0.0, 1000.0, l) * widthK;
	// возврат
	return col;
}
...
void main() {
	...
	// направляющий вектор
	vec2 s = vec2(cos(-time * PI / 30.0 * TimeK + PI / 2.0), sin(time * PI / 30.0 * TimeK + PI / 2.0));
	...
	s = vec2(cos(-time * PI / 1800.0 * TimeK + PI / 2.0), sin(time * PI / 1800.0 * TimeK + PI / 2.0));
	// рисование минутной стреки
	outColor += DrawClockLine(p, vec2(0.0), ClockRadius, s, 1.2, clockLineMinCol);
	
	// изменяем направляющий вектор
	s = vec2(cos(-time / 21600.0 * TimeK + PI / 2.0), sin(time / 21600.0 * TimeK + PI / 2.0));
	// рисование часовой стрелки
	outColor += DrawClockLine(p, vec2(0.0), ClockRadius * 0.5, s, 1.2, clockLineHourCol);
