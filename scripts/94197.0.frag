...
vec4 fVGame_Airplane01(vec4 VP){
	...
	VP.yz *=fM2Rotate(-2.0 *VMouse.y);
	VP.xy *=fM2Rotate(+4.0 *VMouse.x);
	VP.yz *=fM2Rotate(+2.0 *abs(VMouse.x));
...
vec4 fVGame_Drone01(vec4 VP){
	VP.z -=24.0;
	VP.y -=8.0;
	VP.xyz +=sin(vec3(3,2,1) *nTime *0.5) *vec3(24,16, 4);
	VP.xyz *=fM3Rotate(vec3(sin(nTime) *0.2));
...
sface fSFEffect_Before(sface SF, smarch SM){
	...
	if(SF.ID_Object ==9)		VColor.rgb =sign(sin(VP.x) *sin(VP.z +nSpeed *VP.w *acos(-1.0))) *0.05 +0.95 *vec3(1);
	...
	if(true){	/*機関銃*/
		if(ID_Object !=11){
			...
			if(VPi.z >0.0){
				...
				if(ID_Object ==12){
					VColor.rgb +=vec3(1,0.25,0) /nL *4.0 *(sin(nL +VP.w) *0.5 +0.5);

				}else{
					VColor.rgb +=vec3(1,0.25,0) /nL;
				}
