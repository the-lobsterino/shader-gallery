...
void main(void){
    // fragment position
	vec2 position = ( gl_FragCoord.xy / resolution.x ) * 2.0 + (mouse - 0.5) - 1.0;
	vec2 p = position;
	float t = time;
	...
	for(float x = -1.0; x < 2.0; x++)
	{
		for(float y = -1.0; y < 2.0; y++)
		{
		    ...
		    ray.direction = normalize(vec3(p.x, p.y, -1.0) + vec3(x, y, 0) / resolution.x);
			...
			totalColor += destColor / 3.0 / 3.0;
		}
	}
	totalColor = pow(totalColor, vec3(1.0 / 2.0));
