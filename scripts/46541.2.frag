...
// FY Sara,  Not as good as a kinder egg
...
void main() {
   ...
   float fog = clamp(totdist/20.0, 0.0, 1.0);
    gl_FragColor.rgb = mix(gl_FragColor.rgb, vec3(0.4, 0.5, 0.7), fog);
    gl_FragColor.rgb = pow(gl_FragColor.rgb, vec3(1.0/2.2));
    vec2 vig = gl_FragColor.xy/resolution.xy*2.0-1.0;
    gl_FragColor.rgb = mix(gl_FragColor.rgb, vec3(0), dot(vig, vig)*0.2);
