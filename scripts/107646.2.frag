...
vec3 rgbFromHue(in float h) {
	h = h - floor(h);
	float r = smoothstep( 2.0*K, 1.3*K, h) + smoothstep( 5.*K, 5.0*K, h);
...
void main( void ) {
	...
	vec2 P0 = vec2(sin(time*2.0013),cos(time*0.5999))*8.;
	vec2 P1 = vec2(cos(time*1.4011),sin(time*2.0007))*0.5;
	
	float d0 = distance(rP,P0);
	float d1 = distance(rP,P1);

	vec3 color =  vec3( rgbFromHue(2.0/d0 - 6./d1 + time) * smoothstep(10.01, 10.0, 1.0/d0 + 1.0/d1));
