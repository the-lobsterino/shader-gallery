...
void main( void ) {
	...
	float radius = abs(length(normalize(center) - normalize(mouse.xy)) * length(center)); 		
	...
	float within = length(center - gl_FragCoord.xy) - 1. * radius;
	rim = vec4(1,1,1,1) * within;
	...
	gl_FragColor = ((normalize((_mV+_hV+_dV+_sV)) + (sV+mV+hV+dV) - distanceToCenter)) * rim;
