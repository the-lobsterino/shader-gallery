...
{
	float curve = cos(p.x*(5.35+sin(time)*1.25))*.33;
	p.y += curve;
	a.y += (curve*curve);
	return length(max(abs(p) - a,0.0)) -.33;
}
...
{	// Floor...
	...
	// Mouth...
	float mo = -Mouth(p2-vec3(0.0, -.057-fuck, -0.3), vec3(.155-you, -.01+fuck, .2));
