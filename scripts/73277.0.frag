...
void main( void ) {
	...
	gl_FragColor = max(gl_FragColor, T2(0.)-1./256.);
	
	return;
	
	float q1 = .1;
	float d1 = 2.;
	vec2 r1 = normalize(surfacePosition)*10.5;
	gl_FragColor = max(gl_FragColor, mix(gl_FragColor, T2(vec2(1,0) *d1+r1), q1)-1./256.);
	//gl_FragColor = max(gl_FragColor, mix(gl_FragColor, T2(vec2(-1,0)*d1+r1), q1)-1./256.);
	//gl_FragColor = max(gl_FragColor, mix(gl_FragColor, T2(vec2(0,1) *d1+r1), q1)-1./256.);
	//gl_FragColor = max(gl_FragColor, mix(gl_FragColor, T2(vec2(0,-1)*d1+r1), q1)-1./256.);
}