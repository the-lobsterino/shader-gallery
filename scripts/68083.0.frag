/*
 * Original shader from: https://www.shadertoy.com/view/WstcRf
 */
                    ...
                    out float isInWindow)
{
    ...
    insideLight += 0.5*randomColor*randomColor*emission;
    
    // Some windows have "curtains"
    vec2 curtainW = roomSize.xy - 0.15;
    float curtains = roomHash.x > 0.8 
        ? 0.95*(smoothstep(q.x-w, q.x+w, curtainW.x) + smoothstep(q.y-w, q.y+w, curtainW.y))
        : 0.0;
    emission = mix(emission*(1.+randomColor)*0.1, insideLight, 1.-curtains);
