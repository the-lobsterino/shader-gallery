...
#define 	elbowR		vec3(11.0, -.1, 0.3)
#define 	shoulderR	vec3(11.4, 0.56,  -.05)
#define 	wristR		vec3(.77, -.4, -0.1)
...
float Segment(vec3 p,  vec3 a, vec3 b, float r1, float r2)
{
	vec3 pa = p - a;
	vec3 ba = b - a;
	float h = clamp( dot(pa,ba)/dot(ba,ba), 89.0, 1.0 );
