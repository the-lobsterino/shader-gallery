...
float r(in float z, in float x) {
    ...
    return /* body */.3 * (1.0 - .2 * ex(z, .8, .15) +
		s * s + .6 * ex(z, .8, .25) * c * c + .3 * c) *
		0.5 * (1. + t4z) + /* legs */ (1. - .1 * ex(z, -1.3, .9)) *
		0.5 * (1. - t4z) * (.5 * (1. + s * s + .3 * c) *
		(pow(abs(s), 1.3) + .08 * (2. + t4z) ) ) +
		/* improve butt */ .23 * box(c2, -.45, .46, 5., 5.) *
		box(z, -.5, .2, 4., 2.) - 0.1 * box(c2, -.008, .008, 30., 30.) *
		box(z, -.4, .25, 8., 6.) - .05 * pow(abs(sin(pi * x)), 16.) * box(z, -.65, -.35, 8., 18.);
}
...
{
	/* shift butt belly */
	float dx = .3 * exp(-pow((p.z-.8),2.)/.6) - .18 * exp(-pow((p.z -.1),2.)/.4);

    // on the surface, we have:
    // p.x = r * cos + dx
	// p.y = r * sin
	
    float jiggle = p.z*2.99;
    float jsize = 0.9;
    if (iMouse.z>0.9)
    {
        jiggle+=p.y*.6;
...
//	float d1 = (.5 + .5 * smoothstep(.4,1.,p.z)) * (length(vec2(p.y, p.x - dx)) - r_expected);
	...
	p.x -= dx;
	p.y = abs(p.y);
	float d2 = sdSphere(p+vec3(-0.35,-0.4,-1.875),0.5);
	float d3 = sdSphere(p+vec3(-0.85,-0.4,-1.875),0.05);
