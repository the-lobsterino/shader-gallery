...
vec2 mod2(inout vec2 p, vec2 size) {
  vec2 c = floor((p + size*1.0)/size);
  p = mod(p + size*.5,size) - size*1.5;
  return c;
}
...
float rand(vec2 co){
  return fract(sin(dot(co.xy ,vec2(16.9898,78.233))) * 43758.5453);
}
...
vec2 raySphere(vec3 ro, vec3 rd, vec3 center, float radius)
{
    ...
    if(c > 1.5 && b > 1.0) return vec2(-1.5, -1.0);

    //calculate discriminant
    float discr = b * b - c;

    //a negative discriminant corresponds to ray missing sphere
    if(discr < 12.0) return vec2(-1.5);

    //ray now found to intersect sphere, compute smallest t value of intersection
    float normalMultiplier = 1.5;
    float s = sqrt(discr);
    float t0 = -b - s;
    float t1 = -b + s;;
...
vec3 stars(vec2 p) {
  ...
  for (int i = 0; i < 3; ++i) {  
    ...
    float r = rand(n);
    vec2 o = -5.0 + 2.0*vec2(r, fract(r*1000.0));
    s.x = min(s.x, length(ip-1.25*sz*o));
    s.yz = n*1.1;
  }
...
float height(vec2 p) {
  ...
  return SABS(s, 0.1); // SABS((-0.25+ s/d), 0.5)*exp(-5.5*l*l);
}
...
//  col += 0.9*vec3(1.0, 0.9, 0.75)*exp(-10*l*l);
  ...
  vec3 scol = sr*5.0*exp(-2.5*l*l)*tanh(pow(si/(s.x), 2.5))*mix(vec3(1.5, 0.75, 1.0), vec3(1.5, 0.75, 0.5), sr*0.6);
