// https://www.shadertoy.com/view/XsVSzV

/*

in plane normal form, the solution for the plane offset `w` is:

w = dot(abs(plane_normal),box_scale)

*/

// undefine to see visualization of circular plane distances
#define VIZ_MAP

// undefine to see box tumble
// #define TUMBLE
...
void paint() {
    ...
    vec2 p = get_origin();
    vec2 pn = normalize(p);
    float pw = length(p) - dot(abs(pn),s);
    add_field(pw);
    debug_gradient();
