...
float stopThreshold = 2.0/resolution.y;
...
float sinusoidBumps(in vec3 p){
    ...
    return mix((0.75*(cos(p.x*17.+time*0.97)*cos(p.y*29.+time*1.27))*sin(p.z*23.-time*1.31)), (0.5*(sin(p.x*13.+time*0.71)*cos(p.y*37.-time*1.03)*cos(p.z*11.-time*1.13))),clamp(sin(0.75+time*0.25),0.1,0.9));
}
...
void main(void) {
	...
	vec3 lp = vec3(cos(2.5*sin(time*0.125)), 1.75-0.5*cos(time*0.125), 0.0);
