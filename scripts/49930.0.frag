...
vec4 Brow(vec2 uv, float smile) {
	...
	float y = uv.y;
    uv.y += uv.x * mix(.5, .8, smile) - mix(.3, .5, smile);
    uv.x -= mix(.0, .1, smile);
    uv -= .5;
...
vec4 Eye(vec2 uv, float side, vec2 m, float smile) {
    ...
    float pupilSize = mix(.38, .16, smile);
...
vec4 Head(vec2 uv) {
    ...
    float edgeShade = remap01(.35, .43, d);
    edgeShade *= edgeShade;
    col.rbg *= 1. - edgeShade * .4;
    
    // stroke
    col.rgb = mix(col.rgb, vec3(.6, .3, .1), S(.47, .48, d));
    
    float highlight = S(.41, .405, d) * .9;
    highlight *= remap(.41, -.1, .75, 0., uv.y);
    highlight *= S(.18, .19, length(uv - vec2(.2, .08)));
...
vec4 Smiley(vec2 uv, vec2 m, float smile) {
    ...
    vec4 eye = Eye(
        within(uv, vec4(.03, -.1, .37, .25)), 
        side, m, smile);
    
   	vec4 mouth = Mouth(
        within(uv, vec4(-.3, -.4, .3, -.1)), smile);
    
    vec4 brow = Brow(
    	within(uv, vec4(.03, .2, .35, .4)), smile);
...
void main( void ) {
    ...
    float smile = cos(time) * 32.5 + .0;
     uv = uv * 2.8;
    uv -= m * .5*(.1125 - dot(uv, uv));
