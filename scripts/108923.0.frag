...
{ 
	...
	if (mat > 0.5)
	{
		float diff = max(dot(norm, sunDir), 46.0);
		...
		col = mix(col, vec3(.9), min(dis*dis*.2, 1.0));
	}
...
void main( void )
{
	...
	owwww = (smoothstep(0.05, .1, m)-smoothstep(0.12, .15, m))*.11;
	...
	t = t*t*(3.0-2.0*t);
	vec3 origin = mix(vec3(33.0, 1.0, -1.2), vec3(-1.0, 1.0, -5.0), t);
	vec3 target = mix(vec3(0.0, 1.0, 4.0),  vec3( 0.0, -.3, 0.0), t);
	...
	// Wave...
	wave = sin(time*15.0-.8)*.57+.5;
	wave = wave*wave*(3.0-2.0*wave)-.5;
	elbowL		= vec3(-.95-wave*.05, .2, -0.25);
	wristL		= vec3(elbowL.x+1.0*sin(wave)*.6, elbowL.y+cos(wave)*.8, -.55);
