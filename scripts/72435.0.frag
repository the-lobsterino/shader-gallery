...
const int range=2; int threshold=4; int states=5; bool moore=true;
const float fStates = 5.;
...
void main( void ) {
        ...
        (length(position-mouse) < 0.025)) //random pixel around mouse location
    {
        ...
        int tmp = int(rnd1*fStates);
        //convert back to floating point between 0 and 1
        float shade=float(tmp)/fStates;
        ...
        for (int y=-range; y<=range; y++) {
            for (int x=-range; x<=range; x++) {
                if (x==0 && y==0) {
                    ...
                    int neighborCellState = int(getCell(position+offset)*fStates);
