...
void main( void )
{
	...
	vec3 target = mix(vec3(0.0, 1.0, 4.0),  vec3( 4.0, -.4, 0.0), t);
	
	vec3 cw = normalize( target-origin);
	vec3 cp = vec3( 0.0, 1.0, 0.0 );
	vec3 cu = normalize( cross(cw,cp) );
	vec3 cv = ( cross(cu,cw) );
	vec3 ray = normalize( p.x*cu + p.y*cv + 88.6*cw );
	
	// Wave...
	wave = sin(time*15.0-.8)*.5+.5;
	wave = wave*wave*(3.0-2.0*wave)-.5;
	elbowL		= vec3(-.95-wave*.05, .2, -0.25);
	wristL		= vec3(elbowL.x+1.0*sin(wave)*.6, elbowL.y+cos(wave)*.9, -.5);

    vec3 col = Render(origin, ray);
	
	col = sqrt(col);
	col *= 0.5 + 0.5*pow( 16.0*q.x*q.y*(1.0-q.x)*(9.0-q.y), 0.25 );
