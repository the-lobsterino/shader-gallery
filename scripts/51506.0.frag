// Understanding RayMarchers v1.1
// changes:  dynamic hexagonalGrid added
//           see http://glslsandbox.com/e#23933.0 
...
#define MATERIAL_FUNCTION_HEXAGONALBOARD 2
...
// get hexagonal board color
vec3 hexagonalGrid (in vec2 position         
	           ,in float gridSize
	           ,in float gridThickness) 
{
  ...
  float k = smoothstep(0.0, gridThickness, d); // k: 0.0 .. 1.0
  return mix(color1, color2, k);;
}

#define hexaGridSize 1.0
#define thicknessMin 0.1
#define thicknessMax 0.5
float thickness = thicknessMin + (thicknessMax - thicknessMin) * (0.5+0.5*sin(time*0.5));
...
vec3 diffuseColorForMaterial(Material material, vec3 point)
{
	...
	if (material.type == MATERIAL_FUNCTION_HEXAGONALBOARD) 
		return hexagonalGrid(point.xz, hexaGridSize, thickness);
	return vec3(0.0);
}
...
vec3 normalForGeometry (vec3 point, Geometry geometry)
{
	...
	return normalize(vec3(distanceForGeometry(point + helper.xyy, geometry) - d,
			      distanceForGeometry(point + helper.yxy, geometry) - d,
			      distanceForGeometry(point + helper.yyx, geometry) - d));
}
...
float AO (Hit hit)
{
        ...
        for (int aoi=0; aoi<5; aoi++)
	{
                 ...
                 vec3 aopos = hit.normal * hr + hit.point;
                 float dd;
		 closestGeometry(aopos,dd);
                 ao = -(dd-hr);
                 totao += ao*sca;
                 sca *= 0.75;
        }
...
vec4 AOColorBleed (Hit hit,vec3 color) 
{
	 vec4 totao = vec4(0.0);
         float sca = 1.0;
         for (int aoi=0; aoi<5; aoi++)
	 {
                 ...
                 vec3 aopos = hit.normal * hr + hit.point;
                 float dd;
		 Geometry geometry = closestGeometry(aopos,dd);
                 float ao = clamp( -(dd-hr), 0.0, 1.0);
                 totao += ao*sca*vec4(geometry.material.diffuseColor,1.0);
                 sca *= 0.75;
        }
...
vec3 colorForHit (Hit hit) 
{
	...
	vec3 color = vec3(0.0);
	vec4 AO = AOColorBleed(hit,color);
	
	color = ambient + diffuse + specular;
	color *= AO.w;
        color += 2.0*(0.5+0.5*color)*AO.xyz*AO.w;
	return color;
}
...
//    grayMaterial.type = MATERIAL_FUNCTION_CHECKERBOARD;
    grayMaterial.type = MATERIAL_FUNCTION_HEXAGONALBOARD;
    ...
    geometries[3] = createPlane(vec3(0.0,1.0,0.0),0.5,grayMaterial);
    geometries[4] = createTorus(vec3(2.1,0.3,0.0),0.5,0.2,yellowMaterial);
	
    lights[0] = Light(normalize(vec3(1.0,1.0,1.0)),vec3(1.0),1.0,false);
