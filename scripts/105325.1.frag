...
float _R(vec2 p,float d){d=line2(d,p,vec4(0.5,-0.25,2,-3.25)*CHS);d=line2(d,p,vec4(-2,-3.25,-2,0.0)*CHS);p.y-=1.5*CHS;return min(d, abs(sdBox2(p,vec2(2.0,1.75)*CHS)));} 
float T(vec2 p,float d){d=line2(d,p,vec4(0,-3.25,0,3.25)*CHS);return line2(d,p,vec4(2,3.25,-2,3.25)*CHS);} 
float X(vec2 p,float d){d = line2(d,p,vec4(-2,3.25,2,-3.25)*CHS);return line2(d,p,vec4(-2,-3.25,2,3.25)*CHS);} // DOGSHIT

float GetText(vec2 uv)
{
//	CHS = 0.05;//0.04+sin(time*3.1+uv.x*2.5)*0.01;
	float g = 0.3;
	float d = B(uv,1.0);uv.x -= g;
	d = _R(uv,d);uv.x -= g;
	d = E(uv,d);uv.x -= g;
	d = X(uv,d);uv.x -= g;
	d = I(uv,d);uv.x -= g;
...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
	...
	col+=spec;
	float sca = clamp(length(p), 1.5, 10.0);
