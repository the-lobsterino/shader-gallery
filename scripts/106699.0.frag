...
void main( void )
{
       ...
       float noise = pnoise(vec3(position * vec2(30.0, 2.0) - vec2(0, time * 3.0), time / .50), vec3(100.0, 100.0, 100.0));
	noise *= 2.;
	noise += 1.2+sin(time)*0.5;
	noise *=(1.0 - position.y);
	noise = noise * noise * noise;
	vec3 fc = vec3(noise/2.0,noise / 3.4, noise / 5.0);
	vec2 uv = (gl_FragCoord.xy * 2.0 - resolution) / min(resolution.x, resolution.y);
	uv.y += 0.1+(sin(fc.r*0.2+time+uv.x-(sin(time*1.4+uv.y*uv.x)))*0.2);
	float dd= GetText(uv*2.0);
	fc = mix(fc+vec3(.76,0.5,.1)+uv.y, fc,dd);
