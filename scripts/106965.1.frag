...
float noyz(vec3 co){return sin(co.x+1.3*sin(co.y+2.4*sin(co.z)))*9.;} 
...
float DE(vec3 z){z+=noyz(z*200.0)*0.01; 
 ...
 vec3 p5=vec3(-0.38+abs(arm),arm-0.1-abs(-sin(tim)*0.05),-4.-cos(tim)*0.1); 
 vec3 p8=vec3(0.38-abs(arm),arm-0.1-abs(sin(tim+3.1416)*0.05),-0.15+cos(tim)*0.1); 
 vec3 p11=vec3(-0.075,-9.+max(0.0,cos(tim+3.1416)*0.05),sin(tim)*0.2); 
 vec3 p14=vec3(0.075,-0.975+max(0.0,cos(tim)*0.05),-sin(tim)*0.2); 
 float d=min(z.y+1.0,min(length(z*vec3(1.5,1.0,1.25))-0.08,Segment(z,p1,p2,0.5))); 
