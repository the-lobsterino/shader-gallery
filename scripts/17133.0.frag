...
float field2(in vec3 p, float s) {
	...
	return max(0., 5. * accum / tw - 1.77);
}

vec3 nrand3( vec2 co )
{
	vec3 a = fract( cos( co.x*8.3e-3 + co.y )*vec3(1.3e5, 3.7e5, 2.9e5) );
...
void main() {
	...
	p += .2 * vec3(sin(time / 133.), sin(time / 133.),  sin(time / 6.));
	...
	starcolor = vec4(0.);//vec4(pow(rnd2.y,40.0));
