...
float light_density(vec2 p, float p_length) {
    float color_factor = SQRT_2 - p_length + 0.3; // Enhanced color vibrancy

    float r = (atan(p.y, p.x) + PI) / (PI);
    float dynamicTime = time * SPEED * 0.1; // More dynamic time factor for movement

    // Increase the twistiness of the spiral by introducing a higher spiral coefficient
    float spiral_coefficient_dynamic = spiral_coefficient + 0.5 * sin(dynamicTime); // More twisty spiral

    float result;
    
    // Adjusting the fract function to increase twistiness
    float part_1 = fract(r + dynamicTime * 2.0 - sqrt(p_length) * spiral_coefficient_dynamic); // Increased twist
    float part_2 = 1.0 - part_1;
    float spiral = parting(part_1) + parting(part_2);

    vec2 foo, bar, moo, meh;
    float noisy = fbm(vec2(spiral, p_length + dynamicTime * 1.5)); // Time-modulated noise for more dynamics
    noisy *= noisy * noisy;
    float general_noise = fbm(p + vec2(dynamicTime * 1.5)) / 1.8; // More dynamic noise
    result = (spiral * noisy) * color_factor + general_noise * (color_factor + 0.15); // Enhanced overall effect
