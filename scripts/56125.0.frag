//Copyright (c) 2019-06-23 - 2019-07-07 by Angelo Logahd
...
#define FOGMAP				true
#if FOGMAP
const float FOGMAP_INTENSITY	 	= 0.95;
const vec3  FOGMAP_SPEED		= vec3(7.0, 0.5, 7.0);
...
#define FILMIC_TONEMAP_UNCHARTED2	3
#define FILMIC_TONEMAP_ACES		4
...
const float TONEMAP_EXPOSURE		= 0.6;
const float TONEMAP_EXPOSURE_BIAS 	= 0.2;
#endif
#endif
...
{    
    ...
    //float specular = pow(max(dot(reflect(light, normal), eye), 0.0), SEA_SPECULAR_FACTOR) * nrm;
...
float fresnel(const in vec3 normal, const in vec3 eye, const in vec3 diffuse, const in vec3 specular)
{
    ...
    float fFresnel = Schlick(vHalf, eye);
...
vec3 sea(const in vec3 p, const in vec3 lightDir, const in vec3 eye) 
{
    ...
    //diffuse *= SEA_DIFFUSE_CONTRIBUTION * color * max(dot(normal, lightDir), 0.0);
    ambient *= color;
...
float fogNoise(in vec3 p)
{
    ...
    p += tri3(p);
    //p *= 1.1;
    float innerTri = tri(p.x + tri(p.y));
    float resultDepth = tri(p.z + innerTri) / z;
    return resultDepth; 
}

float fogmap(in vec3 p, in float d)
{
    p += time * FOGMAP_SPEED * FOGMAP_DIR;
...
void postProcess(inout vec3 color)
{
    #if APPLY_LUMINANCE	
