...
void main(void){
	...
	float black = 2.0;
  	float blackout = 2.4 - 1.5*bdist;
  	if (blackout > 3.5) {
    	  blackout = 3.4 - (-1.2 + sine);
  	}
  	if (black != 2.0) {
    	  blackout = black;
  	}
	...
	vec2 mouseV = ( gl_FragCoord.xy / (resolution.xy) ) * 7. - 5.*mouse;
	
	//gl_FragColor = vec4(tdist, 0., 0., 1.);
	gl_FragColor = vec4(ccc,cc,c, 1.0);
}