...
vec2 map( in vec3 pos )
{
  ...
  vec2 res = vec2( sdPlane(     pos), 1.0 ); 
	...
	res = opU(res,vec2(sdSphere(pos - vec3(0.0,0.8,0.0), 0.5),5.));
...
vec3 reflectionRender( in vec3 ro, in vec3 rd )
{ 
...
{ 
  ...
  if ( m > -0.5 )
  {
    ...
    vec3 res2 = reflectionRender(pos, ref);
    // primitive's material        
    col = vec3(0.5, 0.6, 0.7) * res2 ;
    if ( m<1.5 )
    {
      float f = mod( floor(5.0*pos.z) + floor(5.0*pos.x), 2.0);
      col = 0.4 + 0.2 *f*res2;
    ...
    float dom = smoothstep( -10., 10., ref.y );
...
void main( void )
{
  ...
  vec3 ro = vec3( rx, (3.0 * mouse.y) , rz );
