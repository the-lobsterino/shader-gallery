...
void main( void ) 
{
	...
	uv = ( floor(gl_FragCoord.xy / 2.) * 2. / resolution.y ) - aspect / 2.0;
	...
	vec3 ch_color = hsv2rgb_smooth(vec3(time*0.4+uv.y*0.07,0.4,1.0));

	float c1 = abs(sin(time*0.4+uv.y*0.3+uv.x*2.0));
	vec3 bg_color = vec3(c1*0.7,0.0,c1*0.9)*(0.7+sin(uv.x*0.2-uv.y*0.3+time)*0.5);
	
	uv.y += -1.+sin(uv.y*0.4+time*0.87+uv.x*.45)*0.5;
	...
	float xx = uv.x;
		   ...
		   uv.x += sin(time*1.4);
	             T H E  _ W O R D _ I D I O T
			   uv.x = xx;
			 nl1 uv.x+=8.;
			 D E S C R I B E S _ A P E S _ W H O 
			   nl2 
			   P A R R O T _ W H A T _ T H  E Y _ A R E _ T O L D				 			 
			   uv.x += sin(time);
			   nl3  W I T H O U T _ Q U E S T I O N
			 
		
			   float dd = smoothstep(-0.1,0.4,d*d);
	vec3 color = mix(ch_color, bg_color*0.75, dd);  // shading
