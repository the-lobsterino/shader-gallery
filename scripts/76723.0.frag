...
void main( void ) 
{
	...
	uv *= 18.0 ;
	uv.y -= 3.;
	uv *= 1.0+(sin(time+length(uv)*0.6)*0.1);
	//uv *= rotate(time+uv.x*0.05);

	
	
	vec3 ch_color = hsv2rgb_smooth(vec3(time*0.4+uv.x*0.1+length(uv*0.05),0.5,1.0));

	//vec3 bg_color = vec3(_d*0.8*sin(time)*3.0, _d*0.0, _d*0.00);
	vec3 bg_color = vec3(0.1,_d*0.8*abs(sin(time+length(uv)*0.4))*0.7,0.1);
	...
	ch_pos = ch_start;
			         nl1  _ O M O _ A R T I F E X _ 
			  	 nl2  V I L L A M A R E _ N I C O L A    
					 uv.x += sin(uv.x*0.5+time*0.45);
			   		 	 nl3 _ _ _ _ O N L I N E
