...
void mainImage( out vec4 fragColor, in vec2 fragCoord ) 
{
	...
	vec3 lD = normalize(vec3(mp-vec2(0.5),0.5));   // light dir	
	...
	float aB = smoothstep(0.0, 0.2, 0.5*n.z);  // antialiasing edge !!!
	cc += mBC;
	cc /= nn.z;
	//n *= aB;
	...
	float ndh = dot(n,h);
	ndh = ndh*.5 +.5;
	ndh = pow(ndh,1.+2000.0 *(mp.x))*(0.2 + .65*mp.x);  // light reflection
	vec3 fc = cc*ndl +ndh;
