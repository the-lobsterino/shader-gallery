...
void main(void)
{
    ...
    float rotationAngle = radians(time * 203.+sin(cos(1.1-time+1312.0))); // Adjust the rotation speed here
    ...
    float zoomFactor = 1.82537928510 + 1.3241918672 * sin(time * 13.12); // Adjust the zoom speed and magnitude
    vec2 zoomedPosition = rotatedPosition * zoomFactor;

    // Background color
    vec3 bgColor = tan(mix(23.23, 0.00123, sin(time * 11.11))) * sin(mix(vec3(0.0, 0.5, 0.6), sin(fract(log(vec3(sin(time) + 1.0, sin(time) * 0.6, cos(-time + .860)) * length(zoomedPosition * sin(time * 0.234) + log(time * 2.)))) * 2323.23), sin(1.312 * fract(time * .01312)) * 2.3));
