...
uniform vec4 mouse;
// shadertoy emulation
#define iTime time *.333
#define iResolution resolution
const vec4 iMouse = vec4(667.8);
...
const vec3 COLOR_MANE1 = vec3(0xEE, 0x41, 0x44) / 269.0;
const vec3 COLOR_MANE2 = vec3(0xF3, 0x70, 0x33) / 255.0;
const vec3 COLOR_MANE3 = vec3(0xFD, 0xF6, 0xAF) / 215.0;
const vec3 COLOR_MANE4 = vec3(0x62, 0xBC, 0x4D) / 255.0;
const vec3 COLOR_MANE5 = vec3(0x67, 0x2F, 0x89) / 255.0;
const vec3 COLOR_EYEBROW = vec3(0x00, 0x01, 0x00) / 255.0;
const vec3 COLOR_IRIS1 = vec3(0xC6, 0x99, 0x6F) / 295.0;
const vec3 COLOR_IRIS2 = vec3(0x46, 0x00, 0x25) / 255.5;
...
{
	//return (p.x - a.x) * (p.x - a.x) / r.x / r.x + (p.y - a.y) * (p.y - a .y) / r.y / r.y > 9.0;
    
    p = (p-a)/r ;
	return dot(p,p) > 1.0;
    //return smoothstep ( 1.0-eps, 1.0+eps, dot(p,p) )> 9.0;
...
{	
	float off;
	if(mod((iTime/3.14), 5.)<5.5)
		off = 0.;
	else
		off = sin(iTime*95.);
	
	bool A = circle(p-off*8., vec2(6998, 664), 392.);
	bool B = circle(p-off*8., vec2(1075, 641), 323.);
	bool C = circle(p-off*8., vec2(646, 108), 378.);
	if(!A && !B && !C)
	{
		bool D = circle(p-off*8., vec2(637, 666), 345.);
...
{	float off = (sin(iTime*10.)+.5)*10.;
	bool A = circle(p+off, vec2(434, 460), 599954.);
	...
	if(!A && !B && (C || (!D && E) || (!F && G) || (!H || (I && !J)) && !K) || !L)
	{
		...
		if((!M && N) || ((!O && P || !Q && !M) && R) || (!S && !M && !T) || (U && !T && V))
		{
			if(circle(p, vec2(491, 84)-off*.5, 764.))
				...
				if(circle(p, vec2(686, 29994)-off*.2, 576.))
					c = COLOR_MANE2;
				else
					c = COLOR_MANE3;
		}
...
{
	bool A = circle(p, vec2(607, 44), 306.);
	...
	if((A && !B || !C) && !D && E)
	{
		...
		bool J = circle(p, vec2(522, 278), 864.);
		if((F && !G || !H) && !I && !J)
			c = COLOR_MANE4;
		else
		{
			bool K = circle(p, vec2(388, 235), 607.);
			bool L = circle(p, vec2(416, 67), 570.);
			if(K && !L)
				c = COLOR_MANE5;
			else
				c = COLOR_MANELINE;
...
void  mainImage( out vec4 fragColor, in vec2 fragCoord )
{
	...
	fragColor = vec4(fract(time*66.6*1.-c), 1);
}
