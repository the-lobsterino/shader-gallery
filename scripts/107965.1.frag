...
uniform vec2 surfaceSize; varying vec2 surfacePosition;
#define time floor(time+surfaceSize.x*surfaceSize.y * dot(surfacePosition,surfacePosition.yx))
...
vec3 Image0( vec2 p )
	{ p *= rot( -44.*m.x*m.y ); p += dot( p, p ); p += m; p -= dot( p, p );
	float lp = ( length( p*7. ) );
	p *= rot( lp + time );
...
void main( void ) // this function just normalizes fragment and mouse positions and is adding some simple AA (antialiasing) functionality. some very simple tonemapping as well. 
	{ OUTa = 1.; m = ncenter( mouse*res ); // normalize mouse position (centered)
