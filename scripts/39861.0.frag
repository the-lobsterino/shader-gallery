...
{
	pos += sin(pos.zxy*3. * sin(time*.8))*0.5;
	pos += sin(pos.zxy*1.5 * sin(time*.4))*.2;
	pos += sin(pos.zxy*2. * sin(time*1.4))*0.3;
...
float world(vec3 pos)
{
  ...
  return min(box(rotX(sin(time/4.)*4.) * rotY(time*0.7) * pos, vec3(.4)), pos.y + 1.5); // a rotating box with horizontal plane
...
float trace(vec3 origin, vec3 direction, float current_dist, float max_dist)
{
  for (int i = 0; i < 128; ++i) {
    ...
    current_dist += new_dist * .3;
    if (log(current_dist/new_dist/1e6)>0.)
      break;
...
void main( void ) {
  ...
  if (resultDist < TRACE_MAX_DIST) {
    ...
    resultColor = enlight(resultSurfacePosition, normal(resultSurfacePosition), vec3(1.), lightPos, vec3(3.), vec3(0.05));
