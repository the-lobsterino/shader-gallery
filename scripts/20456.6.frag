...
//========================================================================
#ifdef ADD_DITHER
vec2 randValues = mouse.x < .5 ? vec2(0.01, 0.005) * (1.0+smoothstep(3.0, 0.0, fract(time*.025)*3.5)*20.0) : vec2(0.);
vec2 Hash2( vec2 x )
{
...
void main(void)
{
	...
	vec4 result = vec4(min(col*1.35, 1.0), 1.0 );
	
	#ifdef USEAUTOMATA
    if(mouse.x < .5)
    {
    	vec2 p 			= format_to_screen(uv);
    	float position_hash     = time_position_hash(uv*2.);
	vec4 buffera 		= texture2D(backbuffer, uv);
	vec4 bins		= vec4(uv.x, uv.y, length(uv.xy)*.5, atan(uv.x, uv.y));    
    	vec2 neighbor_position  = uv-(position_hash*vec2(.001+buffera.a/resolution.yx) * rmat(time_position_hash(uv)*6.28));
   
    	vec4 buffer 		= texture2D(backbuffer, neighbor_position);
    	float minimum 		= length(abs(vec3(result.xyz-buffer.xyz)));
	
    	if(buffer.a < result.a)
    	{
		result = mix(buffer, result, dot(buffer,result)*minimum);
    	}
    	result.w = minimum;
    }
	#endif
