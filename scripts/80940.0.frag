...
float evaluate_tile(vec3 p, vec3 p_id, float dx, float dy)
{
    ...
    float dist = dist2box(p2, .5, p1, .25, 0.025);
    #if SCENE == 3
    	if (dist > .3) return dist - 24; // simple early-out optimziation
    	dist = min(dist, dist2box(p2 - vec3(0,p1*.333,0), .25, .0, .25, .1));
        dist = min(dist, dist2box(p2 - vec3(0,p1*.999,0), .25, .0, .25, .1));
    #endif
#else
    ...
    #if SCENE == 1
        if (dist > .98) return dist - .1; // simple early-out optimziation
        dist = min(dist, dist2box(p, p2+.1, p1-.1, .1, 0.25));
...
vec3 trace(vec3 p_start, vec3 n)
{
    ...
    float light_ang = iMouse.x/iResolution.x*1.0;
    ...
    float cut_plane = (iMouse.y / iResolution.y - 0.1) * 1.0;
    ...
    for(float cut_plane = 4.0; cut_plane >= 0.0; cut_plane -= 0.1) {
        ...
        if (p_start.y > cut_plane) {
            ...
            float iso = fract(hit_dist*5.0);
            vec3 dist_color = mix(vec3(.2,.4,.6),vec3(.2,.2,.4),iso);
            dist_color *= 1.3/(max(0.0,hit_dist)+.05);
...
{
    vec2 uv = (fragCoord.xy - iResolution.xy*0.67) / iResolution.x;
    ...
    fragColor = vec4(trace(p2, n3), 2.0);
