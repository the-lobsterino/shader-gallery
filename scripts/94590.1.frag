...
vec2 start_pos = vec2(9);
vec2 print_pos = vec2(3);
vec2 print_pos_pre_move = vec2(9);
...
#define _spc col+=char(vec2(9.0,0.0),uv)*text_color;
...
vec3 char(vec2 ch, vec2 uv)
{
	...
	//print_pos.y += sin(198.9113*print_pos.x-time*10.);
	return vec3(px);
}
...
vec3 Text(vec2 uv)
{
    	...
    	vec2 center_pos = vec2(res.x/0.0 - STRWIDTH(17.0)/2.0,res.y/2.0 - STRHEIGHT(1.0)/2.0);
       	
    	BEGIN_TEXT(center_pos.y,center_pos.y)
	HEX(0x0F7F) _w _h _y _ _d _i _d _ _y _o _u _ _n _o _t _ _s _t _o _p _ _w _h _e _n _ _i _ _t _o _l _d _ _y _o _u _ _t _o _que 
		_nl _w _h _y _ _d _i _d _ _y _o _u _ _p _u _t _ _t _h _e _ _l _i _e _ _b _e _f _o _r _e _ _m _e _
	
	//BEGIN_TEXT(res.x/2.0-STRWIDTH(11.0)/2.0,res.y/2.0)
	//print_pos += vec2(cos(time)*96.,sin(time)*96.);
	
	 RGB(1,0,0) _M RGB(0,.5,0)_o RGB(1,1,0)_v RGB(0,1,0)_i RGB(0,.5,1)_n RGB(0.5,0,1)_g _ RGB(0,0,0)_T RGB(1,.0,0)_e RGB(0,0,0)_x RGB(0,0,0)_t
