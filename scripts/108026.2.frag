...
vec2 obj1(in vec3 p){
  ...
  return vec2(length(max(abs(p)-vec3(0.25,0.25,0.25),0.0))-0.05,1);
}
...
void main(void){
  ...
  if (f<maxd){
    ...
    vec3 ccc = (b*c+pow(b,8.0))*(1.0-f*.02);
	...
	float fog = 0.0005;
	fog *= 1.; // fog strength
       	c = mix(c, fogCol, 1.0 - (1.0) / (1.0 + f*f*f*fog));; //ändrom3da4twist

  	gl_FragColor=vec4(ccc,1.0);//simple phong LightPosition=CameraPosition
  }
