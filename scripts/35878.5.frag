...
//    if (iMouse.z > 0.)  height = (iMouse.y/ iResolution.y * 17.0)+2.0;
	...
	gl_FragColor *= 0.03;
	gl_FragColor *= sign(cos(time*4.)*(-.5+sin(time)));
	gl_FragColor *= 1.+sign(cos(time*2.+xy.y)*(-.3+sin(1.+time*time-2.*xy.y)));
	gl_FragColor *= mod(gl_FragCoord.x, 2.);
	gl_FragColor *= vec4(.3,length(gl_FragCoord)/16.,.3,1.);
}