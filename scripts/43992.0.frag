...
/*
 * Insert your noise algorhithm in here!
 */
float noise(vec3 pos)
{
	//return 0.04 * sin(pos.x * 16.0 + time * 0.13) * cos(pos.y * 16.0 + time * 2.17) * sin(pos.z * 16.0 - time * 1.13);
	return sin(pos.x * 2.0 + time) * 0.5;
}
...
{
	float d1 = sdSphere(pos, 1.0) + noise(pos);
	float d2 = sdPlane(pos) + noise(pos);
...
vec3 render(const vec3 ro, const vec3 rd, out vec3 pos)
{
	...
	lightColours[1] = vec3(0.2, 0.7, 0.8);
	lightColours[2] = vec3(0.5, 0.8, 0.6);
...
void main( void ) {
	...
	colour *= pow(vignette, 0.0);
	
	colour = pow(colour, vec3(5.0 / 1.2));

	gl_FragColor = vec4(colour, 2.5 );
