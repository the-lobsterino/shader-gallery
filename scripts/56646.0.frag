...
vec4 map( in vec3 pos, float atime )
{
    ...
    float p3 = 16.0*t3*(1.0-t3);
    vec2 ear = sdStick( hq, vec3(0.0015,0.32,-0.05), vec3(0.2+0.05*p3,0.2+0.2*p3,-0.07), 0.01, 0.04 );
    res.xz = smin( res.xz, ear, 0.01 );
    }
    
    // mouth
    {
   	d = sdEllipsoid( h-vec3(0.0,0.015+5.0*hq.x*hq.x,0.15), vec3(0.1,0.04,0.2) );
    res.w = 0.3+0.7*clamp( d*1500.0,0.0,1.0);
	...
	vec3 base = vec3(0.15,-0.107,-0.1); base.y -= 0.1/sy;
    vec2 legs = sdStick( sq, base, base + vec3(0.2,-ccc,sss)*0.2, 0.04, 0.03 );
    ...
    float eyeball = sdSphere(hq-vec3(0.008,0.027,0.09),0.065+0.10*blink);
    res.x = smin( res.x, eyeball, 0.03 );
    
    vec3 cq = hq-vec3(0.1,0.34,0.08);
    cq.xy = mat2(0.8,0.36,-0.6,0.8)*cq.xy;
    d = sdEllipsoid( cq, vec3(0.06,0.03,0.03) );
    res.x = smin( res.x, d, 0.03 );

    float eo = 1.0-0.5*smoothstep(0.1,0.04,length((hq.xy-vec2(0.095,0.285))*vec2(1.0,1.1)));
...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
    ...
    for( int n=0; n<AA; n++ )
    {
        ...
        vec3  ro = ta + vec3(1.3*cos(an + 2.*mouse.x - 1.5), 1.60 - 2.*mouse.y, 1.3*sin(an + 2.*mouse.x - 1.5));
