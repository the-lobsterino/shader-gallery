...
void main( void ) 
{
	...
	float red = (smoothstep(.5,.8, col.y)-smoothstep(.5,.8, col.x))*1.5;
	float green = (smoothstep(.3,.9, col.y)-smoothstep(.3,.9, col.x))*1.5;
	float blue = (smoothstep(.1,.7, col.y)-smoothstep(.1,.7, col.x))*1.5;
