...
float kraken(vec2 pixel, vec2 krakenPos) {
  ...
  for (int i = 1; i <= 10; i++) {
    dist = min(dist, arm(
	    pixel,
	    krakenPos,
	    float(i) * TAU / arms - time/3.,
	    0.5,
	    1.1 * abs(sin(time-9.)*3.)
    ));
  }
...
void main( void )
{
  ...
  float kr = kraken(-pos*1.5, -vec2(0.8, 0.8));
  if (kr < 0.0) {
    c = vec3(10.9-kr*2., 0.8-kr*0.5, 0.3+kr);
    c *= surface(kr, -0.8);
  }
  ...
  for (int j = 1; j <= 15; j+=1) {	
    float i = float(j);
	  ...
	  float wavephase = 12.0*x*13.3*PI+i;
	  //float wavephase = 0.0;
	  float t = time*(6.-i*.1);	
	  float wavetime = 0.0;
	  float waveSize = 0.1025;
	  float waveAmp = max(0.0,waveSize - i*0.0036);
	
	  float k = 0.1*PI*PI-waveSize-waveSize;
	  float waveshort = 5.0+i;	
	  float wavestokes = ((1.0-1.0/16.0-pow((k*k*k*k*waveSize*waveSize),2.0))*cos(pos.x*waveshort+t+wavephase) + 0.5*k*waveSize*cos(2.0*waveshort*pos.x+t+wavephase));
	  
	  float wave = waveAmp*pow(wavestokes,1.0)+sin(t+x*12.2)*0.01/i+(i*(0.06-i*0.0009)-0.5);	 
	  
	  if (pos.y < wave) {
            c = mix(vec3(0.0,0.3,0.2), vec3(0.6, 0.6+i/1160., 0.9), 0.05*i);
