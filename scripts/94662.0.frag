/*R041125作例　二酸化ジルコニウム　by ニシタマオ*/
/*サンドボックス上での試験用、簡易レイトレシステム、反射・屈折、ぼかし影、カラーパレット機能付（R041125版）*/
/*直近の改修：パレット彩色の改良、一部の精度削って軽量化*/
...
sskelton fSSRSet_VolleyBear01P(sskelton SS){
	...
	SS.VR_Head =vec4(0,	0,	0,	1);
	SS.VR_ArRU =vec4(+30,	0,	-5,	1);
	SS.VR_ArLU =vec4(+30,	0,	+5,	1);
...
sskelton fSSRSet_VolleyBear01N(sskelton SS){
	float nD =acos(-1.0) /180.0;
	SS.VR_Cntr;
...
sskelton fSSRSet_Soccer01P(sskelton SS){
	float nD =acos(-1.0) /180.0;
	SS.VR_Cntr;
	SS.VR_Body =vec4(-15,	0,	0,	0);
	SS.VR_Shld =vec4(0,	-15,	0,	0);
	SS.VR_Neck =vec4(-15,	0,	0,	0);
	SS.VR_Head =vec4(0,	-15,	0,	1);
	SS.VR_ArRU =vec4(+60,	0,	+15,	1);
	SS.VR_ArLU =vec4(-60,	0,	-15,	1);
	SS.VR_ArRL =vec4(+60,	0,	-15,	1);
	SS.VR_ArLL =vec4(+15,	0,	+15,	1);
...
sskelton fSSRSet_Soccer01N(sskelton SS){
	float nD =acos(-1.0) /180.0;
	SS.VR_Cntr;
	SS.VR_Body =vec4(+15,	0,	0,	0);
	SS.VR_Shld =vec4(0,	+15,	0,	0);
	...
	SS.VR_ArRU =vec4(-60,	0,	+15,	1);
	SS.VR_ArLU =vec4(+60,	0,	-15,	1);
	SS.VR_ArRL =vec4(+15,	0,	-15,	1);
	SS.VR_ArLL =vec4(+60,	0,	+15,	1);
	SS.VR_LeRU =vec4(+75,	0,	0,	0);
	SS.VR_LeLU =vec4(0,	0,	0,	0);
...
/*全て統合した完成部*/
float fNObject_WalkWarrior01(vec4 VP){
	sskelton SS;
	SS.VP =VP;
	SS =fSSPSet_Man01( SS);
...
float fNObject_WalkDilbert01(vec4 VP){
	sskelton SS;
	SS.VP =VP;
	SS =fSSPSet_Man01( SS);
	SS =fSSRSet_Walker01( SS);
...
float fNObject_RandomBear01(vec4 VP, float nSQ){
	sskelton SS;
	SS.VP =VP;
	SS =fSSPSet_Bear01( SS);
...
float fNObject_RandomDilbert01(vec4 VP, float nSQ){
	sskelton SS;
	SS.VP =VP;
	SS =fSSPSet_Man01( SS);
...
float fNObject_VolleyBear01(vec4 VP){
	sskelton SS;
	SS.VP =VP;
	SS =fSSPSet_Bear01( SS);
	SS =fSSChange_Inbetween(fSSRSet_VolleyBear01P( SS), fSSRSet_VolleyBear01N( SS), sin(VP.w *acos(-1.0) *2.0));
	SS =fSSkeltonMake( SS);
...
float fNObject_SoccerBear01(vec4 VP){
	sskelton SS;
	SS.VP =VP;
	SS =fSSPSet_Bear01( SS);
	SS =fSSChange_Inbetween(fSSRSet_Soccer01P( SS), fSSRSet_Soccer01N( SS), sin(VP.w *acos(-1.0) *2.0));
	SS =fSSkeltonMake( SS);
...
vec4 fVPSet(vec3 V3P){
	...
	/*座標系とマウス操作の関係等を記述*/
	VP.xyz *=fM3Rotate(-VMouse.yx *2.0 +vec2(0.25,0));
	return VP;
}
...
float fNMap(vec3 V3P){
	...
	int iSQ =fISequencer(60, 3);
	int iSQ_Soccer =fISequencer(60 *2, 2);
	iSQ_Soccer =1;

	if(iSQ ==0 || iSQ ==1){	/*ニャンパスマン・ベアー*/
		vec4 VPi =VP.zyxw;
		VPi.y -=2.0;
		VPi.w *=0.25;
		...
		if(iSQ ==1)	{
			VPi.x +=VP.w *0.5;
			float nX_Mod =mod(VPi.x, 8.0) -4.0, nX_Dom =VPi.x -nX_Mod;
			VPi.x =nX_Mod;
			VPi.w +=nX_Dom /8.0 *0.5;
		}

		float nPi;
		nPi =fNObject_VolleyBear01(VPi);
		if(iSQ_Soccer ==1)	nPi =fNObject_SoccerBear01(VPi);
		sface SFi =fSFSet_Default();
		SFi.nDistance =nPi;
		SFi.ID_Object =11;
		SFi.ID_Pallet =(nSignZ >0.0)?	2:5;
		SFi.VP =VPi;
		SF =fSFMin(SF, SFi);
	}
	...
	if(iSQ ==2){
		...
		float nA =nTh_Dom /acos(-1.0) /2.0 *float(iMirage);
		VPi.w +=nA;
	...
	if(true){	/*ボール*/
		vec4 VPi =VP.zyxw;
		VPi.y -=2.0;
		if(iSQ_Soccer ==1)	VPi.y +=2.0;
		VPi.w *=0.25;
		if(iSQ ==1)	{
			VPi.x -=VP.w *(8.0 -0.5) +8.0;
			float nX_Mod =mod(VPi.x, 8.0) -4.0, nX_Dom =VPi.x -nX_Mod;
			VPi.x =nX_Mod;
			VPi.w +=nX_Dom /8.0 *0.5;
		}
		...
		VPi.y +=pow(VPi.w -0.5, 2.0) *16.0 -6.5;
...
sface fSFEffect_Before(sface SF, smarch SM){
	...
	if(true){	/*基本*/
		...
		VColor.rgb =VColor.rgb *0.8 +0.2;
	}
	if(ID_Object ==21)	VColor.rgb =sin(vec3(0,1,2) /3.0 *acos(-1.0) *2.0 +atan(SF.VP.x, SF.VP.z) *4.0 +SF.VP.y *4.0) *0.4 +0.6;
	if(ID_Object ==22)	V3RRR =vec3(0,1,2);
	if(ID_Object ==12)	VColor.rgb =sin(vec3(0,1,2) /3.0 *acos(-1.0) *2.0 +atan(VP.x, VP.z) +VP.w) *0.3 +0.7;
	if(ID_Object ==101)	VColor.rgb =sign(sin(VP.x) *sin(VP.z)) *0.2 +vec3(0.8);
	/*ここまで*/
...
vec4 fVMain(vec2 V2UV){
	...
	if(cbSetting_March2nd_Refract){
		...
		smarch SM =fSMRayMarch(SM2nd_Refract.V3P, SM2nd_Refract.V3Direction, 0.1, ciDefinition /4);
		if(SM.bTouch){
			{
				...
				SM2nd_RefractPlus =fSMRayMarch(SM2nd_RefractPlus.V3P, SM2nd_RefractPlus.V3Direction, 0.1, ciDefinition /2);
