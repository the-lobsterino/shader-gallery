...
vec4 flash( vec2 uv ) {
	...
	for (float i1=0.0;i1<=PI;i1+=0.3) {
		...
		for (float i2=0.0;i2<=TWO_PI;i2+=0.3) {
			...
			p = rotate(p, vec3(1., 0., 0.), PI*sin(t));
					
			float dist = distance(p, vec3(uv.x,uv.y, -1. + sin(0.1*time)));
...
void main( void ) 
{
	...
	vec3 ch_color = vec3(1.); // hsv2rgb_smooth(vec3(time*0.4+uv.y*0.1,0.5,1.0));

	vec3 bg_color = vec3(_d*0.4, _d*0.2, _d*0.1);
	uv.x += 0.5+sin(time+uv.y*0.7)*0.5;
	ch_pos = ch_start;
	ch_pos.x += 5.0 + 10.*sin(time);
	...
	gl_FragColor = flash(uv) + vec4(color, 1.0);
}