...
void main( void ) {
	...
	float speed = 5.0; // larger -> slower
	float speed2 = 2.5; // larger -> slower
	float freq = 0.7; // ripples
	float xflow = 1.5; // flow speed in x direction
	float yflow = 0.0; // flow speed in y direction
	vec4 baseCol = vec4(0.5, 0.0, 0.0, 1.0);
	vec3 specularCol = vec3(0.8, 0.95, 0.0);
	...
	c /= float(MAX_ITER);
	c = 1.5 - sqrt(c);
	gl_FragColor = vec4(vec3(c * c * c * c) * specularCol, 0.0) + baseCol;
