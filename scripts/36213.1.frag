...
// DUDE YOU ARE A FUCKING HACK

// STOP STEALING PEOPLES SHADERS AND SLAPPING YOUR FUCKING TAG ON THEM

// THE ONLY PERSON YOU ARE FOOLING IS YOURSELF

// THE REST OF US REAL PROGRAMMERS KNOW YOU ARE NOTHING BUT A FUCKING HACK

// STOP HIJACKING SHADERS AND GO DIE IN A FIRE
...
void main( void ) 
{
	...
	ch[ 6] = 0.1;
	ch[ 7] = mix(0.0, c_s, clamp(an - 6.0, 0.0, 1.0));	ch[ 8] = mix(0.0, c_u, clamp(an - 7.0, 0.0, 1.0));
	ch[ 9] = mix(0.0, c_c, clamp(an - 8.0, 0.0, 1.0));	ch[10] = mix(0.0, c_k, clamp(an - 9.0, 0.0, 1.0));
	ch[11] = mix(0.0, c_s, clamp(an - 8.0, 0.0, 1.0));
