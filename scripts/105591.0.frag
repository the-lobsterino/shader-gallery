...
vec3   iResolution = vec3(resolution, 22.0);
float  iGlobalTime = time;
vec4   iMouse = vec4(mouse, 12., 12.0);
...
float featureSize = 450./((iResolution.x*aspect+iResolution.y));

float f(vec2 p)
{
	p.x = sin(p.x*20.+time*12.2)*sin(time+p.x*12.)*2.;	
    p += sin(p.x*1.5)*.1;
...
void main(void)
{
	...
	{
		vec3 col2 = (sin(vec3(3.33,2.5,2.2)+i*0.15)*1.5+0.9)*(1.-f(p));
		col = max(col,col2);
        ...
        p = mix(pa,pb,smoothstep(-.07,.7,sin(time*0.)+.1));
	}
