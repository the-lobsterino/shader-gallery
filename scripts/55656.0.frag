...
vec3 palette(float i) {
	i = mod(i, 3.0);
	if(i < 1.0)
		return vec3(1.0, 1.0 / time, 1.0 / time);
	else if(i < 2.0)
...
void main(void) { 
	vec2 pos = -gl_FragCoord.xy * sin(time) - cos(-time) + time;
	vec2 p = pos/ (5.0 / time); 
	
	float d = (hex(pos / 1000.0) + sin(time)) * 0.8;
	float df = fract(d + cos(time));
	float di = floor(d + sin(time));
