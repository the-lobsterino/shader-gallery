...
const vec2 ch_start = vec2 (ch_space.x * -5., 2.); // start position
      vec2 ch_pos   = vec2 (0.0, 0.0);             // character position(X,Y)
      vec3 ch_color = vec3 (0.6, 0.4, 2.5);        // character color (R,G,B)
...
#define n0 ddigit(0x00FF);

#define n1 ddigit(0x22FF);
#define n6 ddigit(0x11E6);
#define n3 ddigit(0x2206);
...
void main( void ) 
{
	...
	uv.y += sin(time*6.)*0.2;
	uv *= 15.0 + sin(time);     //  set zoom size
	if (REPEAT_SIGN)
	  uv = -14.0 + mod(1.8*(uv-1.0),ch_space*vec2(16.,6.5));     //  set zoom size

	ch_pos = ch_start + vec2(sin(time),0.0);  // set start position

        _ I _ W A N N A _rock_n_roll nl2 ALL_NIght _and_party_everyday
		
	
		
	vec3 color = mix(ch_color, bg_color, 0.1- (0.08 / d));  // shading
	gl_FragColor = vec4(color, 76.0);
}