...
#define time (4.*time-length(surfacePosition)*64.)

const float timestep = 0.2; //lower = more accurate/stable, higher = faster.

//Life function parameters.
#define gw (0.08 + 0.03*sin(pow(time, 1.1) * 0.235432))
#define gsl (0.26 + 0.03*sin(pow(time, 1.2) * 1.295784873))
#define gel (0.40 + 0.05*sin(time * 0.198542834))
#define gsh (0.70 + 0.3*sin(time * 0.74534345))
#define geh (0.95 + 0.03*sin(time * 0.205423454))
#define mxs (0.12 + 0.04*sin(time * 0.193734736))
#define mxe (0.60 + 0.06*sin(time * 0.4847373733))
...
void main( void ) 
{
	...
	gl_FragColor = vec4( vec3( value + 0.8*cell.x, 1.1*value + cell.y, value*(1.+cos(length(cell)*10.)) ), value );
