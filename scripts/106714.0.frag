...
// ... [previous definitions, such as extensions, precision, and uniforms]

// Function to create a 3D Torus Knot
vec3 torusKnot(float t) {
    float r = 0.8 + 0.2 * sin(3.0 * t);
    vec3 p;
    p.x = r * cos(2.0 * t);
    p.y = r * sin(2.0 * t);
    p.z = 0.2 * sin(3.0 * t);
...
void main()
{
    ...
    float t = time;
    vec3 knotPos = torusKnot(t);

    // Check proximity of fragment to knot (rudimentary)
    float d = length(knotPos.xy - q);
    if(d < 0.1) {
        col = vec3(1.0); // color the knot white
    }
