...
void main( void ) 
{
	...
	vec3 bg_color = vec3(_d*0.8*sin(time*11.21)*0.44, _d*sin(time*3.115)*.1, _d*cos(time*11.21)*0.3) * (uv.y*.5);

	
	uv.y += -1.+sin(time*2.+uv.x*0.7)*0.5;
	ch_pos = ch_start;
	nl1 
	ch_pos.x -= ch_space.x; ddigit(0x4406);
	ddigit(0x911F);
	ddigit(0x00F9);
	ddigit(0x0A99);
	ddigit(0x111F);
	ch_pos.x += ch_space.x;
	ddigit(0x4466); ddigit(0x8899); ch_pos.x += ch_space.x; ddigit(0x4436); ddigit(0x119F); ddigit(0x4466); ddigit(0x0078);
	nl2 ch_pos.x += ch_space.x*3.5; ddigit(0x1177); ddigit(0x22FF); ddigit(0x1177); ddigit(0x11E7);
