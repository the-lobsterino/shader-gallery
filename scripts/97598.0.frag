/*R041206作例　サーカス　by ニシタマオ*/
/*サンドボックス上での試験用、簡易レイトレシステム、廉価ＰＣ用軽量版（R041206版）*/
/*直近の改修：機能整理*/
...
float fNLetters00(vec4 VP){
	float NP =1e+6;
	{ vec3 V3C =vec3(2.5,2.5,1),  V3D =vec3(5,0,0), V3P;V3P.x -=V3C.x;V3P.x -=V3C.x;V3P.x -=V3C.x; {   vec4 VP_Keep =VP;   VP.xyz -=V3P;{ vec4 VA, VB; VA.w =float(1);{ VA.xy =vec2(-2,2); VB.xy =vec2(2,1); NP =min(NP, fNLL(VP, VA, VB));}{ VA.xy =vec2(2,1); VB.xy =vec2(-2,0); NP =min(NP, fNLL(VP, VA, VB));}{ VA.xy =vec2(-2,0); VB.xy =vec2(2,-1); NP =min(NP, fNLL(VP, VA, VB));}{ VA.xy =vec2(2,-1); VB.xy =vec2(-2,-2); NP =min(NP, fNLL(VP, VA, VB));}{ VA.xy =vec2(-2,-2); VB.xy =vec2(-2,2); NP =min(NP, fNLL(VP, VA, VB));}{ VA.xy =vec2(2,1); VB.xy =vec2(-2,0); NP =min(NP, fNLL(VP, VA, VB));}{ VA.xy =vec2(-2,0); VB.xy =vec2(2,-1); NP =min(NP, fNLL(VP, VA, VB));}{ VA.xy =vec2(2,-1); VB.xy =vec2(-2,-2); NP =min(NP, fNLL(VP, VA, VB));}{ VA.xy =vec2(-2,-2); VB.xy =vec2(-2,2); NP =min(NP, fNLL(VP, VA, VB));}{ VA.xy =vec2(-2,0); VB.xy =vec2(2,-1); NP =min(NP, fNLL(VP, VA, VB));}{ VA.xy =vec2(2,-1); VB.xy =vec2(-2,-2); NP =min(NP, fNLL(VP, VA, VB));}{ VA.xy =vec2(-2,-2); VB.xy =vec2(-2,2); NP =min(NP, fNLL(VP, VA, VB));}{ VA.xy =vec2(2,-1); VB.xy =vec2(-2,-2); NP =min(NP, fNLL(VP, VA, VB));}{ VA.xy =vec2(-2,-2); VB.xy =vec2(-2,2); NP =min(NP, fNLL(VP, VA, VB));}{ VA.xy =vec2(-2,-2); VB.xy =vec2(-2,2); NP =min(NP, fNLL(VP, VA, VB));}}   VP =VP_Keep; }V3P +=V3D; {   vec4 VP_Keep =VP;   VP.xyz -=V3P;{ vec4 VA, VB; VA.w =float(1);{ VA.xy =vec2(2,2); VB.xy =vec2(-2,2); NP =min(NP, fNLL(VP, VA, VB));}{ VA.xy =vec2(-2,2); VB.xy =vec2(-2,-2); NP =min(NP, fNLL(VP, VA, VB));}{ VA.xy =vec2(-2,-2); VB.xy =vec2(2,-2); NP =min(NP, fNLL(VP, VA, VB));}{ VA.xy =vec2(-2,2); VB.xy =vec2(-2,-2); NP =min(NP, fNLL(VP, VA, VB));}{ VA.xy =vec2(-2,-2); VB.xy =vec2(2,-2); NP =min(NP, fNLL(VP, VA, VB));}{ VA.xy =vec2(-2,-2); VB.xy =vec2(2,-2); NP =min(NP, fNLL(VP, VA, VB));}{ VA.xy =vec2(-2,0); VB.xy =vec2(2,0); NP =min(NP, fNLL(VP, VA, VB));}}   VP =VP_Keep; }V3P +=V3D; {   vec4 VP_Keep =VP;   VP.xyz -=V3P;{ vec4 VA, VB; VA.w =float(1);{ VA.xy =vec2(-2,-2); VB.xy =vec2(0,2); NP =min(NP, fNLL(VP, VA, VB));}{ VA.xy =vec2(0,2); VB.xy =vec2(2,-2); NP =min(NP, fNLL(VP, VA, VB));}{ VA.xy =vec2(0,2); VB.xy =vec2(2,-2); NP =min(NP, fNLL(VP, VA, VB));}{ VA.xy =vec2(-1,0); VB.xy =vec2(1,0); NP =min(NP, fNLL(VP, VA, VB));}}   VP =VP_Keep; }V3P +=V3D; {   vec4 VP_Keep =VP;   VP.xyz -=V3P;{ vec4 VA, VB; VA.w =float(1);{ VA.xy =vec2(-2,-2); VB.xy =vec2(-2,2); NP =min(NP, fNLL(VP, VA, VB));}{ VA.xy =vec2(-2,2); VB.xy =vec2(2,1); NP =min(NP, fNLL(VP, VA, VB));}{ VA.xy =vec2(2,1); VB.xy =vec2(-2,0); NP =min(NP, fNLL(VP, VA, VB));}{ VA.xy =vec2(-2,2); VB.xy =vec2(2,1); NP =min(NP, fNLL(VP, VA, VB));}{ VA.xy =vec2(2,1); VB.xy =vec2(-2,0); NP =min(NP, fNLL(VP, VA, VB));}{ VA.xy =vec2(2,1); VB.xy =vec2(-2,0); NP =min(NP, fNLL(VP, VA, VB));}{ VA.xy =vec2(0,0); VB.xy =vec2(2,-2); NP =min(NP, fNLL(VP, VA, VB));}}   VP =VP_Keep; }V3P +=V3D;}
	return NP;
}
/*文字列形状生成機能ここまで*/
...
/*動作部*/
sskelton fSSRSet_Bear02(sskelton SS){
	float nD =acos(-1.0) /180.0;
	vec4 VS =sin(SS.VP.w +vec4(0,1,2,3) /4.0 *acos(-1.0) *2.0);
	SS.VR_Cntr;
	SS.VR_Body =vec4(-90.0,	0,	0,	0);
	SS.VR_Shld =vec4(0,	0,	0,	0);
	SS.VR_Neck =vec4(-45.0,	0,	0,	1);
	SS.VR_Head =vec4(0,	0,	0,	1);
	SS.VR_ArRU =vec4(15.0 *VS.x,	0,	0,	1);
	SS.VR_ArLU =vec4(15.0 *VS.z,	0,	0,	1);
	SS.VR_ArRL =vec4(15.0 +15.0 *VS.x,	0,	0,	1);
	SS.VR_ArLL =vec4(15.0 +15.0 *VS.z,	0,	0,	1);
	SS.VR_LeRU =vec4(15.0 *VS.y,	0,	0,	0);
	SS.VR_LeLU =vec4(15.0 *VS.w,	0,	0,	0);
	SS.VR_LeRL =vec4(-15.0 +15.0 *VS.y,	0,	0,	1);
	SS.VR_LeLL =vec4(-15.0 +15.0 *VS.w,	0,	0,	1);
...
/*全て統合した完成部*/
float fNObject_WalkBear02(vec4 VP){
	sskelton SS;
	VP.y -=2.0;
	SS.VP =VP;
	SS =fSSPSet_Bear01( SS);
	SS =fSSRSet_Bear02( SS);
	SS =fSSkeltonMake( SS);
...
float fNMap(vec3 V3P){
	...
	if(true){
		...
		VPi.xz +=VPi.w *vec2(0.1, 1);
		vec2 V2P_Mod =mod(VPi.xz, 16.0) -8.0;
		VPi.xz =V2P_Mod;

		float nPi =fNObject_WalkBear02(VPi);
...
vec4 fVFront(vec4 VColor, vec2 V2UV){
	...
	if(nC <0.0){
		VColor.rgb =mix(VColor.rgb, (sin(vec3(0,1,2) /3.0 *acos(-1.0) *2.0 +VP.w) *0.3 +0.7) *clamp(-nC *2.0, 0.0, 1.0), sin(VP.w) *0.5 +0.5);
	}
...
void fCameraSet(vec2 V2UV, inout vec3 V3P, inout vec3 V3D){
