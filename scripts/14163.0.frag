...
#define AMBIENT_COLOR vec3(0.2,0,0)
#define FLOOR_COLOR vec3(1.,0,0)
#define ENERGY_COLOR vec3(2.5,1,1)
#define BRIGHTNESS .9
#define GAMMA 2
...
// cold mod by ohioan: we are getting plenty of snow here and i want to leave
// hats off to the original hacker, this is what i would want an endless rail shooter to look like
// vwumvwumvwumvwumvwumvwumvwumvwumvwum need sounds
...
vec3 path(float ti) {
	return vec3(ti,0.37+sin(ti*0.5+0.3)*0.33,ti+0.2);
}
...
vec3 move(inout mat2 rotview1,inout mat2 rotview2) {
	float ttt = 1.1;
	vec3 go=path(ttt);
	vec3 adv=path(ttt+.7) + vec3(-sin(ttt*0.25),0.0,sin(ttt*0.25));
...
vec2 HmdWarp(vec2 uv)
{
	...
	uv = vec2((mod(uv.x,1.0)-0.5)*2.0, uv.y);

	// HMD Parameters
	vec2 ScaleIn = vec2(1.0);
	vec2 LensCenter = vec2(0,0);
	vec4 HmdWarpParam = vec4(1.0, 1.0, 1.0, 1.0);
	vec2 Scale = vec2(1.0);
...
void main()
{
	...
	vec3 eyespan = vec3(0.0, 0,0)*sign(uv.x) ;
	uv = HmdWarp(uv);
	...
	vec3 from=origin+eyespan+move(rotview1,rotview2);
