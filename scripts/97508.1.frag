...
#define smoothstep_Mode          0               // smoothstep 0 = off, 1 = on                  
#define smoothstep_EdgeLen       (1./128.)
#define backbuffer_Mode          0
...
#define gammaCorrection_Value    0.85
#define onlyBlackWhite           0
#define dithering_Mode           0

// options > raymarch options
#define rm_MaxSteps              60
#define rm_MaxDist               35.0
...
{
    p.xz *= rot(-(mouse.x*7.-.5));
    p.yz *= rot(mouse.y*5.-.5);
...
vec3 render3Dsdf( vec2 p )
{
	...
	if ( d < rm_MaxDist )        
	{
		...
		float fog = 0.005;
		fog *= rm_FogStrength;
       	        c = mix(c, vec3(0.1, 0.0, 0.1), 1.0 - (1.0) / (1.0 + d*d*d*fog));;
	#endif
	return c;
 }
