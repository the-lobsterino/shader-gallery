...
{
	float col = 0.0;
	float zoom = ZOOM_LEVEL+50.0;	
	float ratio = resolution.x/resolution.y;
	
	vec2 cCoord = gl_FragCoord.xy-resolution/2.0;
	...
	for(int i=0;i<=60;i++){
		...
		if(length(z)>4.0){
			...
			col=smoothstep(-0.,.2,1.3*float(i)/128.0);
	...
	gl_FragColor=vec4(gl_FragCoord.xy/resolution*col,(sin(time*0.5)*0.5+0.5)*col,1.0);
