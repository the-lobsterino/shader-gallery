...
const vec2 ch_space = ch_size + vec2(1.0, 1.0);    // character distance Vector(X,Y)
  vec2 ch_start = vec2 (ch_space.x * 2., 1.); // start position
...
void main( void ) 
{
	...
	uv = ( gl_FragCoord.xy / resolution.xy )*2.-1.0 ;
	//uv = -0.5+atan(uv)+sin(time);
	...
	uv.x+=10.00;
	ch_start.x = 13.0-mod(time*3.0,22.0) ;
	ch_pos = ch_start;
	
	
	n5 U n8 s_minus M A K E  M O R E  F R A C T A L S  P L E A S E
		 
		
		vec3 color = mix(ch_color, bg_color, 1.0- (0.08 / d*0.5));  // shading
