...
#define nl1 ch_pos = ch_start;  ch_pos.y -= 1.0;
#define nl2 ch_pos = ch_start;  ch_pos.y -= 4.0;
#define nl3 ch_pos = ch_start;	ch_pos.y -= 7.0;
...
void main( void ) 
{
	...
	ch_pos = ch_start;
	          ...
	          nl1  _ T H E _ T R U T H 
		  nl2  I S s_dot 
                  nl3 F O R T N I T E _ S U X
			 
		
	vec3 color = mix(ch_color, bg_color, 1.0- (0.08 / d*2.0));  // shading
	gl_FragColor = vec4(1.-pow(length(color)-0.25, 4.)*vec3(.5,.5,.5), 1.0);
}