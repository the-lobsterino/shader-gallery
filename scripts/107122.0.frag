...
void main() {
	...
	vec2 p = 2.0 * gl_FragCoord.xy / resolution.xy - 1.;
	...
	s = vec2(cos(-time * PI / 4. * TimeK + PI / 2.0), sin(time * PI / 1. * TimeK + PI / 2.0));
	// рисование минутной стреки
	outColor += DrawClockLine(p, vec2(0.0), ClockRadius, s, 1.2, clockLineMinCol);
	
	// изменяем направляющий вектор
	s = vec2(cos(-time * PI / 9999. * TimeK + PI / 2.0), sin(time * PI / 2. * TimeK + PI / 2.0));
