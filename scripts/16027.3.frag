...
// https://glsl.heroku.com/e#16027.1
...
float gameStep() {
  ...
  if (isLive) {
    ...
    if (neighbors==3)
      return 1.0;
    else if (neighbors==0 && rand(uv.xy * time) < EPS)
      return 1.0;
    else
      return 0.0;
  }
...
void main( void ) {
  ...
  uv = ( gl_FragCoord.xy / resolution.xy ) + vec2(-uvUnit.x*1.0, -uvUnit.y*3.0);
  ...
  color *= smallnormsin(8.0*time)*live;
  color += ghosting();
