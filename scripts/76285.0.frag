...
void main() {
	...
	float fx=(pattern(p/3.0)+0.5)/3.0;
	f1=0.5/(abs(sin(p.x+time)*2.0-p.y));
	f2=0.5/(abs(cos(p.x-time)*4.0-p.y));
	f1*=fx*f3.x;f2*=fx*f3.y;
	gl_FragColor=vec4(f1+f2*6.0,f1*2.0+f2*2.0,f1*4.0+f2,1.0);
}
...
float fbm(vec2 p) {
	...
	float f=0.0;
	f+=perlin(p+tme);
	f+=perlin(vec2(p.x+tme*0.5,p.y)*0.5)*0.25;
	f+=perlin(vec2(p.x,p.y-tme*0.25)*0.25)*0.25;
	f+=perlin(p*0.125)*0.125;
	f+=perlin(p*0.0625)*0.0625;
	return f/1.0;
}
...
vec2 noise(vec2 n) {
	vec2 ret;
	ret.x=fract(sin(dot(n.xy, vec2(12.9898, 58.233)))* 43758.5453)*2.0-1.0;
	ret.y=fract(sin(dot(n.xy, vec2(34.9865, 55.946)))* 28618.3756)*2.0-1.0;
