...
void main( void ) {
	...
	vec2 P0 = vec2(sin(time*2.0013),cos(time*0.5999))*50.5;
	vec2 P1 = vec2(cos(time*1.4011),sin(time*2.0007))*50.5;
	
	float d0 = distance(rP,P0);
	float d1 = distance(rP,P1);

	vec3 color =  vec3( rgbFromHue(2.0/d0 - 2.0/d1 + time) * smoothstep(10.01, 0.0, 1.0/d0 + 1.0/d1));
