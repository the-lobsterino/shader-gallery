precision mediump float; 
uniform vec2 resolution;
uniform vec4 mouse;
uniform float time;
const float pi = 3.141592653589793;

float hash2(vec2 p) {
    return fract(sin(p.x * 15.73 + p.y * 35.29) * 43758.29);    
}

float shash2(vec2 p) {
    return hash2(p) * 2.0 - 1.0;
}

vec2 hash22(vec2 p) {
    return fract(sin(mat2(15.23, 35.76, 75.48, 153.69) * p) * 43758.29);
}

float noise2(vec2 p) {
    vec2 g = floor(p);
    vec2 f = fract(p);
    f = f*f*(3.0 - 2.0*f);
    
    float lt = hash2(g + vec2(0.0, 1.0));
    float rt = hash2(g + vec2(1.0, 1.0));
    float lb = hash2(g + vec2(0.0, 0.0));
    float rb = hash2(g + vec2(1.0, 0.0));
    
    float t = mix(lt, rt, f.x);
    float b = mix(lb, rb, f.x);
    return mix(b, t, f.y);
}

float voronoi(vec2 p) {
    vec2 g = floor(p);
    vec2 f = fract(p);
    
    float d = 2.0;
    for(int i = -1; i <= 1; i++) {
        for(int j = -1; j <= 1; j++) {
            vec2 b = vec2(i, j);
            d = min(d, length(hash22(g + b) + b - f));
        }
    }
    return d;
}

float sdPlane(vec3 p) {
    return p.y - 0.1 * clamp(voronoi(p.xz * 2.0), 0.0, 1.0) + noise2(p.xz) * 0.1;
}

float sdSphere(vec3 p, float r) {
    return length(p) - r;
}

float smin(float a, float b, float k) {
    float h = clamp(0.5 + 0.5 * (b - a) / k, 0.0, 1.0);
    return mix(b, a, h) - k * h * (1.0 - h);
}

float map(vec3 p) {
    float d = sdPlane(p);
    d = smin(d, sdSphere(p - vec3(0.0, 0.25, 0.0), 0.5), 0.5);
    return d;
}

vec3 calcNormal(vec3 p) {
    vec2 e = vec2(-1.0, 1.0) * 0.0001;
    return normalize(
        e.xyy * map(p + e.xyy) +
        e.yxy * map(p + e.yxy) +
        e.yyx * map(p + e.yyx) +
        e.xxx * map(p + e.xxx)
    );
}

void main() { 
    vec2 p = gl_FragCoord.xy / resolution;
    p = 2.0 * p - 1.0;
    p.x *= resolution.x / resolution.y;
    
    vec3 ro = vec3(0.0, 0.2, 3.0);
    vec3 ta = vec3(0.0, 0.0, 0.0);
    vec3 cw = normalize(ta - ro);
    vec3 up = vec3(0.0, 1.0, 0.0);
    vec3 cu = normalize(cross(cw, up));
    vec3 cv = normalize(cross(cu, cw));
    vec3 rd = normalize(p.x * cu + p.y * cv + ( ( sin(time) + 1.2 ) / 0.5 ) * cw);
    
    float e = 0.01;
    float t = 0.0;
    float h = e * 2.0;
    for(int i = 0; i < 50; i++) {
        if(h < e || t > 20.0) continue;
        h = map(ro + rd * t);
        t += h;
    }
    
    vec3 pos = ro + rd * t;
    vec3 nor = calcNormal(pos + time);
    vec3 lig = normalize(vec3(1.0));
    float dif = dot(nor, lig);
    float fre = 1.0 + dot(nor, rd);
    float spe = clamp(dot(reflect(rd, nor), lig), 0.0, 1.0);
    spe = pow(spe, 32.0);
    vec3 col = vec3(0.4, 1.0, 0.9) * (dif + spe);
    
    if(t > 20.0) {
        col = vec3(0.3, 0.2, 0.4);
    }
    gl_FragColor = vec4(col, 1.0);
}