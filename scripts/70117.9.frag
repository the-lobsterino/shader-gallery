...
void main(void)
{
	...
	f *= cos(f+2.+time*3.);
	f *= cos(f+3.+time*4.);
	if(f < 0.1) gl_FragColor *= 0.;
	if(gl_FragColor.r + gl_FragColor.b < 0.01){
		vec2 w = (cos(f*33.)-7.*sin(f*f*10000.))*5.*normalize(surfacePosition);
		gl_FragColor = texture2D(bb, (gl_FragCoord.xy+w+vec2(sin(f*100.),cos(f*100.)))/resolution)-1./256.;
	}
}