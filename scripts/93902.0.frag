...
/*人体形状生成機能、中割機能付（R041022版）by ニシタマオ*/
...
sskelton fSSPSet_Dora01(sskelton SS){
	...
	SS.VP_LeRU =vec4(+0.8,	-0.2,	0,	0.45);
	SS.VP_LeLU =vec4(-0.8,	-0.2,	0,	0.45);
...
vec4 fVPSet(vec3 V3P){
	...
	VP.xyz *=fM3Rotate(-VMouse.yx *4.0);
	return VP;
}
...
float fNMap(vec3 V3P){
	...
	if(true){	/*ニャンパスマン*/
		...
		float nTh =atan(VPi.x, VPi.z) +VPi.w *0.1, nL =length(VPi.xz);
		float nTh_Mod =mod(nTh +VPi.w *0.1, acos(-1.0) *2.0 /3.0) -acos(-1.0) /3.0;
		VPi.xz =vec2(cos(nTh_Mod), sin(nTh_Mod)) *nL;

		VPi.x -=10.0;
		VPi.xy *=fM2Rotate(nTh *0.5);
		VPi.y =abs(VPi.y) -4.0;

		float nPi =fNWalkBear01(VPi, 0.0);
	...
	if(true){
		vec4 VPi =VP.xzyw;
		float nTh =atan(VPi.x, VPi.z) +VPi.w *0.1;
		vec2 V2Pi =vec2(length(VPi.xz) -10.0, VPi.y);
		V2Pi *=fM2Rotate(nTh *0.5);
		float nPi =length(max(abs(V2Pi) -vec2(2,1), 0.0)) -0.1;
	...
	if(false){	/*水面*/
		...
		sface SFi =fSFSet_Default();
		SFi.nDistance = abs(VPi.y +5.0 +sin(VPi.w) *5.0 +sin(VPi.x +sin(VPi.z +VPi.w)) *sin(VPi.z +sin(VPi.x +VPi.w)) *0.02) -0.5;
		SFi.ID_Object =9;
		SFt =fSFSmoothMin(SFt, SFi, 2.0);
 	}
...
sface fSFEffect_Before(sface SF, smarch SM){
	...
	if(SF.ID_Object ==21){
		VColor.rgb =sin(vec3(0,1,2) /3.0 *acos(-1.0) *2.0 +VP.w) *0.1 +0.9;
	}

	if(SF.ID_Object ==22){
		VColor.rgb =sin(vec3(1,2,0) /3.0 *acos(-1.0) *2.0 +VP.w) *0.1 +0.9;
	}
