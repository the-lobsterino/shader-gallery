...
vec3 char(vec2 ch, vec2 uv)
{
	...
	print_pos.y += sin(198.9113*print_pos.x-time*3.0);
	return vec3(px);
}
...
vec3 Text(vec2 uv)
{
	...
	HEX(0x00FF7F) _H _E _Y _spc _E _V _E _R _Y _O _N _E _I _apo _M _spc _T _R _Y _I _N _G _spc _T _O _spc _L _E _A _R _N _spc _G _L _S _L
		      _nl _spc _A _N _D _spc _W _A _N _T _spc _T _O _spc _K _N _O _W _spc _W _H _E _R _E _spc _T _O _spc _S _T _A _R _T 
	BEGIN_TEXT(res.x/3.0-STRWIDTH(11.0)/2.0,res.y/2.0)
