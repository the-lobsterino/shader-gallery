...
void main( void ) {
	...
	// central disc
	float c = 1.0 - smoothstep(0.1+mouse.x/10.0, 0.11+mouse.x/10.0, centerDist);
	...
	float hazControlMax = 1.0 - sstep(0.3+mouse.x, centerDist);
	float hazControl = sstep(0.27, centerDist) * hazControlMax;
	float wingz = b * hazControl;
	vec4 COLOR = vec4(mouse.y, 0.5+abs(sin(time*3.0+centerDist*position.x*8.0-position.y*8.0))*0.26, centerDist*0.5, 1.0);
