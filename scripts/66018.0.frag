/*
 * Original shader from: https://www.shadertoy.com/view/wsVXRV
 */
...
float w_intersect_sphere( float max_t, vec3 ray_dir, vec3 origin, 
    ...
    inout vec3 pos, inout vec3 norm, inout int idx ) {
...
//        );
    ...
    for( int bounce =2; bounce >0 ; bounce -- ) {
        ...
        if( current_t <= maximum_dist ) {
            ...
            if ( ( int_mod( int( pos.x/5.0) + int(pos.z/5.0), 2) ) != 0){
