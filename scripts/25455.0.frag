...
void main()
{
	...
	for(float i=0.;i<150.;i++) {
		...
		vec4 leftRing = vec4(1.,1.0,0.,smoothstep(0.,blur,distL-radius));       // Red
		vec4 middleRing = vec4(.91,1,0.,smoothstep(0.,blur,distM-radius));
		vec4 bgCircles = vec4(.0,.0,1.,smoothstep(0.,blur/2.0,bg-radius*38.0));
