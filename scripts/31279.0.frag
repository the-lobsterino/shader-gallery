...
//Sorry for destroying the truth but there was no tree at reach to carve in......
//....and yeah SRUMP TUCKS
//yours PSYRECO
...
const float groundPosition = -0.9;
const vec3 boundingSize = vec3(50,20,12.8)*scale;
...
vec3 processColor(vec3 p)
{
	...
	if (ground<d) { col = vec3(diff+spec*0.3)*vec3(0.1,0.3,0.0); d = ground; }
	if (letters<d) { col = vec3(p.y*0.91+0.0,0.1,p.y+.3)+diff+spec; }
...
{	
	...
	p.x -= 0.25;
	float a = -4.;
	letters = min(letters,L(p.xy-vec2(a/2.46,1.1))+cos(time*5.1+3.0+p.z*2.)*0.); a++;
	letters = min(letters,E(p.xy-vec2(a/2.46,1.1))+cos(time*5.1+3.0+p.z*2.)*0.); a++;
	letters = min(letters,T(p.xy-vec2(a/2.46,1.1))+cos(time*5.1+3.0+p.z*2.)*0.); a++;
	letters = min(letters,S(p.xy-vec2(a/2.46,1.1))+cos(time*5.1+3.0+p.z*2.)*0.); a++;
	a = -5.;
	letters = min(letters,M(p.xy-vec2(a/2.5,0.4))+cos(time*5.1+3.0+p.z*2.)*0.); a++;
	letters = min(letters,A(p.xy-vec2(a/2.5,0.4))+cos(time*5.1+3.0+p.z*2.)*0.); a++;
	letters = min(letters,S(p.xy-vec2(a/2.5,0.4))+cos(time*5.1+3.0+p.z*2.)*0.); a++;
	letters = min(letters,T(p.xy-vec2(a/2.5,0.4))+cos(time*5.1+3.0+p.z*2.)*0.); a++;
	letters = min(letters,U(p.xy-vec2(a/2.5,0.4))+cos(time*5.1+3.0+p.z*2.)*0.); a++;
	letters = min(letters,R(p.xy-vec2(a/2.5,0.4))+cos(time*5.1+3.0+p.z*2.)*0.); a++;
	letters = min(letters,B(p.xy-vec2(a/2.5,0.4))+cos(time*5.1+3.0+p.z*2.)*0.); a++;
	letters = min(letters,A(p.xy-vec2(a/2.5,0.4))+cos(time*5.1+3.0+p.z*2.)*0.); a++;
	letters = min(letters,T(p.xy-vec2(a/2.5,0.4))+cos(time*5.1+3.0+p.z*2.)*0.); a++;
	letters = min(letters,E(p.xy-vec2(a/2.5,0.4))+cos(time*5.1+3.0+p.z*2.)*0.); a++;
	letters = max(bounding, letters+.016);
