...
float GetText(vec2 uv) {
	...
	d = I(uv,d);uv.y += 1.9;
	d = T(uv,d);
	return smoothstep(0.0,.001,d-0.75*CHS);
}
...
void main(void) {
    ...
    for (int i = 0; i < buildingCount; ++i) {
        ...
        vec2 charPos = (uv - signCenter + halfSize*vec2(.15,-.9))*vec2(1.,2.) / charSize;
		float char = 4.*(GetText(charPos));//1.5 + 4.0 * noise12(id + 6.0 * charPos);
