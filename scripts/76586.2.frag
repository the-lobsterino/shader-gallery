...
void main( void ) 
{
	...
	vec2 uv2 = uv;
	uv2 *= rotate(time*0.3+uv.x*0.1+length(uv)*0.2);
	
	float c1 = abs(sin(time*0.4+uv2.y*1.3+uv2.x*0.5));
	vec3 bg_color = vec3(c1*0.9,c1,c1*0.8)*(0.7+sin(uv.x*2.2-uv.y*8.3+time)*0.5);
	bg_color.r *= c1;
	bg_color.gb += c1*0.1+(sin(uv2.x*2.2+length(uv))*0.3);
	bg_color.g *= 0.77;
	
	uv.y += -1.+sin(uv.y*0.4+time*0.47+uv.x*.45)*0.5;
	uv += sin(uv.x*0.3+time*0.2+length(uv))*0.3;
	...
	float xx = uv.x;
		   ...
		   uv.x += sin(uv.y*0.1+time*1.4);
	           T H E _ 
			   uv.x = xx;
			   uv.x += sin(uv.x*0.3+uv.y*0.1+time*.4);
	
			 nl1
			I L L U M I N A T I 
			   nl2
			   uv.x += sin(time);
			 _  A R E _ C O M I N G
				 nl3
				 uv.x = xx;
				 _ _ _ T O D A Y
			 
		
			   float dd = smoothstep(-0.1,0.3,d);
	vec3 color = mix(ch_color, bg_color*0.45, dd);  // shading
