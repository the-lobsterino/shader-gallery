/*
 * Original shader from: https://www.shadertoy.com/view/dlc3Rs
 */
...
vec4 map(vec3 p) {
    ...
    U(m, sdBox(p, vec3(0.8, 1, 1)), SOL, 0.5, 1.);
    U(m, sdBox(p, vec3(0.0, 9.1, 9.1)), VOL, 0.3 + pos.z / 8., saturate(cos(TAU * (pos.z / 8. + time))));
...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
    ...
    for (int i = 0; i < 226; i++) {
        ...
        if (m.y == SOL) {
            if (d < 0.001) {
                col += vec3(1) * float(i) * .01;
                break;
            }
            t += 0.5 * d;
        } else {
            col += clamp(pal(m.z) * m.w * 0.09 / abs(d), 0.0, 0.3);
            t += 0.01 * abs(d) + 0.09;
        }
    }
