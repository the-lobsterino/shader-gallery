...
//Forked from http://glsl.heroku.com/e#5096.0
...
vec4 computeLightShadow(in Intersection isect)
{
	...
	lisect.hit = 0;
	lisect.t = 1.0e+30;
	lisect.n = lisect.p = vec3(0.0);
	lisect.col = vec4(vec3(0), 1.0);
...
{//sdkghalskdgh
	...
	for (int j = 0; j < raytraceDepth; j++)
	{
		...
		i.t = 1.0e+30;
		i.n = i.p = vec3(0.0);
		i.col = vec4(vec3(0.0), 1.0);
			
		Intersect(r, i);
		if (i.hit != 0)
		{
			col += bcol * i.col * computeLightShadow(i);
			if(col.a < 1.0)
			{
				Intersection transparency_i;
				transparency_i.hit = 0;
				transparency_i.t = 1.0e+30;
				transparency_i.n = vec3(0.0);
				transparency_i.p = vec3(0.0);
				transparency_i.col = vec4(vec3(0.0), 1.0);
				Ray rt;
				rt.org = i.p + eps * r.dir;
				rt.dir = r.dir;
				Intersect(rt, transparency_i);
				
				if (transparency_i.hit != 0)
				{
					col += bcol * transparency_i.col * computeLightShadow(transparency_i);
					/*if(col.a < 1.0)
					{
						Intersection transparency_i2;
						transparency_i2.hit = 0;
						transparency_i2.t = 1.0e+30;
						transparency_i2.n = vec3(0.0);
						transparency_i2.p = vec3(0.0);
						transparency_i2.col = vec4(vec3(0.0), 1.0);
						Ray rt2;
						rt2.org = transparency_i.p + eps * -transparency_i.n;
						rt2.dir = rt.dir;
						Intersect(rt2, transparency_i2);
				
						if (transparency_i2.hit != 0)
						{
							col *= bcol * transparency_i2.col * computeLightShadow(transparency_i2);
						}
					}*/
	...
	gl_FragColor = col + vec4(0.1, 0.1, 0.1, 1.0 - col.a);
/*	
#if 0	
	    Ray r;
	    r.org = org;
