...
void main( void ){
	...
	// Camera Setup.
	vec3 lookAt = vec3(0, 1, time*2. + 0.1);  // "Look At" position.
    ...
    vec3 sky = vec3(-1., .9, .8);//* mix(1., .75, mist);//*(rd.y*.25 + 1.);
	...
	gl_FragColor = vec4((clamp(sceneCol, 0., 1.)), -.01);
}