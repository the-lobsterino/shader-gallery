...
void main( void ) {
	...
	float flash = 0.1*cos(time);
	float uflash = 1.0*(sin(time)+1.001);//flash*0.5+0.5;
	...
	float d1 = rect(pos - vec2(-0.3,11.0), vec2(0.1,1), 0.1); 
	...
	float d4 = rect(pos - vec2(1.0*tan(time),0.0), vec2(1.0,0.1), 0.2);
