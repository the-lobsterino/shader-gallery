...
float unmodified_time = time*1e-2;
#define time ((time+length(surfacePosition))*1e-3)
...
void main( )
{
   float t = 30100.0+10050.0*sin(time*0.000021) + time * .0000053;
   float spin = 330.0+cos(t*2.0+unmodified_time*0.00092)*150.0;
   ...
   float dirt = (step(sin(unmodified_time*0.01),0.0)*2.0 - 1.0) * unmodified_time;
   ...
   mainImage( b, sp2.xy * log(10100.0+501.0*cos(t*1.3)), spin*abs(sin( t * 1000.2 + time*0.001 )) + spin*5.0*cos(t*10.02) );	
   ...
   gl_FragColor = clamp( mix( 1.-bak, mix( a, b, sin(st*1.5+3.0*cos(t*21.5))*0.5+0.5 ), 1.-0.75*sin(t+cos(0.3*t))+0.25), 0.0, 1.0 );
	gl_FragColor = vec4(length(gl_FragColor)/sqrt(4.));
