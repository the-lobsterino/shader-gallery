...
void main( void ) 
{
	...
	m +=metaball(uv,vec2(0.1 , 0.25) , 0.07 * (1.0 + 0.05 * sin(time * Freq * 0.5)));
	
	//uv.x  = uv.x + sin(time * 2.0) * 0.02;
	uv.x  = 0.5 + uv.x * 0.0003 + fract(t * Freq) / Freq;
	
	m += metaball(uv,center , 0.03);
	...
	gl_FragColor = vec4( vec3(1.3,0.4,0.4 ) * w , 1.0 );
}