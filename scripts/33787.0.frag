...
void main()
{
	...
	//	 a = abs(atan(uvs.y,uvs.x));
		float r = pow( pow(uvs.x*uvs.x,1.3) + pow(uvs.y*uvs.y,1.3), 1.0/6.0 );
...
//	uv = uvs;
	...
	vec4 newCol = (forCol2 + vec4(backCol2, 1.0)) * pow(r,1.5*(clamp((0.5 - 3.*cos(0.02*time)),0.,1.)));
