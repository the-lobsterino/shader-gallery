...
{
	return fract(vec2(sin(p.x * 23591.32 + p.y * 154.077 + time), cos(p.x * 391.32 + p.y * 49.077 + time)));
}
...
float noise1(float p)
{
	float fl = floor(p);
	float fc = fract(p);
	return mix(rand(fl), rand(fl + 22.0), fc);
}
