/*
 * Original shader from: https://www.shadertoy.com/view/XsB3W3
 */
...
vec2 ppnoise2v( float seed, float p )
{
    ...
    return vec2(mix(noise1f(x + 0.), noise1f(x + 1.),  nf),
                mix(noise1f(y + 0.), noise1f(y + 1.), nf));
}
...
vec2 ellington_df( vec3 pos ) 
{
    ...
    // squetch off screen
    float squash_stretch_exit = step(0., exit_t) * sin(1. * exit_t) * exp(-4. * exit_t);
