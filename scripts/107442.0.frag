...
vec2 Running( vec2 p)
{
	...
	if (d > 0.005)
	{
		...
		d2 = h.x - 0.04 + h.y*0.01;
		d = min(d, d2 );
		wrist = Rotate(elbow+vec2(.13, -.01), elbow, -arm1*0.5+.1);
		h = Segment(elbow,  wrist, p );
		d2 = h.x - 0.03 + h.y*0.01;
		d = min(d, d2 );
		vec2 hand = Rotate(wrist+vec2(.005, -0.0), wrist, -arm1*1.5-.3);
		...
		d2 = h.x - 0.03 + h.y*0.001;
		d = min(d, d2 );
		knee = Rotate(vec2(0.4,-.55), vec2(.4,-0.35), leg1-99.9);
		h = Segment(vec2(0.4,-.35), knee, p );
...
void main() 
{
	vec2 p = (-resolution.xy + 2.4 * gl_FragCoord.xy) / min(resolution.x, resolution.y);
	vec2 m = (mouse * 2.0 - 1.0) * vec2(resolution.x / resolution.y, 3.0);
	vec3 col;
	runCycle = time*4.0;
