/*
 * Original shader from: https://www.shadertoy.com/view/MtsXRX
 */
...
void mainVR( out vec4 fragColor, in vec2 fragCoord, in vec3 fragRayOri, in vec3 fragRayDir ) {
    ...
    if (iTime < transitionEnd) {
        
        
        float fwd = (encounterStart-iTime)*1.0;
        fwd = max(0.0,fwd);
        scrCoord.z += fwd;
        ...
        float straightUfoDist = (encounterStart - iTime)*4.0;
        ...
        if (iTime > encounterStart) {
            ...
            if (mat < 5) {
                ...
                float swirlOff = (0.5/stripeWidth)+(iTime-transitionStart)*10.0;
        ...
        float fadeOutAmt = min(1.0,(transitionEnd-iTime)*fadeSpd);
        ...
        if (f < nearestD) {
            ...
            if (mat == 4) {
                ...
                if ( (pX + 150*rownum) > int((iTime-(transitionEnd+2.0))*300.0) ) {
                    bit = false;
                }
			...
			float fadeInAmt = min(1.0,(iTime-transitionEnd)*fadeSpd);
