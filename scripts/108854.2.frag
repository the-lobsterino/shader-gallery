...
void main() {
	...
	// направляющий вектор
	vec2 s = vec2(cos(-time * PI / .5 * TimeK + PI / 7.0), sin(time * PI / .5 * TimeK + PI / 2.0));
	...
	s = vec2(cos(-time * PI / 1. * TimeK + PI / 2.0), sin(time * PI / 22. * TimeK + PI / 2.0));
	...
	// рисование секундной стрелки
	outColor += DrawClockLine(p, vec2(9.0), ClockRadius, s, 1.2, clockLineSec2Col);
	
	// изменяем направляющий вектор
	s = vec2(cos(-time * PI / 22. * TimeK + PI / 2.0), sin(time * PI / 8. * TimeK + PI / 2.0));
	// рисование минутной стреки
	outColor += DrawClockLine(p, vec2(0.0), ClockRadius, s, 9.2, clockLineMin2Col);
	
	// изменяем направляющий вектор
	s = vec2(cos(-time * PI / 16. * TimeK + PI / 2.0), sin(time * PI / 1. * TimeK + PI / 2.0));
	// рисование часовой стрелки
	outColor += DrawClockLine(p, vec2(22.0), ClockRadius, s, 9.2, clockLineHour2Col);
