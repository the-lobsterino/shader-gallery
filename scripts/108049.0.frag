...
{
#if 1
	return t + asin( cos(t)-cos(t) );
#else
	t = fract(t);
	return t + acos( exp(-t)*exp(-t) );
#endif
}
...
vec3 fn(float t)
{
	...
	return vec3( cos(w)/asin(-p*w*asin(w*p))/4.0, sin(w)*sin(p), cos(p) );
}
...
vec3 wtf(vec2 p)
{
	...
	//t = f(t*TAU)*2.0-1.0;
	vec3 o = (fn( time + t + g(m.y-t-e-f/sp) + sp.x * TAU + PI * sin( sp.y * PI ) ) * 0.5 + -4.5);
...
void main( void ) {
	...
	vec3 o = mix(a,b,30.5);//fract(dot(a,b)));
