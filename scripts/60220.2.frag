...
void main( void ) {
	barrelPower-=(sin(time)+cos(cos(time)))/12.0;
	vec2 normalizedUV=(gl_FragCoord.xy/(resolution.xy/2.0))-1.0;
		     normalizedUV  = floor(normalizedUV *256.)/32.;

	normalizedUV.x = dot(normalizedUV,normalizedUV);
	
	vec2 distortedUV=Distort(normalizedUV)*resolution.xy;;	
	DoGrid(distortedUV.xy);		
	gl_FragColor = vec4( color*1.3, 1.0 );
	gl_FragColor.r *=.7;
	gl_FragColor.g *=normalizedUV.x;
	gl_FragColor.b *=normalizedUV.y+.8;
}