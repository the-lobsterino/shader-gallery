...
void main( void ) {
	...
	vec2 bpos = read_vec2(bposPtr)/100.0;
	vec2 lbpos = bpos;
		
	bpos += (mpos-bpos)/16.0;
	
	float col = texture2D(last,pos/resolution).r;
	
	col = max(col,line(pos,lbpos,bpos,1.5));
	
	gl_FragColor = vec4( vec3( col ), 1.0 );
	
	write_vec2(bposPtr,bpos*100.0);
}
...
{	
	...
	for(int i = 0;i < NUM_NYBLES;i++)
	{
		...
		float bit = float(i)*4.0;
		
		acc += nyble.x * pow(2.0,bit+0.0);
		acc += nyble.y * pow(2.0,bit+1.0);
		acc += nyble.z * pow(2.0,bit+2.0);
		acc += nyble.w * pow(2.0,bit+3.0);	
	}
