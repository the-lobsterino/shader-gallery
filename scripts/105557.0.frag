...
vec3 calculateGoldenShape(vec2 p){
	...
	for (int i = steps; i > 0; --i)
	{
		...
		float inc = n * rSteps;
		vec2 offset = rotateAroundPoint(fract(time*1.055)*6.28+n * goldenAngle*cos(time*0.0025)) * inc * 0.45;
		
		vec3 dist = vec3(distance(p, offset));
		     dist = exp2(-dist * 334.0) * hsv2rgb(vec3(fract(time*0.2)+inc*0.75, 1.0, 1.0));
