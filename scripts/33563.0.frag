...
#define BROKEN c += vec3(tan(p * 10.0 + sin(p)), tan(p * 10.0 + cos(p)))*Sprite3x5(time * time + time * time * time - time,floor(p-cpos));cpos.x += 4.;if(cpos.x > 61.) slashN
...
void main( void ) {
	goTYPE ;
	cpos.y -= 3.;
	{inCYA}_ _ H O M O S _ G O N N A
	slashN ;
	{inGRN}_ _ _ _ _ 
	BROKEN H A T E 
	BROKEN
