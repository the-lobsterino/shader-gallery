...
void main( void ) {
	...
	float st = 0.1 * sin(time);
	finalColor += color * circlify(p, vec2(0.,0.), RADIUS+0.1+st);
	finalColor += color1 * circlify(p, vec2(0.5,0.2), RADIUS+st);
	finalColor += color2 * circlify(p, vec2(-0.6,-0.2), RADIUS+0.2+st);
