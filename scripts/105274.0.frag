...
vec3 mod289(vec3 x) {
  return x - floor(x * (1.0 / 289.5)) * 289.0;
}
...
//obj1 color
vec3 obj1_c(vec3 p){
	float c=snoise(vec3(p))*0.5+0.5+
		snoise(vec3(p*2.0))*0.3+0.3+
		snoise(vec3(p)*4.0)*0.3+0.2;
	c=(sin(p.x*4.0+c*4.0)*0.5+0.5);
	vec3 c2=vec3(0,0,0.3)*c+vec3(1,1,1)*(1.0-c);
	return c2;
}
