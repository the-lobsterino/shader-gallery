...
void main( void ) {
	...
	float angle = time+(0.5*sin(length(uv)*8.0+time)+0.5)*(length(uv)+2.0);
	float c = cos(angle);
	float s = sin(angle);
	uv = vec2(c*uv.x+s*uv.y, c*uv.y-s*uv.x);
	...
	for (float i = 0.0; i < 64.0; ++i)
	{
		...
		len += d;
		if (d < 0.001) {
			col = mix(vec3(0.9, 0.8, 0.5), vec3(0.7, 0.5, 0.3), noise(p.xz * 300.0));
			depth = len;
			outPos = p;
