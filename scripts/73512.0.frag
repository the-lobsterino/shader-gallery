...
float map(vec3 position)
{
	...
	origin.xy 	*= rmat(cos(mouse.x * TAU + TAU * .5));
	origin.xz 	*= rmat(sin(mouse.y * TAU));
	...
	range 		= min(min(min(rtp(k-origin, radius), rtp(origin.zyx, radius)), rtp(k-origin.xzy, radius)), range);
...
void main( void ) 
{
	...
	if(range < .01)
	{
		...
		vec3 light_position 	= vec3(32., 32., -218.);
		vec3 light_direction	= normalize(light_position - position);
		
		float light		= max(dot(surface_direction, -light_direction), 0.);
		float bounce		= max(dot(surface_direction, light_direction), -1.);
		
		
		color 			+= material_color + material_color * light + pow(bounce, 5.) * .25;
	...
	color 				= 1.-pow(color.xxx * .6, vec3(1., 1., 1.) * 2.);
