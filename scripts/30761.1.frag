...
{
	t=mod(t,13.0);
	if     (t< 1.0) return vec4( 8.0,  4.0,  4.0, 16.0);
	else if(t< 2.0) return vec4(12.0,  8.0, 32.0,  4.0);
	else if(t< 3.0) return vec4( 5.0,  1.8,  6.0,  7.0);
  	else if(t< 4.0) return vec4( 4.0,  0.8,  1.0,  1.0);
 	else if(t< 5.0) return vec4( 8.0,  0.6,  1.0,  8.0);
	else if(t< 6.0) return vec4( 2.0,  2.0,  2.0,  2.0);
	else if(t< 7.0) return vec4( 5.0,  1.0,  1.0,  1.0);
	else if(t< 8.0) return vec4( 3.0,  4.5, 10.0, 10.0);
	else if(t< 9.0) return vec4( 4.0,  4.0,  8.0,  8.0);
	else if(t<10.0) return vec4( 9.0,  0.5,  1.5,  1.5);
	else if(t<11.0) return vec4( 3.0,  14.,  25.,  25.);
	else if(t<12.0) return vec4( 3.0,  14.,  25.,  25.);
	else            return vec4( 1.0,   8.,  22.,  22.);
}
...
void main()
{
	...
	float tim = time*0.2;	
//	tim = 10.0;	   // for direct select shape
