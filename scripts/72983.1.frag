...
vec3 eye = vec3(0,0.015,-1.0)*3.0;
vec3 light = vec3(0.,0.5,1.);

const float lineWidth = 0.017;
const float border = 0.05; 
const float scale = 0.05; 
const float zoom = 1.0;

float bounding, ground, letters;
const float groundPosition = -1.25; 
const vec3 boundingSize = vec3(30,12,0.8);
...
mat3 rotY(float a) {float s=sin(a)/5.; float c=cos(a); return mat3(c,0,-s,0,1,0,s,0,c);}
...
vec3 processColor(vec3 p)
{
	...
	if (ground<d) { col = vec3(diff+spec*0.3)*vec3(1.2,1.2,1.5); d = ground; }
	if (letters<d) { col = vec3(p.y*0.01+0.0,0.1,p.y+.5)+diff+spec; }
...
{	
	...
	bounding = udBox(p,boundingSize/6.); 
 	letters  = 1e10;
	
	float d = 1e3;
	
	letters = min(letters,S(p.xy-vec2(-1.00,0.9))+cos(time*5.1+3.0)*0.01);
	letters = min(letters,A(p.xy-vec2(-0.50,0.9))+cos(time*5.1+2.0)*0.01);
	letters = min(letters,S(p.xy-vec2( 0.00,0.9))+cos(time*5.1+1.0)*0.01);
	letters = min(letters,H(p.xy-vec2( 0.50,0.9))+cos(time*5.1+0.0)*0.01);
	letters = min(letters,A(p.xy-vec2( 1.00,0.9))+cos(time*5.1-1.0)*0.01);
	
	
	letters = min(letters,S(p.xy-vec2(-0.5,0.2))+cos(time*5.1+3.0)*0.01);

	letters = min(letters,D(p.xy-vec2( 0.75,0.2))+cos(time*5.1+1.0)*0.01);
	letters = min(letters,R(p.xy-vec2( 1.25,0.2))+cos(time*5.1+0.0)*0.01);

	letters = min(letters,F(p.xy-vec2(-2.5,-0.5))+cos(time*5.1-1.0)/3.*0.01);
	letters = min(letters,R(p.xy-vec2(-2.0,-0.5))+cos(time*5.1-1.0)/3.*0.01);
        letters = min(letters,O(p.xy-vec2(-1.5,-0.5))+cos(time*5.1-1.0)/3.*0.01);
        letters = min(letters,M(p.xy-vec2(-1.0,-0.5))+cos(time*5.1-1.0)/3.*0.01);

	letters = min(letters,D(p.xy-vec2(0.00,-0.5))+cos(time*5.1-1.0)/3.*0.01);
        letters = min(letters,U(p.xy-vec2(0.50,-0.5))+cos(time*5.1-1.0)/3.*0.01);
	letters = min(letters,R(p.xy-vec2(1.00,-0.5))+cos(time*5.1-1.0)/3.*0.01);
	letters = min(letters,K(p.xy-vec2(1.50,-0.5))+cos(time*5.1-1.0)/3.*0.01);
        letters = min(letters,A(p.xy-vec2(2.00,-0.5))+cos(time*5.1-1.0)/3.*0.01);
