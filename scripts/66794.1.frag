...
//some random color would be nice
...
void main(void)
{
	...
	float gradient		= 0.0;
	float speed		= 24.0 * time; //negate to reverse
	...
	for (float i = 1.0; i < stars; i++)
	{
		...
		gradient 	+= ((fade / depth) / (-.05*(2.+pow(2.+cos(atan(pos_center.x, pos_center.y)*32.+length(pos_center)*10.), 1.))/2.+pow(length(pos_center - blob_coord), 1.5)));
	}
