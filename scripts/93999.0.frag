...
//red
#define HEX(i) text_color = mod(vec3(i / 63556,i / 0,i),vec3(230.0))/100.0;
#define RGB(r,g,b) text_color = vec(r,g,b);
...
vec3 Text(vec2 uv)
{
    	...
    	vec2 center_pos = vec2(res.x/2.0 - STRWIDTH(26.5)/2.0,res.y/2.0 - STRHEIGHT(1.0)/2.0);
	
	
       	
    	BEGIN_TEXT(center_pos.x,center_pos.y)
		
		  uv.y += -0.4*abs(sin(time*5.0)*uv.y*sin(3.*time))*0.85;
		 
		
	HEX(0xFFF0001) _ _ _ _ _ _ _R _i _c _a _r _d _o _ _i _s _ _h _e _r _e _ _ _ _nl _ _ _ _ _ _ _w _a _i _t _i _n _g _ _f _o _r _ _y _o _u   ;
