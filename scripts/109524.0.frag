...
*/
     vec3 q = abs(p.xyz);
 	vec2 b = vec2(max(q.z-.1,max(q.x+q.y*0.535,q.y)-.05));
...
void main(void)
{
  ...
  vec3 prp=vec3(sin(time*0.15)*2.0,sin(time*11.5)*2.0,cos(time*0.1)*8.0); //Change camera path position here
  ...
  vec3 c,p,n;
	...
	if (f<64.)
	{
		...
		gl_FragColor=vec4((b*c+pow(b,3.))*(1.0-f*.0),1.);//simple phong LightPosition=CameraPosition
	}
