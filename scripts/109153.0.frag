...
void main( void ) {
	...
	if (length(position-mouse) < 0.01) {
		float rnd1 = mod(fract(sin(dot(position + time * 11.001, vec2(1.9898,78.233))) * 43758.5453), 1.0);
		if (rnd1 > 0.5) {
			gl_FragColor = live;
		} else {
			gl_FragColor = blue;
		...
		sum += texture2D(backbuffer, position + pixel * vec2(io., 1.)).g;
		...
		if (me.g <= 0.1) {
			if ((sum >= 2.9) && (sum <= 3.1)) {
				...
				gl_FragColor = vec4(0., 0., max(me.b - 0.004, 0.25), oo.);
			} else {
				gl_FragColor = dead;
			}
		} else {
