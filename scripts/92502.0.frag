...
#define spiralFactor     (1./2.)         // makes it a spiral if not 0.
...
vec3 hsv2rgb(vec3 c)
{
    ...
    vec3 p = abs(fract(c.xxx + K.xyz) * 60.0 - K.www);
    return c.z * mix(K.xxx, clamp(p - K.xxx, 0.0, 1.0), c.y);
}

float tw(float a) { return nsin( a*time*tau ); }     // helper "timewave" function...
...
#define border   0.002      // 0.0 for no border...
{
	...
	o *= cosPalette( (floor(tiles*p1.x + 0.50*tiles*time))/tiles, PAL1 );
        p1 += rectPos;
...
{
	// domain
	vec2 p = 20./40.*surfacePosition;
	p *= rot( tau/40. );    // just rotating one quarter...
	...
	float lp = length(p);
	p *= rot( spiralFactor*lp*32.*nsin(5.*time) );
	...
	// adding "sun"
	c = step( length ( p ), (10./81920.) )*colorSun;
