...
float Cir (vec2 uv, float r, bool blur) {
    float a = blur ? 0.025 : 0.;
    float b = blur ? 0.075 : 1.5/R.y;
...
void mainImage( out vec4 fragColor, in vec2 fragCoord){
    ...
    vec3 Col0 = vec3(.9);
    vec3 Col1 = vec3(.21+uv.y*2., .4+uv.x*-1.1, .8)*.828;
    vec3 Col2 = vec3(.86);
    
    float cir1 = Cir(uv - m, .15, false);
    float cir2 = Cir(uv, .2, false);
    float cir2B = Cir(uv, .15, true);
