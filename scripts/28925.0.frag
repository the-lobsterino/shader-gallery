...
float noise( in vec3 x )	// noise from Inigo quilez.
{
    vec3 p = floor(x);
    vec3 f = fract(x);
    f = f*f*(3.0-2.0*f);
...
{
    float d = dot(uv, vec2(12.9898, 78.233));
	return fract(sin(mod(d, PI / 2.0)) * 43758.5453);
}
////////////////////
...
void main( void ) {
	...
	vec2 mo = (mouse.xy-vec2(0.5,0.))*vec2(10.,1.);
	...
	if(dd.x<20.){
		...
		if(dd.y == 1. ){col = vec3(1.,1.,.9)*max(sh,ssh);}
		if(dd.y == 1.1){col = vec3(.9,1.,1.)*max(sh,ssh);}
		if(dd.y == 1.2){col = vec3(1.,.9,1.)*max(sh,ssh);}
		if(dd.y == 1.3){col = vec3(1.,1.,1.)*max(sh,ssh);}
		if(dd.y == 1.4){col = vec3(0.9,.9,1.)*max(sh,ssh);}
		if(dd.y == 1.5){col = vec3(0.9,1.+(sin(time)+1.)*0.8,0.9)*max(sh,ssh);}
		if(dd.y == 2.){col = vec3(0.6,0.6,0.6)*ssh;}
		//if(dd.y == 3.){col = vec3(0.,1.,1.)*sh;}
		if(dd.y == 4.){col = vec3(.7,0.6,.7)*ssh/1.2;}
		if(dd.y == 5.){col = vec3(1.,1.,1.)*max(ssh,sh);}
		
		if(dd.y == 4.1){col = vec3(1.,.5,0.);}
		if(dd.y == 4.2){col = vec3(.5,.5,0.);}
