...
{
    vec3 q = abs(p);
    return max(q.z-h.y,max(q.x+q.y*0.57735,q.y*1.47)-h.x);
}
...
vec2 map( vec3 p ){
	...
	vec3 p1 = p;
	p1.x += 0.11;
	p1 = rotXaxis(p1,time - 3.15/2. + p.x * 3. * cos(time));
...
void main( void )
{
    	...
    	vec3 camUp  = normalize(vec3(0.0, 1.0, 11.0));
	...
	float light = b*c+pow(b,118.)*3.;

	vec3 col = vec3(1.0*r.y,0.8-0.2*r.y,.3);
