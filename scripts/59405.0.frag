...
float noise(vec2 p) {
	p=(p);
	return fract(time*0.0001*sin(p.x*45.11+p.y*97.23)*878.73+733.17)*2.0-1.0;
}
...
void main( void ) {
	...
	for(int i = 0; i < 50; i ++)
	{
		...
		float dist = mouse.x > 0.5 ? abs(dotPos.x-position.x) + abs(dotPos.y-position.y) : distance(dotPos, position);
		if(closestDist > dist)
		{
			closestDist = dist;	
