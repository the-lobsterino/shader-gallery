/*
 * Original shader from: https://www.shadertoy.com/view/MdcXzn
 */
...
AnimationChannels g_animationChannels = AnimationChannels(0., 0., 0., 0., 0., vec3(0.), vec3(0.));
...
float RAYMARCH_DFAO( vec3 o, vec3 N, float isoSurfaceValue)
{
    ...
    //25percent to allow those cracks to get darker.
...
vec3 PBR_ABL_Equation(vec3 V, vec3 L, vec3 N, float roughness, float metallic, vec3 ior_n, vec3 ior_k)
{
	...
	//It is not necessarily 100percent mathematically/physically correct : this is still just a function which has a curve that decently
