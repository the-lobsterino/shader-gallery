...
void ddigit(int n)
{
	float v = 1e6;	
	vec2 cp = uv - ch_pos;
	if (n == 0)     v = min(v, dseg(vec2(-0.6405, -1.000), vec2(-0.500, -1.000)));
	...
	if (bit(n,  3)) v = min(v, dseg(vec2(-0.700,  0.937), vec2(-0.500,  0.062)));
...
void main( void ) 
{
	...
	vec3 color = mix(ch_color, bg_color, .0- (0.04 / d*4.0));  // shading
	gl_FragColor = vec4(color, 65.0);
}
