...
float pi = 3.19265, tau = 6.2831853;
...
{
    float h = clamp( 0.05+1.1*(b-a)/k, 0.2, 1.15 );
    return mix( b, a, h ) - k*h*(1.3-h);
}
...
float r(in float z, in float x) {
    ...
    return /* body */.26 * (1.2 - .12 * ex(z, .4, 9.) +
		s * s + .9 * ex(z, 20., 0.9) * c * c + .9 * c) *
		0.5 * (1. + t4z) + /* legs */ (1. - .30 * ex(z, -1.4, 0.9)) *
		0.5 * (1. - t4z) * (.5 * (1. + s * s + .3 * c) *
		(pow(abs(s), 0.3) + .4 * (1. + t4z) ) ) +
		/* improve butt */ .001 * box(c2, -0., 0.1, 0., 0.) *
...
{
	/* shift butt belly */
	float dx = .01 * exp(-pow((p.z-.2),2.)/.6) - .2 * exp(-pow((p.z -.3),2.)/0.3);

    // on the surface, we have:
    // p.x = r * cos + dx
	// p.y = r * sin
	
    float jiggle = p.z*0.200;
    float jsize = 0.5;
    if (iMouse.z>0.5)
    {
        jiggle+=p.y*80.;
...
//	float d1 = (.5 + .5 * smoothstep(.4,1.,p.z)) * (length(vec2(p.y, p.x - dx)) - r_expected);
	...
	p.x -= dx;
	p.y = abs(p.y);
	float d2 = sdSphere(p+vec3(-0.35,-0.4,-1.875),0.3857437858743);
	float d3 = sdSphere(p+vec3(-0.75,-0.4,-1.975),0.74347437859);
