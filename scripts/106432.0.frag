...
void main( )
{
    ...
    uvv = vec2(1,-1)*uvv.yx;
    vec2 uv2 = uv + uvv*sin(0.9*time);// twisting    
    ...
    float lowClouds = fbm(5.*uv + 0.*time, 8);
    float midClouds = fbm(3.*uv + vec2(0.26,-0.03)*time, 8);
    float hiClouds = fbm(uv2 + vec2(0.1,0.01)*time, 8) - 0.5;
    float hiClouds2 = fbm(uv + 10. + vec2(0.062,-1.03)*time, 8) - 0.5;
    col = mix(col, cloudCol3, 0.5*smoothstep(0.,1., -uv.y+3.*hiClouds));
    col = mix(col, cloudCol3, 0.3*smoothstep(0.,1., -uv.y+3.*hiClouds2));
    ...
    col = (col*(2.51*col+0.03))/(col*(2.43*col+0.59)+.54); // tonemapping
