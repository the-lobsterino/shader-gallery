...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
    ...
    //vec3 menger = light+vec3(sin(iTime+0.8),cos(iTime*10.4),sin(iTime/3.14));
    
   // vec4 l;// = mengersponge(ro, rd, 1.0, FAR);
	
	vec4 l=vec4(ro.z,-sign(rd));
   ...
   // int id = ID_MENGER;
    ...
    lightn /= lightd;
        ...
        fragColor = vec4(l.yzwx);//+cos(l.x),l.x);//;//l.yzw*0.0+sin(p)*0.5+0.5,1.0);
