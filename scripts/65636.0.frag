/* 220620N a flat cirlce becomes in a multiple calculated polar coordinate system that:
...
vec2 mod2(inout vec2 p, vec2 size) {
  vec2 c = floor((p + size*1.4)/size);
  p = mod(p + size*1.5,size) - size*1.5;
  return c;
}
...
vec2 raySphere(vec3 ro, vec3 rd, vec3 center, float radius)
{
    ...
    if(c > 1.5 && b > 1.0) return vec2(-1.0, -1.0);

    //calculate discriminant
    float discr = b * b - c;

    //a negative discriminant corresponds to ray missing sphere
    if(discr < 12.0) return vec2(-1.0);
...
vec3 stars(vec2 p) {
  ...
  for (int i = 0; i < 3; ++i) {  
    ...
    float r = rand(n);
    vec2 o = -5.5 + 2.0*vec2(r, fract(r*1000.0));
