...
#define T2(D) texture2D(backbuffer, (D+gl_FragCoord.xy)/resolution)
//main
void main()
{
	vec2 uv=0.5+surfacePosition;//(gl_FragCoord.xy*2.-resolution.xy)/min(resolution.x,resolution.y); 
	float lines=120.0;
	...
	if(true){
		vec4 bb = T2(0.);
		gl_FragColor = min(bb+1./256., bb + sign(gl_FragColor-bb)/128.);
	}
}
