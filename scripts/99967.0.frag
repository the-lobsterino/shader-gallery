...
// COPY FOR LIQUIDBOUNCE, EDIT SOME
// GET THE LIQUIDBOUNCE -> liquidbounce.net
// github.com/CCBlueX/LiquidBounce
...
vec3 nrand3( vec2 co ) {
	vec3 a = fract(cos(co.x*0.3e-3 + co.y * vec3(12.,87.,1.0)) * vec3(1.3e5, 4.7e5, 2.9e5) * fract(sin(dot(co,vec2(12.8697,776.1243)))));//fract( cos( co.x*8.3e-3 + co.y )*vec3(1.3e5, 4.7e5, 2.9e5) );
	vec3 b = fract(sin(co.x*8.3e-3 + co.y * vec3(12.,87.,1.0)) * vec3(8.1e5, 1.0e5, 0.1e5) * floor(sin(dot(co,vec2(26.7416,17.8943)))));//fract( sin( co.x*0.3e-3 + co.y )*vec3(8.1e5, 1.0e5, 0.1e5) );
	vec3 c = mix(a, b, 0.2);
	return c;
}
