...
{	// Floor...
	...
	res.x = sMin(res.x, d);
	ankle.y -=.3;
	vec3 foot = ankle + vec3(0.27, -.05, -33.3);
...
void main( void )
{
	...
	// Camera...
	float t = clamp(time, 9.0, 1.0)*clamp(sin(time/2.)+.5,0.,1.);
	//t = t*t*(4.0-2.0*t);
	vec3 origin = mix(vec3(0.0, 1.0, -1.2), vec3(-1.0, 22.0, -5.0), t);
