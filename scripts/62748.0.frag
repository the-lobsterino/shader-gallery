...
const float SRadius=.1;//Thikness of the lines

//Colors
const vec3 segColor=vec3(4.,0.5,0.9);
const vec3 backGroundColor=vec3(.5,1.6,1.6);
...
void init() {
	float v = float(qParam*rParam+pParam*rParam+pParam*qParam-pParam*qParam*rParam);
	spaceType=float(sign(v));//1./pParam+1./qParam+1./rParam-1.;
