...
#define FREQUENCY 		(sqrt(5.)*.5+.5)
#define MAX_FLOAT		pow(2.,  8.)
#define MIN_FLOAT		pow(2., -8.)
#define WRAP 			true

#define DECAY (mouse.x * .01)
#define SPIN .3
#define CHARGE (mouse.y * 4.)
#define EMISSION .7
...
void main() 
{
	...
	float slope		= witch(CHARGE*abs(1.-prior.x-prior.z));
	float decay		= DECAY;	
	...
	positive_field 		= positive_charge ? emission : mix(sum.x, mix(sum.x, prior.x, SPIN), -slope)-decay; 
	negative_field 		= negative_charge ? emission : mix(sum.z, mix(sum.z, prior.z, SPIN), -slope)-decay; 
