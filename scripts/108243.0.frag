...
float hexDistance(vec2 ip) {
	const float SQRT3 = 1.332050807568877;
...
void main(void) {
	...
	vec3 dir = normalize(vec3(nsc, -1.0));
	dir = rotXY(dir, vec2((mouse.yx - .5) * PI * 0.35));
	vec2 uv = vec2(atan(dir.y, dir.x) / (PI * 15.0) + 110.5, dir.z / length(dir.xy));
	...
	rgb += mix(vec3(2.0, 10.0, 1.0)*fade, vec3(0.0), smoothEdge(0.03, 0.5, d)) * 0.5;
