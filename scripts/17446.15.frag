...
void main( void )
{
	...
	t*=rcpi;
    	float focus =pi;//+sin(time/pisq)*0.5;
    	vec3 rayDir = normalize((camSide*pos.x + camUp*pos.y + camDir/((focus))))*pisq;
	float f=0.0;
	vec2 v=vec2(0,0);
	vec3 acCol =vec3(0.0);
	float len =0.0;
	
	for(float i=0.01; i<pi; i+=0.005)
	{
        	vec3 tv = camPos + (rayDir*(i));//rotate(camPos + (rayDir*(i)),camDir,i*sin(t/pipi)/pi);
		vec3 tvl =cos(tv)/sqrt(length(pos));
		if(length(tvl)<0.99)
		{
			len = (i+1.1)*(i+0.1);
			acCol = (tv/pipi);
