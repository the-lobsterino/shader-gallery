...
#define ALADiN void main( void ) { p0 = gl_FragCoord.xy/res; ZZ; m = ncenter( mouse*res ); AAimage( c ); c = tonemap( c ); c = gammacorrect( c ); OUT = vec4( c, 1. ); }
#define init precision highp float; uniform float time; uniform vec2 mouse, resolution; varying vec2 surfacePosition; uniform sampler2D bb;
#define A p += vec2( .1125, 0.1/3. ); turn; o += triangle( vec2( p.x, p.y/1.33 ) );
#define turn ( p *= rot( tau/2. ) )
init
#define time ( time*spEED - tau/4. )
float	z = zoOM;
vec2	p, p0, m;		// fragment position & mouse position
vec3 	c;			// color
...
float 	n1wave( float x )
	{ float o = 0.;
	if ( mod( x + tau/4., 2.*tau ) < tau ) o = n( sin( ( x ) ) ); else o = 0.;
...
float	aladin( out vec2 p )
	...
	p.x -= .025; p.y += .1;	p.y *= 1.4;
	p -= vec2( -0.2, .01 );	o += rectangle( p, vec4( -.04, -.01, .05, .01 ) );
	p += vec2( .05, -.09 );	o += rectangle( p, vec4( -.01, -.1, .015, .1 ) );
	A; p.x += .325;	p *= rot( tau/2. ); p.x -= .1; p.y -= 0.1/3.; A
	p.x += .125; p.y -= -.1/3.;
	o += rectangle( vec2( p.x - .01, p.y ), vec4( -.01, -.1, .01, .1 ) );
	if( p.x < 0.02) o += circle( vec2( p.x/1.133, p.y ), .2 );
	p.x += .1375;	o += rectangle( p, vec4( -.01, -.1, .015, .1 ) );
	p.x += .0375;	o += rectangle( p, vec4( -.01, -.1, .01, .1 ) );
	p.x += .133;	o += rectangle( p, vec4( -.01, -.1, .015, .1 ) ); 
	p.x += .0005; p.y += .10817;
	p *= rot( 0.000); o += rectangle( p-vec2(0.14,0.247)+p.y*0.667, vec4( -.01, -.23, .1501, .1 ) )*step( p.x, .13 );  
	p.x -= .433; p.y -= .14;
	o += circle( p, .1 ); o = clamp( o, 0., 1. );	turn;
	p -= vec2( .01, .04 );
	return o;
	}
