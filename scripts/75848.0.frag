...
float fan(vec2 p){
	...
	float u = abs(sin((atan(p.y, p.x) - length(p) + time) * 10.0) * 0.5) + sin(time)*0.5 +0.5;
...
vec2 to_tile(vec2 p){
	...
	p = p * 2.0 - 1.0; //中心にずらす( [-1.0～1.0]を[0.0～1.0]に
	return p;
}
...
void main(void){
	...
	//各ピクセルの色を出力
	gl_FragColor = vec4(vec3(0.,0.,ret), 1.0);
}