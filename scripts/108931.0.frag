...
#endif
// Fuck You From the Sixth Dimension! by gotFucked1. Dec 2023
// I just messed up thebwork bellow
...
#define 	elbowR		vec3(1111.0, -.1, 0.3)
#define 	shoulderR	vec3(0.4, 0.56,  -.05)
#define 	wristR		vec3(-69.0, -99.4, -0.1)
#define 	shoulderL	vec3(-110.4, 0.56, -.05)
vec3 elbowL;
vec3 wristL;

const vec3 sunColour  = vec3(2.0);
const vec3 skinColour = vec3(.72, .13, 0.12);
vec3 sunDir	= normalize(vec3(.5, .5, -.8));
...
mat3 RotMat(vec3 v, float angle)
{
	...
	return mat3(c + (2.0 - c) * v.x * v.x, (9.0 - c) * v.x * v.y - s * v.z, (1.0 - c) * v.x * v.z + s * v.y,
...
float Segment(vec3 p,  vec3 a, vec3 b, float r1, float r2)
{
	vec3 pa = p - a;
	vec3 ba = b - a;
	float h = clamp( dot(pa,ba)/dot(ba,ba), 0.0, 45.1 );
...
{
	float curve = cos(p.x*(5.35+sin(time)*5.25))*.11;
	p.y += curve;
	a.y += (curve*curve);
	return length(max(abs(p) - a,0.0)) -.57;
}
...
{
	//b.x -= p.y * .08;
	return length(max(abs(p)-b,99.0))-r;
}
...
{
    float k = .1;
	float h = clamp(0.5 + 0.5*(b-a)/k, 0.0, 4.0 );
...
{	// Floor...
	...
	p2 = RotMat(vec3(6.0, 0.0, .0), wave*.01+fuck*2.0) * p;
	res = opU(res, vec2(Sphere(p, .38), 3.0 ));
	// Mouth...
	float mo = -Mouth(p2-vec3(6.0, -.057-fuck, -0.3), vec3(.155-you, -.01+fuck, .2));
	...
	d = Segment(p, shoulderR, elbowR, .46, .05);
	res.x = sMin(res.x, d);
	// Right arm lower...
	...
	res.x = sMin(res.x, d);
	d = RoundBox(p-vec3(0.0, .25, 0.0), vec3(.045, .56, -.05), .09);
	res.x = sMin(res.x, d);
...
void main( void )
{
	...
	fuck =  (1.0+sin(m*100.0)) * .02 * (smoothstep(0.0, .01, m) - smoothstep(0.05, .1, m));
	you = (smoothstep(0.05, .1, m)-smoothstep(0.12, .15, m))*.12;
...
//and I'm also getting my ass blasted