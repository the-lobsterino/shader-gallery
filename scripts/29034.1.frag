...
#define SKY_COLOR    vec3(1.00, 1.00, 1.00)
#define HAZE_COLOR   vec3(0.00, 0.5, 0.90)
#define WALL_COLOR   vec3(0.00, 0.5, 0.90)
#define STRIPE_COLOR vec3(2.00, 2.0, 2.00)
...
vec2 sdMaze(vec3 p, float id)
{
    ...
    return vec2(max((d / MAZE_SCALE) - WALL_WIDTH / 2.0, -p.z - (WALL_HEIGHT + 2. * sin(p.x*p.y))), id);
}
