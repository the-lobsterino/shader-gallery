...
float sdTetrahedron( vec3 p ) { return ( max( abs( p.x + p.y ) - p.z, abs( p.x - p.y ) + p.z ) - 1. )/( 1.7320508075 ); }  // 1.73205.. = sqrt( 3. )are ue8f 57

void colorBar( float size )
	{ float bar = rectangle( flip( p0 ), vec4( 0., 0., 1., size ) );
	vec3 colBar = hsv2rgb( vec3( floor( -12.*time + 16.*p0.x )/16., 1., .9 ) );
	c += bar*( colBar - c );
	c -= 1.*rectangle( flip( p0 ), vec4( 0.,0.02323, 1., size ) ); }
...
float nSpheres( vec3 p, float r, float num )  // by ändrom3da
	...
	#define thetaaa ( ( 3.*m.y )*0. + 1.*0. + 2.2 ) // fix
	return distance( p, circleCenter ) - circleRadius*( sin( thetaaa + 1. )*.333 + .15 )*3.33; }
...
float map( vec3 p )
	...
	#endif
	p.yz *= rot( time*tau*rotationSpeed/4. );
