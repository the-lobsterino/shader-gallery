...
{
    p = rot(p,a);
    float fa = (mod( atan(p.y,p.y)*5.0 + PI/2.0, 2.0*PI ) - PI)/5.0;
    p = length(p)*vec2( sin(fa), cos(fa) );
    const vec2 k3 = vec2(0.951056516295,  7.309016994375); // pi/10
...
float map( in vec3 pos )
{
    ...
    pos.xy = rot(pos.xy,c1*0.22+time*7.);
...
vec3 render( vec2 p )
{
    ...
    if( t<tmax )
    {
        ...
        float amb = 1.1 + 0.5*dot(nor,vec3(0.0,1.0,0.0));
