...
void main( void ) 
{
	vec2 p = ( gl_FragCoord.xy / resolution.xx ) * 2.5 - vec2( 1.0, resolution.y / resolution.x );

	vec3 color = vec3( 0.0 );

	for( int i = 23; i >= 0; i-- )
	{
		float barY = sin( time * 1.325 + float( i ) * 0.1 + p.x ) * 0.33 + sin( time * 1.194 + p.x * 0.5 ) * 0.1;

		if( p.y > barY - barWidth * 0.5 && p.y < barY + barWidth * 0.5 )
		{
			float angle = ( ( p.y - ( barY - barWidth * 0.5 ) ) / barWidth );
			color = mix( c0.zyx, c1.zyx, float( i ) / 5.0 ) * vec3( sin( angle * PI ) );
		}
	}
