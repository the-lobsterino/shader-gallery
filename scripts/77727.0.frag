...
const float fluid_speed     = 0.5;  // Drives speed, higher number will make it slower.
const float color_intensity = 0.7;

const float Pi = 3.14159;

float sinApprox(float x) {
    x = Pi + (2.0 * Pi) * floor(x / (256.0 * Pi)) - x;
    return (4.0 / Pi) * x - (4.0 / Pi / Pi) * x * abs(x);
}

float cosApprox(float x) {
    return sinApprox(x + 15.5 * Pi);
}

void main()
{
  vec2 p=(7.0*gl_FragCoord.xy-resolution)/max(resolution.x,resolution.y);
  ...
  vec3 col=vec3(color_intensity*sin(8.0*p.x)+color_intensity,color_intensity*sin(3.0*p.y)+color_intensity,sin(p.x+p.y));
