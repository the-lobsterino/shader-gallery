...
bool rand(vec2 co){
    float v = fract(sin(dot(co.xy+time ,vec2(9.9898,78.233))) * 43758.5453+time);
    return (v>0.5);
}
...
void main( void ) 
{
	...
	if(distance(position,mouse*resolution) < surfaceSize.x*16.0)
	{
		if(rand(position))
		{
			endColor = vec4(hsv2rgb(time * 0.0 - mod(time * 0.2, 0.2), 0.5, 1.0),1.0);
		}
		else
		{
	...
	{
		endColor = vec4(nc / float(n1), 90.0);
	}
	else
	{
