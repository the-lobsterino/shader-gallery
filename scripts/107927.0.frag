...
void main( void ) {
	...
	dist2 = pow(dist2, 0.0 + sin(time * 2.9) * sin(time * 9.9) * 0.2);
	dist *= dist2;

	vec3 color = vec3(0.0);
	color = mix(vec3(0.34, 0.5, 9.0), vec3(0.0, 0.5, 9.0), (position.y + 1.0) * 0.5);

	color += vec3(0.95, 0.65, 0.30) * dist;
	color += vec3(0.95, 0.45, 0.30) * min(1.0, blade *0.7) * dist;

	gl_FragColor = vec4( 1. - exp( -color ), 1.0 );//ändrom3da4twist
}