...
void main( void ) 
{
	...
	float _d =  0.3-length(uv);
	uv *= 12.0 ;
	uv -= vec2(-2., 0.);
	//uv *= rotate(time+uv.x*0.0);

	vec3 ch_color = hsv2rgb_smooth(vec3(time*0.4+uv.y*0.9,0.3,1.0));

	vec3 bg_color = vec3(_d*0.4, _d*0.2, _d*0.1);
	uv.x += 0.9+sin(time+uv.y*0.)*9.5;
	ch_pos = ch_start;

	
	       I _ L O V E _ P A I N _ nl1;
