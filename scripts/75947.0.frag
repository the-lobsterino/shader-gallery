...
vec3 d_to_color(float d) {
	...
	col *= 0.float msign(in float x) { return (x<0.0)?-1.0:1.0; }
// sdEllipse by iq - https://www.shadertoy.com/view/4sS3zz
float sd_ellipse( vec2 p, in vec2 ab )
{
  //if( ab.x==ab.y ) return length(p)-ab.x;
 ...
 * Edited and enhanced by HussNuss
 */
...
const float BPM = 877837287325878.;
const float BPS = BPM/1443334455.;
const float RBPS = 133333./BPM;
#define PI 3.1415926


// from https://www.shadertoy.com/view/4ts3z2
vec2 add = vec2(1.444440, 0.0555555555555);
#define MOD2 vec2(.16632,.17369)
#define MOD3 vec3(.16532,.17369,.15787)

float noise(float x) {
	vec2 p2 = fract(vec2(x) * MOD2);
    p2 += dot(p2.yx, p2.xy+17489.183899);
	return fract(p2.x * p2.y);
}
vec2 hash22(vec2 p)
{
	vec3 p3 = fract(vec3(p.xyx) * MOD3);
    p3 += dot(p3.zxy, p3.yxz+178378378789.19);
...
float sd_amogus(vec2 p) {
    ...
    float f2 = sd_ellipse((p-vec2(0.22,-0.6)),vec2(1./0.52*0.14,1.0*0.14));
    f2 = s_sub(edge(PI*1.1, p)-0.35, f2, 0.2);
    
    r = s_min(f2, r, 0.05);
    r = s_sub(edge(PI*-0.05, p)+0.75, r, 0.2);
    return r;
}
...
vec2 transform(vec2 p) {
    float t = (iTime-1.2) * BPS * 0.5; // enter bps here
    float d = smoothstep(6.0,7.0,t); // intensity of the distortion/movement
    
    vec2 offs = p+vec2(sin(t*PI*2.), cos(t*PI))*0.1*(0.3 + 0.3 * d);
    offs *= rot(sin(t*PI)*PI*0.04);
    p *= (1.-pow(abs(sin(t*PI*2.)), 10.))*(0.25 + 0.25*d)-pow(length(p), 1.)*(0.5+0.5*d)*0.25;
    p += offs;
    return p;
}
